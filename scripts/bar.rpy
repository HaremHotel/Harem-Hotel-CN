# TODO: Translation updated at 2023-12-30 22:03
# game/scripts/bar.rpy:51
translate schinese prebar_fe710902:

    # "Sports day is going on, the crowd is too big..."
    "体育日正在进行,人太多了......"

# game/scripts/bar.rpy:181
translate schinese bar_9b3fde8d:

    # "I should come back near closing time."
    "我应该在快打烊的时候再过来."

# game/scripts/bar.rpy:183
translate schinese bar_9b3fde8d_1:

    # "I should come back near closing time."
    "我应该在快打烊的时候再过来."

# game/scripts/bar.rpy:185
translate schinese bar_c950e013:

    # "I should increase Maria's friendship."
    "我得提升玛丽亚的友谊值."

# game/scripts/bar.rpy:187
translate schinese bar_9b3fde8d_2:

    # "I should come back near closing time."
    "我应该在快打烊的时候再过来."

# game/scripts/bar.rpy:189
translate schinese bar_9b3fde8d_3:

    # "I should come back near closing time."
    "我应该在快打烊的时候再过来."

# game/scripts/bar.rpy:203
translate schinese firstspank_666d3373:

    # moon "No! Wait! PLEASE! I can fix this!"
    moon "不!等等!求你了!我会搞好的!"

# game/scripts/bar.rpy:204
translate schinese firstspank_4cf4c7d5:

    # bart "There's not much we can do today..."
    bart "我们今天做不了什么......"

# game/scripts/bar.rpy:205
translate schinese firstspank_6871f93b:

    # pat "What a dump..."
    pat "真是个垃圾场......"

# game/scripts/bar.rpy:206
translate schinese firstspank_ec700bea:

    # pat "I want a refund!"
    pat "我要退款!"

# game/scripts/bar.rpy:207
translate schinese firstspank_90f35ed7:

    # pat "I'm outta here!"
    pat "我走了!"

# game/scripts/bar.rpy:209
translate schinese firstspank_4732b4ca:

    # c "What's going on here?"
    c "这里发生什么事了?"

# game/scripts/bar.rpy:210
translate schinese firstspank_e5119cb1:

    # bart "You're going to want to talk to Moon... Just give me a call next time I'm able to come to work."
    bart "你得和文谈谈......下次我可以过来上班的时候打个电话给我就行."

# game/scripts/bar.rpy:211
translate schinese firstspank_297d93f3:

    # c "What?"
    c "什么?"

# game/scripts/bar.rpy:213
translate schinese firstspank_7f330ee1:

    # moon "I'm so sorry Mr. [name]! I'm so sorry! I ruined everything! *sob*"
    moon "对不起[name]先生!对不起!我把一切都毁了!*抽泣*"

# game/scripts/bar.rpy:214
translate schinese firstspank_0c5aed51:

    # c "Calm down, what happened?"
    c "冷静点,发生什么事了?"

# game/scripts/bar.rpy:215
translate schinese firstspank_d3c7f6df:

    # moon "I broke everything! All the glasses! We only have a couple left!"
    moon "我把东西都打坏了!所有的玻璃杯!我们只剩下两只了!"

# game/scripts/bar.rpy:216
translate schinese firstspank_8af7f45e:

    # c "Fuck, you broke all the glasses...?"
    c "操了,你把所有玻璃杯都打坏了......?"

# game/scripts/bar.rpy:217
translate schinese firstspank_8b1274dc:

    # moon "I was trying to reach for one off the shelf and I took the whole thing down!"
    moon "我想伸手去拿架子上的玻璃杯,结果把整个架子都拉下来了!"

# game/scripts/bar.rpy:218
translate schinese firstspank_6185b544:

    # moon "Bart and Maria are going to be out of a job now, and it's all my fault! I'm so sorry! Please forgive me!"
    moon "巴特和玛丽亚现在要失业了,这都是我的错!我很抱歉!请原谅我!"

# game/scripts/bar.rpy:220
translate schinese firstspank_a27e8514:

    # moon "I-I'll pay for everything! You can have all my money!"
    moon "我-我会支付一切费用的!你可以用我所有的钱!"

# game/scripts/bar.rpy:221
translate schinese firstspank_e831aa4d:

    # c "I'm not taking any of your money..."
    c "我不会拿你的钱的......"

# game/scripts/bar.rpy:222
translate schinese firstspank_52296cfd:

    # moon "You have to! The punishment must fit the crime, it's what my father always enforced! I have to pay for this!"
    moon "你必须拿!惩罚必须和过错相匹配,这也是我爸爸一直以来所执行的!我必须为此付出代价!"

# game/scripts/bar.rpy:223
translate schinese firstspank_82cc2713:

    # c "You worked for that money, you'll keep it."
    c "你是来这里赚钱的,你可以留着它."

# game/scripts/bar.rpy:224
translate schinese firstspank_322f5e1a:

    # moon "What!? That doesn't make any sense! I messed up! I have to make this right!"
    moon "什么!?那没道理!我把一切都搞砸了!我必须把它改正过来!"

# game/scripts/bar.rpy:225
translate schinese firstspank_86836fa5:

    # moon "I-I'll work for only half my pay then! I can afford that for while! I don't want Maria or Bart to be homeless!"
    moon "那-那我就只拿一半工资!我暂时还可以负担得起!我不希望玛丽亚或者巴特无家可归!"

# game/scripts/bar.rpy:226
translate schinese firstspank_d19f855d:

    # c "..."
    c "......"

# game/scripts/bar.rpy:227
translate schinese firstspank_d97a7eae:

    # moon "Please...!"
    moon "求你了......!"

# game/scripts/bar.rpy:228
translate schinese firstspank_62636ae3:

    # c "You want to be punished?"
    c "你想被惩罚吗?"

# game/scripts/bar.rpy:229
translate schinese firstspank_0a7dbe13:

    # moon "It must be done!"
    moon "你必须这样做!"

# game/scripts/bar.rpy:230
translate schinese firstspank_efda75ff:

    # c "...Then I'll spank you. {size=-15}please don't hit me{/size}"
    c "......那我会打你屁股.{size=-15}请别打我{/size}"

# game/scripts/bar.rpy:232
translate schinese firstspank_4984c9d6:

    # moon "..."
    moon "......"

# game/scripts/bar.rpy:234
translate schinese firstspank_e2519fa6:

    # moon "DO YOU WANT TO LOSE YOUR TEETH!?" with ssshake
    moon "你想满地找牙吗!?" with ssshake

# game/scripts/bar.rpy:235
translate schinese firstspank_d32e8a64:

    # c "Are you saying your father never spanked you as a punishment?"
    c "你是说你爸爸从来没打你屁股?"

# game/scripts/bar.rpy:236
translate schinese firstspank_400ee6fd:

    # moon "Of course not! He never laid a hand on me!"
    moon "当然没有!他从来都没有对我动过手!"

# game/scripts/bar.rpy:237
translate schinese firstspank_ec9b52e0:

    # c "It's a very common punishment."
    c "这是一个非常常见的惩罚."

# game/scripts/bar.rpy:238
translate schinese firstspank_be3d2126:

    # moon "But I'm not a child!"
    moon "但我不是小孩了!"

# game/scripts/bar.rpy:239
translate schinese firstspank_a9bb6210:

    # c "You don't need to be one. And besides, you're the one asking for a punishment."
    c "你不需要是.而且,你才是那个要求惩罚的人."

# game/scripts/bar.rpy:240
translate schinese firstspank_dc981473:

    # moon "Choose something else!"
    moon "那就选点别的!"

# game/scripts/bar.rpy:241
translate schinese firstspank_8233ebc2:

    # c "No more hot springs?"
    c "不能去温泉呢?"

# game/scripts/bar.rpy:242
translate schinese firstspank_50b97277:

    # moon "Not good enough!"
    moon "还不够!"

# game/scripts/bar.rpy:243
translate schinese firstspank_30a10ae1:

    # c "No more alcohol?"
    c "不能喝酒呢?"

# game/scripts/bar.rpy:244
translate schinese firstspank_ec045a7d:

    # moon "Not a punishment!"
    moon "这不是惩罚!"

# game/scripts/bar.rpy:245
translate schinese firstspank_5cc8cb2a:

    # c "How about you undress, I tie you up outside and write demeaning words on your body for 7 days?"
    c "那我把你衣服脱了,然后在你身上写很多不好的词语,然后把你绑在外面晾上七天,如何?"

# game/scripts/bar.rpy:247
translate schinese firstspank_54c2182b:

    # moon "Grrrr..."
    moon "呃......"

# game/scripts/bar.rpy:248
translate schinese firstspank_4bfa4eb0:

    # c "Okay, nevermind..."
    c "好吧,算了......"

# game/scripts/bar.rpy:250
translate schinese firstspank_ffd941e7:

    # moon "Take my money!"
    moon "拿我的钱!"

# game/scripts/bar.rpy:251
translate schinese firstspank_e69e42b8:

    # c "No."
    c "不."

# game/scripts/bar.rpy:252
translate schinese firstspank_23d8c261:

    # moon "TAKE MY MONEY!"
    moon "拿我的钱!!!"

# game/scripts/bar.rpy:253
translate schinese firstspank_e69e42b8_1:

    # c "No."
    c "不."

# game/scripts/bar.rpy:255
translate schinese firstspank_d6976022:

    # moon "You're more stubborn than my father..."
    moon "你比我爸爸还固执......"

# game/scripts/bar.rpy:257
translate schinese firstspank_fc6ded52:

    # moon "Fine, you want to spank me? That's both humiliating and embarrassing! So I guess it's good enough."
    moon "好吧,你想打我屁股是吧?这既令人难堪也令人羞耻.我想这也算够了."

# game/scripts/bar.rpy:258
translate schinese firstspank_871ecce0:

    # c "Wait, really?"
    c "等等,真的吗?"

# game/scripts/bar.rpy:259
translate schinese firstspank_20f5c79d:

    # moon "Nobody else is here, and so long as you don't take pictures or touch me anywhere you shouldn't, you'll keep your head!"
    moon "这也没有别人,只要你不拍照,不碰我不该碰的地方,你就可以保住你的脑袋!"

# game/scripts/bar.rpy:262
translate schinese firstspank_7210720c:

    # moon "You had better not get hard..."
    moon "你最好不要硬起来......"

# game/scripts/bar.rpy:263
translate schinese firstspank_6cce0516:

    # c "Nice panties."
    c "内裤不错."

# game/scripts/bar.rpy:264
translate schinese firstspank_e1e94a60:

    # moon "Don't look at them!"
    moon "别看它们!"

# game/scripts/bar.rpy:265
translate schinese firstspank_96d04c5f:

    # moon "What are you waiting for!? Spank me, you stubborn bastard!"
    moon "你还在等什么!?打我屁股啊,你这顽固的混蛋!"

# game/scripts/bar.rpy:267
translate schinese firstspank_c4b3b790:

    # moon "Keep going...!"
    moon "继续......!"

# game/scripts/bar.rpy:269
translate schinese firstspank_3888c1ee:

    # moon "E-Eghh... Not enough...!"
    moon "呃-嗯......还不够......!"

# game/scripts/bar.rpy:271
translate schinese firstspank_dad59b7f:

    # moon "Fine... Enough... I think that's enough..."
    moon "好吧......够了......我想已经够了......"

# game/scripts/bar.rpy:275
translate schinese firstspank_84381bf3:

    # moon "Enough!"
    moon "够了!"

# game/scripts/bar.rpy:281
translate schinese firstspank_0dc45d60:

    # moon "Oh! Sorry!"
    moon "噢!对不起!"

# game/scripts/bar.rpy:284
translate schinese firstspank_13a409dc:

    # c "...Maybe that was too much..."
    c "......也许有点太多了......"

# game/scripts/bar.rpy:293
translate schinese firstspank_684ac16e:

    # moon "Hmph... I suppose that was good enough."
    moon "嗯......我想这已经够了."

# game/scripts/bar.rpy:294
translate schinese firstspank_4c9dd033:

    # c "A little spanking for hundreds of dollars worth of glasses?"
    c "价值几百美元的玻璃杯换一个小小的打屁股?"

# game/scripts/bar.rpy:296
translate schinese firstspank_2b749f59:

    # moon "But you said you didn't want my money!"
    moon "但是你说你不想要我的钱的!"

# game/scripts/bar.rpy:297
translate schinese firstspank_235fc483:

    # c "I did, and I'm not going to take it."
    c "我是这么说的,我也不会拿."

# game/scripts/bar.rpy:298
translate schinese firstspank_87cf91e9:

    # c "But a couple minutes of spanking doesn't make up for everything. We'll have to do this again."
    c "但是打几分钟屁股并不能弥补这一切.我们必须再做一次."

# game/scripts/bar.rpy:300
translate schinese firstspank_73d73c4d:

    # moon "*sigh*... You're probably right..."
    moon "*叹气*......你可能是对的......"

# game/scripts/bar.rpy:301
translate schinese firstspank_52cabc51:

    # moon "Give me a call if I'm able to come into work tomorrow..."
    moon "如果我明天可以来上班了就给我打电话......"

# game/scripts/bar.rpy:303
translate schinese firstspank_37b3cb69:

    # "Moon gives you her phone number."
    "文给了你她的手机号码."

# game/scripts/bar.rpy:305
translate schinese firstspank_df8bbcf6:

    # c "Moon broke all the glasses in the bar. Do we have the funds to replace them?"
    c "文把酒吧里的玻璃杯都打碎了.我们有钱去替换它们吗?"

# game/scripts/bar.rpy:306
translate schinese firstspank_9b5aa049:

    # e "She did...? That's not good..."
    e "她......?那可不好......"

# game/scripts/bar.rpy:307
translate schinese firstspank_0f6ffd14:

    # e "Yes, we have some money in reserve, I'll go tonight and replace our glasses."
    e "是的,我们有一些储备的资金,我今晚就去更换我们的玻璃杯."

# game/scripts/bar.rpy:321
translate schinese moonspankingnude_e9b8976d:

    # moon "Yes..."
    moon "好的......"

# game/scripts/bar.rpy:323
translate schinese moonspankingnude_ff39f9d3:

    # moon "Mmm... Oh..."
    moon "嗯......噢......"

# game/scripts/bar.rpy:325
translate schinese moonspankingnude_8988c88e:

    # moon "Hnng..."
    moon "嗯......"

# game/scripts/bar.rpy:327
translate schinese moonspankingnude_37f78821:

    # moon "Hnn!"
    moon "嗯!"

# game/scripts/bar.rpy:329
translate schinese moonspankingnude_8925a046:

    # moon "Ah~!"
    moon "啊~!"

# game/scripts/bar.rpy:336
translate schinese moonspankingnude_ec900443:

    # moon "Hmmm..."
    moon "嗯......"

# game/scripts/bar.rpy:338
translate schinese moonspankingnude_34b75dae:

    # moon "Ahhh~!"
    moon "啊~!"

# game/scripts/bar.rpy:340
translate schinese moonspankingnude_b8bee2d3:

    # moon "Hnn, mmm..."
    moon "嗯,嗯......"

# game/scripts/bar.rpy:342
translate schinese moonspankingnude_37f78821_1:

    # moon "Hnn!"
    moon "嗯!"

# game/scripts/bar.rpy:344
translate schinese moonspankingnude_f724519e:

    # moon "Ah!"
    moon "啊!"

# game/scripts/bar.rpy:352
translate schinese moonspankingnude_e9b8976d_1:

    # moon "Yes..."
    moon "好的......"

# game/scripts/bar.rpy:354
translate schinese moonspankingnude_ff39f9d3_1:

    # moon "Mmm... Oh..."
    moon "嗯......噢......"

# game/scripts/bar.rpy:356
translate schinese moonspankingnude_8988c88e_1:

    # moon "Hnng..."
    moon "嗯......"

# game/scripts/bar.rpy:358
translate schinese moonspankingnude_37f78821_2:

    # moon "Hnn!"
    moon "嗯!"

# game/scripts/bar.rpy:360
translate schinese moonspankingnude_8925a046_1:

    # moon "Ah~!"
    moon "啊~!"

# game/scripts/bar.rpy:367
translate schinese moonspankingnude_ec900443_1:

    # moon "Hmmm..."
    moon "嗯......"

# game/scripts/bar.rpy:369
translate schinese moonspankingnude_34b75dae_1:

    # moon "Ahhh~!"
    moon "啊~!"

# game/scripts/bar.rpy:371
translate schinese moonspankingnude_b8bee2d3_1:

    # moon "Hnn, mmm..."
    moon "嗯,嗯......"

# game/scripts/bar.rpy:373
translate schinese moonspankingnude_37f78821_3:

    # moon "Hnn!"
    moon "嗯!"

# game/scripts/bar.rpy:375
translate schinese moonspankingnude_f724519e_1:

    # moon "Ah!"
    moon "啊!"

# game/scripts/bar.rpy:383
translate schinese moonspankingnude_e9b8976d_2:

    # moon "Yes..."
    moon "好的......"

# game/scripts/bar.rpy:385
translate schinese moonspankingnude_ff39f9d3_2:

    # moon "Mmm... Oh..."
    moon "嗯......噢......"

# game/scripts/bar.rpy:387
translate schinese moonspankingnude_8988c88e_2:

    # moon "Hnng..."
    moon "嗯......"

# game/scripts/bar.rpy:389
translate schinese moonspankingnude_37f78821_4:

    # moon "Hnn!"
    moon "嗯!"

# game/scripts/bar.rpy:391
translate schinese moonspankingnude_8925a046_2:

    # moon "Ah~!"
    moon "啊~!"

# game/scripts/bar.rpy:398
translate schinese moonspankingnude_ec900443_2:

    # moon "Hmmm..."
    moon "嗯......"

# game/scripts/bar.rpy:400
translate schinese moonspankingnude_34b75dae_2:

    # moon "Ahhh~!"
    moon "啊~!"

# game/scripts/bar.rpy:402
translate schinese moonspankingnude_b8bee2d3_2:

    # moon "Hnn, mmm..."
    moon "嗯,嗯......"

# game/scripts/bar.rpy:404
translate schinese moonspankingnude_37f78821_5:

    # moon "Hnn!"
    moon "嗯!"

# game/scripts/bar.rpy:406
translate schinese moonspankingnude_f724519e_2:

    # moon "Ah!"
    moon "啊!"

# game/scripts/bar.rpy:417
translate schinese moonspankingnophappy_0debc013:

    # moon "Ahh..."
    moon "啊......"

# game/scripts/bar.rpy:419
translate schinese moonspankingnophappy_3bc3e1f4:

    # moon "Mmm..."
    moon "嗯......"

# game/scripts/bar.rpy:421
translate schinese moonspankingnophappy_8988c88e:

    # moon "Hnng..."
    moon "嗯......"

# game/scripts/bar.rpy:423
translate schinese moonspankingnophappy_37f78821:

    # moon "Hnn!"
    moon "嗯!"

# game/scripts/bar.rpy:425
translate schinese moonspankingnophappy_f724519e:

    # moon "Ah!"
    moon "啊!"

# game/scripts/bar.rpy:432
translate schinese moonspankingnophappy_814e5466:

    # moon "Hahhn!"
    moon "哈!"

# game/scripts/bar.rpy:434
translate schinese moonspankingnophappy_33c32dd1:

    # moon "Nn, mmm..."
    moon "嗯,嗯......"

# game/scripts/bar.rpy:436
translate schinese moonspankingnophappy_082ce360:

    # moon "Ohh..."
    moon "噢......"

# game/scripts/bar.rpy:438
translate schinese moonspankingnophappy_f724519e_1:

    # moon "Ah!"
    moon "啊!"

# game/scripts/bar.rpy:440
translate schinese moonspankingnophappy_8c7e88fb:

    # moon "Hnnnn...!"
    moon "嗯......!"

# game/scripts/bar.rpy:448
translate schinese moonspankingnophappy_0debc013_1:

    # moon "Ahh..."
    moon "啊......"

# game/scripts/bar.rpy:450
translate schinese moonspankingnophappy_3bc3e1f4_1:

    # moon "Mmm..."
    moon "嗯......"

# game/scripts/bar.rpy:452
translate schinese moonspankingnophappy_8988c88e_1:

    # moon "Hnng..."
    moon "嗯......"

# game/scripts/bar.rpy:454
translate schinese moonspankingnophappy_37f78821_1:

    # moon "Hnn!"
    moon "嗯!"

# game/scripts/bar.rpy:456
translate schinese moonspankingnophappy_f724519e_2:

    # moon "Ah!"
    moon "啊!"

# game/scripts/bar.rpy:463
translate schinese moonspankingnophappy_814e5466_1:

    # moon "Hahhn!"
    moon "哈!"

# game/scripts/bar.rpy:465
translate schinese moonspankingnophappy_33c32dd1_1:

    # moon "Nn, mmm..."
    moon "嗯,嗯......"

# game/scripts/bar.rpy:467
translate schinese moonspankingnophappy_082ce360_1:

    # moon "Ohh..."
    moon "噢......"

# game/scripts/bar.rpy:469
translate schinese moonspankingnophappy_f724519e_3:

    # moon "Ah!"
    moon "啊!"

# game/scripts/bar.rpy:471
translate schinese moonspankingnophappy_8c7e88fb_1:

    # moon "Hnnnn...!"
    moon "嗯......!"

# game/scripts/bar.rpy:479
translate schinese moonspankingnophappy_0debc013_2:

    # moon "Ahh..."
    moon "啊......"

# game/scripts/bar.rpy:481
translate schinese moonspankingnophappy_3bc3e1f4_2:

    # moon "Mmm..."
    moon "嗯......"

# game/scripts/bar.rpy:483
translate schinese moonspankingnophappy_8988c88e_2:

    # moon "Hnng..."
    moon "嗯......"

# game/scripts/bar.rpy:485
translate schinese moonspankingnophappy_37f78821_2:

    # moon "Hnn!"
    moon "嗯!"

# game/scripts/bar.rpy:487
translate schinese moonspankingnophappy_f724519e_4:

    # moon "Ah!"
    moon "啊!"

# game/scripts/bar.rpy:494
translate schinese moonspankingnophappy_814e5466_2:

    # moon "Hahhn!"
    moon "哈!"

# game/scripts/bar.rpy:496
translate schinese moonspankingnophappy_33c32dd1_2:

    # moon "Nn, mmm..."
    moon "嗯,嗯......"

# game/scripts/bar.rpy:498
translate schinese moonspankingnophappy_082ce360_2:

    # moon "Ohh..."
    moon "噢......"

# game/scripts/bar.rpy:500
translate schinese moonspankingnophappy_f724519e_5:

    # moon "Ah!"
    moon "啊!"

# game/scripts/bar.rpy:502
translate schinese moonspankingnophappy_8c7e88fb_2:

    # moon "Hnnnn...!"
    moon "嗯......!"

# game/scripts/bar.rpy:512
translate schinese moonspankingnop_0debc013:

    # moon "Ahh..."
    moon "啊......"

# game/scripts/bar.rpy:514
translate schinese moonspankingnop_3bc3e1f4:

    # moon "Mmm..."
    moon "嗯......"

# game/scripts/bar.rpy:516
translate schinese moonspankingnop_8988c88e:

    # moon "Hnng..."
    moon "嗯......"

# game/scripts/bar.rpy:518
translate schinese moonspankingnop_37f78821:

    # moon "Hnn!"
    moon "嗯!"

# game/scripts/bar.rpy:520
translate schinese moonspankingnop_f724519e:

    # moon "Ah!"
    moon "啊!"

# game/scripts/bar.rpy:527
translate schinese moonspankingnop_814e5466:

    # moon "Hahhn!"
    moon "哈!"

# game/scripts/bar.rpy:529
translate schinese moonspankingnop_33c32dd1:

    # moon "Nn, mmm..."
    moon "嗯,嗯......"

# game/scripts/bar.rpy:531
translate schinese moonspankingnop_082ce360:

    # moon "Ohh..."
    moon "噢......"

# game/scripts/bar.rpy:533
translate schinese moonspankingnop_f724519e_1:

    # moon "Ah!"
    moon "啊!"

# game/scripts/bar.rpy:535
translate schinese moonspankingnop_8c7e88fb:

    # moon "Hnnnn...!"
    moon "嗯......!"

# game/scripts/bar.rpy:543
translate schinese moonspankingnop_0debc013_1:

    # moon "Ahh..."
    moon "啊......"

# game/scripts/bar.rpy:545
translate schinese moonspankingnop_3bc3e1f4_1:

    # moon "Mmm..."
    moon "嗯......"

# game/scripts/bar.rpy:547
translate schinese moonspankingnop_8988c88e_1:

    # moon "Hnng..."
    moon "嗯......"

# game/scripts/bar.rpy:549
translate schinese moonspankingnop_37f78821_1:

    # moon "Hnn!"
    moon "嗯!"

# game/scripts/bar.rpy:551
translate schinese moonspankingnop_f724519e_2:

    # moon "Ah!"
    moon "啊!"

# game/scripts/bar.rpy:558
translate schinese moonspankingnop_814e5466_1:

    # moon "Hahhn!"
    moon "哈!"

# game/scripts/bar.rpy:560
translate schinese moonspankingnop_33c32dd1_1:

    # moon "Nn, mmm..."
    moon "嗯,嗯......"

# game/scripts/bar.rpy:562
translate schinese moonspankingnop_082ce360_1:

    # moon "Ohh..."
    moon "噢......"

# game/scripts/bar.rpy:564
translate schinese moonspankingnop_f724519e_3:

    # moon "Ah!"
    moon "啊!"

# game/scripts/bar.rpy:566
translate schinese moonspankingnop_8c7e88fb_1:

    # moon "Hnnnn...!"
    moon "嗯......!"

# game/scripts/bar.rpy:574
translate schinese moonspankingnop_0debc013_2:

    # moon "Ahh..."
    moon "啊......"

# game/scripts/bar.rpy:576
translate schinese moonspankingnop_3bc3e1f4_2:

    # moon "Mmm..."
    moon "嗯......"

# game/scripts/bar.rpy:578
translate schinese moonspankingnop_8988c88e_2:

    # moon "Hnng..."
    moon "嗯......"

# game/scripts/bar.rpy:580
translate schinese moonspankingnop_37f78821_2:

    # moon "Hnn!"
    moon "嗯!"

# game/scripts/bar.rpy:582
translate schinese moonspankingnop_f724519e_4:

    # moon "Ah!"
    moon "啊!"

# game/scripts/bar.rpy:589
translate schinese moonspankingnop_814e5466_2:

    # moon "Hahhn!"
    moon "哈!"

# game/scripts/bar.rpy:591
translate schinese moonspankingnop_33c32dd1_2:

    # moon "Nn, mmm..."
    moon "嗯,嗯......"

# game/scripts/bar.rpy:593
translate schinese moonspankingnop_082ce360_2:

    # moon "Ohh..."
    moon "噢......"

# game/scripts/bar.rpy:595
translate schinese moonspankingnop_f724519e_5:

    # moon "Ah!"
    moon "啊!"

# game/scripts/bar.rpy:597
translate schinese moonspankingnop_8c7e88fb_2:

    # moon "Hnnnn...!"
    moon "嗯......!"

# game/scripts/bar.rpy:605
translate schinese moonspankingcatnchard_d7b8ec89:

    # moon "Gah!"
    moon "咕!"

# game/scripts/bar.rpy:607
translate schinese moonspankingcatnchard_030c8ab8:

    # moon "HNNG!"
    moon "嗯!"

# game/scripts/bar.rpy:609
translate schinese moonspankingcatnchard_0b570100:

    # moon "Too hard!"
    moon "太大力了!"

# game/scripts/bar.rpy:611
translate schinese moonspankingcatnchard_bc959e7e:

    # moon "Softer, please!"
    moon "轻一点,求你了!"

# game/scripts/bar.rpy:613
translate schinese moonspankingcatnchard_b268297c:

    # moon "AHH!"
    moon "啊!"

# game/scripts/bar.rpy:619
translate schinese moonspankingcatnchard_1bdca328:

    # moon "EGH!"
    moon "呃!"

# game/scripts/bar.rpy:621
translate schinese moonspankingcatnchard_b84f908b:

    # moon "Hnnguh!"
    moon "嗯!"

# game/scripts/bar.rpy:623
translate schinese moonspankingcatnchard_b70451e4:

    # moon "Ouch!"
    moon "嗷!"

# game/scripts/bar.rpy:625
translate schinese moonspankingcatnchard_0b570100_1:

    # moon "Too hard!"
    moon "太大力了!"

# game/scripts/bar.rpy:627
translate schinese moonspankingcatnchard_4a0e8d32:

    # moon "AHH...!"
    moon "啊......!"

# game/scripts/bar.rpy:633
translate schinese moonspankingcatnchard_d7b8ec89_1:

    # moon "Gah!"
    moon "咕!"

# game/scripts/bar.rpy:635
translate schinese moonspankingcatnchard_030c8ab8_1:

    # moon "HNNG!"
    moon "嗯!"

# game/scripts/bar.rpy:637
translate schinese moonspankingcatnchard_0b570100_2:

    # moon "Too hard!"
    moon "太大力了!"

# game/scripts/bar.rpy:639
translate schinese moonspankingcatnchard_bc959e7e_1:

    # moon "Softer, please!"
    moon "轻一点,求你了!"

# game/scripts/bar.rpy:641
translate schinese moonspankingcatnchard_b268297c_1:

    # moon "AHH!"
    moon "啊!"

# game/scripts/bar.rpy:647
translate schinese moonspankingcatnchard_1bdca328_1:

    # moon "EGH!"
    moon "呃!"

# game/scripts/bar.rpy:649
translate schinese moonspankingcatnchard_b84f908b_1:

    # moon "Hnnguh!"
    moon "嗯!"

# game/scripts/bar.rpy:651
translate schinese moonspankingcatnchard_b70451e4_1:

    # moon "Ouch!"
    moon "嗷!"

# game/scripts/bar.rpy:653
translate schinese moonspankingcatnchard_0b570100_3:

    # moon "Too hard!"
    moon "太大力了!"

# game/scripts/bar.rpy:655
translate schinese moonspankingcatnchard_4a0e8d32_1:

    # moon "AHH...!"
    moon "啊......!"

# game/scripts/bar.rpy:661
translate schinese moonspankingcatnchard_d7b8ec89_2:

    # moon "Gah!"
    moon "咕!"

# game/scripts/bar.rpy:663
translate schinese moonspankingcatnchard_030c8ab8_2:

    # moon "HNNG!"
    moon "嗯!"

# game/scripts/bar.rpy:665
translate schinese moonspankingcatnchard_0b570100_4:

    # moon "Too hard!"
    moon "太大力了!"

# game/scripts/bar.rpy:667
translate schinese moonspankingcatnchard_bc959e7e_2:

    # moon "Softer, please!"
    moon "轻一点,求你了!"

# game/scripts/bar.rpy:669
translate schinese moonspankingcatnchard_b268297c_2:

    # moon "AHH!"
    moon "啊!"

# game/scripts/bar.rpy:675
translate schinese moonspankingcatnchard_1bdca328_2:

    # moon "EGH!"
    moon "呃!"

# game/scripts/bar.rpy:677
translate schinese moonspankingcatnchard_b84f908b_2:

    # moon "Hnnguh!"
    moon "嗯!"

# game/scripts/bar.rpy:679
translate schinese moonspankingcatnchard_b70451e4_2:

    # moon "Ouch!"
    moon "嗷!"

# game/scripts/bar.rpy:681
translate schinese moonspankingcatnchard_0b570100_5:

    # moon "Too hard!"
    moon "太大力了!"

# game/scripts/bar.rpy:683
translate schinese moonspankingcatnchard_4a0e8d32_2:

    # moon "AHH...!"
    moon "啊......!"

# game/scripts/bar.rpy:692
translate schinese moonspankingcatnophard_d7b8ec89:

    # moon "Gah!"
    moon "咕!"

# game/scripts/bar.rpy:694
translate schinese moonspankingcatnophard_030c8ab8:

    # moon "HNNG!"
    moon "嗯!"

# game/scripts/bar.rpy:696
translate schinese moonspankingcatnophard_0b570100:

    # moon "Too hard!"
    moon "太大力了!"

# game/scripts/bar.rpy:698
translate schinese moonspankingcatnophard_bc959e7e:

    # moon "Softer, please!"
    moon "轻一点,求你了!"

# game/scripts/bar.rpy:700
translate schinese moonspankingcatnophard_b268297c:

    # moon "AHH!"
    moon "啊!"

# game/scripts/bar.rpy:706
translate schinese moonspankingcatnophard_1bdca328:

    # moon "EGH!"
    moon "呃!"

# game/scripts/bar.rpy:708
translate schinese moonspankingcatnophard_b84f908b:

    # moon "Hnnguh!"
    moon "嗯!"

# game/scripts/bar.rpy:710
translate schinese moonspankingcatnophard_b70451e4:

    # moon "Ouch!"
    moon "嗷!"

# game/scripts/bar.rpy:712
translate schinese moonspankingcatnophard_0b570100_1:

    # moon "Too hard!"
    moon "太大力了!"

# game/scripts/bar.rpy:714
translate schinese moonspankingcatnophard_4a0e8d32:

    # moon "AHH...!"
    moon "啊......!"

# game/scripts/bar.rpy:721
translate schinese moonspankingcatnophard_d7b8ec89_1:

    # moon "Gah!"
    moon "咕!"

# game/scripts/bar.rpy:723
translate schinese moonspankingcatnophard_030c8ab8_1:

    # moon "HNNG!"
    moon "嗯!"

# game/scripts/bar.rpy:725
translate schinese moonspankingcatnophard_0b570100_2:

    # moon "Too hard!"
    moon "太大力了!"

# game/scripts/bar.rpy:727
translate schinese moonspankingcatnophard_bc959e7e_1:

    # moon "Softer, please!"
    moon "轻一点,求你了!"

# game/scripts/bar.rpy:729
translate schinese moonspankingcatnophard_b268297c_1:

    # moon "AHH!"
    moon "啊!"

# game/scripts/bar.rpy:735
translate schinese moonspankingcatnophard_1bdca328_1:

    # moon "EGH!"
    moon "呃!"

# game/scripts/bar.rpy:737
translate schinese moonspankingcatnophard_b84f908b_1:

    # moon "Hnnguh!"
    moon "嗯!"

# game/scripts/bar.rpy:739
translate schinese moonspankingcatnophard_b70451e4_1:

    # moon "Ouch!"
    moon "嗷!"

# game/scripts/bar.rpy:741
translate schinese moonspankingcatnophard_0b570100_3:

    # moon "Too hard!"
    moon "太大力了!"

# game/scripts/bar.rpy:743
translate schinese moonspankingcatnophard_4a0e8d32_1:

    # moon "AHH...!"
    moon "啊......!"

# game/scripts/bar.rpy:750
translate schinese moonspankingcatnophard_d7b8ec89_2:

    # moon "Gah!"
    moon "咕!"

# game/scripts/bar.rpy:752
translate schinese moonspankingcatnophard_030c8ab8_2:

    # moon "HNNG!"
    moon "嗯!"

# game/scripts/bar.rpy:754
translate schinese moonspankingcatnophard_0b570100_4:

    # moon "Too hard!"
    moon "太大力了!"

# game/scripts/bar.rpy:756
translate schinese moonspankingcatnophard_bc959e7e_2:

    # moon "Softer, please!"
    moon "轻一点,求你了!"

# game/scripts/bar.rpy:758
translate schinese moonspankingcatnophard_b268297c_2:

    # moon "AHH!"
    moon "啊!"

# game/scripts/bar.rpy:764
translate schinese moonspankingcatnophard_1bdca328_2:

    # moon "EGH!"
    moon "呃!"

# game/scripts/bar.rpy:766
translate schinese moonspankingcatnophard_b84f908b_2:

    # moon "Hnnguh!"
    moon "嗯!"

# game/scripts/bar.rpy:768
translate schinese moonspankingcatnophard_b70451e4_2:

    # moon "Ouch!"
    moon "嗷!"

# game/scripts/bar.rpy:770
translate schinese moonspankingcatnophard_0b570100_5:

    # moon "Too hard!"
    moon "太大力了!"

# game/scripts/bar.rpy:772
translate schinese moonspankingcatnophard_4a0e8d32_2:

    # moon "AHH...!"
    moon "啊......!"

# game/scripts/bar.rpy:779
translate schinese moonspankingcatnophard_d7b8ec89_3:

    # moon "Gah!"
    moon "咕!"

# game/scripts/bar.rpy:781
translate schinese moonspankingcatnophard_030c8ab8_3:

    # moon "HNNG!"
    moon "嗯!"

# game/scripts/bar.rpy:783
translate schinese moonspankingcatnophard_0b570100_6:

    # moon "Too hard!"
    moon "太大力了!"

# game/scripts/bar.rpy:785
translate schinese moonspankingcatnophard_bc959e7e_3:

    # moon "Softer, please!"
    moon "轻一点,求你了!"

# game/scripts/bar.rpy:787
translate schinese moonspankingcatnophard_b268297c_3:

    # moon "AHH!"
    moon "啊!"

# game/scripts/bar.rpy:793
translate schinese moonspankingcatnophard_1bdca328_3:

    # moon "EGH!"
    moon "呃!"

# game/scripts/bar.rpy:795
translate schinese moonspankingcatnophard_b84f908b_3:

    # moon "Hnnguh!"
    moon "嗯!"

# game/scripts/bar.rpy:797
translate schinese moonspankingcatnophard_b70451e4_3:

    # moon "Ouch!"
    moon "嗷!"

# game/scripts/bar.rpy:799
translate schinese moonspankingcatnophard_0b570100_7:

    # moon "Too hard!"
    moon "太大力了!"

# game/scripts/bar.rpy:801
translate schinese moonspankingcatnophard_4a0e8d32_3:

    # moon "AHH...!"
    moon "啊......!"

# game/scripts/bar.rpy:810
translate schinese moonspankingcatwethard_d7b8ec89:

    # moon "Gah!"
    moon "咕!"

# game/scripts/bar.rpy:812
translate schinese moonspankingcatwethard_030c8ab8:

    # moon "HNNG!"
    moon "嗯!"

# game/scripts/bar.rpy:814
translate schinese moonspankingcatwethard_0b570100:

    # moon "Too hard!"
    moon "太大力了!"

# game/scripts/bar.rpy:816
translate schinese moonspankingcatwethard_bc959e7e:

    # moon "Softer, please!"
    moon "轻一点,求你了!"

# game/scripts/bar.rpy:818
translate schinese moonspankingcatwethard_b268297c:

    # moon "AHH!"
    moon "啊!"

# game/scripts/bar.rpy:824
translate schinese moonspankingcatwethard_1bdca328:

    # moon "EGH!"
    moon "呃!"

# game/scripts/bar.rpy:826
translate schinese moonspankingcatwethard_b84f908b:

    # moon "Hnnguh!"
    moon "嗯!"

# game/scripts/bar.rpy:828
translate schinese moonspankingcatwethard_b70451e4:

    # moon "Ouch!"
    moon "嗷!"

# game/scripts/bar.rpy:830
translate schinese moonspankingcatwethard_0b570100_1:

    # moon "Too hard!"
    moon "太大力了!"

# game/scripts/bar.rpy:832
translate schinese moonspankingcatwethard_4a0e8d32:

    # moon "AHH...!"
    moon "啊......!"

# game/scripts/bar.rpy:839
translate schinese moonspankingcatwethard_d7b8ec89_1:

    # moon "Gah!"
    moon "咕!"

# game/scripts/bar.rpy:841
translate schinese moonspankingcatwethard_030c8ab8_1:

    # moon "HNNG!"
    moon "嗯!"

# game/scripts/bar.rpy:843
translate schinese moonspankingcatwethard_0b570100_2:

    # moon "Too hard!"
    moon "太大力了!"

# game/scripts/bar.rpy:845
translate schinese moonspankingcatwethard_bc959e7e_1:

    # moon "Softer, please!"
    moon "轻一点,求你了!"

# game/scripts/bar.rpy:847
translate schinese moonspankingcatwethard_b268297c_1:

    # moon "AHH!"
    moon "啊!"

# game/scripts/bar.rpy:853
translate schinese moonspankingcatwethard_1bdca328_1:

    # moon "EGH!"
    moon "呃!"

# game/scripts/bar.rpy:855
translate schinese moonspankingcatwethard_b84f908b_1:

    # moon "Hnnguh!"
    moon "嗯!"

# game/scripts/bar.rpy:857
translate schinese moonspankingcatwethard_b70451e4_1:

    # moon "Ouch!"
    moon "嗷!"

# game/scripts/bar.rpy:859
translate schinese moonspankingcatwethard_0b570100_3:

    # moon "Too hard!"
    moon "太大力了!"

# game/scripts/bar.rpy:861
translate schinese moonspankingcatwethard_4a0e8d32_1:

    # moon "AHH...!"
    moon "啊......!"

# game/scripts/bar.rpy:868
translate schinese moonspankingcatwethard_d7b8ec89_2:

    # moon "Gah!"
    moon "咕!"

# game/scripts/bar.rpy:870
translate schinese moonspankingcatwethard_030c8ab8_2:

    # moon "HNNG!"
    moon "嗯!"

# game/scripts/bar.rpy:872
translate schinese moonspankingcatwethard_0b570100_4:

    # moon "Too hard!"
    moon "太大力了!"

# game/scripts/bar.rpy:874
translate schinese moonspankingcatwethard_bc959e7e_2:

    # moon "Softer, please!"
    moon "轻一点,求你了!"

# game/scripts/bar.rpy:876
translate schinese moonspankingcatwethard_b268297c_2:

    # moon "AHH!"
    moon "啊!"

# game/scripts/bar.rpy:882
translate schinese moonspankingcatwethard_1bdca328_2:

    # moon "EGH!"
    moon "呃!"

# game/scripts/bar.rpy:884
translate schinese moonspankingcatwethard_b84f908b_2:

    # moon "Hnnguh!"
    moon "嗯!"

# game/scripts/bar.rpy:886
translate schinese moonspankingcatwethard_b70451e4_2:

    # moon "Ouch!"
    moon "嗷!"

# game/scripts/bar.rpy:888
translate schinese moonspankingcatwethard_0b570100_5:

    # moon "Too hard!"
    moon "太大力了!"

# game/scripts/bar.rpy:890
translate schinese moonspankingcatwethard_4a0e8d32_2:

    # moon "AHH...!"
    moon "啊......!"

# game/scripts/bar.rpy:897
translate schinese moonspankingcatwethard_d7b8ec89_3:

    # moon "Gah!"
    moon "咕!"

# game/scripts/bar.rpy:899
translate schinese moonspankingcatwethard_030c8ab8_3:

    # moon "HNNG!"
    moon "嗯!"

# game/scripts/bar.rpy:901
translate schinese moonspankingcatwethard_0b570100_6:

    # moon "Too hard!"
    moon "太大力了!"

# game/scripts/bar.rpy:903
translate schinese moonspankingcatwethard_bc959e7e_3:

    # moon "Softer, please!"
    moon "轻一点,求你了!"

# game/scripts/bar.rpy:905
translate schinese moonspankingcatwethard_b268297c_3:

    # moon "AHH!"
    moon "啊!"

# game/scripts/bar.rpy:911
translate schinese moonspankingcatwethard_1bdca328_3:

    # moon "EGH!"
    moon "呃!"

# game/scripts/bar.rpy:913
translate schinese moonspankingcatwethard_b84f908b_3:

    # moon "Hnnguh!"
    moon "嗯!"

# game/scripts/bar.rpy:915
translate schinese moonspankingcatwethard_b70451e4_3:

    # moon "Ouch!"
    moon "嗷!"

# game/scripts/bar.rpy:917
translate schinese moonspankingcatwethard_0b570100_7:

    # moon "Too hard!"
    moon "太大力了!"

# game/scripts/bar.rpy:919
translate schinese moonspankingcatwethard_4a0e8d32_3:

    # moon "AHH...!"
    moon "啊......!"

# game/scripts/bar.rpy:928
translate schinese moonspankingcathard_d7b8ec89:

    # moon "Gah!"
    moon "咕!"

# game/scripts/bar.rpy:930
translate schinese moonspankingcathard_030c8ab8:

    # moon "HNNG!"
    moon "嗯!"

# game/scripts/bar.rpy:932
translate schinese moonspankingcathard_0b570100:

    # moon "Too hard!"
    moon "太大力了!"

# game/scripts/bar.rpy:934
translate schinese moonspankingcathard_bc959e7e:

    # moon "Softer, please!"
    moon "轻一点,求你了!"

# game/scripts/bar.rpy:936
translate schinese moonspankingcathard_b268297c:

    # moon "AHH!"
    moon "啊!"

# game/scripts/bar.rpy:942
translate schinese moonspankingcathard_1bdca328:

    # moon "EGH!"
    moon "呃!"

# game/scripts/bar.rpy:944
translate schinese moonspankingcathard_b84f908b:

    # moon "Hnnguh!"
    moon "嗯!"

# game/scripts/bar.rpy:946
translate schinese moonspankingcathard_b70451e4:

    # moon "Ouch!"
    moon "嗷!"

# game/scripts/bar.rpy:948
translate schinese moonspankingcathard_0b570100_1:

    # moon "Too hard!"
    moon "太大力了!"

# game/scripts/bar.rpy:950
translate schinese moonspankingcathard_4a0e8d32:

    # moon "AHH...!"
    moon "啊......!"

# game/scripts/bar.rpy:957
translate schinese moonspankingcathard_d7b8ec89_1:

    # moon "Gah!"
    moon "咕!"

# game/scripts/bar.rpy:959
translate schinese moonspankingcathard_030c8ab8_1:

    # moon "HNNG!"
    moon "嗯!"

# game/scripts/bar.rpy:961
translate schinese moonspankingcathard_0b570100_2:

    # moon "Too hard!"
    moon "太大力了!"

# game/scripts/bar.rpy:963
translate schinese moonspankingcathard_bc959e7e_1:

    # moon "Softer, please!"
    moon "轻一点,求你了!"

# game/scripts/bar.rpy:965
translate schinese moonspankingcathard_b268297c_1:

    # moon "AHH!"
    moon "啊!"

# game/scripts/bar.rpy:971
translate schinese moonspankingcathard_1bdca328_1:

    # moon "EGH!"
    moon "呃!"

# game/scripts/bar.rpy:973
translate schinese moonspankingcathard_b84f908b_1:

    # moon "Hnnguh!"
    moon "嗯!"

# game/scripts/bar.rpy:975
translate schinese moonspankingcathard_b70451e4_1:

    # moon "Ouch!"
    moon "嗷!"

# game/scripts/bar.rpy:977
translate schinese moonspankingcathard_0b570100_3:

    # moon "Too hard!"
    moon "太大力了!"

# game/scripts/bar.rpy:979
translate schinese moonspankingcathard_4a0e8d32_1:

    # moon "AHH...!"
    moon "啊......!"

# game/scripts/bar.rpy:986
translate schinese moonspankingcathard_d7b8ec89_2:

    # moon "Gah!"
    moon "咕!"

# game/scripts/bar.rpy:988
translate schinese moonspankingcathard_030c8ab8_2:

    # moon "HNNG!"
    moon "嗯!"

# game/scripts/bar.rpy:990
translate schinese moonspankingcathard_0b570100_4:

    # moon "Too hard!"
    moon "太大力了!"

# game/scripts/bar.rpy:992
translate schinese moonspankingcathard_bc959e7e_2:

    # moon "Softer, please!"
    moon "轻一点,求你了!"

# game/scripts/bar.rpy:994
translate schinese moonspankingcathard_b268297c_2:

    # moon "AHH!"
    moon "啊!"

# game/scripts/bar.rpy:1000
translate schinese moonspankingcathard_1bdca328_2:

    # moon "EGH!"
    moon "呃!"

# game/scripts/bar.rpy:1002
translate schinese moonspankingcathard_b84f908b_2:

    # moon "Hnnguh!"
    moon "嗯!"

# game/scripts/bar.rpy:1004
translate schinese moonspankingcathard_b70451e4_2:

    # moon "Ouch!"
    moon "嗷!"

# game/scripts/bar.rpy:1006
translate schinese moonspankingcathard_0b570100_5:

    # moon "Too hard!"
    moon "太大力了!"

# game/scripts/bar.rpy:1008
translate schinese moonspankingcathard_4a0e8d32_2:

    # moon "AHH...!"
    moon "啊......!"

# game/scripts/bar.rpy:1015
translate schinese moonspankingcathard_d7b8ec89_3:

    # moon "Gah!"
    moon "咕!"

# game/scripts/bar.rpy:1017
translate schinese moonspankingcathard_030c8ab8_3:

    # moon "HNNG!"
    moon "嗯!"

# game/scripts/bar.rpy:1019
translate schinese moonspankingcathard_0b570100_6:

    # moon "Too hard!"
    moon "太大力了!"

# game/scripts/bar.rpy:1021
translate schinese moonspankingcathard_bc959e7e_3:

    # moon "Softer, please!"
    moon "轻一点,求你了!"

# game/scripts/bar.rpy:1023
translate schinese moonspankingcathard_b268297c_3:

    # moon "AHH!"
    moon "啊!"

# game/scripts/bar.rpy:1029
translate schinese moonspankingcathard_1bdca328_3:

    # moon "EGH!"
    moon "呃!"

# game/scripts/bar.rpy:1031
translate schinese moonspankingcathard_b84f908b_3:

    # moon "Hnnguh!"
    moon "嗯!"

# game/scripts/bar.rpy:1033
translate schinese moonspankingcathard_b70451e4_3:

    # moon "Ouch!"
    moon "嗷!"

# game/scripts/bar.rpy:1035
translate schinese moonspankingcathard_0b570100_7:

    # moon "Too hard!"
    moon "太大力了!"

# game/scripts/bar.rpy:1037
translate schinese moonspankingcathard_4a0e8d32_3:

    # moon "AHH...!"
    moon "啊......!"

# game/scripts/bar.rpy:1048
translate schinese moonspankingcat_2726388d:

    # moon "Ahn..."
    moon "啊......"

# game/scripts/bar.rpy:1050
translate schinese moonspankingcat_3f97d95f:

    # moon "Hnnn..."
    moon "嗯......"

# game/scripts/bar.rpy:1052
translate schinese moonspankingcat_d7b8ec89:

    # moon "Gah!"
    moon "咕!"

# game/scripts/bar.rpy:1054
translate schinese moonspankingcat_3b71fc85:

    # moon "Fffuh!"
    moon "操!"

# game/scripts/bar.rpy:1056
translate schinese moonspankingcat_48ca5d2e:

    # moon "Nn..."
    moon "嗯......"

# game/scripts/bar.rpy:1062
translate schinese moonspankingcat_d1ae4883:

    # moon "Ahhh..."
    moon "啊......"

# game/scripts/bar.rpy:1064
translate schinese moonspankingcat_f20a25fa:

    # moon "Ssss!"
    moon "嘶!"

# game/scripts/bar.rpy:1066
translate schinese moonspankingcat_b84f908b:

    # moon "Hnnguh!"
    moon "嗯!"

# game/scripts/bar.rpy:1068
translate schinese moonspankingcat_596fc351:

    # moon "Eee!"
    moon "噫!"

# game/scripts/bar.rpy:1070
translate schinese moonspankingcat_7cb79743:

    # moon "Ah...!"
    moon "啊......!"

# game/scripts/bar.rpy:1078
translate schinese moonspankingcat_2726388d_1:

    # moon "Ahn..."
    moon "啊......"

# game/scripts/bar.rpy:1080
translate schinese moonspankingcat_3f97d95f_1:

    # moon "Hnnn..."
    moon "嗯......"

# game/scripts/bar.rpy:1082
translate schinese moonspankingcat_d7b8ec89_1:

    # moon "Gah!"
    moon "咕!"

# game/scripts/bar.rpy:1084
translate schinese moonspankingcat_3b71fc85_1:

    # moon "Fffuh!"
    moon "操!"

# game/scripts/bar.rpy:1086
translate schinese moonspankingcat_48ca5d2e_1:

    # moon "Nn..."
    moon "嗯......"

# game/scripts/bar.rpy:1092
translate schinese moonspankingcat_d1ae4883_1:

    # moon "Ahhh..."
    moon "啊......"

# game/scripts/bar.rpy:1094
translate schinese moonspankingcat_f20a25fa_1:

    # moon "Ssss!"
    moon "嘶!"

# game/scripts/bar.rpy:1096
translate schinese moonspankingcat_b84f908b_1:

    # moon "Hnnguh!"
    moon "嗯!"

# game/scripts/bar.rpy:1098
translate schinese moonspankingcat_596fc351_1:

    # moon "Eee!"
    moon "噫!"

# game/scripts/bar.rpy:1100
translate schinese moonspankingcat_7cb79743_1:

    # moon "Ah...!"
    moon "啊......!"

# game/scripts/bar.rpy:1108
translate schinese moonspankingcat_2726388d_2:

    # moon "Ahn..."
    moon "啊......"

# game/scripts/bar.rpy:1110
translate schinese moonspankingcat_3f97d95f_2:

    # moon "Hnnn..."
    moon "嗯......"

# game/scripts/bar.rpy:1112
translate schinese moonspankingcat_d7b8ec89_2:

    # moon "Gah!"
    moon "咕!"

# game/scripts/bar.rpy:1114
translate schinese moonspankingcat_3b71fc85_2:

    # moon "Fffuh!"
    moon "操!"

# game/scripts/bar.rpy:1116
translate schinese moonspankingcat_48ca5d2e_2:

    # moon "Nn..."
    moon "嗯......"

# game/scripts/bar.rpy:1122
translate schinese moonspankingcat_d1ae4883_2:

    # moon "Ahhh..."
    moon "啊......"

# game/scripts/bar.rpy:1124
translate schinese moonspankingcat_f20a25fa_2:

    # moon "Ssss!"
    moon "嘶!"

# game/scripts/bar.rpy:1126
translate schinese moonspankingcat_b84f908b_2:

    # moon "Hnnguh!"
    moon "嗯!"

# game/scripts/bar.rpy:1128
translate schinese moonspankingcat_596fc351_2:

    # moon "Eee!"
    moon "噫!"

# game/scripts/bar.rpy:1130
translate schinese moonspankingcat_7cb79743_2:

    # moon "Ah...!"
    moon "啊......!"

# game/scripts/bar.rpy:1140
translate schinese moonspankingcatwet_2726388d:

    # moon "Ahn..."
    moon "啊......"

# game/scripts/bar.rpy:1142
translate schinese moonspankingcatwet_3f97d95f:

    # moon "Hnnn..."
    moon "嗯......"

# game/scripts/bar.rpy:1144
translate schinese moonspankingcatwet_d7b8ec89:

    # moon "Gah!"
    moon "咕!"

# game/scripts/bar.rpy:1146
translate schinese moonspankingcatwet_3b71fc85:

    # moon "Fffuh!"
    moon "操!"

# game/scripts/bar.rpy:1148
translate schinese moonspankingcatwet_48ca5d2e:

    # moon "Nn..."
    moon "嗯......"

# game/scripts/bar.rpy:1154
translate schinese moonspankingcatwet_d1ae4883:

    # moon "Ahhh..."
    moon "啊......"

# game/scripts/bar.rpy:1156
translate schinese moonspankingcatwet_f20a25fa:

    # moon "Ssss!"
    moon "嘶!"

# game/scripts/bar.rpy:1158
translate schinese moonspankingcatwet_b84f908b:

    # moon "Hnnguh!"
    moon "嗯!"

# game/scripts/bar.rpy:1160
translate schinese moonspankingcatwet_596fc351:

    # moon "Eee!"
    moon "噫!"

# game/scripts/bar.rpy:1162
translate schinese moonspankingcatwet_7cb79743:

    # moon "Ah...!"
    moon "啊......!"

# game/scripts/bar.rpy:1170
translate schinese moonspankingcatwet_2726388d_1:

    # moon "Ahn..."
    moon "啊......"

# game/scripts/bar.rpy:1172
translate schinese moonspankingcatwet_3f97d95f_1:

    # moon "Hnnn..."
    moon "嗯......"

# game/scripts/bar.rpy:1174
translate schinese moonspankingcatwet_d7b8ec89_1:

    # moon "Gah!"
    moon "咕!"

# game/scripts/bar.rpy:1176
translate schinese moonspankingcatwet_3b71fc85_1:

    # moon "Fffuh!"
    moon "操!"

# game/scripts/bar.rpy:1178
translate schinese moonspankingcatwet_48ca5d2e_1:

    # moon "Nn..."
    moon "嗯......"

# game/scripts/bar.rpy:1184
translate schinese moonspankingcatwet_d1ae4883_1:

    # moon "Ahhh..."
    moon "啊......"

# game/scripts/bar.rpy:1186
translate schinese moonspankingcatwet_f20a25fa_1:

    # moon "Ssss!"
    moon "嘶!"

# game/scripts/bar.rpy:1188
translate schinese moonspankingcatwet_b84f908b_1:

    # moon "Hnnguh!"
    moon "嗯!"

# game/scripts/bar.rpy:1190
translate schinese moonspankingcatwet_596fc351_1:

    # moon "Eee!"
    moon "噫!"

# game/scripts/bar.rpy:1192
translate schinese moonspankingcatwet_7cb79743_1:

    # moon "Ah...!"
    moon "啊......!"

# game/scripts/bar.rpy:1200
translate schinese moonspankingcatwet_2726388d_2:

    # moon "Ahn..."
    moon "啊......"

# game/scripts/bar.rpy:1202
translate schinese moonspankingcatwet_3f97d95f_2:

    # moon "Hnnn..."
    moon "嗯......"

# game/scripts/bar.rpy:1204
translate schinese moonspankingcatwet_d7b8ec89_2:

    # moon "Gah!"
    moon "咕!"

# game/scripts/bar.rpy:1206
translate schinese moonspankingcatwet_3b71fc85_2:

    # moon "Fffuh!"
    moon "操!"

# game/scripts/bar.rpy:1208
translate schinese moonspankingcatwet_48ca5d2e_2:

    # moon "Nn..."
    moon "嗯......"

# game/scripts/bar.rpy:1214
translate schinese moonspankingcatwet_d1ae4883_2:

    # moon "Ahhh..."
    moon "啊......"

# game/scripts/bar.rpy:1216
translate schinese moonspankingcatwet_f20a25fa_2:

    # moon "Ssss!"
    moon "嘶!"

# game/scripts/bar.rpy:1218
translate schinese moonspankingcatwet_b84f908b_2:

    # moon "Hnnguh!"
    moon "嗯!"

# game/scripts/bar.rpy:1220
translate schinese moonspankingcatwet_596fc351_2:

    # moon "Eee!"
    moon "噫!"

# game/scripts/bar.rpy:1222
translate schinese moonspankingcatwet_7cb79743_2:

    # moon "Ah...!"
    moon "啊......!"

# game/scripts/bar.rpy:1233
translate schinese moonspankingwaitresshard_d7b8ec89:

    # moon "Gah!"
    moon "咕!"

# game/scripts/bar.rpy:1235
translate schinese moonspankingwaitresshard_030c8ab8:

    # moon "HNNG!"
    moon "嗯!"

# game/scripts/bar.rpy:1237
translate schinese moonspankingwaitresshard_0b570100:

    # moon "Too hard!"
    moon "太大力了!"

# game/scripts/bar.rpy:1239
translate schinese moonspankingwaitresshard_bc959e7e:

    # moon "Softer, please!"
    moon "轻一点,求你了!"

# game/scripts/bar.rpy:1241
translate schinese moonspankingwaitresshard_b268297c:

    # moon "AHH!"
    moon "啊!"

# game/scripts/bar.rpy:1247
translate schinese moonspankingwaitresshard_1bdca328:

    # moon "EGH!"
    moon "呃!"

# game/scripts/bar.rpy:1249
translate schinese moonspankingwaitresshard_b84f908b:

    # moon "Hnnguh!"
    moon "嗯!"

# game/scripts/bar.rpy:1251
translate schinese moonspankingwaitresshard_b70451e4:

    # moon "Ouch!"
    moon "嗷!"

# game/scripts/bar.rpy:1253
translate schinese moonspankingwaitresshard_0b570100_1:

    # moon "Too hard!"
    moon "太大力了!"

# game/scripts/bar.rpy:1255
translate schinese moonspankingwaitresshard_4a0e8d32:

    # moon "AHH...!"
    moon "啊......!"

# game/scripts/bar.rpy:1262
translate schinese moonspankingwaitresshard_d7b8ec89_1:

    # moon "Gah!"
    moon "咕!"

# game/scripts/bar.rpy:1264
translate schinese moonspankingwaitresshard_030c8ab8_1:

    # moon "HNNG!"
    moon "嗯!"

# game/scripts/bar.rpy:1266
translate schinese moonspankingwaitresshard_0b570100_2:

    # moon "Too hard!"
    moon "太大力了!"

# game/scripts/bar.rpy:1268
translate schinese moonspankingwaitresshard_bc959e7e_1:

    # moon "Softer, please!"
    moon "轻一点,求你了!"

# game/scripts/bar.rpy:1270
translate schinese moonspankingwaitresshard_b268297c_1:

    # moon "AHH!"
    moon "啊!"

# game/scripts/bar.rpy:1276
translate schinese moonspankingwaitresshard_1bdca328_1:

    # moon "EGH!"
    moon "呃!"

# game/scripts/bar.rpy:1278
translate schinese moonspankingwaitresshard_b84f908b_1:

    # moon "Hnnguh!"
    moon "嗯!"

# game/scripts/bar.rpy:1280
translate schinese moonspankingwaitresshard_b70451e4_1:

    # moon "Ouch!"
    moon "嗷!"

# game/scripts/bar.rpy:1282
translate schinese moonspankingwaitresshard_0b570100_3:

    # moon "Too hard!"
    moon "太大力了!"

# game/scripts/bar.rpy:1284
translate schinese moonspankingwaitresshard_4a0e8d32_1:

    # moon "AHH...!"
    moon "啊......!"

# game/scripts/bar.rpy:1291
translate schinese moonspankingwaitresshard_d7b8ec89_2:

    # moon "Gah!"
    moon "咕!"

# game/scripts/bar.rpy:1293
translate schinese moonspankingwaitresshard_030c8ab8_2:

    # moon "HNNG!"
    moon "嗯!"

# game/scripts/bar.rpy:1295
translate schinese moonspankingwaitresshard_0b570100_4:

    # moon "Too hard!"
    moon "太大力了!"

# game/scripts/bar.rpy:1297
translate schinese moonspankingwaitresshard_bc959e7e_2:

    # moon "Softer, please!"
    moon "轻一点,求你了!"

# game/scripts/bar.rpy:1299
translate schinese moonspankingwaitresshard_b268297c_2:

    # moon "AHH!"
    moon "啊!"

# game/scripts/bar.rpy:1305
translate schinese moonspankingwaitresshard_1bdca328_2:

    # moon "EGH!"
    moon "呃!"

# game/scripts/bar.rpy:1307
translate schinese moonspankingwaitresshard_b84f908b_2:

    # moon "Hnnguh!"
    moon "嗯!"

# game/scripts/bar.rpy:1309
translate schinese moonspankingwaitresshard_b70451e4_2:

    # moon "Ouch!"
    moon "嗷!"

# game/scripts/bar.rpy:1311
translate schinese moonspankingwaitresshard_0b570100_5:

    # moon "Too hard!"
    moon "太大力了!"

# game/scripts/bar.rpy:1313
translate schinese moonspankingwaitresshard_4a0e8d32_2:

    # moon "AHH...!"
    moon "啊......!"

# game/scripts/bar.rpy:1320
translate schinese moonspankingwaitresshard_d7b8ec89_3:

    # moon "Gah!"
    moon "咕!"

# game/scripts/bar.rpy:1322
translate schinese moonspankingwaitresshard_030c8ab8_3:

    # moon "HNNG!"
    moon "嗯!"

# game/scripts/bar.rpy:1324
translate schinese moonspankingwaitresshard_0b570100_6:

    # moon "Too hard!"
    moon "太大力了!"

# game/scripts/bar.rpy:1326
translate schinese moonspankingwaitresshard_bc959e7e_3:

    # moon "Softer, please!"
    moon "轻一点,求你了!"

# game/scripts/bar.rpy:1328
translate schinese moonspankingwaitresshard_b268297c_3:

    # moon "AHH!"
    moon "啊!"

# game/scripts/bar.rpy:1334
translate schinese moonspankingwaitresshard_1bdca328_3:

    # moon "EGH!"
    moon "呃!"

# game/scripts/bar.rpy:1336
translate schinese moonspankingwaitresshard_b84f908b_3:

    # moon "Hnnguh!"
    moon "嗯!"

# game/scripts/bar.rpy:1338
translate schinese moonspankingwaitresshard_b70451e4_3:

    # moon "Ouch!"
    moon "嗷!"

# game/scripts/bar.rpy:1340
translate schinese moonspankingwaitresshard_0b570100_7:

    # moon "Too hard!"
    moon "太大力了!"

# game/scripts/bar.rpy:1342
translate schinese moonspankingwaitresshard_4a0e8d32_3:

    # moon "AHH...!"
    moon "啊......!"

# game/scripts/bar.rpy:1354
translate schinese moonspankingwaitress_2726388d:

    # moon "Ahn..."
    moon "啊......"

# game/scripts/bar.rpy:1356
translate schinese moonspankingwaitress_3f97d95f:

    # moon "Hnnn..."
    moon "嗯......"

# game/scripts/bar.rpy:1358
translate schinese moonspankingwaitress_d7b8ec89:

    # moon "Gah!"
    moon "咕!"

# game/scripts/bar.rpy:1360
translate schinese moonspankingwaitress_3b71fc85:

    # moon "Fffuh!"
    moon "操!"

# game/scripts/bar.rpy:1362
translate schinese moonspankingwaitress_48ca5d2e:

    # moon "Nn..."
    moon "嗯......"

# game/scripts/bar.rpy:1368
translate schinese moonspankingwaitress_d1ae4883:

    # moon "Ahhh..."
    moon "啊......"

# game/scripts/bar.rpy:1370
translate schinese moonspankingwaitress_f20a25fa:

    # moon "Ssss!"
    moon "嘶!"

# game/scripts/bar.rpy:1372
translate schinese moonspankingwaitress_b84f908b:

    # moon "Hnnguh!"
    moon "嗯!"

# game/scripts/bar.rpy:1374
translate schinese moonspankingwaitress_596fc351:

    # moon "Eee!"
    moon "噫!"

# game/scripts/bar.rpy:1376
translate schinese moonspankingwaitress_7cb79743:

    # moon "Ah...!"
    moon "啊......!"

# game/scripts/bar.rpy:1384
translate schinese moonspankingwaitress_2726388d_1:

    # moon "Ahn..."
    moon "啊......"

# game/scripts/bar.rpy:1386
translate schinese moonspankingwaitress_3f97d95f_1:

    # moon "Hnnn..."
    moon "嗯......"

# game/scripts/bar.rpy:1388
translate schinese moonspankingwaitress_d7b8ec89_1:

    # moon "Gah!"
    moon "咕!"

# game/scripts/bar.rpy:1390
translate schinese moonspankingwaitress_3b71fc85_1:

    # moon "Fffuh!"
    moon "操!"

# game/scripts/bar.rpy:1392
translate schinese moonspankingwaitress_48ca5d2e_1:

    # moon "Nn..."
    moon "嗯......"

# game/scripts/bar.rpy:1398
translate schinese moonspankingwaitress_d1ae4883_1:

    # moon "Ahhh..."
    moon "啊......"

# game/scripts/bar.rpy:1400
translate schinese moonspankingwaitress_f20a25fa_1:

    # moon "Ssss!"
    moon "嘶!"

# game/scripts/bar.rpy:1402
translate schinese moonspankingwaitress_b84f908b_1:

    # moon "Hnnguh!"
    moon "嗯!"

# game/scripts/bar.rpy:1404
translate schinese moonspankingwaitress_596fc351_1:

    # moon "Eee!"
    moon "噫!"

# game/scripts/bar.rpy:1406
translate schinese moonspankingwaitress_7cb79743_1:

    # moon "Ah...!"
    moon "啊......!"

# game/scripts/bar.rpy:1414
translate schinese moonspankingwaitress_2726388d_2:

    # moon "Ahn..."
    moon "啊......"

# game/scripts/bar.rpy:1416
translate schinese moonspankingwaitress_3f97d95f_2:

    # moon "Hnnn..."
    moon "嗯......"

# game/scripts/bar.rpy:1418
translate schinese moonspankingwaitress_d7b8ec89_2:

    # moon "Gah!"
    moon "咕!"

# game/scripts/bar.rpy:1420
translate schinese moonspankingwaitress_3b71fc85_2:

    # moon "Fffuh!"
    moon "操!"

# game/scripts/bar.rpy:1422
translate schinese moonspankingwaitress_48ca5d2e_2:

    # moon "Nn..."
    moon "嗯......"

# game/scripts/bar.rpy:1428
translate schinese moonspankingwaitress_d1ae4883_2:

    # moon "Ahhh..."
    moon "啊......"

# game/scripts/bar.rpy:1430
translate schinese moonspankingwaitress_f20a25fa_2:

    # moon "Ssss!"
    moon "嘶!"

# game/scripts/bar.rpy:1432
translate schinese moonspankingwaitress_b84f908b_2:

    # moon "Hnnguh!"
    moon "嗯!"

# game/scripts/bar.rpy:1434
translate schinese moonspankingwaitress_596fc351_2:

    # moon "Eee!"
    moon "噫!"

# game/scripts/bar.rpy:1436
translate schinese moonspankingwaitress_7cb79743_2:

    # moon "Ah...!"
    moon "啊......!"

# game/scripts/bar.rpy:1443
translate schinese moonspa_17cca8de:

    # m "Hey. Gonna close up now, you about to head out?"
    m "嘿.我们要打样了,你要出去了吗?"

# game/scripts/bar.rpy:1444
translate schinese moonspa_caba381f:

    # moon "Ehhrrrnn..."
    moon "呃......"

# game/scripts/bar.rpy:1445
translate schinese moonspa_944e92ed:

    # m "Everything okay?"
    m "你还好吗?"

# game/scripts/bar.rpy:1446
translate schinese moonspa_5c7db9a1:

    # moon "It's nothing... Work was just hard today..."
    moon "没事......只是今天工作有点累......"

# game/scripts/bar.rpy:1448
translate schinese moonspa_85e03706:

    # c "Do you need some time off?"
    c "你需要休息会吗?"

# game/scripts/bar.rpy:1449
translate schinese moonspa_f5a89e5c:

    # m "Oh, hey!"
    m "噢,嘿!"

# game/scripts/bar.rpy:1450
translate schinese moonspa_73c0e08d:

    # moon "No, no... I'm fine. I'm just really tired..."
    moon "不,不是......我没事.我只是真的很累......"

# game/scripts/bar.rpy:1451
translate schinese moonspa_39a7b02f:

    # c "Why don't you relax in the hot springs with me? You'll feel much better."
    c "不如陪我去温泉里放松一下?你会感觉好很多的."

# game/scripts/bar.rpy:1452
translate schinese moonspa_08ae6070:

    # moon "Thank you Mr. [name]... But I couldn't do that... I'm not a resident here."
    moon "谢谢你[name]先生......但我不能那么做......我不是这里的居民."

# game/scripts/bar.rpy:1453
translate schinese moonspa_0495833a:

    # c "Who's going to tell you to leave? I'm the boss here."
    c "谁会让你离开呢?我才是这里的老板."

# game/scripts/bar.rpy:1454
translate schinese moonspa_212fcfa6:

    # moon "Well, that is true..."
    moon "嗯,那确实......"

# game/scripts/bar.rpy:1455
translate schinese moonspa_81bfb5b3:

    # moon "I don't have a swimsuit with me anyway..."
    moon "但是我也没有带泳衣......"

# game/scripts/bar.rpy:1457
translate schinese moonspa_8e7a74ab:

    # m "I've got a spare! Just use mine!"
    m "我有一件备用的!就用我的吧!"

# game/scripts/bar.rpy:1458
translate schinese moonspa_95ee0a5b:

    # moon "I couldn't wear your clothes!"
    moon "我不能穿你的衣服!"

# game/scripts/bar.rpy:1459
translate schinese moonspa_d05e41aa:

    # m "Oh come on, I'm sure I have something that can fit you."
    m "噢来嘛,我肯定有合你身的东西."

# game/scripts/bar.rpy:1460
translate schinese moonspa_bece692e:

    # moon "But they're your clothes! I would never ask to wear them!"
    moon "但是它们是你的衣服!我不会去穿它们的!"

# game/scripts/bar.rpy:1461
translate schinese moonspa_289ed337:

    # m "Quit being so nice and go relax..."
    m "别当好好小姐了,去放松下吧......"

# game/scripts/bar.rpy:1462
translate schinese moonspa_72ccbec9:

    # moon "...Thank you, Maria..."
    moon "......谢谢你,玛丽亚......"

# game/scripts/bar.rpy:1464
translate schinese moonspa_4ae7e13c:

    # m "Come on, go wait outside. I'll bring her something to wear."
    m "来吧,去外面等着.我去给她拿点东西穿."

# game/scripts/bar.rpy:1466
translate schinese moonspa_abe57e2f:

    # "You leave the bar and change into your swim clothes. Not long after, Moon arrives..."
    "你离开了酒吧换上了你的泳衣.没过多久,文就过来了......"

# game/scripts/bar.rpy:1468
translate schinese moonspa_d99f55b4:

    # moon "Wow... The stars are really pretty here. It reminds me of home..."
    moon "哇哦......这里的星星真的很美.这让我想起了我的家......"

# game/scripts/bar.rpy:1470
translate schinese moonspa_fde53961:

    # moon "Thank you for bringing me here Mr. [name], It's beautiful."
    moon "谢谢你带我过来,[name]先生,这里真美."

# game/scripts/bar.rpy:1471
translate schinese moonspa_3087afff:

    # c "Just [name] is fine."
    c "叫[name]就行了."

# game/scripts/bar.rpy:1472
translate schinese moonspa_ddc2cf7a:

    # moon "Hmm... Okay."
    moon "嗯......好的."

# game/scripts/bar.rpy:1474
translate schinese moonspa_e5407e6b:

    # moon "Up for a challenge, [name]?"
    moon "准备好迎接挑战了吗,[name]?"

# game/scripts/bar.rpy:1475
translate schinese moonspa_bb00a52c:

    # c "What do you mean?"
    c "你这是什么意思?"

# game/scripts/bar.rpy:1476
translate schinese moonspa_e2b44ae3:

    # moon "Let's climb! The view will be better up there."
    moon "我们爬上去吧!上面风景更好."

# game/scripts/bar.rpy:1477
translate schinese moonspa_39cde236:

    # c "There are stairs right there..."
    c "那里有楼梯......"

# game/scripts/bar.rpy:1478
translate schinese moonspa_d56468c4:

    # moon "Stairs are boring..."
    moon "走楼梯太无聊了......"

# game/scripts/bar.rpy:1480
translate schinese moonspa_5f9e35cf:

    # moon "Come on, grab my hand!"
    moon "来吧,抓住我的手!"

# game/scripts/bar.rpy:1481
translate schinese moonspa_f238ed27:

    # c "This is so dangerous..."
    c "这太危险了......"

# game/scripts/bar.rpy:1482
translate schinese moonspa_02707f78:

    # moon "Stop being a pussy!"
    moon "别像个懦夫一样!"

# game/scripts/bar.rpy:1484
translate schinese moonspa_7fe859db:

    # moon "Amazing... See? I told you. We have a beautiful view of both moons!"
    moon "太美了......看到了吗?我都告诉过你了.两个月都很好看!"

# game/scripts/bar.rpy:1485
translate schinese moonspa_37cc56d9:

    # c "Three."
    c "三个."

# game/scripts/bar.rpy:1486
translate schinese moonspa_ad85b0e9:

    # moon "Hm?"
    moon "嗯?"

# game/scripts/bar.rpy:1487
translate schinese moonspa_e40a79c1:

    # c "Three beautiful Moons."
    c "三个美丽的月(韩文姓氏中'文'的英文也是Moon)."

# game/scripts/bar.rpy:1488
translate schinese moonspa_4984c9d6:

    # moon "..."
    moon "......"

# game/scripts/bar.rpy:1489
translate schinese moonspa_c86aaa5e:

    # moon "Smooth."
    moon "油嘴滑舌."

# game/scripts/bar.rpy:1493
translate schinese moonspa_a1a134a8:

    # moon "I used to visit hot springs all the time back home. I really missed this feeling..."
    moon "我在家里的时候经常会去泡温泉.我真的很怀念这种感觉......"

# game/scripts/bar.rpy:1495
translate schinese moonspa_804da753:

    # moon "And it's so quiet here... It's always so loud in the bar."
    moon "而且这里很安静......酒吧里一直很吵."

# game/scripts/bar.rpy:1496
translate schinese moonspa_e1f86f84:

    # moon "There's always someone trying to start a fight..."
    moon "总有人试图挑起争端......"

# game/scripts/bar.rpy:1497
translate schinese moonspa_2e27572b:

    # c "And you've been stopping them?"
    c "你一直在阻止他们吗?"

# game/scripts/bar.rpy:1499
translate schinese moonspa_d070582c:

    # moon "Of course! It's my job, isn't it?"
    moon "当然了!这是我的工作,对吧?"

# game/scripts/bar.rpy:1500
translate schinese moonspa_ff80205a:

    # c "Not really."
    c "不太是."

# game/scripts/bar.rpy:1501
translate schinese moonspa_e9b19af9:

    # moon "Well, I don't mind... Nobody really sees me coming anyway. They never expect such a small girl to have such great strength."
    moon "嗯,我不介意......反正也没人指望我过去.他们从来没想到这么小的一个女孩会有这么大的力量."

# game/scripts/bar.rpy:1502
translate schinese moonspa_4e40ede4:

    # c "You don't look very strong."
    c "你看起来也不是很强壮."

# game/scripts/bar.rpy:1503
translate schinese moonspa_fb59d847:

    # moon "Exactly why they don't expect it Mr-, I mean [name]!"
    moon "这也是为什么他们没有料到,[name]先-,我是说,[name]!"

# game/scripts/bar.rpy:1504
translate schinese moonspa_94050268:

    # moon "Remember the other day when I mentioned living in a forest?"
    moon "记得那天我提到我住在森林里吗?"

# game/scripts/bar.rpy:1506
translate schinese moonspa_92306967:

    # moon "Or... Is it called a jungle? Tree land?"
    moon "或者说......它叫丛林?绿地?"

# game/scripts/bar.rpy:1508
translate schinese moonspa_5c397a69:

    # moon "This language isn't my first..."
    moon "这不是我的母语......"

# game/scripts/bar.rpy:1510
translate schinese moonspa_ca412b3b:

    # moon "Well anyway, my father and I lived in a very secluded area..."
    moon "嗯,不管怎么说,我爸爸和我住在一个很隐蔽的地方......"

# game/scripts/bar.rpy:1511
translate schinese moonspa_ee99ed39:

    # c "And your mother?"
    c "你妈妈呢?"

# game/scripts/bar.rpy:1512
translate schinese moonspa_0e4ddb18:

    # moon "I never got to meet her..."
    moon "我从来没见过她......"

# game/scripts/bar.rpy:1513
translate schinese moonspa_ef2fa0da:

    # moon "The closest area of civilization to where we lived was half a day's walk, so going there for food was very unreasonable."
    moon "离我们住的地方最近的文明地区也要走半天的路,所以去那里吃东西是很不合理的."

# game/scripts/bar.rpy:1514
translate schinese moonspa_5ae529b7:

    # moon "Because of this, my father taught me how to hunt, fish, and fight! Among many other skills I needed to survive in the wilderness."
    moon "正因为如此,我爸爸教会了我如何打猎,捕鱼和战斗!还有其他我在荒野中生存所需要的许多技能."

# game/scripts/bar.rpy:1515
translate schinese moonspa_9ee11a95:

    # moon "Oh, and of course he taught me how to climb. There were many mountains and trees where I lived."
    moon "噢,当然了,他也教了我怎么攀爬.我住的地方有很多山和树."

# game/scripts/bar.rpy:1517
translate schinese moonspa_4b487cd3:

    # moon "Here's a tip for you, [name]! Don't try climbing a mountain when it's raining. I broke a few too many bones that way..."
    moon "给你一个建议,[name]!下雨的时候不要尝试去爬山.我因为那样断了好几条骨头......"

# game/scripts/bar.rpy:1519
translate schinese moonspa_86b6144b:

    # moon "Naturally, most of our food lived atop trees, like monkeys! I never could swing like they did, but I could sneak up on them!"
    moon "自然,我们的大部分食物都住在树上,比如猴子!虽然我永远不可能像他们那样荡来荡去,但我可以偷偷摸摸地接近他们!"

# game/scripts/bar.rpy:1520
translate schinese moonspa_d766178e:

    # c "Monkeys?"
    c "猴子?"

# game/scripts/bar.rpy:1522
translate schinese moonspa_d715e0b5:

    # moon "They're like little furry humans."
    moon "他们就像小的毛茸茸的人类."

# game/scripts/bar.rpy:1523
translate schinese moonspa_a2ddb870:

    # c "But you ate them?"
    c "但是你吃它们?"

# game/scripts/bar.rpy:1524
translate schinese moonspa_0c2388a8:

    # moon "Yes? Have you never eaten monkey?"
    moon "怎么了?你没吃过猴子吗?"

# game/scripts/bar.rpy:1525
translate schinese moonspa_09944933:

    # c "I'm the one that should be looking at you like that."
    c "这个眼神应该是我来看你才对."

# game/scripts/bar.rpy:1527
translate schinese moonspa_aa51627d:

    # moon "Hah, well I guess so. I've never seen any monkey for sale here."
    moon "哈哈,我想也是.我从来没见过这里卖过猴子."

# game/scripts/bar.rpy:1529
translate schinese moonspa_1c262293:

    # moon "I hope you don't think I'm crazy..."
    moon "我希望你不要认为我疯了......"

# game/scripts/bar.rpy:1531
translate schinese moonspa_c798f3fd:

    # moon "Well at least not completely crazy!"
    moon "嗯,至少没有完全疯了!"

# game/scripts/bar.rpy:1533
translate schinese moonspa_06c5ae72:

    # moon "I learned what was needed to survive."
    moon "我学到了生存所需的东西."

# game/scripts/bar.rpy:1534
translate schinese moonspa_4837d4c3:

    # c "Coming to civilization must have been a shock."
    c "来到文明世界对你来说一定是一个很大的冲击."

# game/scripts/bar.rpy:1535
translate schinese moonspa_36f13a9c:

    # moon "I remember the day very well, [name]. I was twelve years old when my father was able to move us over here."
    moon "我还非常清楚地记得那一天,[name].我十二岁的时候,爸爸就把我们搬到这里来了."

# game/scripts/bar.rpy:1536
translate schinese moonspa_d6469deb:

    # c "But you were arrested?"
    c "但是你们被抓了?"

# game/scripts/bar.rpy:1537
translate schinese moonspa_29a87386:

    # moon "We didn't know the language... Honestly, we didn't know there {i}were{/i} other languages!"
    moon "我们不知道这里的语言......实际上,我们都不知道{i}有{/i}其他的语言!"

# game/scripts/bar.rpy:1538
translate schinese moonspa_edf2d891:

    # moon "Suffice to say, we didn't know what we were doing. And we weren't imprisoned or anything, just held until someone could translate for us."
    moon "只能说,我们不知道自己在做什么.我们也没有被囚禁,只是被关押起来,直到有人能给我们翻译为止."

# game/scripts/bar.rpy:1539
translate schinese moonspa_1f3239f5:

    # c "How were you able to move?"
    c "你们怎么搬家的?"

# game/scripts/bar.rpy:1540
translate schinese moonspa_562a44a2:

    # moon "Well, you're correct in assuming we were poor."
    moon "嗯,你猜得对,我们很穷."

# game/scripts/bar.rpy:1542
translate schinese moonspa_e79e9799:

    # moon "My father never did tell me how he got the money, or why we moved here, but I know I'm grateful."
    moon "我爸爸从来没告诉我他是怎么得到这笔钱的,或者我们为什么要搬到这里,但我知道我很感激."

# game/scripts/bar.rpy:1543
translate schinese moonspa_636abcd0:

    # c "Are you still talking with him?"
    c "你还和他说话吗?"

# game/scripts/bar.rpy:1545
translate schinese moonspa_9ae4f32f:

    # moon "I love my father dearly, [name]... But we live very far apart. He lives in the Elvish trees."
    moon "我深爱着我爸爸,[name]......但是我们离得很远.他住在精灵的森林里."

# game/scripts/bar.rpy:1546
translate schinese moonspa_229b8e15:

    # c "The what?"
    c "什么?"

# game/scripts/bar.rpy:1548
translate schinese moonspa_29a76251:

    # moon "Forest...? Jungle!"
    moon "森林......?丛林!"

# game/scripts/bar.rpy:1550
translate schinese moonspa_be7744ec:

    # moon "That's where he works. He sent me to school where he could, which happened to be in this town. Since I was thirteen, I've been living by myself."
    moon "那是他工作的地方.他送我到他有能力送到的地方去上学,恰好就是在这个镇上.我十三岁起,我就一直自己生活."

# game/scripts/bar.rpy:1551
translate schinese moonspa_57f8e55c:

    # c "That must have been hard."
    c "那一定很困难."

# game/scripts/bar.rpy:1552
translate schinese moonspa_8c0ec12a:

    # moon "Not for a wild girl like me, haha!"
    moon "像我这样的野丫头可不会,哈哈!"

# game/scripts/bar.rpy:1554
translate schinese moonspa_359b0c36:

    # moon "School was lonely for a girl who could beat up the boys and didn't speak their language. But I survived, like I was raised to do!"
    moon "对于一个可以打败男孩们却听不懂他们说的话的女孩来说,学校是孤独的.但我活了下来,就像我被培养出来的那样!"

# game/scripts/bar.rpy:1556
translate schinese moonspa_616ee83a:

    # moon "I think that's why my father decided to move to here of all places. Your school systems give out so many grants. Especially to immigrants and the poor."
    moon "我觉得这就是为什么我爸爸决定搬到这里.你们的教育系统发了很多的补助金.特别是针对移民和穷人的."

# game/scripts/bar.rpy:1557
translate schinese moonspa_0cc8ae4e:

    # moon "And of course the free health care is nice!"
    moon "当然了,免费的医疗保健也很好!"

# game/scripts/bar.rpy:1558
translate schinese moonspa_dbceef9d:

    # moon "Naturally I went to college after schooling, I majored in biochemistry and engineering."
    moon "自然,我上学当然是直接上到大学了,我的专业是生物化学专业与工程专业."

# game/scripts/bar.rpy:1559
translate schinese moonspa_e2d716c0:

    # c "But you're a waitress?"
    c "但是你现在是个服务员?"

# game/scripts/bar.rpy:1560
translate schinese moonspa_c4cb07c1:

    # moon "I'm still in college but I should be finished soon. I've been looking around for job openings but I haven't been able to find anyone who will accept me."
    moon "我现在还在上大学,但是我应该很快就会毕业.我一直在四处寻找工作机会,但我一直没找到愿意接受我的人."

# game/scripts/bar.rpy:1561
translate schinese moonspa_50c8224d:

    # moon "Your hotel is very close to me, so when I saw you were hiring I figured why not!"
    moon "你的酒店离我家很近,所以你们在招聘的时候,我就想为什么不呢!"

# game/scripts/bar.rpy:1562
translate schinese moonspa_71411fd1:

    # c "We're glad to have you."
    c "我们很高兴能拥有你."

# game/scripts/bar.rpy:1564
translate schinese moonspa_835bab8d:

    # moon "...Ehhh, thank you..."
    moon "......嗯,谢谢你......"

# game/scripts/bar.rpy:1565
translate schinese moonspa_3fc15ae0:

    # c "Are you okay?"
    c "你还好吗?"

# game/scripts/bar.rpy:1566
translate schinese moonspa_11d5d9a2:

    # moon "You'll have to excuse me, I'm not very used to male admiration... My father was a good man, but he was tough on me."
    moon "你得原谅我,我不太习惯来自男性的倾慕......我爸爸是个好人,但是他对我很严厉."

# game/scripts/bar.rpy:1568
translate schinese moonspa_a2994dc1:

    # moon "Ehm, or female admiration I guess... Why does Maria look at me like that?"
    moon "嗯,女性的倾慕可能也会吧......为什么玛丽亚要那样看着我?"

# game/scripts/bar.rpy:1569
translate schinese moonspa_45b1f4df:

    # c "She swings both ways."
    c "她是双性恋."

# game/scripts/bar.rpy:1571
translate schinese moonspa_ac05dd11:

    # moon "Oh, I see!"
    moon "噢,我懂了!"

# game/scripts/bar.rpy:1573
translate schinese moonspa_5a8d2838:

    # moon "But why does she want me to hurt her...?"
    moon "但是为什么她想要我伤害她......?"

# game/scripts/bar.rpy:1574
translate schinese moonspa_3cfafa8b:

    # c "Your guess is as good as mine."
    c "我也想知道."

# game/scripts/bar.rpy:1576
translate schinese moonspa_ff424c3a:

    # moon "Well, I don't really enjoy hurting people... But she's a good friend of mine..."
    moon "嗯,我不是很喜欢伤害别人......但是她是我的好朋友......"

# game/scripts/bar.rpy:1578
translate schinese moonspa_7747b302:

    # moon "If she asks again, I'll slap her!"
    moon "如果她再问,我就给她一巴掌!"

# game/scripts/bar.rpy:1579
translate schinese moonspa_4256f56d:

    # c "Haha, alright."
    c "哈哈,好吧."

# game/scripts/bar.rpy:1581
translate schinese moonspa_9dc1d8a1:

    # moon "I feel so much better now, [name]. Thank you very much for allowing me to visit your hot springs."
    moon "我现在感觉好多了,[name].谢谢你允许我参观你的温泉."

# game/scripts/bar.rpy:1582
translate schinese moonspa_49396692:

    # c "You can come here any time you'd like."
    c "你随时都可以来这里."

# game/scripts/bar.rpy:1583
translate schinese moonspa_a0f2bfd6:

    # moon "Thank you so much! I should get back home. Thanks again."
    moon "谢谢你!但是我现在应该回家了!谢谢."

# game/scripts/bar.rpy:1597
translate schinese twotruths_1f633b67:

    # m "Hahaha, no way! Really?"
    m "哈哈哈,不可能!真的吗?"

# game/scripts/bar.rpy:1598
translate schinese twotruths_f87e6e76:

    # moon "It's true..."
    moon "真的......"

# game/scripts/bar.rpy:1600
translate schinese twotruths_43c3f241:

    # m "I honestly never would have guessed!"
    m "说实话,我从来没有想到!"

# game/scripts/bar.rpy:1601
translate schinese twotruths_9b1c066b:

    # c "What are you girls doing?"
    c "你们这些女孩在做什么?"

# game/scripts/bar.rpy:1603
translate schinese twotruths_0bb17db1:

    # m "Hey [name]! It's been pretty slow today, so we've been playing some drinking games. Right now we're playing Never Have I Ever, wanna play?"
    m "嘿[name]!今天没多少人,所以我们在玩一些喝酒游戏.我们在玩'我没'(Never Have I Ever,大家轮流说出一个事,如果有人做过这件事就喝酒,有点类似掰手指),想玩吗?"

# game/scripts/bar.rpy:1604
translate schinese twotruths_c48dc32f:

    # c "Sure."
    c "当然了."

# game/scripts/bar.rpy:1606
translate schinese twotruths_bd97788c:

    # moon "Big mistake!"
    moon "你犯了个大错误!"

# game/scripts/bar.rpy:1607
translate schinese twotruths_acf19163:

    # m "Alright fine, let's go clockwise. Go ahead [name]."
    m "好吧好吧,顺时针走.[name],继续."

# game/scripts/bar.rpy:1612
translate schinese tt_c76caf41:

    # c "Never have I ever..."
    c "我没......"

# game/scripts/bar.rpy:1617
translate schinese tt_6b63fd2c:

    # moon "Whaaat? That's an embarrassing question..."
    moon "什么?这个问题好尴尬......"

# game/scripts/bar.rpy:1619
translate schinese tt_16f8c576:

    # m "*gulp*"
    m "*咕噜*"

# game/scripts/bar.rpy:1620
translate schinese tt_7eaf6228:

    # moon "...{i}{size=-10}That's disgusting...{/size}{/i}"
    moon "......{i}{size=-10}恶心......{/size}{/i}"

# game/scripts/bar.rpy:1623
translate schinese tt_996fc0e7:

    # m "Cheater..."
    m "你作弊......"

# game/scripts/bar.rpy:1625
translate schinese tt_052072d7:

    # m "My turn!"
    m "到我了!"

# game/scripts/bar.rpy:1627
translate schinese tt_d826ef42:

    # m "Never have I ever..."
    m "我没......"

# game/scripts/bar.rpy:1628
translate schinese tt_ffac805a:

    # m "Been attracted to someone over twice my age."
    m "被超过我两倍年龄的人吸引过."

# game/scripts/bar.rpy:1629
translate schinese tt_31221591:

    # moon "Nope!"
    moon "当然不了!"

# game/scripts/bar.rpy:1634
translate schinese tt_2333518f:

    # m "Interesting..."
    m "有趣......"

# game/scripts/bar.rpy:1637
translate schinese tt_ac2bea30:

    # c "Me neither."
    c "我也没有."

# game/scripts/bar.rpy:1639
translate schinese tt_ed2bd186:

    # moon "Never have I ever..."
    moon "我没......"

# game/scripts/bar.rpy:1640
translate schinese tt_1d494f55:

    # moon "Flashed in public!"
    moon "在公共场合露出!"

# game/scripts/bar.rpy:1642
translate schinese tt_16f8c576_1:

    # m "*gulp*"
    m "*咕噜*"

# game/scripts/bar.rpy:1643
translate schinese tt_6acf8071:

    # moon "Oh...!"
    moon "噢......!"

# game/scripts/bar.rpy:1648
translate schinese tt_946b617b:

    # m "Hahahahaha!"
    m "哈哈哈哈哈!"

# game/scripts/bar.rpy:1649
translate schinese tt_d8966511:

    # moon "Ehhh...?"
    moon "嗯......?"

# game/scripts/bar.rpy:1652
translate schinese tt_9a60c2bb:

    # c "Nope."
    c "没有."

# game/scripts/bar.rpy:1656
translate schinese tt_86f264f8:

    # m "You're just trying to get me drunk..."
    m "你就是想让我喝醉是吧......"

# game/scripts/bar.rpy:1658
translate schinese tt_16f8c576_2:

    # m "*gulp*"
    m "*咕噜*"

# game/scripts/bar.rpy:1660
translate schinese tt_4984c9d6:

    # moon "..."
    moon "......"

# game/scripts/bar.rpy:1661
translate schinese tt_32899f05:

    # moon "...Y-Your turn..."
    moon "......到-到你了......"

# game/scripts/bar.rpy:1663
translate schinese tt_9062ced7:

    # m "Never have I ever stole someone's panties..."
    m "我没偷过别人的内裤......"

# game/scripts/bar.rpy:1667
translate schinese tt_b458aa19:

    # m "I knew it! Haha!"
    m "我就知道!哈哈!"

# game/scripts/bar.rpy:1668
translate schinese tt_b0797162:

    # moon "Really?"
    moon "真的吗?"

# game/scripts/bar.rpy:1671
translate schinese tt_9a60c2bb_1:

    # c "Nope."
    c "没有."

# game/scripts/bar.rpy:1673
translate schinese tt_35184d51:

    # moon "*gulp*..."
    moon "*咕噜*......"

# game/scripts/bar.rpy:1674
translate schinese tt_03dc7223:

    # m "What!? What happened? Tell the story!"
    m "什么!?发生什么事了?告诉我那个故事!"

# game/scripts/bar.rpy:1676
translate schinese tt_5b0a02bd:

    # moon "I was young! I stole my friend's panties because they were cute... Don't judge me!"
    moon "我那时候还很小!我偷了我朋友的内裤,因为它们很可爱......不要批评我!"

# game/scripts/bar.rpy:1678
translate schinese tt_ed2bd186_1:

    # moon "Never have I ever..."
    moon "我没......"

# game/scripts/bar.rpy:1679
translate schinese tt_ee673ec4:

    # moon "Been spanked!"
    moon "被打过屁股!"

# game/scripts/bar.rpy:1681
translate schinese tt_b5c5bd56:

    # m "Bullshit! You've never been spanked? Not once in your life?"
    m "胡扯!你没被打过屁股?一次都没有?"

# game/scripts/bar.rpy:1682
translate schinese tt_bf5a0bbc:

    # moon "My father never believed in physical punishment. He's a good man!"
    moon "我爸爸从来都不体罚我.他是个好人!"

# game/scripts/bar.rpy:1686
translate schinese tt_b500adb8:

    # m "Most of us have... That question is bullshit!"
    m "我们大多数都有过......这个问题就是胡扯!"

# game/scripts/bar.rpy:1687
translate schinese tt_bee43091:

    # moon "Got you!"
    moon "抓到你了!"

# game/scripts/bar.rpy:1690
translate schinese tt_9a60c2bb_2:

    # c "Nope."
    c "没有."

# game/scripts/bar.rpy:1692
translate schinese tt_21ed94e1:

    # m "*gulp* I can't believe you've never been spanked..."
    m "*咕噜*我不敢相信你居然从来没被打过屁股......"

# game/scripts/bar.rpy:1696
translate schinese tt_5f79ce11:

    # m "Nope!"
    m "没有!"

# game/scripts/bar.rpy:1697
translate schinese tt_cf6aead7:

    # moon "Definitely not!"
    moon "当然没有!"

# game/scripts/bar.rpy:1701
translate schinese tt_0e02c5b2:

    # moon "[name] was a bad boy!"
    moon "[name]是个坏人!"

# game/scripts/bar.rpy:1702
translate schinese tt_4fd76a76:

    # m "Oooh..."
    m "噢......"

# game/scripts/bar.rpy:1705
translate schinese tt_ac2bea30_1:

    # c "Me neither."
    c "我也没有."

# game/scripts/bar.rpy:1707
translate schinese tt_d826ef42_1:

    # m "Never have I ever..."
    m "我没......"

# game/scripts/bar.rpy:1708
translate schinese tt_86e03853:

    # m "Fucked myself with a vegetable..."
    m "用蔬菜自慰......"

# game/scripts/bar.rpy:1710
translate schinese tt_16f8c576_3:

    # m "*gulp*"
    m "*咕噜*"

# game/scripts/bar.rpy:1711
translate schinese tt_6e562d95:

    # moon "What? You have!?"
    moon "什么?你有过!?"

# game/scripts/bar.rpy:1713
translate schinese tt_4574f867:

    # moon "*gulp*"
    moon "*咕噜*"

# game/scripts/bar.rpy:1714
translate schinese tt_9fad45bc:

    # m "*gasp*!"
    m "*惊叹*!"

# game/scripts/bar.rpy:1716
translate schinese tt_2c687c14:

    # moon "Why did you ask a question you were going to drink to? That's not how you play!"
    moon "为什么你要问一个你会喝酒的问题?游戏不是这么玩的!"

# game/scripts/bar.rpy:1717
translate schinese tt_be1fbd8c:

    # m "I know, heheh... But I wanted to know if you did."
    m "我知道,呵呵呵......但是我想知道你有没有做过."

# game/scripts/bar.rpy:1718
translate schinese tt_f0f2d0bd:

    # moon "Don't ask such embarassing questions, Maria!"
    moon "别再问那么尴尬的问题了,玛丽亚!"

# game/scripts/bar.rpy:1719
translate schinese tt_cf41a62c:

    # m "So what's the story with that?"
    m "所以那是什么故事?"

# game/scripts/bar.rpy:1720
translate schinese tt_339a9d63:

    # moon "My family was poor until we moved to Syl'anar... I had to use something..."
    moon "直到我们搬到西尔阿纳尔之前,我家里一直都很穷......我不得不用点东西......"

# game/scripts/bar.rpy:1724
translate schinese tt_4984c9d6_1:

    # moon "..."
    moon "......"

# game/scripts/bar.rpy:1725
translate schinese tt_3a8ec408:

    # m "Hot."
    m "可真够火辣的."

# game/scripts/bar.rpy:1728
translate schinese tt_82b8e8e8:

    # c "Next question..."
    c "下一个问题......"

# game/scripts/bar.rpy:1733
translate schinese tt_16f8c576_4:

    # m "*gulp*"
    m "*咕噜*"

# game/scripts/bar.rpy:1734
translate schinese tt_9373a7c8:

    # moon "!"
    moon "!"

# game/scripts/bar.rpy:1736
translate schinese tt_a46b02d6:

    # moon "!!"
    moon "!!"

# game/scripts/bar.rpy:1738
translate schinese tt_dd67bde8:

    # moon "!!!"
    moon "!!!"

# game/scripts/bar.rpy:1739
translate schinese tt_e6e0925f:

    # moon "Next question please..."
    moon "还是下一个问题吧......"

# game/scripts/bar.rpy:1741
translate schinese tt_d826ef42_2:

    # m "Never have I ever..."
    m "我没......"

# game/scripts/bar.rpy:1742
translate schinese tt_7aaa5b6c:

    # m "Killed an animal."
    m "杀过动物."

# game/scripts/bar.rpy:1744
translate schinese tt_68d84001:

    # moon "What a strange question... *gulp*"
    moon "这是什么奇怪问题......*咕噜*"

# game/scripts/bar.rpy:1746
translate schinese tt_547f11e9:

    # m "No way, a cute little girl like you?"
    m "不可能,你这样的可爱小女孩怎么会?"

# game/scripts/bar.rpy:1747
translate schinese tt_a7a681e6:

    # moon "Hah, don't call me that! My father taught me how to hunt in forests! I could snap a monkey's neck."
    moon "哈哈,别那么叫我!我爸爸教过我如何在森林里打猎!我可以扭断猴子的脖子."

# game/scripts/bar.rpy:1748
translate schinese tt_984baceb:

    # m "There's no way! I bet I could pin you on the ground right now, and there would be nothing you could do about it."
    m "不可能!我敢打赌,我现在就可以把你按在地上,你却无能为力."

# game/scripts/bar.rpy:1749
translate schinese tt_98f12dc7:

    # moon "Don't make any claims if you don't plan on enforcing them!"
    moon "光说不练假把式!"

# game/scripts/bar.rpy:1750
translate schinese tt_e4868af1:

    # m "I'm just joking, I wouldn't want to hurt you."
    m "我只是在开玩笑,我不想伤害你."

# game/scripts/bar.rpy:1751
translate schinese tt_7c5268c2:

    # c "I say we test it out anyway..."
    c "要不我们还是测试一下吧......"

# game/scripts/bar.rpy:1753
translate schinese tt_dfacece0:

    # moon "I could take you on too..."
    moon "我也可以带上你一起......"

# game/scripts/bar.rpy:1757
translate schinese tt_97cd67f1:

    # m "Such a tragedy..."
    m "真是个悲剧......"

# game/scripts/bar.rpy:1759
translate schinese tt_82b8e8e8_1:

    # c "Next question..."
    c "下一个问题......"

# game/scripts/bar.rpy:1761
translate schinese tt_aed74215:

    # moon "Never have I ever climbed a mountain..."
    moon "我没爬过山......"

# game/scripts/bar.rpy:1763
translate schinese tt_503d6303:

    # moon "...I just wanted to brag... *gulp*"
    moon "......我只是想吹吹牛......*咕噜*"

# game/scripts/bar.rpy:1764
translate schinese tt_bffde9b1:

    # m "No way!"
    m "不可能!"

# game/scripts/bar.rpy:1766
translate schinese tt_8cb454aa:

    # m "You climbed a mountain!?"
    m "你爬过山!?"

# game/scripts/bar.rpy:1767
translate schinese tt_7cf6888a:

    # moon "Yep! The first time was when I was 9."
    moon "是的!我第一次是在9岁的时候爬山的."

# game/scripts/bar.rpy:1768
translate schinese tt_eecf9139:

    # m "Nine!? What are you, super human?"
    m "9岁!?你是什么,超人吗?"

# game/scripts/bar.rpy:1769
translate schinese tt_d0ae2840:

    # moon "I lived in what is essentially a jungle when I was young... All I had was my father, we lived a very solitary life. He taught me how to survive!"
    moon "我年轻的时候住的地方基本上就是一片丛林......我身边只有我的爸爸,我们过着非常孤独的生活.他教会了我如何生存!"

# game/scripts/bar.rpy:1770
translate schinese tt_022bb473:

    # m "That's so cool!"
    m "那太酷了!"

# game/scripts/bar.rpy:1774
translate schinese tt_499de3c5:

    # moon "Liar!"
    moon "骗子!"

# game/scripts/bar.rpy:1775
translate schinese tt_2132a680:

    # m "Liar..."
    m "骗子......"

# game/scripts/bar.rpy:1777
translate schinese tt_9a60c2bb_3:

    # c "Nope."
    c "没有."

# game/scripts/bar.rpy:1781
translate schinese tt_2627a62c:

    # moon "Ew, no!"
    moon "呃,没有!"

# game/scripts/bar.rpy:1782
translate schinese tt_391709aa:

    # c "Maria?"
    c "玛丽亚?"

# game/scripts/bar.rpy:1783
translate schinese tt_d746c1b9:

    # m "What? No, I haven't peed on anyone."
    m "什么?没有,我没有尿到别人身上过."

# game/scripts/bar.rpy:1787
translate schinese tt_2e506dfb:

    # moon "Grossss!"
    moon "恶心!"

# game/scripts/bar.rpy:1788
translate schinese tt_c7c630b5:

    # m "Haha..."
    m "哈哈......"

# game/scripts/bar.rpy:1790
translate schinese tt_72165f1b:

    # c "Next question."
    c "下一个问题."

# game/scripts/bar.rpy:1792
translate schinese tt_85702b02:

    # m "Never have I ever fucked a dog!"
    m "我没有操过狗!"

# game/scripts/bar.rpy:1794
translate schinese tt_4984c9d6_2:

    # moon "..."
    moon "......"

# game/scripts/bar.rpy:1795
translate schinese tt_b41c5836:

    # m "Just checking..."
    m "我只是随便问问......"

# game/scripts/bar.rpy:1799
translate schinese tt_4984c9d6_3:

    # moon "..."
    moon "......"

# game/scripts/bar.rpy:1800
translate schinese tt_e612037c:

    # m "..."
    m "......"

# game/scripts/bar.rpy:1802
translate schinese tt_13804b5a:

    # m "Ehem..."
    m "嗯......"

# game/scripts/bar.rpy:1803
translate schinese tt_4984c9d6_4:

    # moon "..."
    moon "......"

# game/scripts/bar.rpy:1805
translate schinese tt_eb1bc23b:

    # c "...Just kidding..."
    c "......我只是在开玩笑......"

# game/scripts/bar.rpy:1806
translate schinese tt_e612037c_1:

    # m "..."
    m "......"

# game/scripts/bar.rpy:1807
translate schinese tt_4984c9d6_5:

    # moon "..."
    moon "......"

# game/scripts/bar.rpy:1808
translate schinese tt_d23dd1d9:

    # c "...Next question please..."
    c "......下一个问题吧......"

# game/scripts/bar.rpy:1809
translate schinese tt_50d016ad:

    # moon "Right..."
    moon "好吧......"

# game/scripts/bar.rpy:1811
translate schinese tt_a59f8f81:

    # c "Of course not."
    c "当然没有."

# game/scripts/bar.rpy:1813
translate schinese tt_bd80db23:

    # moon "Never have I ever eaten french fries!"
    moon "我没吃过炸薯条!"

# game/scripts/bar.rpy:1814
translate schinese tt_8b2d3a72:

    # m "What!?"
    m "什么!?"

# game/scripts/bar.rpy:1815
translate schinese tt_54e5d726:

    # moon "We don't have them where I'm from..."
    moon "我的家乡没有这种东西......"

# game/scripts/bar.rpy:1816
translate schinese tt_a291cdef:

    # m "Urgh!"
    m "呃!"

# game/scripts/bar.rpy:1818
translate schinese tt_578142fe:

    # m "Fine! *gulp*"
    m "好吧!*咕噜*"

# game/scripts/bar.rpy:1822
translate schinese tt_4ee9b626:

    # c "Obviously."
    c "当然了."

# game/scripts/bar.rpy:1824
translate schinese tt_251ff2a2:

    # c "*gulp*"
    c "*咕噜*"

# game/scripts/bar.rpy:1826
translate schinese tt_9a60c2bb_4:

    # c "Nope."
    c "没有."

# game/scripts/bar.rpy:1827
translate schinese tt_a6e6d960:

    # m "Liar!"
    m "骗子!"

# game/scripts/bar.rpy:1831
translate schinese tt_755a2933:

    # m "*sigh*..."
    m "*叹气*......"

# game/scripts/bar.rpy:1833
translate schinese tt_16f8c576_5:

    # m "*gulp*"
    m "*咕噜*"

# game/scripts/bar.rpy:1834
translate schinese tt_27210761:

    # moon "Maria!"
    moon "玛丽亚!"

# game/scripts/bar.rpy:1836
translate schinese tt_bb3160ed:

    # m "What can I say...? Underage drinking... I was at a party."
    m "我还能说什么......?未成年喝酒......我当时在参加聚会."

# game/scripts/bar.rpy:1838
translate schinese tt_eac49e5f:

    # moon "...*gulp*"
    moon "......*咕噜*"

# game/scripts/bar.rpy:1839
translate schinese tt_4480f35e:

    # m "*gasp*! What!? Why!?"
    m "*惊叹*!什么!?为什么!?"

# game/scripts/bar.rpy:1841
translate schinese tt_460ce553:

    # moon "Immigration... I didn't have any ID when I came over here. Me and my father were completely off the grid."
    moon "移民局......我来这里的时候没有身份证.我和我爸爸当时完全不在状态."

# game/scripts/bar.rpy:1842
translate schinese tt_0868b29c:

    # m "What are you, some sort of a mountain girl?"
    m "你到底是什么人啊,山中野人吗?"

# game/scripts/bar.rpy:1843
translate schinese tt_98714dca:

    # moon "I guess so?"
    moon "我想应该是吧?"

# game/scripts/bar.rpy:1847
translate schinese tt_47cb927f:

    # m "Oooohhhh... [name] has been bad..."
    m "噢......[name]做过坏事......"

# game/scripts/bar.rpy:1848
translate schinese tt_5564345f:

    # moon "Naughty..."
    moon "调皮......"

# game/scripts/bar.rpy:1850
translate schinese tt_9a60c2bb_5:

    # c "Nope."
    c "没有."

# game/scripts/bar.rpy:1852
translate schinese tt_cfff66bb:

    # m "Never have I ever had multiple girlfriends at once."
    m "我没同时拥有过多个女朋友."

# game/scripts/bar.rpy:1854
translate schinese tt_9624c1aa:

    # m "Heheheh..."
    m "呵呵呵呵......"

# game/scripts/bar.rpy:1855
translate schinese tt_3dda5a49:

    # moon "Oh wow..."
    moon "噢哇哦......"

# game/scripts/bar.rpy:1857
translate schinese tt_419c625b:

    # c "Cheater..."
    c "你作弊......"

# game/scripts/bar.rpy:1859
translate schinese tt_82bae930:

    # moon "Never have I ever had a boyfriend!"
    moon "我没交过男朋友!"

# game/scripts/bar.rpy:1860
translate schinese tt_a5bd9fb3:

    # m "Oh, come on! You're targeting me..."
    m "噢,拜托!你在针对我......"

# game/scripts/bar.rpy:1862
translate schinese tt_16f8c576_6:

    # m "*gulp*"
    m "*咕噜*"

# game/scripts/bar.rpy:1864
translate schinese tt_38835dd8:

    # m "Are you saying you're a virgin?"
    m "你是说你是处女吗?"

# game/scripts/bar.rpy:1865
translate schinese tt_0b2c76b1:

    # moon "I'm not a virgin... But I've never had a boyfriend."
    moon "我不是处女......但我从来没有有过男朋友."

# game/scripts/bar.rpy:1866
translate schinese tt_fc69603d:

    # m "How!? You're cute!"
    m "怎么可能!?你这么可爱!"

# game/scripts/bar.rpy:1867
translate schinese tt_58c92286:

    # moon "'Mountain girl', remember?"
    moon "'山中野人',你还记得吗?"

# game/scripts/bar.rpy:1871
translate schinese tt_fa1eaaa1:

    # m "Hot!"
    m "太火辣啦!"

# game/scripts/bar.rpy:1872
translate schinese tt_39bc007a:

    # moon "Hot..."
    moon "确实很火辣......"

# game/scripts/bar.rpy:1874
translate schinese tt_72165f1b_1:

    # c "Next question."
    c "下一个问题."

# game/scripts/bar.rpy:1879
translate schinese tt_e9f2e846:

    # m "Fuck you..."
    m "操你妈......"

# game/scripts/bar.rpy:1881
translate schinese tt_16f8c576_7:

    # m "*gulp*"
    m "*咕噜*"

# game/scripts/bar.rpy:1882
translate schinese tt_367218a2:

    # moon "Huh?"
    moon "嗯?"

# game/scripts/bar.rpy:1884
translate schinese tt_bb92d5b7:

    # moon "What's the story there?"
    moon "那是什么故事?"

# game/scripts/bar.rpy:1887
translate schinese tt_e612037c_2:

    # m "..."
    m "......"

# game/scripts/bar.rpy:1889
translate schinese tt_95e3e7b9:

    # m "There you go..."
    m "懂了吧......"

# game/scripts/bar.rpy:1890
translate schinese tt_d72d5879:

    # moon "Ohh... Wow..."
    moon "噢......哇哦......"

# game/scripts/bar.rpy:1892
translate schinese tt_98548848:

    # m "Secret..."
    m "秘密......"

# game/scripts/bar.rpy:1894
translate schinese tt_d826ef42_3:

    # m "Never have I ever..."
    m "我没......"

# game/scripts/bar.rpy:1895
translate schinese tt_0a783e14:

    # m "Gotten a little too close with my father..."
    m "和我爸爸有点关系太亲密了......"

# game/scripts/bar.rpy:1897
translate schinese tt_d9032d1f:

    # moon "N-No!"
    moon "没-没有!"

# game/scripts/bar.rpy:1898
translate schinese tt_b41c5836_1:

    # m "Just checking..."
    m "只是随便问问......"

# game/scripts/bar.rpy:1900
translate schinese tt_fea03229:

    # m "[name]...?"
    m "[name]......?"

# game/scripts/bar.rpy:1901
translate schinese tt_72165f1b_2:

    # c "Next question."
    c "下一个问题."

# game/scripts/bar.rpy:1902
translate schinese tt_42d07288:

    # m "Heheh..."
    m "呵呵呵......"

# game/scripts/bar.rpy:1904
translate schinese tt_289140e6:

    # moon "Never have I ever gone to elementary school."
    moon "我没上过小学."

# game/scripts/bar.rpy:1906
translate schinese tt_fb11f6bf:

    # m "Bullshit... *gulp*"
    m "胡扯......*咕噜*"

# game/scripts/bar.rpy:1908
translate schinese tt_483d0f6d:

    # m "Come on, what's the story with that?"
    m "来吧,什么故事?"

# game/scripts/bar.rpy:1909
translate schinese tt_9e0c52e2:

    # moon "My father schooled me. There were no schools for miles... Or anything for miles..."
    moon "我爸爸自己教我的.我们那里几英里内都没有学校......或者应该说,什么都没有......"

# game/scripts/bar.rpy:1914
translate schinese tt_71b7041f:

    # m "Come on!"
    m "拜托!"

# game/scripts/bar.rpy:1916
translate schinese tt_16f8c576_8:

    # m "*gulp*"
    m "*咕噜*"

# game/scripts/bar.rpy:1918
translate schinese tt_0599a445:

    # m "Hey."
    m "嘿."

# game/scripts/bar.rpy:1919
translate schinese tt_8f58f800:

    # moon "Ehh..."
    moon "嗯......"

# game/scripts/bar.rpy:1923
translate schinese tt_651e3308:

    # moon "Woo! Equality!"
    moon "喔!这就公平了!"

# game/scripts/bar.rpy:1925
translate schinese tt_2132a680_1:

    # m "Liar..."
    m "骗子......"

# game/scripts/bar.rpy:1927
translate schinese tt_d826ef42_4:

    # m "Never have I ever..."
    m "我没......"

# game/scripts/bar.rpy:1928
translate schinese tt_5b097076:

    # m "Wanted to suck you off in front of someone so badly..."
    m "很想在别人面前给你口交......"

# game/scripts/bar.rpy:1929
translate schinese tt_4984c9d6_6:

    # moon "..."
    moon "......"

# game/scripts/bar.rpy:1931
translate schinese tt_4984c9d6_7:

    # moon "..."
    moon "......"

# game/scripts/bar.rpy:1933
translate schinese tt_4984c9d6_8:

    # moon "..."
    moon "......"

# game/scripts/bar.rpy:1935
translate schinese tt_b287f729:

    # moon "WAIT WHAT!?" with ssshake
    moon "等等什么!?" with ssshake

# game/scripts/bar.rpy:1937
translate schinese tt_44d086c2:

    # moon "Jokes like that will land you in the hospital!"
    moon "你再讲一次这种笑话就等着进医院吧!"

# game/scripts/bar.rpy:1939
translate schinese tt_16f8c576_9:

    # m "*gulp*"
    m "*咕噜*"

# game/scripts/bar.rpy:1941
translate schinese tt_34b41b3b:

    # m "Hurt me, baby..."
    m "来伤害我吧,宝贝......"

# game/scripts/bar.rpy:1943
translate schinese tt_0c4fa538:

    # moon "I won't be participating in this joke any longer..."
    moon "我不会再参与这个笑话了......"

# game/scripts/bar.rpy:1944
translate schinese tt_ee62d9f8:

    # moon "Now... Back to the game..."
    moon "现在......回到游戏上......"

# game/scripts/bar.rpy:1946
translate schinese tt_6e506f36:

    # moon "Never have I ever worn a bra!"
    moon "我没穿过胸罩!"

# game/scripts/bar.rpy:1948
translate schinese tt_bba49ccf:

    # m "Aw, come on! [name] can't drink to that! That's not fair!"
    m "啊,拜托!这样[name]不可能喝的!这不公平!"

# game/scripts/bar.rpy:1949
translate schinese tt_5eac26f3:

    # moon "You never know... Hahah!"
    moon "你怎么知道呢......哈哈哈!"

# game/scripts/bar.rpy:1950
translate schinese tt_7b5fa4b0:

    # m "What do you mean you've never worn a bra?"
    m "你说没穿过胸罩是什么意思?"

# game/scripts/bar.rpy:1951
translate schinese tt_c44530dc:

    # moon "The nearest shopping center to where I used to live was {i}miles{/i} away! We only went every month... And we were poor..."
    moon "我以前住的地方那里最近的商店都得有好几{i}英里{/i}远!我们一个月才去一次......而且我们很穷......"

# game/scripts/bar.rpy:1952
translate schinese tt_acfaf7f9:

    # moon "And... I don't really need to wear a bra anyway..."
    moon "而且......反正我也不是很需要穿胸罩......"

# game/scripts/bar.rpy:1954
translate schinese tt_16294f88:

    # m "Hot..."
    m "那可真够火辣的......"

# game/scripts/bar.rpy:1958
translate schinese tt_4c27615d:

    # m "Oh!"
    m "噢!"

# game/scripts/bar.rpy:1959
translate schinese tt_8eea4679:

    # moon "Ehhh..."
    moon "嗯......"

# game/scripts/bar.rpy:1961
translate schinese tt_9a60c2bb_6:

    # c "Nope."
    c "没有."

# game/scripts/bar.rpy:1965
translate schinese tt_5736d741:

    # c "That's all the time I have, it was fun!"
    c "好了,我现在有事了,这很好玩!"

# game/scripts/bar.rpy:1966
translate schinese tt_81187fcc:

    # moon "Thank you for joining us!"
    moon "谢谢你加入我们!"

# game/scripts/bar.rpy:1967
translate schinese tt_0a7f333a:

    # m "Bye [name]!"
    m "再见[name]!"

# game/scripts/bar.rpy:1977
translate schinese felashcomp_909b1b5e:

    # f "Well if you're so bold, then shut up!"
    f "你要是真这么大胆就给我闭嘴!"

# game/scripts/bar.rpy:1978
translate schinese felashcomp_5f9c68dd:

    # a "I don't want to hear it! Leave!"
    a "我不想听!离开!"

# game/scripts/bar.rpy:1979
translate schinese felashcomp_31b588f5:

    # f "You're going to say {i}that{/i} after making that claim?"
    f "你在提完这个要求后还要说{i}那个{/i}?"

# game/scripts/bar.rpy:1980
translate schinese felashcomp_ca19da25:

    # a "I'm ignoring you!"
    a "我不想理你!"

# game/scripts/bar.rpy:1981
translate schinese felashcomp_47d1f3df:

    # moon "Guys... Please..."
    moon "伙计们......求你们了......"

# game/scripts/bar.rpy:1982
translate schinese felashcomp_4732b4ca:

    # c "What's going on here?"
    c "这里发生什么事了?"

# game/scripts/bar.rpy:1984
translate schinese felashcomp_c49e573e:

    # moon "Ashley was very curious about the drinks! So we got to talking and, well... Then she challenged me to a drinking competition..."
    moon "阿什莉很好奇这些饮料!所以我们就开始聊了起来,嗯......然后她向我发起了一个喝酒挑战......"

# game/scripts/bar.rpy:1985
translate schinese felashcomp_9d5b4a0e:

    # f "And then I sat down ready to show her up, but she's too scared I'll beat her!"
    f "然后我坐下来准备让她看看我的实力,但是她害怕我会打败她!"

# game/scripts/bar.rpy:1986
translate schinese felashcomp_3d13fd70:

    # a "I said I'm ignoring you!"
    a "我说了我不想理你!"

# game/scripts/bar.rpy:1987
translate schinese felashcomp_591747b2:

    # c "Ashley, a mouse could out drink you..."
    c "阿什莉,一只老鼠喝的都比你多......"

# game/scripts/bar.rpy:1989
translate schinese felashcomp_ffa2897a:

    # a "*GASP*! You can't say that! You're my boyfriend, you're supposed to cheer me on!"
    a "*惊叹*!你不能这么说!你是我的男朋友,你应该为我加油的!"

# game/scripts/bar.rpy:1990
translate schinese felashcomp_ab42f9e9:

    # f "He is NOT your boyfriend!"
    f "他不是你的男朋友!"

# game/scripts/bar.rpy:1992
translate schinese felashcomp_13876bb2:

    # a "SHUT UP!"
    a "闭嘴!"

# game/scripts/bar.rpy:1993
translate schinese felashcomp_f74e3091:

    # c "Why don't you just let her drink, Ashley?"
    c "为什么你不让她喝呢,阿什莉?"

# game/scripts/bar.rpy:1994
translate schinese felashcomp_02249166:

    # f "Because she's worried I'll beat her..."
    f "因为她害怕我会打败她......"

# game/scripts/bar.rpy:1995
translate schinese felashcomp_7897e219:

    # a "No I'm not! I just don't like you! I want you gone!"
    a "不,我不是!我只是不喜欢你!我要你离开!"

# game/scripts/bar.rpy:1996
translate schinese felashcomp_b6bb541f:

    # c "Ashley, just let her drink..."
    c "阿什莉,让她喝吧......"

# game/scripts/bar.rpy:1998
translate schinese felashcomp_f8fc96ef:

    # a "Hm! Fine."
    a "哼!好吧."

# game/scripts/bar.rpy:1999
translate schinese felashcomp_bc3a3c0b:

    # f "Thank you, [name]~"
    f "谢谢你,[name]~"

# game/scripts/bar.rpy:2001
translate schinese felashcomp_cd1b9481:

    # f "Wait, why are we doing this?"
    f "等等,为什么我们要做这个?"

# game/scripts/bar.rpy:2002
translate schinese felashcomp_79cc9b6e:

    # moon "Haha, no reason! Ashley just challenged me, and I couldn't pass that up!"
    moon "哈哈,没为什么!阿什莉只是想挑战我,我不能错过这个机会!"

# game/scripts/bar.rpy:2003
translate schinese felashcomp_7ec25200:

    # f "That's boring..."
    f "那好无聊......"

# game/scripts/bar.rpy:2004
translate schinese felashcomp_11ad1585:

    # moon "Let's make it fun then! You have to drink the whole bottle without taking a single breath! If you put your glass down, you lose!"
    moon "那就把它变得好玩点吧!你必须一口气喝完一整杯酒!如果你中途把杯子放下,你就输了!"

# game/scripts/bar.rpy:2005
translate schinese felashcomp_d95bd03d:

    # f "What!? That's crazy! We're going to get wasted!"
    f "什么!?那太疯狂了!我们会喝个烂醉的!"

# game/scripts/bar.rpy:2007
translate schinese felashcomp_0292a072:

    # a "I told you! I knew you couldn't do it!"
    a "我都告诉你了!我就知道你不行!"

# game/scripts/bar.rpy:2008
translate schinese felashcomp_0a3d5a12:

    # f "Urgh, shut up! You're going to pass out on your first sip!"
    f "呃,闭嘴!你喝第一口就会晕倒的!"

# game/scripts/bar.rpy:2009
translate schinese felashcomp_345e2343:

    # f "Fine, I'll do it! There's no way I could lose this!"
    f "好吧,我做!我可不会输的!"

# game/scripts/bar.rpy:2011
translate schinese felashcomp_cfa4f2a7:

    # moon "Great! Ready?"
    moon "很好!准备好了吗?"

# game/scripts/bar.rpy:2012
translate schinese felashcomp_c385465d:

    # a "I'm ready!"
    a "准备好了!"

# game/scripts/bar.rpy:2013
translate schinese felashcomp_5a2859d5:

    # f "Of course!"
    f "当然!"

# game/scripts/bar.rpy:2014
translate schinese felashcomp_f8d3fec0:

    # moon "Go!"
    moon "开始!"

# game/scripts/bar.rpy:2016
translate schinese felashcomp_7ce8a9bb:

    # f "*GULP, GULP, GULP*!" with ssshake
    f "*咕噜,咕噜,咕噜*!" with ssshake

# game/scripts/bar.rpy:2017
translate schinese felashcomp_6fa3bdfa:

    # a "*GULP, GULP, GULP*!" with ssshake
    a "*咕噜,咕噜,咕噜*!" with ssshake

# game/scripts/bar.rpy:2018
translate schinese felashcomp_5529d2bf:

    # moon "*GULP, GULP, GULP*!" with ssshake
    moon "*咕噜,咕噜,咕噜*!" with ssshake

# game/scripts/bar.rpy:2020
translate schinese felashcomp_dc924c6e:

    # a "Nnn! W-We're all out! Get us some more [name]!" with ssshake
    a "嗯!我-我们都喝完了![name],再给我们来点!" with ssshake

# game/scripts/bar.rpy:2023
translate schinese felashcomp_913bac39:

    # f "R- *hic*! Ready to go?"
    f "准-*打嗝*!准备好了吗?"

# game/scripts/bar.rpy:2024
translate schinese felashcomp_0513fec9:

    # moon "Aw, you guys can't be drunk already? Heheh!"
    moon "啊,你们不会已经醉了吧?呵呵呵!"

# game/scripts/bar.rpy:2025
translate schinese felashcomp_f8d3fec0_1:

    # moon "Go!"
    moon "来!"

# game/scripts/bar.rpy:2027
translate schinese felashcomp_7ce8a9bb_1:

    # f "*GULP, GULP, GULP*!" with ssshake
    f "*咕噜,咕噜,咕噜*!" with ssshake

# game/scripts/bar.rpy:2028
translate schinese felashcomp_6fa3bdfa_1:

    # a "*GULP, GULP, GULP*!" with ssshake
    a "*咕噜,咕噜,咕噜*!" with ssshake

# game/scripts/bar.rpy:2029
translate schinese felashcomp_5529d2bf_1:

    # moon "*GULP, GULP, GULP*!" with ssshake
    moon "*咕噜,咕噜,咕噜*!" with ssshake

# game/scripts/bar.rpy:2031
translate schinese felashcomp_33c8f5c3:

    # f "Give up already!" with ssshake
    f "放弃吧!" with ssshake

# game/scripts/bar.rpy:2032
translate schinese felashcomp_9dc63bcd:

    # a "N- *hic*! N- *hic*! Never! Ooofggh..."
    a "不-*打嗝*!不-*打嗝*!不可能!噢......"

# game/scripts/bar.rpy:2033
translate schinese felashcomp_9abad278:

    # a "MORE!"
    a "再来!"

# game/scripts/bar.rpy:2036
translate schinese felashcomp_d349d21d:

    # f "Go!"
    f "来!"

# game/scripts/bar.rpy:2038
translate schinese felashcomp_7ce8a9bb_2:

    # f "*GULP, GULP, GULP*!" with ssshake
    f "*咕噜,咕噜,咕噜*!" with ssshake

# game/scripts/bar.rpy:2039
translate schinese felashcomp_6fa3bdfa_2:

    # a "*GULP, GULP, GULP*!" with ssshake
    a "*咕噜,咕噜,咕噜*!" with ssshake

# game/scripts/bar.rpy:2040
translate schinese felashcomp_5529d2bf_2:

    # moon "*GULP, GULP, GULP*!" with ssshake
    moon "*咕噜,咕噜,咕噜*!" with ssshake

# game/scripts/bar.rpy:2042
translate schinese felashcomp_c59b009a:

    # a "AGH!" with sshake
    a "啊!" with sshake

# game/scripts/bar.rpy:2043
translate schinese felashcomp_d22f45ce:

    # f "Ooogh..."
    f "噢......"

# game/scripts/bar.rpy:2045
translate schinese felashcomp_db8647f7:

    # f "Geh- Get yer hand off my tit!"
    f "拔-把你的手从我奶子上拿开!"

# game/scripts/bar.rpy:2046
translate schinese felashcomp_7f7ace17:

    # a "Shut up! I- *hic* won!"
    a "闭嘴!我-*打嗝*赢了!"

# game/scripts/bar.rpy:2048
translate schinese felashcomp_2e36171d:

    # f "You didn't win! I waz thuh winner!"
    f "你没赢!我才湿真正的阴家!"

# game/scripts/bar.rpy:2050
translate schinese felashcomp_3e455b14:

    # a "EHH!?" with ssshake
    a "嗯!?" with ssshake

# game/scripts/bar.rpy:2051
translate schinese felashcomp_a87fd285:

    # a "I wuz the winner! You fell first!"
    a "我菜是赢家!你先倒的!"

# game/scripts/bar.rpy:2053
translate schinese felashcomp_8f2e8d7d:

    # f "Whaaaaat!? I don't remember that! You're lying!" with ssshake
    f "什么!?我不记得了!你在说谎!" with ssshake

# game/scripts/bar.rpy:2055
translate schinese felashcomp_14840686:

    # a "Are yuo callin' me a liar?" with ssshake
    a "你说我是骗子吗?" with ssshake

# game/scripts/bar.rpy:2057
translate schinese felashcomp_f752e842:

    # f "I don't remember." with ssshake
    f "我忘了." with ssshake

# game/scripts/bar.rpy:2059
translate schinese felashcomp_398b1bdb:

    # a "Huh!?" with ssshake
    a "嗯!?" with ssshake

# game/scripts/bar.rpy:2061
translate schinese felashcomp_dd7a83d9:

    # f "What!?" with ssshake
    f "什么!?" with ssshake

# game/scripts/bar.rpy:2063
translate schinese felashcomp_b1a64de2:

    # f "I think you need ta shtop talkin' smack!"
    f "我觉得你得别说废话了!"

# game/scripts/bar.rpy:2064
translate schinese felashcomp_76a7e46b:

    # a "I ain't talkin' shit!"
    a "我什么都没说!"

# game/scripts/bar.rpy:2065
translate schinese felashcomp_6ced7dc9:

    # f "Then stop it!"
    f "那就停下!"

# game/scripts/bar.rpy:2066
translate schinese felashcomp_2b516c17:

    # a "You stop it first!"
    a "你先停!"

# game/scripts/bar.rpy:2068
translate schinese felashcomp_1c81803d:

    # f "Stop what!? Why!?"
    f "停什么!?为什么!?"

# game/scripts/bar.rpy:2070
translate schinese felashcomp_630c29f6:

    # a "I don't know, jusht shtop it already!"
    a "我不知道,翻正就是听下!"

# game/scripts/bar.rpy:2072
translate schinese felashcomp_0cd2147a:

    # f "I ain't shtopping until you stoppin' it!"
    f "你停下我才停下!"

# game/scripts/bar.rpy:2074
translate schinese felashcomp_bb8e4b4e:

    # a "..."
    a "......"

# game/scripts/bar.rpy:2075
translate schinese felashcomp_3ec57d49:

    # f "..."
    f "......"

# game/scripts/bar.rpy:2077
translate schinese felashcomp_d4a7c552:

    # f "Mmmph!" with ssshake
    f "嗯!" with ssshake

# game/scripts/bar.rpy:2078
translate schinese felashcomp_34a3f994:

    # a "Mmmm!"
    a "嗯!"

# game/scripts/bar.rpy:2079
translate schinese felashcomp_884c51eb:

    # c "Woah."
    c "喔."

# game/scripts/bar.rpy:2081
translate schinese felashcomp_5776ae1a:

    # a "We should have a threesome...!"
    a "我们应该来个3P......!"

# game/scripts/bar.rpy:2082
translate schinese felashcomp_b28a74a7:

    # f "Yeah, can we have a threesome?"
    f "是的,我们可以来个3P吗?"

# game/scripts/bar.rpy:2083
translate schinese felashcomp_0611d83d:

    # c "Wait, what? After what just happened?"
    c "等等,什么?就在刚刚发生那些事情之后?"

# game/scripts/bar.rpy:2085
translate schinese felashcomp_11beeaf7:

    # a "H- *hic*! Huh? What happened?"
    a "呃-*打嗝*!嗯?发生什么事了?"

# game/scripts/bar.rpy:2086
translate schinese felashcomp_b680b6ad:

    # f "Did something happen?"
    f "怎么了?"

# game/scripts/bar.rpy:2088
translate schinese felashcomp_95c58eb0:

    # f "Oh! We should have a threesome!"
    f "噢!我们要3P!"

# game/scripts/bar.rpy:2089
translate schinese felashcomp_bbff2bbb:

    # a "Good idea, can we have a threesome?"
    a "好主意,我们可以3P吗?"

# game/scripts/bar.rpy:2090
translate schinese felashcomp_d19f855d:

    # c "..."
    c "......"

# game/scripts/bar.rpy:2094
translate schinese felashcomp_bcdf0bcd:

    # f "Huh?"
    f "嗯?"

# game/scripts/bar.rpy:2095
translate schinese felashcomp_11ad5510:

    # a "A nap? Yeah... I'm tired..."
    a "休息一下?对......我累了......"

# game/scripts/bar.rpy:2096
translate schinese felashcomp_264b1133:

    # f "Me too..."
    f "我也是......"

# game/scripts/bar.rpy:2099
translate schinese felashcomp_237008b8:

    # "You bring Felicity and Ashley to their rooms, turn off the light, and let the rest for a couple of hours..."
    "你带着菲丽希蒂和阿什莉回了她们房间,关上了灯,让她们休息几小时......"

# game/scripts/bar.rpy:2103
translate schinese felashcomp_da8e8f3d:

    # c "...I don't know what's going on, but I kind of like it."
    c "......我不知道发生什么事了,但是我有点喜欢."

# game/scripts/bar.rpy:2104
translate schinese felashcomp_cf23c3dc:

    # a "Yay!"
    a "耶!"

# game/scripts/bar.rpy:2105
translate schinese felashcomp_44418ffb:

    # f "Come on! Let's go ta ya room..."
    f "来吧!我们去尼的放间......"

# game/scripts/bar.rpy:2108
translate schinese felashcomp_e71ee215:

    # f "Heheh... Your tits are small!"
    f "呵呵呵......你的奶子好小!"

# game/scripts/bar.rpy:2109
translate schinese felashcomp_d1d1598f:

    # a "Whaaat...?"
    a "什么......?"

# game/scripts/bar.rpy:2111
translate schinese felashcomp_44df49a1:

    # a "Well... Your pussy is small!"
    a "嗯......你的小穴好小!"

# game/scripts/bar.rpy:2112
translate schinese felashcomp_561d6b47:

    # f "But... That's a good thing...?"
    f "但是......这不是好事吗......?"

# game/scripts/bar.rpy:2114
translate schinese felashcomp_bb8e4b4e_1:

    # a "..."
    a "......"

# game/scripts/bar.rpy:2115
translate schinese felashcomp_d849f5c7:

    # a "Oh..."
    a "噢......"

# game/scripts/bar.rpy:2117
translate schinese felashcomp_274e9b78:

    # a "[name]~~~ Get on the bed so I can fuck you!"
    a "[name]~~~快去床上,这样我就可以上你了!"

# game/scripts/bar.rpy:2118
translate schinese felashcomp_0da6088a:

    # f "No, I want his dick!"
    f "不,我要他的肉棒!"

# game/scripts/bar.rpy:2119
translate schinese felashcomp_c121c1a2:

    # a "Oh... Then I guesshhh... We can do a pussy sandwich!"
    a "噢......那我想......我们可以来一个小穴三明治!"

# game/scripts/bar.rpy:2120
translate schinese felashcomp_1063037d:

    # c "A what?"
    c "什么?"

# game/scripts/bar.rpy:2121
translate schinese felashcomp_0e912a83:

    # f "Come on, let's do it! Get on tha bed!"
    f "来吧,我们做吧!快去床上!"

# game/scripts/bar.rpy:2122
translate schinese felashcomp_05588169:

    # a "Hurry, hurry!"
    a "快点,快点!"

# game/scripts/bar.rpy:2124
translate schinese felashcomp_119a37c4:

    # a "Nn! Nn! Nn!"
    a "嗯!嗯!嗯!"

# game/scripts/bar.rpy:2125
translate schinese felashcomp_71a77c78:

    # a "How does our pussy sandwich feel? Two pussies for the price of one!"
    a "我们的小穴三明治感觉如何?两面包夹之势!"

# game/scripts/bar.rpy:2126
translate schinese felashcomp_48c63894:

    # a "Nnn! Ahh~ Thish ish amashing!"
    a "嗯!啊~好树服!"

# game/scripts/bar.rpy:2130
translate schinese felashcomp_66dbb533:

    # f "It's like we're jerking him off! But with our pussies!"
    f "就好像我们在帮他打飞机!但是是用我们的小穴!"

# game/scripts/bar.rpy:2131
translate schinese felashcomp_efdee53c:

    # f "I wish you had two cocks, [name]..."
    f "我希望你有两根肉棒,[name]......"

# game/scripts/bar.rpy:2132
translate schinese felashcomp_7b9823ff:

    # a "*gasp*! That would be amashing [name]! You could fuck us both at the same time!"
    a "*惊叹*!那台棒了[name]!你可以同时操我们两个!"

# game/scripts/bar.rpy:2133
translate schinese felashcomp_9d88fb6e:

    # f "I don't think that's possible though..."
    f "但是我觉得那不可能......"

# game/scripts/bar.rpy:2134
translate schinese felashcomp_e745ce7c:

    # a "Aww..."
    a "啊......"

# game/scripts/bar.rpy:2138
translate schinese felashcomp_8b471513:

    # f "Nnnn~ Mmm!"
    f "嗯~嗯!"

# game/scripts/bar.rpy:2139
translate schinese felashcomp_80c1d588:

    # a "Ah~ [name]!"
    a "啊~[name]!"

# game/scripts/bar.rpy:2140
translate schinese felashcomp_cdcc1ade:

    # f "[name]! You cum all over our pussies!"
    f "[name]!你可以全射到我们小穴上!"

# game/scripts/bar.rpy:2141
translate schinese felashcomp_d4458237:

    # a "Yeah! Do it! This pussy sandwhich needs mayo!"
    a "是的!射吧!小穴三明治需要来点蛋黄酱!"

# game/scripts/bar.rpy:2142
translate schinese felashcomp_ba562e79:

    # f "That'sh gross..."
    f "恶心......"

# game/scripts/bar.rpy:2145
translate schinese felashcomp_ad71340c:

    # a "Cum [name]! Cum!"
    a "射吧[name]!射吧!"

# game/scripts/bar.rpy:2146
translate schinese felashcomp_a4a99fb8:

    # f "Cummmmm~!"
    f "射吧~!"

# game/scripts/bar.rpy:2148
translate schinese felashcomp_45497c9e:

    # a "Ahh~" with ssshake
    a "啊~" with ssshake

# game/scripts/bar.rpy:2149
translate schinese felashcomp_f6cfe3a7:

    # f "Mmm..."
    f "嗯......"

# game/scripts/bar.rpy:2150
translate schinese felashcomp_d19f855d_1:

    # c "..."
    c "......"

# game/scripts/bar.rpy:2151
translate schinese felashcomp_e74ae97f:

    # c "Girls?"
    c "女孩们?"

# game/scripts/bar.rpy:2152
translate schinese felashcomp_2710fe0c:

    # a "*snore*..."
    a "*鼾声*......"

# game/scripts/bar.rpy:2153
translate schinese felashcomp_4e98467e:

    # c "They fell asleep..."
    c "她们睡着了......"

# game/scripts/bar.rpy:2154
translate schinese felashcomp_3dc08cb5:

    # c "Well, it doesn't look like I'm going anywhere..."
    c "嗯,那我也无处可去了......"

# game/scripts/bar.rpy:2156
translate schinese felashcomp_09bf0796:

    # "You lay around a while. You attempt to get out from under their bodies, but figure you may as well let them rest."
    "你躺了一会.你试图从她们身体下脱身,但是想来想去还是让她们休息吧."

# game/scripts/bar.rpy:2157
translate schinese felashcomp_cf9fdc76:

    # "After some time, you too fall asleep..."
    "一会后,你睡着了......"

# game/scripts/bar.rpy:2160
translate schinese felashcomp_160e5338:

    # f "WHAT THE HELL ARE YOU DOING!?" with ssshake
    f "你他妈在干什么!?" with ssshake

# game/scripts/bar.rpy:2161
translate schinese felashcomp_3c395357:

    # a "SHUT UP! YOU SHOULDN'T BE HERE!"
    a "闭嘴!你不应该在这里的!"

# game/scripts/bar.rpy:2162
translate schinese felashcomp_f80627b2:

    # c "What's going on?"
    c "怎么了?"

# game/scripts/bar.rpy:2163
translate schinese felashcomp_07b7ddb8:

    # a "Felicity snuck into your bed after I did!"
    a "菲丽希蒂在我来之后溜到了你的床上!"

# game/scripts/bar.rpy:2164
translate schinese felashcomp_c8f25fd7:

    # f "That's not true! You came in after me!"
    f "那不是真的!你比我慢来!"

# game/scripts/bar.rpy:2165
translate schinese felashcomp_8fb907e0:

    # c "You guys don't remember what happened, do you?"
    c "你们不记得发生什么事了吗?"

# game/scripts/bar.rpy:2166
translate schinese felashcomp_068d0e2b:

    # f "Of course I remember! I came in at night to sleep with you!"
    f "我当然记得了!我晚上过来跟你睡觉的!"

# game/scripts/bar.rpy:2167
translate schinese felashcomp_0a62cbf6:

    # a "After me!"
    a "在我之后!"

# game/scripts/bar.rpy:2168
translate schinese felashcomp_8f04d579:

    # f "There's no way I would have gotten into bed with [name] while you were in it!"
    f "你在[name]房间里的时候我不可能进来的!"

# game/scripts/bar.rpy:2169
translate schinese felashcomp_043ba03b:

    # a "Same here!"
    a "我也一样!"

# game/scripts/bar.rpy:2171
translate schinese felashcomp_9cd4ac7e:

    # "*SLAM*" with sshake
    "*啪*" with sshake

# game/scripts/bar.rpy:2172
translate schinese felashcomp_cd9e4207:

    # c "*sigh*... Well, it was nice while it lasted..."
    c "*叹气*......好吧,至少刚才很美好......"

# game/scripts/bar.rpy:2182
translate schinese mareldrink_4b54968d:

    # m "You're on!"
    m "你来吧!"

# game/scripts/bar.rpy:2183
translate schinese mareldrink_3dbb2c35:

    # moon "Don't you guys think you've had enough?"
    moon "你们不觉得你们已经够了吗?"

# game/scripts/bar.rpy:2184
translate schinese mareldrink_064e0256:

    # e "Absolutely not! I won't let this young girl beat me! I was drinking since before she was born!"
    e "当然没有!我不会让这个年轻的女孩打败我的!我在她出生前就开始喝酒了!"

# game/scripts/bar.rpy:2185
translate schinese mareldrink_32268cae:

    # m "Yer just an old lady now! Look at you! You're already drunk!"
    m "你现在只是个老妇人!看看你!你已经喝醉了!"

# game/scripts/bar.rpy:2186
translate schinese mareldrink_b5562cbd:

    # e "I'm barely- *hic* tipsy! You're the drunk one!"
    e "我才没有-*打嗝*醉!你才是喝醉的那个!"

# game/scripts/bar.rpy:2187
translate schinese mareldrink_966572ce:

    # moon "Guys..."
    moon "伙计们......"

# game/scripts/bar.rpy:2188
translate schinese mareldrink_6c144784:

    # m "Then we'll have a competition! Loser has to... Eh..."
    m "那我们比赛吧!输的人要......嗯......"

# game/scripts/bar.rpy:2190
translate schinese mareldrink_cbc9be92:

    # m "Oh! Loser has to suck [name]'s cock right here!"
    m "噢!输的人要在这里给[name]口交!"

# game/scripts/bar.rpy:2191
translate schinese mareldrink_69e6b179:

    # e "Whaat? I can't do that..."
    e "什么?我不能那么做......"

# game/scripts/bar.rpy:2193
translate schinese mareldrink_833076f0:

    # m "Then you admit you lost!"
    m "那你就是承认你输了!"

# game/scripts/bar.rpy:2194
translate schinese mareldrink_8f40646d:

    # e "Absolutely not!"
    e "当然不是!"

# game/scripts/bar.rpy:2195
translate schinese mareldrink_b1fb8efc:

    # e "Fine, I'll do this challenge, you're about to pass out anyway!"
    e "好吧,我接受挑战,反正你也马上要晕倒了!"

# game/scripts/bar.rpy:2196
translate schinese mareldrink_d915769d:

    # m "G- *hic* Great! Moon, get some drinks!"
    m "好-*打嗝*好的!文,来点喝的!"

# game/scripts/bar.rpy:2197
translate schinese mareldrink_c951055f:

    # moon "I think I'm just going to step outside for a while..."
    moon "我想我还是先出去一会吧......"

# game/scripts/bar.rpy:2198
translate schinese mareldrink_fd1a19df:

    # m "Great! [name], get some drinks!"
    m "好的![name],来点喝的!"

# game/scripts/bar.rpy:2199
translate schinese mareldrink_eec7ed34:

    # c "Alright..."
    c "好吧......"

# game/scripts/bar.rpy:2201
translate schinese mareldrink_a9028231:

    # "You walk up to Bart..."
    "你走向了巴特......"

# game/scripts/bar.rpy:2202
translate schinese mareldrink_44076629:

    # c "Hmm... Whoever loses is going to give me a blowjob... I might be able to influence who the loser will be..."
    c "嗯......谁输了谁就给我口交......那也许我可以影响谁会输......"

# game/scripts/bar.rpy:2208
translate schinese mareldrink_2f3f1bb8:

    # bart "Here's your drinks!"
    bart "这是你的饮料!"

# game/scripts/bar.rpy:2210
translate schinese mareldrink_404de09a:

    # m "Woo! Alright, you ready to lose, woman?"
    m "喔!好了,你已经输了,女人?"

# game/scripts/bar.rpy:2211
translate schinese mareldrink_9bdfdae9:

    # e "You're going down young lady!"
    e "你才要晕倒了,年轻的小姐!"

# game/scripts/bar.rpy:2212
translate schinese mareldrink_45a7cc2e:

    # m "Go!"
    m "来!"

# game/scripts/bar.rpy:2214
translate schinese mareldrink_88615573:

    # m "*gulp*!"
    m "*咕噜*!"

# game/scripts/bar.rpy:2215
translate schinese mareldrink_01440cc7:

    # e "*gulp*!"
    e "*咕噜*!"

# game/scripts/bar.rpy:2217
translate schinese mareldrink_1d3483fe:

    # e "Ready to give up?" with ssshake
    e "准备投降了吗?" with ssshake

# game/scripts/bar.rpy:2218
translate schinese mareldrink_f1b5510c:

    # m "Never!"
    m "才不!"

# game/scripts/bar.rpy:2220
translate schinese mareldrink_88615573_1:

    # m "*gulp*!"
    m "*咕噜*!"

# game/scripts/bar.rpy:2221
translate schinese mareldrink_01440cc7_1:

    # e "*gulp*!"
    e "*咕噜*!"

# game/scripts/bar.rpy:2223
translate schinese mareldrink_67e5fea7:

    # m "Ahh! Like drinking water!" with ssshake
    m "啊!就像喝水一样!" with ssshake

# game/scripts/bar.rpy:2224
translate schinese mareldrink_f90daa8f:

    # e "Bluffing won't help you here!"
    e "虚张声势在这里可帮不了你!"

# game/scripts/bar.rpy:2226
translate schinese mareldrink_88615573_2:

    # m "*gulp*!"
    m "*咕噜*!"

# game/scripts/bar.rpy:2227
translate schinese mareldrink_01440cc7_2:

    # e "*gulp*!"
    e "*咕噜*!"

# game/scripts/bar.rpy:2230
translate schinese mareldrink_b4bb1a1d:

    # e "Ohhh..." with ssshake
    e "噢......" with ssshake

# game/scripts/bar.rpy:2231
translate schinese mareldrink_f8666273:

    # m "I knew it! You can't handle your drinks!"
    m "我就知道!你都拿不起你的饮料了!"

# game/scripts/bar.rpy:2233
translate schinese mareldrink_669954d5:

    # e "Be quiet young lady!"
    e "安静,年轻的小姐!"

# game/scripts/bar.rpy:2235
translate schinese mareldrink_88615573_3:

    # m "*gulp*!"
    m "*咕噜*!"

# game/scripts/bar.rpy:2236
translate schinese mareldrink_01440cc7_3:

    # e "*gulp*!"
    e "*咕噜*!"

# game/scripts/bar.rpy:2238
translate schinese mareldrink_f80085e7:

    # e "It feels like my throat is burning..." with ssshake
    e "我的喉咙好像在燃烧......" with ssshake

# game/scripts/bar.rpy:2239
translate schinese mareldrink_0a799024:

    # m "Shut up and drink!"
    m "闭嘴,喝!"

# game/scripts/bar.rpy:2241
translate schinese mareldrink_88615573_4:

    # m "*gulp*!"
    m "*咕噜*!"

# game/scripts/bar.rpy:2242
translate schinese mareldrink_01440cc7_4:

    # e "*gulp*!"
    e "*咕噜*!"

# game/scripts/bar.rpy:2244
translate schinese mareldrink_1c41824e:

    # e "Nn! I can't take another drink!" with ssshake
    e "嗯!我不能再喝了!" with ssshake

# game/scripts/bar.rpy:2245
translate schinese mareldrink_430accc1:

    # m "You'll never be able to beat the vitality of a young woman!"
    m "你永远无法战胜一个年轻女人的活力!"

# game/scripts/bar.rpy:2246
translate schinese mareldrink_4112267f:

    # m "Now get to sucking!"
    m "现在去口交!"

# game/scripts/bar.rpy:2247
translate schinese mareldrink_b855b153:

    # e "Ah... I guess I have to..."
    e "啊......我想我必须去了......"

# game/scripts/bar.rpy:2249
translate schinese mareldrink_0f600867:

    # e "Well [name], it looks like I lost..."
    e "嗯[name],看起来我输了......"

# game/scripts/bar.rpy:2250
translate schinese mareldrink_5f8d0ecb:

    # e "Let's get to it, then!"
    e "那就,来吧!"

# game/scripts/bar.rpy:2252
translate schinese mareldrink_6ffbe535:

    # e "Aghhk! Mmmghh!"
    e "啊!嗯!"

# game/scripts/bar.rpy:2253
translate schinese mareldrink_c879512e:

    # m "Damn! You're really good!"
    m "操!你真的好棒!"

# game/scripts/bar.rpy:2254
translate schinese mareldrink_095a4c57:

    # e "Of coursh, I've been shucking dick before you were even born!"
    e "当染了,在你出生之前我就口角过了!"

# game/scripts/bar.rpy:2255
translate schinese mareldrink_e8292414:

    # e "NNGHK! Ghhpphh! Mmm, [name]!"
    e "嗯!咕!嗯,[name]!"

# game/scripts/bar.rpy:2256
translate schinese mareldrink_ef4ee7d9:

    # m "This is so hot!"
    m "好火辣!"

# game/scripts/bar.rpy:2259
translate schinese mareldrink_d43e3c76:

    # e "Mmmgph! [name]! Thish is amashing!"
    e "嗯![name]!台棒了!"

# game/scripts/bar.rpy:2260
translate schinese mareldrink_22b591ae:

    # m "You're making me wet Ellen... I can't believe how good you are!"
    m "你让我湿了艾兰......我不敢相信你居然这么厉害!"

# game/scripts/bar.rpy:2261
translate schinese mareldrink_aa566afa:

    # e "Mmmph! Mmmp! GHK!"
    e "嗯!嗯!咕!"

# game/scripts/bar.rpy:2262
translate schinese mareldrink_c7e7fbc4:

    # e "Oofgh... I'm getting so dizzy... I had way too much to drink..."
    e "噢......我好晕......我喝太多了......"

# game/scripts/bar.rpy:2263
translate schinese mareldrink_0f7e2807:

    # e "Shtill, NGH! I don't mind giving a blowjob every once in a while..."
    e "操,嗯!我不介意每隔一段时间就给人口交......"

# game/scripts/bar.rpy:2264
translate schinese mareldrink_c59e464a:

    # m "Cum all over her tits, [name]! I want to see those bimbo tits covered!"
    m "全射到她奶子上吧,[name]!我想看这个傻大妞的奶子都被覆盖住!"

# game/scripts/bar.rpy:2265
translate schinese mareldrink_ca58b2e2:

    # e "I love being shprayed with cum... Do it! Nghk!"
    e "我喜欢被你的精液福盖......射吧!嗯!"

# game/scripts/bar.rpy:2267
translate schinese mareldrink_7d670ce7:

    # e "Nnn!" with ssshake
    e "嗯!" with ssshake

# game/scripts/bar.rpy:2268
translate schinese mareldrink_12b9c7d6:

    # m "So hot!"
    m "好火辣!"

# game/scripts/bar.rpy:2273
translate schinese mareldrink_839c51d9:

    # m "Ahh..." with ssshake
    m "啊......" with ssshake

# game/scripts/bar.rpy:2274
translate schinese mareldrink_4cc6d550:

    # e "There's no need for you to keep pushing yourself..."
    e "你没有必要逼自己......"

# game/scripts/bar.rpy:2276
translate schinese mareldrink_1a0a36d7:

    # m "Shut it, woman!"
    m "闭嘴,女人!"

# game/scripts/bar.rpy:2278
translate schinese mareldrink_88615573_5:

    # m "*gulp*!"
    m "*咕噜*!"

# game/scripts/bar.rpy:2279
translate schinese mareldrink_01440cc7_5:

    # e "*gulp*!"
    e "*咕噜*!"

# game/scripts/bar.rpy:2281
translate schinese mareldrink_5bc1b2c0:

    # m "Nnh, my throat's on fire!" with ssshake
    m "嗯,我的喉咙着火了!" with ssshake

# game/scripts/bar.rpy:2282
translate schinese mareldrink_50603dd0:

    # e "Save yourself from a hangover!"
    e "别喝了,再喝你就要宿醉了!"

# game/scripts/bar.rpy:2284
translate schinese mareldrink_88615573_6:

    # m "*gulp*!"
    m "*咕噜*!"

# game/scripts/bar.rpy:2285
translate schinese mareldrink_01440cc7_6:

    # e "*gulp*!"
    e "*咕噜*!"

# game/scripts/bar.rpy:2287
translate schinese mareldrink_568eed7b:

    # m "Agh! No more! This is too much..." with sshake
    m "啊!不要了!太多了......" with sshake

# game/scripts/bar.rpy:2288
translate schinese mareldrink_0f27a83f:

    # e "Hahaha! What did I say? You can't beat a veteran!"
    e "哈哈哈!我说什么来着?你是打不过老手的!"

# game/scripts/bar.rpy:2289
translate schinese mareldrink_f6df7abc:

    # m "Oooghh..."
    m "噢......"

# game/scripts/bar.rpy:2290
translate schinese mareldrink_99fe0a2d:

    # e "Well, get to sucking young lady!"
    e "嗯,现在去口交吧年轻的小姐!"

# game/scripts/bar.rpy:2291
translate schinese mareldrink_3df5b466:

    # m "Alright...!"
    m "好吧......!"

# game/scripts/bar.rpy:2293
translate schinese mareldrink_2aeb2419:

    # m "Well, I guess I lost... But maybe I- *hic*! Planned it! Heheh!"
    m "嗯,我想我输了......但是也许我-*打嗝*本来就是这么打算的!呵呵呵!"

# game/scripts/bar.rpy:2295
translate schinese mareldrink_f312154c:

    # m "Ghlllsh... Gphh..."
    m "咕噜......咕......"

# game/scripts/bar.rpy:2296
translate schinese mareldrink_c8bc4ae1:

    # m "Nnguh! [name]..."
    m "嗯![name]......"

# game/scripts/bar.rpy:2297
translate schinese mareldrink_407a88f9:

    # e "Oh my, You're very skilled at this, aren't you?"
    e "哦我天,你很擅长这个,对吗?"

# game/scripts/bar.rpy:2298
translate schinese mareldrink_b8d5fde7:

    # m "Yesh, of coursh! I love shucking cocksh..."
    m "是的,当然了!我喜欢口角......"

# game/scripts/bar.rpy:2299
translate schinese mareldrink_82590478:

    # m "Anggh! Mmphh!"
    m "啊!嗯!"

# game/scripts/bar.rpy:2302
translate schinese mareldrink_ab0d0419:

    # e "Be careful not to throw up! It can be hard when you're drunk."
    e "小心点别吐出来了!你喝醉的时候这可不容易."

# game/scripts/bar.rpy:2303
translate schinese mareldrink_d65d5412:

    # m "Pff... Whatever... I won't throw up! I don't even have a gag reflex!"
    m "切......随便啦......我不会吐的!我甚至都没有咽喉反射(指异物进入喉咙导致的呕吐)!"

# game/scripts/bar.rpy:2304
translate schinese mareldrink_22cb8b39:

    # e "Hm, if that were me, I would have made him cum already..."
    e "嗯,如果是我的话,我已经让他射了......"

# game/scripts/bar.rpy:2305
translate schinese mareldrink_dae1158d:

    # m "Shut up, lady! You couldn't make turtle come out of it's shell!"
    m "闭嘴,女士!就算是只乌龟你也没法让它的头伸出来!"

# game/scripts/bar.rpy:2306
translate schinese mareldrink_8babc9cd:

    # m "Oogh! Nngh... Ooohh, I'm dizzy now..."
    m "噢!嗯......噢,我好晕......"

# game/scripts/bar.rpy:2307
translate schinese mareldrink_a5c621a7:

    # e "[name], you should cum all over her! Her punishment for losing should be cleaning the cum out of her clothes!"
    e "[name],你应该全射到她身上!她输了的惩罚是清理她衣服上的精液!"

# game/scripts/bar.rpy:2308
translate schinese mareldrink_47b68efd:

    # m "I love being shprayed with cum... Do it! Nghk!"
    m "我喜欢被精液福盖......射吧!嗯!"

# game/scripts/bar.rpy:2310
translate schinese mareldrink_7fc21377:

    # m "Ahhh! Yesss..." with ssshake
    m "啊!太棒了......" with ssshake

# game/scripts/bar.rpy:2311
translate schinese mareldrink_cf9998f6:

    # e "I hope that tought you a lesson, young lady!"
    e "我希望你学到教训了,年轻的小姐!"

# game/scripts/bar.rpy:2312
translate schinese mareldrink_8d90f69d:

    # m "If anything, I just want to do it again!"
    m "如果有的话,我还会再来一次的!"

# game/scripts/bar.rpy:2317
translate schinese mareldrink_e2cce33b:

    # m "Let's do this every weekend!"
    m "我们每周末都来一次吧!"

# game/scripts/bar.rpy:2318
translate schinese mareldrink_5ce850b7:

    # e "That sounds fun!"
    e "那听起来很不错!"

# game/scripts/bar.rpy:2320
translate schinese mareldrink_a5b6ebee:

    # "You can now call Ellen and Maria to the bar for a drinking competition on the weekends from the bartender!"
    "你现在可以在周末把艾兰和玛丽亚叫到酒吧,然后由酒保举行喝酒比赛!"

# game/scripts/bar.rpy:2331
translate schinese barsarah_65013f10:

    # sar "Ohh, who are *hic* ...you?"
    sar "噢,你是*打嗝*......谁?"

# game/scripts/bar.rpy:2332
translate schinese barsarah_6f362034:

    # c "I'm [name], I own this place."
    c "我是[name],我拥有这个地方."

# game/scripts/bar.rpy:2333
translate schinese barsarah_439512c0:

    # sar "*gasp*! Really? You do...? You must be pretty rich then, right...?"
    sar "*惊叹*!真的吗?你真的吗......?那你一定很有钱吧......?"

# game/scripts/bar.rpy:2334
translate schinese barsarah_197d6e62:

    # c "I guess...?"
    c "我想......?"

# game/scripts/bar.rpy:2335
translate schinese barsarah_d214be24:

    # sar "Hey... Do you maybe wanna just fuck?"
    sar "嘿......你想做爱吗?"

# game/scripts/bar.rpy:2338
translate schinese barsarah_86db6596:

    # sar "H- *hic* Hey baby... Wanna fuck?"
    sar "嘿-*打嗝*嘿宝贝......想做爱吗?"

# game/scripts/bar.rpy:2342
translate schinese barsarah_f24da8da:

    # sar "Yeahhhh! Let's fuck! ...*hic*!"
    sar "太好了!我们做爱吧!......*打嗝*!"

# game/scripts/bar.rpy:2346
translate schinese barsarah_2313a426:

    # sar "Whaaat?"
    sar "什么?"

# game/scripts/bar.rpy:2351
translate schinese marbarin_5ac5901f:

    # m "Ohh, I love it! This looks amazing! So classy!"
    m "噢,我喜欢!看起来好棒!好有品味!"

# game/scripts/bar.rpy:2352
translate schinese marbarin_0e488bde:

    # moon "Good choice, [name]!"
    moon "好选择,[name]!"

# game/scripts/bar.rpy:2354
translate schinese marbarin_0a24c8f0:

    # m "Thanks [name]! I really appreciate this job offer!"
    m "谢谢你[name]!我真的很感谢这个工作机会!"

# game/scripts/bar.rpy:2355
translate schinese marbarin_ff1a7c12:

    # c "No problem. I'll leave you two to it."
    c "没关系.我会让你们去做的."

# game/scripts/bar.rpy:2356
translate schinese marbarin_f8d108d9:

    # moon "Thank you boss!"
    moon "谢谢你老板!"

# game/scripts/bar.rpy:2357
translate schinese marbarin_ec789916:

    # "Maria will now work as a bartender in the Evenings!"
    "玛丽亚现在会在傍晚担任调酒师!"

# game/scripts/bar.rpy:2363
translate schinese marbar_9ba72ba9:

    # m "Well, I do like wine..."
    m "嗯,我喜欢葡萄酒......"

# game/scripts/bar.rpy:2364
translate schinese marbar_3eb5de86:

    # moon "Aww, wine is for babies! You need something stronger!"
    moon "啊,葡萄酒是给小孩子喝的!你得来点更烈的!"

# game/scripts/bar.rpy:2365
translate schinese marbar_88ac41d3:

    # m "What's your drink of choice then?"
    m "你这里有什么饮料选择吗?"

# game/scripts/bar.rpy:2366
translate schinese marbar_0f65037f:

    # moon "Ethanol!"
    moon "无水乙醇!"

# game/scripts/bar.rpy:2367
translate schinese marbar_1517b933:

    # m "Hahaha! You're joking, right?"
    m "哈哈哈!你在开玩笑,对吧?"

# game/scripts/bar.rpy:2368
translate schinese marbar_f0c0187b:

    # moon "Haha, yes! Just kidding! I like Vodka."
    moon "哈哈,是的!只是在开玩笑!我喜欢伏特加."

# game/scripts/bar.rpy:2369
translate schinese marbar_c968eac1:

    # m "A girl your size would die from one sip..."
    m "你这么大的女孩喝一口就会死的......"

# game/scripts/bar.rpy:2370
translate schinese marbar_ed36d996:

    # moon "Is that a challenge?"
    moon "这是挑战吗?"

# game/scripts/bar.rpy:2371
translate schinese marbar_2ad54806:

    # m "I wouldn't want my new friend to die the same day I met her..."
    m "我不希望我的新朋友在我遇到的第一天就死了......"

# game/scripts/bar.rpy:2372
translate schinese marbar_710e89d9:

    # moon "You're on!"
    moon "你也来!"

# game/scripts/bar.rpy:2373
translate schinese marbar_a92b0fc2:

    # m "I'm just joking around! I need to stay sober today."
    m "我只是在开玩笑!我今天得保持清醒!"

# game/scripts/bar.rpy:2375
translate schinese marbar_9bd7d5df:

    # m "Oh! Speaking of which... Hey [name]!"
    m "噢!说到那个......嘿[name]!"

# game/scripts/bar.rpy:2376
translate schinese marbar_48fc288b:

    # c "What's up?"
    c "怎么了?"

# game/scripts/bar.rpy:2377
translate schinese marbar_60ef9a40:

    # m "Think I can work as a waitress here? I would love to work right next to Moon!"
    m "你觉得我可以在这里当服务员吗?我很愿意在文旁边工作的!"

# game/scripts/bar.rpy:2378
translate schinese marbar_dce73ee8:

    # moon "It would be very fun, boss!"
    moon "这会很有趣的,老板!"

# game/scripts/bar.rpy:2379
translate schinese marbar_1fc93202:

    # c "I don't think we have any open spots for a waitress..."
    c "我想我们没有服务员的空缺......"

# game/scripts/bar.rpy:2381
translate schinese marbar_ec6f5a99:

    # m "What? Come on! You know I need a job! And I would never need to leave the hotel! It would be so fun!"
    m "什么?拜托!你知道我需要一份工作的!而且我永远也不需要离开酒店!这会很有趣的!"

# game/scripts/bar.rpy:2383
translate schinese marbar_a8f767b5:

    # m "I could always suck your cock during my breaks..."
    m "我可以在休息时间给你口交......"

# game/scripts/bar.rpy:2385
translate schinese marbar_648e56d0:

    # m "Or you could just come in and bend me over a counter whenever you want. We wouldn't even have to leave..."
    m "或者你可以随时进来把我压在柜台上.我们甚至都不需要离开......"

# game/scripts/bar.rpy:2386
translate schinese marbar_f9bc8863:

    # moon "Umm..."
    moon "嗯......"

# game/scripts/bar.rpy:2387
translate schinese marbar_e633bb5b:

    # c "Sorry, Moon is enough. You could be our bartender though."
    c "对不起,文一个人就够了.不过你可以当我们的调酒师."

# game/scripts/bar.rpy:2389
translate schinese marbar_7fe78d08:

    # bart "What?"
    bart "什么?"

# game/scripts/bar.rpy:2390
translate schinese marbar_86070833:

    # m "Oh! That would be fun!"
    m "噢!那会很有趣的!"

# game/scripts/bar.rpy:2391
translate schinese marbar_ee3f05d1:

    # moon "Yeah, you said you liked making drinks! I bet you'd be a great bartender!"
    moon "是的,你说你喜欢做饮料的!我打赌你会成为一个伟大的调酒师的!"

# game/scripts/bar.rpy:2393
translate schinese marbar_0c9ee6d6:

    # moon "Oh... But, um..."
    moon "噢......但是,嗯......"

# game/scripts/bar.rpy:2394
translate schinese marbar_569e966a:

    # c "It's fine. Maria can work Evenings, Bart will work the rest of the time."
    c "没事的.玛丽亚可以傍晚来工作,巴特会在其他时间工作."

# game/scripts/bar.rpy:2396
translate schinese marbar_1441b83c:

    # bart "Ah, works for me."
    bart "啊,这对我来说有好处啊."

# game/scripts/bar.rpy:2397
translate schinese marbar_424fdc9a:

    # m "Yes!"
    m "太好了!"

# game/scripts/bar.rpy:2399
translate schinese marbar_33046856:

    # m "Oh, but I don't have anything to wear... Can you buy me something [name]?"
    m "噢,但是我没啥可穿的......你可以给我买点东西吗[name]?"

# game/scripts/bar.rpy:2400
translate schinese marbar_c48dc32f:

    # c "Sure."
    c "当然可以."

# game/scripts/bar.rpy:2402
translate schinese marbar_e8c493cd:

    # m "Awesome!"
    m "太棒了!"

# game/scripts/bar.rpy:2404
translate schinese marbar_a1fd488a:

    # moon "I can't wait to work with you, Maria!"
    moon "我等不及要和你一起工作了,玛丽亚!"

# game/scripts/bar.rpy:2405
translate schinese marbar_a7b1cc51:

    # m "I'm excited!"
    m "我好兴奋!"

# game/scripts/bar.rpy:2406
translate schinese marbar_741743b9:

    # "New items have been unlocked on your computer."
    "你电脑上的新项目已被解锁."

# game/scripts/bar.rpy:2412
translate schinese sarahfucking_461125cf:

    # sar "Ohhh! Oh... Fuck... Mmmm..."
    sar "噢!噢......操......嗯......"

# game/scripts/bar.rpy:2413
translate schinese sarahfucking_1fde0a9e:

    # sar "Ahhhhhnnnnn! That's the spot... Ohhh...!"
    sar "啊!就是那个点......噢......!"

# game/scripts/bar.rpy:2414
translate schinese sarahfucking_db6574a8:

    # pat "Damn, I love a show and a drink!"
    pat "操,我喜欢这里的表演和饮料!"

# game/scripts/bar.rpy:2415
translate schinese sarahfucking_cec2b207:

    # sar "Heh... People are watching...? I'm a whore... Hahhh..."
    sar "呵呵......人们在看噢......?我是个妓女......哈......"

# game/scripts/bar.rpy:2416
translate schinese sarahfucking_88a4054d:

    # sar "Oh, yes baby! Mmmm... Fuck that cunt!"
    sar "噢,是的宝贝!嗯......操我的小穴!"

# game/scripts/bar.rpy:2422
translate schinese sarahbend_cfd6915e:

    # sar "Ahh!" with ssshake
    sar "啊!" with ssshake

# game/scripts/bar.rpy:2423
translate schinese sarahbend_82715544:

    # sar "Ohh... I got drunk again, and now I'm being bred!" with ssshake
    sar "噢......我有喝醉了,现在我在交尾!" with ssshake

# game/scripts/bar.rpy:2424
translate schinese sarahbend_4416f7f5:

    # sar "*hic*! Mmmm, but drunk sex feels so goooood..." with ssshake
    sar "*打嗝*!嗯,但是喝醉了再做爱感觉好棒......" with ssshake

# game/scripts/bar.rpy:2426
translate schinese sarahbend_c5f46b1b:

    # sar "Thanksh baby... That was... Really hot!"
    sar "谢了宝贝......那真的......好火辣!"

# game/scripts/bar.rpy:2427
translate schinese sarahbend_67479876:

    # sar "I'm gonna buy another drink, I think?"
    sar "我想,我要再买一杯饮料了?"

# game/scripts/bar.rpy:2432
translate schinese sarahbend_b6d7f6d9:

    # sar "Ohhhh~!" with ssshake
    sar "噢~!" with ssshake

# game/scripts/bar.rpy:2433
translate schinese sarahbend_f31fd421:

    # sar "Nnnn... I gotta clean my dresshh now? *hic*!"
    sar "嗯......现在我得清理我的衣服了?*打嗝*!"

# game/scripts/bar.rpy:2435
translate schinese sarahbend_c5f46b1b_1:

    # sar "Thanksh baby... That was... Really hot!"
    sar "谢了宝贝......那真的......好火辣!"

# game/scripts/bar.rpy:2436
translate schinese sarahbend_67479876_1:

    # sar "I'm gonna buy another drink, I think?"
    sar "我想,我要再买一杯饮料了?"

# game/scripts/bar.rpy:2441
translate schinese sarahbend_7659f129:

    # sar "Ahhh! Oh baby! Oh my god... That dick is so good!"
    sar "啊!噢宝贝!哦我天......这根肉棒好爽!"

# game/scripts/bar.rpy:2442
translate schinese sarahbend_ca53a534:

    # sar "Mmm! Nnn! Fuck, fuuuuck! Ahhh..."
    sar "嗯!嗯!操,操!啊......"

# game/scripts/bar.rpy:2443
translate schinese sarahbend_89c6717a:

    # pat "Best bar in town!"
    pat "镇子里最棒的酒吧!"

# game/scripts/bar.rpy:2444
translate schinese sarahbend_58dcd575:

    # sar "Mmmmmmmm, baby... Fuck me, fuck me! Ohh!"
    sar "嗯,宝贝......操我,操我!噢!"

# game/scripts/bar.rpy:2445
translate schinese sarahbend_5c13729b:

    # sar "Wait... Wait! Don't cum inside me, I don't wanna another baby..."
    sar "等等......等等!别射进来,我不想要另一个孩子了......"

# game/scripts/bar.rpy:2446
translate schinese sarahbend_36104ee3:

    # sar "Oh god... Drunk sex is so fucking hooooot!"
    sar "哦天......酒后做爱感觉好他妈爽!"

# game/scripts/bar.rpy:2452
translate schinese sarahbend_a4982d6a:

    # sar "Ohh!" with ssshake
    sar "噢!" with ssshake

# game/scripts/bar.rpy:2453
translate schinese sarahbend_a781556c:

    # sar "I can't get *hic*!... Pregnant..." with ssshake
    sar "我不能*打嗝*!......怀孕......" with ssshake

# game/scripts/bar.rpy:2454
translate schinese sarahbend_5127b32e:

    # sar "Mmmm... I already have a son..." with ssshake
    sar "嗯......我已经有儿子了......" with ssshake

# game/scripts/bar.rpy:2455
translate schinese sarahbend_1160e3dc:

    # sar "Well... That's not really good, huh? Ohhh, I'm too drunk..."
    sar "嗯......不是很好,嗯?噢,我太醉了......"

# game/scripts/bar.rpy:2457
translate schinese sarahbend_c5f46b1b_2:

    # sar "Thanksh baby... That was... Really hot!"
    sar "谢了宝贝......那真的......很爽!"

# game/scripts/bar.rpy:2458
translate schinese sarahbend_67479876_2:

    # sar "I'm gonna buy another drink, I think?"
    sar "我想,我要再来一杯喝的?"

# game/scripts/bar.rpy:2463
translate schinese sarahbend_f515e0c2:

    # sar "Oh fuck!" with ssshake
    sar "哦操!" with ssshake

# game/scripts/bar.rpy:2464
translate schinese sarahbend_b231b409:

    # sar "Woahhhh... It's like a fire hose! *hic*!"
    sar "喔......这就像一根消防水管!*打嗝*!"

# game/scripts/bar.rpy:2466
translate schinese sarahbend_c5f46b1b_3:

    # sar "Thanksh baby... That was... Really hot!"
    sar "谢了宝贝......那真的......很爽!"

# game/scripts/bar.rpy:2467
translate schinese sarahbend_1afed28f:

    # sar "I gots ta clean now though... Hah..."
    sar "不过我得去清理一下了......哈哈......"

# game/scripts/bar.rpy:2481
translate schinese barmart_2b258ccb:

    # m "Hey [name]!"
    m "嘿[name]!"

# game/scripts/bar.rpy:2508
translate schinese drinking2_dc3e4a7d:

    # c "Oof... I'm starting to get a little tipsy..."
    c "噢......我开始有点醉了......"

# game/scripts/bar.rpy:2523
translate schinese drinking2_46d1823b:

    # c "Ergh..."
    c "呃......"

# game/scripts/bar.rpy:2531
translate schinese drinking2_58ef29f7:

    # c "I shoulnnnnt a had thhah ohhngge..."
    c "我补应该和这个的......"

# game/scripts/bar.rpy:2537
translate schinese drinking2_d19f855d:

    # c "..."
    c "......"

# game/scripts/bar.rpy:2549
translate schinese drinking2_cad9094b:

    # c "Urgh... Where am I...?"
    c "呃......我在哪......?"

# game/scripts/bar.rpy:2550
translate schinese drinking2_3f42245b:

    # c "I need to get some sleep..."
    c "我得去睡觉了......"

# game/scripts/bar.rpy:2566
translate schinese barbart_a1d771f1:

    # bart "Hey boss."
    bart "嘿老板."

# game/scripts/bar.rpy:2593
translate schinese drinking_dc3e4a7d:

    # c "Oof... I'm starting to get a little tipsy..."
    c "噢......我开始有点醉了......"

# game/scripts/bar.rpy:2608
translate schinese drinking_46d1823b:

    # c "Ergh..."
    c "呃......"

# game/scripts/bar.rpy:2616
translate schinese drinking_58ef29f7:

    # c "I shoulnnnnt a had thhah ohhngge..."
    c "我补应该和这个的......"

# game/scripts/bar.rpy:2622
translate schinese drinking_d19f855d:

    # c "..."
    c "......"

# game/scripts/bar.rpy:2634
translate schinese drinking_cad9094b:

    # c "Urgh... Where am I...?"
    c "呃......我在哪......?"

# game/scripts/bar.rpy:2635
translate schinese drinking_3f42245b:

    # c "I need to get some sleep..."
    c "我得去睡觉了......"

# game/scripts/bar.rpy:2641
translate schinese drinking_78138fef:

    # "They'll only accept a competition during the weekends."
    "他们只在周末接受比赛."

# game/scripts/bar.rpy:2645
translate schinese drinking_57f1c8ae:

    # c "Prepare some drinks."
    c "准备点喝的."

# game/scripts/bar.rpy:2646
translate schinese drinking_3bd66d56:

    # bart "Sure boss."
    bart "好的老板."

# game/scripts/bar.rpy:2649
translate schinese drinking_b9825a46:

    # "Maria isn't here..."
    "玛丽亚不在......"

# game/scripts/bar.rpy:2660
translate schinese barmooncat_e49e664a:

    # moon "Hello!"
    moon "你好!"

# game/scripts/bar.rpy:2663
translate schinese barmooncattalk_ca7682c4:

    # moon "I'm doing aMEOWzing!"
    moon "喵极了!"

# game/scripts/bar.rpy:2664
translate schinese barmooncattalk_0aca6b58:

    # moon "Get it? Haha!"
    moon "听懂了吗?哈哈!"

# game/scripts/bar.rpy:2667
translate schinese barmooncattalk_f44f19ec:

    # "She's not ready yet."
    "她还没准备好."

# game/scripts/bar.rpy:2670
translate schinese barmooncattalk_dc35cdf5:

    # moon "It still stings, [name]... Give me time to recover first."
    moon "现在还是有点痛,[name]......给我点时间恢复一下吧."

# game/scripts/bar.rpy:2678
translate schinese barmooncattalk_5deb73b6:

    # moon "There are people here, [name]... Shouldn't we do this when nobody is around?"
    moon "这里有人,[name]......我们不应该在没人的时候做这个吗?"

# game/scripts/bar.rpy:2679
translate schinese barmooncattalk_0206afc4:

    # c "I'm sure you can agree the humiliation just adds to the punishment."
    c "我相信你会同意的,羞辱也刚好增加了惩罚的部分."

# game/scripts/bar.rpy:2680
translate schinese barmooncattalk_4984c9d6:

    # moon "..."
    moon "......"

# game/scripts/bar.rpy:2681
translate schinese barmooncattalk_7e63ec25:

    # moon "Okay..."
    moon "好吧......"

# game/scripts/bar.rpy:2683
translate schinese barmooncattalk_208f70a7:

    # moon "I can take more...!"
    moon "我还可以再忍受一会......!"

# game/scripts/bar.rpy:2685
translate schinese barmooncattalk_00e1eca0:

    # moon "Please stop looking at me, everyone!"
    moon "大家请不要再看我了!"

# game/scripts/bar.rpy:2687
translate schinese barmooncattalk_dad59b7f:

    # moon "Fine... Enough... I think that's enough..."
    moon "好吧......够了......我想已经够了......"

# game/scripts/bar.rpy:2691
translate schinese barmooncattalk_84381bf3:

    # moon "Enough!"
    moon "够了!"

# game/scripts/bar.rpy:2697
translate schinese barmooncattalk_0dc45d60:

    # moon "Oh! Sorry!"
    moon "噢!对不起!"

# game/scripts/bar.rpy:2700
translate schinese barmooncattalk_13a409dc:

    # c "...Maybe that was too much..."
    c "......也许有点过头了......"

# game/scripts/bar.rpy:2706
translate schinese barmooncattalk_ba314112:

    # c "That's enough for now."
    c "够了."

# game/scripts/bar.rpy:2707
translate schinese barmooncattalk_71d89851:

    # moon "Nnn, okay..."
    moon "嗯,好的......"

# game/scripts/bar.rpy:2711
translate schinese barmooncattalk_ba314112_1:

    # c "That's enough for now."
    c "够了."

# game/scripts/bar.rpy:2712
translate schinese barmooncattalk_7e63ec25_1:

    # moon "Okay..."
    moon "好的......"

# game/scripts/bar.rpy:2721
translate schinese barmooncattalk_41a59255:

    # moon "This outfit is so humiliating..."
    moon "这件衣服好羞耻......"

# game/scripts/bar.rpy:2722
translate schinese barmooncattalk_0206afc4_1:

    # c "I'm sure you can agree the humiliation just adds to the punishment."
    c "我相信你会同意的,羞辱也刚好增加了惩罚的部分."

# game/scripts/bar.rpy:2723
translate schinese barmooncattalk_aeccfdc8:

    # moon "Right, right..."
    moon "好吧,好吧......"

# game/scripts/bar.rpy:2725
translate schinese barmooncattalk_f4ff0555:

    # moon "Everyone is staring at me..."
    moon "每个人都在看着我......"

# game/scripts/bar.rpy:2727
translate schinese barmooncattalk_c37bea86:

    # moon "Stop looking at me!"
    moon "别看我了!"

# game/scripts/bar.rpy:2729
translate schinese barmooncattalk_dad59b7f_1:

    # moon "Fine... Enough... I think that's enough..."
    moon "好吧......够了......我想已经够了......"

# game/scripts/bar.rpy:2733
translate schinese barmooncattalk_84381bf3_1:

    # moon "Enough!"
    moon "够了!"

# game/scripts/bar.rpy:2739
translate schinese barmooncattalk_0dc45d60_1:

    # moon "Oh! Sorry!"
    moon "噢!对不起!"

# game/scripts/bar.rpy:2742
translate schinese barmooncattalk_13a409dc_1:

    # c "...Maybe that was too much..."
    c "......也许有点过头了......"

# game/scripts/bar.rpy:2748
translate schinese barmooncattalk_ba314112_2:

    # c "That's enough for now."
    c "够了."

# game/scripts/bar.rpy:2749
translate schinese barmooncattalk_71d89851_1:

    # moon "Nnn, okay..."
    moon "嗯,好的......"

# game/scripts/bar.rpy:2753
translate schinese barmooncattalk_ba314112_3:

    # c "That's enough for now."
    c "够了."

# game/scripts/bar.rpy:2754
translate schinese barmooncattalk_7e63ec25_2:

    # moon "Okay..."
    moon "好的......"

# game/scripts/bar.rpy:2762
translate schinese barmooncattalk_da9cf4ef:

    # moon "Do we really have to do this again?"
    moon "我们真的得再做一次吗?"

# game/scripts/bar.rpy:2763
translate schinese barmooncattalk_1ec63ee4:

    # c "We can stop if you'd like..."
    c "如果你想的话我们可以停下......"

# game/scripts/bar.rpy:2764
translate schinese barmooncattalk_ec997992:

    # moon "I guess we can at least go at least once more..."
    moon "我想我们至少可以再来一次......"

# game/scripts/bar.rpy:2767
translate schinese barmooncattalk_7966257c:

    # moon "Hnnn... [name]..."
    moon "嗯......[name]......"

# game/scripts/bar.rpy:2768
translate schinese barmooncattalk_68f145e3:

    # c "Are you getting wet?"
    c "你湿了吗?"

# game/scripts/bar.rpy:2769
translate schinese barmooncattalk_8bd2acfd:

    # moon "What!? Stop looking there!"
    moon "什么!?别看那里!"

# game/scripts/bar.rpy:2771
translate schinese barmooncattalk_cfa2dbf5:

    # moon "This feels strange..."
    moon "感觉好奇怪......"

# game/scripts/bar.rpy:2773
translate schinese barmooncattalk_dad59b7f_2:

    # moon "Fine... Enough... I think that's enough..."
    moon "好吧......够了......我想已经够了......"

# game/scripts/bar.rpy:2777
translate schinese barmooncattalk_84381bf3_2:

    # moon "Enough!"
    moon "够了!"

# game/scripts/bar.rpy:2783
translate schinese barmooncattalk_0dc45d60_2:

    # moon "Oh! Sorry!"
    moon "噢!对不起!"

# game/scripts/bar.rpy:2786
translate schinese barmooncattalk_13a409dc_2:

    # c "...Maybe that was too much..."
    c "......也许有点过头了......"

# game/scripts/bar.rpy:2792
translate schinese barmooncattalk_ba314112_4:

    # c "That's enough for now."
    c "够了."

# game/scripts/bar.rpy:2793
translate schinese barmooncattalk_71d89851_2:

    # moon "Nnn, okay..."
    moon "嗯,好的......"

# game/scripts/bar.rpy:2797
translate schinese barmooncattalk_ba314112_5:

    # c "That's enough for now."
    c "够了."

# game/scripts/bar.rpy:2798
translate schinese barmooncattalk_7e63ec25_3:

    # moon "Okay..."
    moon "好的......"

# game/scripts/bar.rpy:2807
translate schinese barmooncattalk_236a448e:

    # moon "[name], I feel a lot less guilty about what I've done now. I think the spankings actually helped!"
    moon "[name],我现在对我做的那些事不再那么内疚了.我觉得打屁股是真的有帮助的!"

# game/scripts/bar.rpy:2812
translate schinese barmooncattalk_d7b8ec89:

    # moon "Gah!"
    moon "咕!"

# game/scripts/bar.rpy:2814
translate schinese barmooncattalk_37f78821:

    # moon "Hnn!"
    moon "嗯!"

# game/scripts/bar.rpy:2816
translate schinese barmooncattalk_f724519e:

    # moon "Ah!"
    moon "啊!"

# game/scripts/bar.rpy:2818
translate schinese barmooncattalk_11aa970e:

    # moon "Sss!"
    moon "嘶!"

# game/scripts/bar.rpy:2820
translate schinese barmooncattalk_b268297c:

    # moon "AHH!"
    moon "啊!"

# game/scripts/bar.rpy:2826
translate schinese barmooncattalk_d7b8ec89_1:

    # moon "Gah!"
    moon "咕!"

# game/scripts/bar.rpy:2828
translate schinese barmooncattalk_37f78821_1:

    # moon "Hnn!"
    moon "嗯!"

# game/scripts/bar.rpy:2830
translate schinese barmooncattalk_f724519e_1:

    # moon "Ah!"
    moon "啊!"

# game/scripts/bar.rpy:2832
translate schinese barmooncattalk_11aa970e_1:

    # moon "Sss!"
    moon "嘶!"

# game/scripts/bar.rpy:2834
translate schinese barmooncattalk_b268297c_1:

    # moon "AHH!"
    moon "啊!"

# game/scripts/bar.rpy:2835
translate schinese barmooncattalk_796ac522:

    # c "That's good to hear."
    c "很高兴听到这个消息."

# game/scripts/bar.rpy:2837
translate schinese barmooncattalk_54066883:

    # moon "Nnh..."
    moon "嗯......"

# game/scripts/bar.rpy:2838
translate schinese barmooncattalk_455f825a:

    # c "Seems like you're enjoying it as well."
    c "看起来你也很享受."

# game/scripts/bar.rpy:2839
translate schinese barmooncattalk_4965796b:

    # moon "Don't be silly..."
    moon "别傻了......"

# game/scripts/bar.rpy:2842
translate schinese barmooncattalk_e67bfb60:

    # moon "Wait, what are you-"
    moon "等等,你在干什-"

# game/scripts/bar.rpy:2844
translate schinese barmooncattalk_c44ef131:

    # moon "Ah!" with ssshake
    moon "啊!" with ssshake

# game/scripts/bar.rpy:2846
translate schinese barmooncattalk_6ab22eb5:

    # moon "You'll die for that!"
    moon "你想找死吗!"

# game/scripts/bar.rpy:2847
translate schinese barmooncattalk_1d76b582:

    # c "You'd just get keep getting them wet..."
    c "你那里一直都是湿的......"

# game/scripts/bar.rpy:2849
translate schinese barmooncattalk_cc285698:

    # moon "T-That only happened once!"
    moon "那-那只发生过一次!"

# game/scripts/bar.rpy:2850
translate schinese barmooncattalk_f8cb617b:

    # c "Twice now."
    c "现在两次了."

# game/scripts/bar.rpy:2851
translate schinese barmooncattalk_9bc5ace8:

    # c "Besides, I'm sure you can agree the humiliation just-"
    c "而且,我相信你同意羞辱只是-"

# game/scripts/bar.rpy:2853
translate schinese barmooncattalk_aef6a62a:

    # moon "Don't give me that! That's my pussy! I don't want people seeing that!"
    moon "别跟我那么说!那是我的小穴!我不想要别人看到它!"

# game/scripts/bar.rpy:2855
translate schinese barmooncattalk_0cea0e5d:

    # moon "Thank you..."
    moon "谢谢你......"

# game/scripts/bar.rpy:2857
translate schinese barmooncattalk_d0b54138:

    # c "Are you going to get wet again?"
    c "你是不是又要湿了?"

# game/scripts/bar.rpy:2858
translate schinese barmooncattalk_3f9f60ea:

    # moon "Of course not! Just punish me already!"
    moon "当然不会!直接来惩罚我吧!"

# game/scripts/bar.rpy:2861
translate schinese barmooncattalk_59932a8c:

    # c "That was fast."
    c "太快了."

# game/scripts/bar.rpy:2862
translate schinese barmooncattalk_4f0bbe92:

    # moon "What was?"
    moon "什么?"

# game/scripts/bar.rpy:2863
translate schinese barmooncattalk_711dc129:

    # c "You're wet again."
    c "你又湿了."

# game/scripts/bar.rpy:2865
translate schinese barmooncattalk_eba3d098:

    # moon "Rgh! Stop looking! Are we done?"
    moon "呃!别看了!我们完事了吗?"

# game/scripts/bar.rpy:2869
translate schinese barmooncattalk_84381bf3_3:

    # moon "Enough!"
    moon "够了!"

# game/scripts/bar.rpy:2875
translate schinese barmooncattalk_0dc45d60_3:

    # moon "Oh! Sorry!"
    moon "噢!对不起!"

# game/scripts/bar.rpy:2878
translate schinese barmooncattalk_13a409dc_3:

    # c "...Maybe that was too much..."
    c "......也许有点过头了......"

# game/scripts/bar.rpy:2884
translate schinese barmooncattalk_ba314112_6:

    # c "That's enough for now."
    c "够了."

# game/scripts/bar.rpy:2885
translate schinese barmooncattalk_71d89851_3:

    # moon "Nnn, okay..."
    moon "嗯,好的......"

# game/scripts/bar.rpy:2889
translate schinese barmooncattalk_ba314112_7:

    # c "That's enough for now."
    c "够了."

# game/scripts/bar.rpy:2890
translate schinese barmooncattalk_7e63ec25_4:

    # moon "Okay..."
    moon "好的......"

# game/scripts/bar.rpy:2899
translate schinese barmooncattalk_4984c9d6_1:

    # moon "..."
    moon "......"

# game/scripts/bar.rpy:2900
translate schinese barmooncattalk_51e1fcda:

    # c "I haven't even done anything yet..."
    c "我什么都还没做呢......"

# game/scripts/bar.rpy:2902
translate schinese barmooncattalk_0ae93eca:

    # moon "Haven't done anything? You've spanked me so much! Look at what you've done!"
    moon "什么都没做?你打我屁股那么多次了!看看你都干了什么!"

# game/scripts/bar.rpy:2903
translate schinese barmooncattalk_7043ba7a:

    # moon "Actually wait, don't look!"
    moon "等等,别看!"

# game/scripts/bar.rpy:2904
translate schinese barmooncattalk_3e0716eb:

    # c "I've only spanked you, it's not my fault if it turns you on."
    c "我只是打了你的屁股,你兴奋了又不是我的错."

# game/scripts/bar.rpy:2905
translate schinese barmooncattalk_3ff210ff:

    # moon "T-Turns me on!? No! That..."
    moon "我-我兴奋了!?不是!那......"

# game/scripts/bar.rpy:2906
translate schinese barmooncattalk_b0d66984:

    # moon "That's just not true! You broke my pussy or something!"
    moon "那不是真的!是你搞坏了我的小穴什么的!"

# game/scripts/bar.rpy:2907
translate schinese barmooncattalk_d12b8add:

    # c "I haven't touched your pussy..."
    c "我都没碰过你的小穴......"

# game/scripts/bar.rpy:2908
translate schinese barmooncattalk_73140e0f:

    # moon "It's the vibrations from the spankings! You do it too hard! This would happen to any girl!"
    moon "是打屁股传过去的!你太大力了!任何女孩都会这样子的!"

# game/scripts/bar.rpy:2910
translate schinese barmooncattalk_db282ef6:

    # moon "...Just... Take them off... I'm tired of cleaning them every day..."
    moon "......就......直接把它们拿下来吧......我已经厌倦了每天清洗它们......"

# game/scripts/bar.rpy:2912
translate schinese barmooncattalk_4984c9d6_2:

    # moon "..."
    moon "......"

# game/scripts/bar.rpy:2914
translate schinese barmooncattalk_3980b5f5:

    # c "You have a nice ass."
    c "屁股不错."

# game/scripts/bar.rpy:2916
translate schinese barmooncattalk_be0de4d2:

    # moon "Rghh..."
    moon "呃......"

# game/scripts/bar.rpy:2918
translate schinese barmooncattalk_048f42ff:

    # moon "Just punish me already..."
    moon "就直接来惩罚我吧......"

# game/scripts/bar.rpy:2923
translate schinese barmooncattalk_4ad7b887:

    # c "Don't say it like that."
    c "别那样说."

# game/scripts/bar.rpy:2924
translate schinese barmooncattalk_c1205426:

    # moon "Excuse me?"
    moon "那怎么说?"

# game/scripts/bar.rpy:2925
translate schinese barmooncattalk_00d76987:

    # c "Say 'Please spank my tight ass [name].'"
    c "说:'请打我的小紧屁股吧[name].'"

# game/scripts/bar.rpy:2927
translate schinese barmooncattalk_ed082e3a:

    # moon "What!? Why should I do that?"
    moon "什么!?为什么我要这么做?"

# game/scripts/bar.rpy:2928
translate schinese barmooncattalk_c6e9fdc4:

    # c "Because it would be hot?"
    c "因为那样很火辣?"

# game/scripts/bar.rpy:2929
translate schinese barmooncattalk_40da1b81:

    # moon "RGH!"
    moon "呃!"

# game/scripts/bar.rpy:2932
translate schinese barmooncattalk_de6d2bc0:

    # moon "You deserved that one!"
    moon "你活该!"

# game/scripts/bar.rpy:2936
translate schinese barmooncattalk_13a409dc_4:

    # c "...Maybe that was too much..."
    c "......也许有点过头了......"

# game/scripts/bar.rpy:2942
translate schinese barmooncattalk_c396f73a:

    # moon "It stings more than usual..."
    moon "比平时更痛了......"

# game/scripts/bar.rpy:2944
translate schinese barmooncattalk_f59df506:

    # c "Are you getting wetter?"
    c "你也更湿了?"

# game/scripts/bar.rpy:2945
translate schinese barmooncattalk_13f97163:

    # moon "Don't say such things! Punish me!"
    moon "别说那些话!惩罚我!"

# game/scripts/bar.rpy:2946
translate schinese barmooncattalk_6ccb6e7f:

    # c "Again."
    c "再来."

# game/scripts/bar.rpy:2947
translate schinese barmooncattalk_f498d30b:

    # moon "What?"
    moon "什么?"

# game/scripts/bar.rpy:2948
translate schinese barmooncattalk_fc17c8cf:

    # c "Say it again."
    c "再说一次."

# game/scripts/bar.rpy:2949
translate schinese barmooncattalk_9b847ee1:

    # moon "P-Punish me...?"
    moon "惩-惩罚我......?"

# game/scripts/bar.rpy:2951
translate schinese barmooncattalk_dad59b7f_3:

    # moon "Fine... Enough... I think that's enough..."
    moon "好吧......够了......我想已经够了......"

# game/scripts/bar.rpy:2955
translate schinese barmooncattalk_84381bf3_4:

    # moon "Enough!"
    moon "够了!"

# game/scripts/bar.rpy:2961
translate schinese barmooncattalk_0dc45d60_4:

    # moon "Oh! Sorry!"
    moon "噢!对不起!"

# game/scripts/bar.rpy:2964
translate schinese barmooncattalk_13a409dc_5:

    # c "...Maybe that was too much..."
    c "......也许有点过头了......"

# game/scripts/bar.rpy:2970
translate schinese barmooncattalk_ba314112_8:

    # c "That's enough for now."
    c "够了."

# game/scripts/bar.rpy:2971
translate schinese barmooncattalk_71d89851_4:

    # moon "Nnn, okay..."
    moon "嗯,好的......"

# game/scripts/bar.rpy:2975
translate schinese barmooncattalk_ba314112_9:

    # c "That's enough for now."
    c "够了."

# game/scripts/bar.rpy:2976
translate schinese barmooncattalk_7e63ec25_5:

    # moon "Okay..."
    moon "好的......"

# game/scripts/bar.rpy:2985
translate schinese barmooncattalk_4984c9d6_3:

    # moon "..."
    moon "......"

# game/scripts/bar.rpy:2986
translate schinese barmooncattalk_27cdeed0:

    # c "Want me to take your panties off?"
    c "想让我把你内裤脱了吗?"

# game/scripts/bar.rpy:2987
translate schinese barmooncattalk_e9b8976d:

    # moon "Yes..."
    moon "是的......"

# game/scripts/bar.rpy:2988
translate schinese barmooncattalk_0d7444fd:

    # c "I'm surprised you even come to work with panties still."
    c "我很惊奇你居然还穿着内裤来上班."

# game/scripts/bar.rpy:2990
translate schinese barmooncattalk_4984c9d6_4:

    # moon "..."
    moon "......"

# game/scripts/bar.rpy:2996
translate schinese barmooncattalk_00d76987_1:

    # c "Say 'Please spank my tight ass [name].'"
    c "说'请打我的小紧屁股吧[name].'"

# game/scripts/bar.rpy:2998
translate schinese barmooncattalk_0d562c6d:

    # moon "What!?"
    moon "什么!?"

# game/scripts/bar.rpy:2999
translate schinese barmooncattalk_0206afc4_2:

    # c "I'm sure you can agree the humiliation just adds to the punishment."
    c "我相信你会同意的,羞辱也刚好增加了惩罚的部分."

# game/scripts/bar.rpy:3000
translate schinese barmooncattalk_925a615a:

    # moon "Stop saying that!"
    moon "别说了!"

# game/scripts/bar.rpy:3002
translate schinese barmooncattalk_44e50adc:

    # moon "Fine... Whatever..."
    moon "好吧......随便了......"

# game/scripts/bar.rpy:3003
translate schinese barmooncattalk_8217c2e5:

    # moon "Please spank my tight ass... [name]..."
    moon "请打我的小紧屁股吧......[name]......"

# game/scripts/bar.rpy:3004
translate schinese barmooncattalk_9696ccb2:

    # c "Good girl."
    c "好女孩."

# game/scripts/bar.rpy:3006
translate schinese barmooncattalk_dff7887f:

    # moon "Grr..."
    moon "呃......"

# game/scripts/bar.rpy:3009
translate schinese barmooncattalk_ae13a16b:

    # moon "...Do you like spanking me?"
    moon "......你喜欢打我屁股吗?"

# game/scripts/bar.rpy:3010
translate schinese barmooncattalk_297d93f3:

    # c "What?"
    c "什么?"

# game/scripts/bar.rpy:3011
translate schinese barmooncattalk_838f40c4:

    # moon "You seem to really enjoy it..."
    moon "你看起来真的很享受......"

# game/scripts/bar.rpy:3012
translate schinese barmooncattalk_c2da66bb:

    # moon "I don't know if you just want to punish me... Or if you just like seeing my ass... Or if you like hurting me..."
    moon "我不知道你是不是只是想惩罚我......还是说你只是喜欢看我的屁股......或者你喜欢伤害我......"

# game/scripts/bar.rpy:3015
translate schinese barmooncattalk_68f4bff9:

    # moon "Oh..."
    moon "噢......"

# game/scripts/bar.rpy:3016
translate schinese barmooncattalk_da436381:

    # moon "I mean... Right."
    moon "我是说......好吧."

# game/scripts/bar.rpy:3018
translate schinese barmooncattalk_4984c9d6_5:

    # moon "..."
    moon "......"

# game/scripts/bar.rpy:3019
translate schinese barmooncattalk_931d1bcc:

    # moon "You... You really think so...?"
    moon "你......你真的这么觉得吗......?"

# game/scripts/bar.rpy:3020
translate schinese barmooncattalk_3bc3e1f4:

    # moon "Mmm..."
    moon "嗯......"

# game/scripts/bar.rpy:3021
translate schinese barmooncattalk_3ac179ce:

    # moon "Spank me, please..."
    moon "请打我屁股吧......"

# game/scripts/bar.rpy:3023
translate schinese barmooncattalk_a3068798:

    # moon "...I see..."
    moon "......我懂了......"

# game/scripts/bar.rpy:3024
translate schinese barmooncattalk_f60c322b:

    # moon "Get back to it, then..."
    moon "那就继续吧......"

# game/scripts/bar.rpy:3027
translate schinese barmooncattalk_cef922d0:

    # moon "*gasp*!"
    moon "*惊叹*!"

# game/scripts/bar.rpy:3029
translate schinese barmooncattalk_bfa75403:

    # moon "What are you-"
    moon "你在干什-"

# game/scripts/bar.rpy:3030
translate schinese barmooncattalk_4f2bc13e:

    # c "-Just cleaning you up."
    c "-只是在帮你清理一下."

# game/scripts/bar.rpy:3031
translate schinese barmooncattalk_da11c7b7:

    # moon "That's a terrible lie! You're not supposed to-"
    moon "那是个糟糕的借口!你不应该-"

# game/scripts/bar.rpy:3033
translate schinese barmooncattalk_f4c0ab1a:

    # moon "-Hnn!"
    moon "-嗯!"

# game/scripts/bar.rpy:3034
translate schinese barmooncattalk_488a4409:

    # moon "Mmmm... Ahhh..."
    moon "嗯......啊......"

# game/scripts/bar.rpy:3035
translate schinese barmooncattalk_3b49566c:

    # moon "Th... That... Ohh... Ohhh..."
    moon "那......那里......噢......噢......"

# game/scripts/bar.rpy:3036
translate schinese barmooncattalk_d15034ef:

    # moon "Mmm... Nn... That's... Ah..."
    moon "嗯......嗯......那......啊......"

# game/scripts/bar.rpy:3040
translate schinese barmooncattalk_7b3a4d59:

    # moon "You're not supposed to touch me there, remember?" with ssshake
    moon "你不应该碰我那里的,记得吗?" with ssshake

# game/scripts/bar.rpy:3041
translate schinese barmooncattalk_881c312e:

    # c "Sorry, you seemed like you enjoyed it."
    c "对不起,你看起来很享受的."

# game/scripts/bar.rpy:3043
translate schinese barmooncattalk_030731d7:

    # moon "Shut up... Spank me..."
    moon "闭嘴......打我屁股......"

# game/scripts/bar.rpy:3048
translate schinese barmooncattalk_84381bf3_5:

    # moon "Enough!"
    moon "够了!"

# game/scripts/bar.rpy:3054
translate schinese barmooncattalk_0dc45d60_5:

    # moon "Oh! Sorry!"
    moon "噢!对不起!"

# game/scripts/bar.rpy:3057
translate schinese barmooncattalk_13a409dc_6:

    # c "...Maybe that was too much..."
    c "......也许有点过头了......"

# game/scripts/bar.rpy:3063
translate schinese barmooncattalk_ba314112_10:

    # c "That's enough for now."
    c "够了."

# game/scripts/bar.rpy:3064
translate schinese barmooncattalk_71d89851_5:

    # moon "Nnn, okay..."
    moon "嗯,好吧......"

# game/scripts/bar.rpy:3068
translate schinese barmooncattalk_ba314112_11:

    # c "That's enough for now."
    c "够了."

# game/scripts/bar.rpy:3069
translate schinese barmooncattalk_7e63ec25_6:

    # moon "Okay..."
    moon "好吧......"

# game/scripts/bar.rpy:3078
translate schinese barmooncattalk_d19f855d:

    # c "..."
    c "......"

# game/scripts/bar.rpy:3079
translate schinese barmooncattalk_5e35f542:

    # c "You're not wearing panties?"
    c "你没穿内裤?"

# game/scripts/bar.rpy:3080
translate schinese barmooncattalk_ab40fe7b:

    # moon "...I... Forgot to bring an extra pair today..."
    moon "......我......我今天忘多带一条了......"

# game/scripts/bar.rpy:3081
translate schinese barmooncattalk_d0ae06d2:

    # c "Is that so?"
    c "真的吗?"

# game/scripts/bar.rpy:3083
translate schinese barmooncattalk_5146e411:

    # moon "Shut up and spank me..."
    moon "闭嘴,打我屁股......"

# game/scripts/bar.rpy:3086
translate schinese barmooncattalk_cef922d0_1:

    # moon "*gasp*!"
    moon "*惊叹*!"

# game/scripts/bar.rpy:3088
translate schinese barmooncattalk_bfa75403_1:

    # moon "What are you-"
    moon "你在干什-"

# game/scripts/bar.rpy:3089
translate schinese barmooncattalk_4f2bc13e_1:

    # c "-Just cleaning you up."
    c "-只是在帮你清理一下"

# game/scripts/bar.rpy:3090
translate schinese barmooncattalk_007e83d9:

    # moon "You said that last time! That excuse still won't-"
    moon "你上次就这么说了!这个借口不能-"

# game/scripts/bar.rpy:3092
translate schinese barmooncattalk_f4c0ab1a_1:

    # moon "-Hnn!"
    moon "-嗯!"

# game/scripts/bar.rpy:3093
translate schinese barmooncattalk_4ce77e5e:

    # c "Hm?"
    c "嗯?"

# game/scripts/bar.rpy:3094
translate schinese barmooncattalk_e4de1040:

    # moon "Ehhh... Ss... Shut up..."
    moon "呃......闭......闭嘴......"

# game/scripts/bar.rpy:3095
translate schinese barmooncattalk_b5780022:

    # moon "Ohh... Nnn... Oh my... Mmm..."
    moon "噢......嗯......哦我天......嗯......"

# game/scripts/bar.rpy:3096
translate schinese barmooncattalk_62f96dc5:

    # moon "S... St... Sto... Mmm... Nn..."
    moon "停......停......停......嗯......嗯......"

# game/scripts/bar.rpy:3098
translate schinese barmooncattalk_0f908422:

    # moon "W-What are you doing!?"
    moon "你-你在干什么!?"

# game/scripts/bar.rpy:3099
translate schinese barmooncattalk_4e1a0168:

    # c "It sounded like you wanted me to stop."
    c "听起来你想让我停下啊."

# game/scripts/bar.rpy:3100
translate schinese barmooncattalk_2aeb2c32:

    # moon "I never...! I- I didn't... I..."
    moon "我没有......!我-我没有......我......"

# game/scripts/bar.rpy:3101
translate schinese barmooncattalk_4984c9d6_6:

    # moon "..."
    moon "......"

# game/scripts/bar.rpy:3103
translate schinese barmooncattalk_1bdc3bd8:

    # moon "...I never said stop..."
    moon "......我没说停下......"

# game/scripts/bar.rpy:3105
translate schinese barmooncattalk_5bde8cb4:

    # moon "Mmmmmm..."
    moon "嗯......"

# game/scripts/bar.rpy:3106
translate schinese barmooncattalk_6941d226:

    # moon "Hahhh...! Nnn...! That's... So... Nnn..."
    moon "哈......!嗯......!那......好......嗯......"

# game/scripts/bar.rpy:3107
translate schinese barmooncattalk_6d3c6a7f:

    # moon "[name]... Nnnmmm..."
    moon "[name]......嗯......"

# game/scripts/bar.rpy:3111
translate schinese barmooncattalk_e3b3c07b:

    # moon "Ahh! Mmmm..." with dissolve
    moon "啊!嗯......" with dissolve

# game/scripts/bar.rpy:3112
translate schinese barmooncattalk_eda4f0af:

    # moon "You're... Inside... Me..."
    moon "你......进入......我了......"

# game/scripts/bar.rpy:3113
translate schinese barmooncattalk_30c9e87c:

    # moon "Ssnnn... Ohh... There..."
    moon "嗯......噢......那里......"

# game/scripts/bar.rpy:3114
translate schinese barmooncattalk_bab4749a:

    # moon "Right... T-There... Ahh...! Hnn!"
    moon "就是......那-那里......啊......!嗯!"

# game/scripts/bar.rpy:3116
translate schinese barmooncattalk_99fa2e42:

    # "*shlick*" with dissolve
    "*轻拍*" with dissolve

# game/scripts/bar.rpy:3117
translate schinese barmooncattalk_90bed004:

    # moon "Mmmmm...?"
    moon "嗯......?"

# game/scripts/bar.rpy:3119
translate schinese barmooncattalk_8a577270:

    # moon "Wait, what are you doing!? You're supposed to... Be spanking me!"
    moon "等等,你在干什么!?你应该......打我屁股的!"

# game/scripts/bar.rpy:3121
translate schinese barmooncattalk_7c0e22f0:

    # moon "...So spank me..."
    moon "......打我屁股吧......"

# game/scripts/bar.rpy:3126
translate schinese barmooncattalk_84381bf3_6:

    # moon "Enough!"
    moon "够了!"

# game/scripts/bar.rpy:3132
translate schinese barmooncattalk_0dc45d60_6:

    # moon "Oh! Sorry!"
    moon "噢!对不起!"

# game/scripts/bar.rpy:3135
translate schinese barmooncattalk_13a409dc_7:

    # c "...Maybe that was too much..."
    c "......也许有点太过了......"

# game/scripts/bar.rpy:3141
translate schinese barmooncattalk_ba314112_12:

    # c "That's enough for now."
    c "够了."

# game/scripts/bar.rpy:3142
translate schinese barmooncattalk_71d89851_6:

    # moon "Nnn, okay..."
    moon "嗯,好吧......"

# game/scripts/bar.rpy:3146
translate schinese barmooncattalk_ba314112_13:

    # c "That's enough for now."
    c "够了."

# game/scripts/bar.rpy:3147
translate schinese barmooncattalk_7e63ec25_7:

    # moon "Okay..."
    moon "好吧......"

# game/scripts/bar.rpy:3156
translate schinese barmooncattalk_1c6c7788:

    # c "No panties... Nice."
    c "没穿内裤......太棒了."

# game/scripts/bar.rpy:3157
translate schinese barmooncattalk_244ff9be:

    # moon "...I forgot my... Nevermind..."
    moon "......我忘了我的......算了......"

# game/scripts/bar.rpy:3158
translate schinese barmooncattalk_ba41627c:

    # c "Why don't you forget the rest of your clothes too?"
    c "为什么你不把其他衣服也忘了?"

# game/scripts/bar.rpy:3160
translate schinese barmooncattalk_e3009336:

    # moon "What...!?"
    moon "什么......!?"

# game/scripts/bar.rpy:3161
translate schinese barmooncattalk_5ba0cf3c:

    # c "Your ass has already been out there, the rest won't be much more."
    c "你的屁股都出来了,剩下的也没多少了......"

# game/scripts/bar.rpy:3163
translate schinese barmooncattalk_b6c34a9b:

    # moon "That's ridiculous..."
    moon "那太荒谬了......"

# game/scripts/bar.rpy:3165
translate schinese barmooncattalk_ea756623:

    # moon "...The rest of my clothes have nothing to do with spanking, [name]..."
    moon "......我剩下的衣服和打屁股无关,[name]......"

# game/scripts/bar.rpy:3167
translate schinese barmooncattalk_7222fcf6:

    # moon "...But I'm sure you can agree the humiliation just adds to the punishment..."
    moon "......但我相信你会同意的,羞辱也刚好增加了惩罚的部分......"

# game/scripts/bar.rpy:3169
translate schinese barmooncattalk_bdf3e631:

    # moon "...Among other effects..."
    moon "......还有别的影响......"

# game/scripts/bar.rpy:3172
translate schinese barmooncattalk_3089766c:

    # moon "Interesting..."
    moon "有趣......"

# game/scripts/bar.rpy:3173
translate schinese barmooncattalk_297d93f3_1:

    # c "What?"
    c "什么?"

# game/scripts/bar.rpy:3174
translate schinese barmooncattalk_c16ea5bd:

    # moon "Oh, nothing..."
    moon "噢,没什么......"

# game/scripts/bar.rpy:3176
translate schinese barmooncattalk_5b61b904:

    # moon "Um... Where's your phone?"
    moon "嗯......你的手机呢?"

# game/scripts/bar.rpy:3177
translate schinese barmooncattalk_c98eb17c:

    # c "In my back pocket?"
    c "在我后面的口袋里?"

# game/scripts/bar.rpy:3178
translate schinese barmooncattalk_68f4bff9_1:

    # moon "Oh..."
    moon "噢......"

# game/scripts/bar.rpy:3180
translate schinese barmooncattalk_1d741ad6:

    # moon "Oh... I almost forgot, I'm naked aren't I?"
    moon "噢......我差点忘了,我是不是裸着?"

# game/scripts/bar.rpy:3181
translate schinese barmooncattalk_ae2abe22:

    # c "You are."
    c "是的."

# game/scripts/bar.rpy:3182
translate schinese barmooncattalk_21d00f0a:

    # moon "I was just so focused..."
    moon "我只是太专注了......"

# game/scripts/bar.rpy:3187
translate schinese barmooncattalk_84381bf3_7:

    # moon "Enough!"
    moon "够了!"

# game/scripts/bar.rpy:3193
translate schinese barmooncattalk_0dc45d60_7:

    # moon "Oh! Sorry!"
    moon "噢!对不起!"

# game/scripts/bar.rpy:3196
translate schinese barmooncattalk_13a409dc_8:

    # c "...Maybe that was too much..."
    c "......也许有点太过了......"

# game/scripts/bar.rpy:3202
translate schinese barmooncattalk_ba314112_14:

    # c "That's enough for now."
    c "够了."

# game/scripts/bar.rpy:3203
translate schinese barmooncattalk_71d89851_7:

    # moon "Nnn, okay..."
    moon "嗯,好吧......"

# game/scripts/bar.rpy:3207
translate schinese barmooncattalk_ba314112_15:

    # c "That's enough for now."
    c "够了."

# game/scripts/bar.rpy:3208
translate schinese barmooncattalk_7e63ec25_8:

    # moon "Okay..."
    moon "好吧......"

# game/scripts/bar.rpy:3217
translate schinese barmooncattalk_038c8a31:

    # moon "...I should really buy new panties some day, hahah..."
    moon "......我真的得找个时间买点新内裤,哈哈哈......"

# game/scripts/bar.rpy:3218
translate schinese barmooncattalk_4bd2b1f0:

    # c "They'd just get in the way."
    c "那它们只会碍手碍脚."

# game/scripts/bar.rpy:3220
translate schinese barmooncattalk_c5bdbc04:

    # moon "Maybe they would..."
    moon "也许会吧......"

# game/scripts/bar.rpy:3222
translate schinese barmooncattalk_6233657e:

    # moon "Besides, it hurts more without them..."
    moon "而且,没有它们疼多了......"

# game/scripts/bar.rpy:3224
translate schinese barmooncattalk_747ace26:

    # moon "So I suppose you can continue..."
    moon "所以我想你可以继续......"

# game/scripts/bar.rpy:3226
translate schinese barmooncattalk_a72ed496:

    # moon "W-Wait...! That's not spanking...!"
    moon "等-等等......!这不是打屁股......!"

# game/scripts/bar.rpy:3233
translate schinese barmooncattalk_0debc013:

    # moon "Ahh..."
    moon "啊......"

# game/scripts/bar.rpy:3235
translate schinese barmooncattalk_3bc3e1f4_1:

    # moon "Mmm..."
    moon "嗯......"

# game/scripts/bar.rpy:3237
translate schinese barmooncattalk_8988c88e:

    # moon "Hnng..."
    moon "嗯......"

# game/scripts/bar.rpy:3239
translate schinese barmooncattalk_37f78821_2:

    # moon "Hnn!"
    moon "嗯!"

# game/scripts/bar.rpy:3241
translate schinese barmooncattalk_f724519e_2:

    # moon "Ah!"
    moon "啊!"

# game/scripts/bar.rpy:3243
translate schinese barmooncattalk_322fa9f0:

    # c "Better?"
    c "好多了?"

# game/scripts/bar.rpy:3244
translate schinese barmooncattalk_f1842f94:

    # moon "Hn..."
    moon "嗯......"

# game/scripts/bar.rpy:3245
translate schinese barmooncattalk_6c4a37c7:

    # moon "...Yes..."
    moon "......是的......"

# game/scripts/bar.rpy:3252
translate schinese barmooncattalk_814e5466:

    # moon "Hahhn!"
    moon "哈!"

# game/scripts/bar.rpy:3254
translate schinese barmooncattalk_33c32dd1:

    # moon "Nn, mmm..."
    moon "嗯,嗯......"

# game/scripts/bar.rpy:3256
translate schinese barmooncattalk_082ce360:

    # moon "Ohh..."
    moon "噢......"

# game/scripts/bar.rpy:3258
translate schinese barmooncattalk_f724519e_3:

    # moon "Ah!"
    moon "啊!"

# game/scripts/bar.rpy:3260
translate schinese barmooncattalk_8c7e88fb:

    # moon "Hnnnn...!"
    moon "嗯......!"

# game/scripts/bar.rpy:3262
translate schinese barmooncattalk_4a41c3f1:

    # moon "...Mmmm... That feels so good...!"
    moon "......嗯......感觉好舒服......!"

# game/scripts/bar.rpy:3263
translate schinese barmooncattalk_0530d3da:

    # c "Sounds like you're opening up."
    c "听起来你终于放开了."

# game/scripts/bar.rpy:3264
translate schinese barmooncattalk_e88e760f:

    # moon "...I don't know... It just feels good, okay?"
    moon "......我不知道......只是感觉很舒服,好吗?"

# game/scripts/bar.rpy:3265
translate schinese barmooncattalk_59cc033d:

    # moon "Mmmm...! Ah!"
    moon "嗯......!啊!"

# game/scripts/bar.rpy:3267
translate schinese barmooncattalk_4984c9d6_7:

    # moon "..."
    moon "......"

# game/scripts/bar.rpy:3269
translate schinese barmooncattalk_6fc2d03e:

    # moon "...What are you doing...?"
    moon "......你在干什么......?"

# game/scripts/bar.rpy:3276
translate schinese barmooncattalk_f724519e_4:

    # moon "Ah!"
    moon "啊!"

# game/scripts/bar.rpy:3281
translate schinese barmooncattalk_7ee2ec2d:

    # moon "Mmmmm!!"
    moon "嗯!!"

# game/scripts/bar.rpy:3283
translate schinese barmooncattalk_b114a925:

    # c "Beg."
    c "求我."

# game/scripts/bar.rpy:3285
translate schinese barmooncattalk_07a8886c:

    # moon "...Like before...?"
    moon "......像以前那样吗......?"

# game/scripts/bar.rpy:3286
translate schinese barmooncattalk_49bfd76d:

    # moon "......"
    moon "......"

# game/scripts/bar.rpy:3287
translate schinese barmooncattalk_8fb6232d:

    # moon "........."
    moon "........."

# game/scripts/bar.rpy:3289
translate schinese barmooncattalk_7b0439e5:

    # moon "Please spank me..."
    moon "请打我屁股吧......"

# game/scripts/bar.rpy:3291
translate schinese barmooncattalk_88ae8d97:

    # moon "I... Um... I've been a bad girl...?"
    moon "我......嗯......我是个坏女孩......?"

# game/scripts/bar.rpy:3293
translate schinese barmooncattalk_de6f8d15:

    # moon "...I broke all your glasses and need to be punished..."
    moon "......我打破了你所有的玻璃杯,我需要被惩罚......"

# game/scripts/bar.rpy:3295
translate schinese barmooncattalk_f1c10d66:

    # moon "And... And rubbed... My wet pussy wants to... Be rubbed..."
    moon "还有......还有抚摸......我湿润的小穴......需要被抚摸......"

# game/scripts/bar.rpy:3300
translate schinese barmooncattalk_f724519e_5:

    # moon "Ah!"
    moon "啊!"

# game/scripts/bar.rpy:3305
translate schinese barmooncattalk_7ee2ec2d_1:

    # moon "Mmmmm!!"
    moon "嗯!!"

# game/scripts/bar.rpy:3307
translate schinese barmooncattalk_139c6bba:

    # c "Good."
    c "很好."

# game/scripts/bar.rpy:3308
translate schinese barmooncattalk_95cc95c1:

    # moon "Ahh... Yes! Mmmm!!"
    moon "啊......是的!嗯!!"

# game/scripts/bar.rpy:3309
translate schinese barmooncattalk_a770cc75:

    # moon "{size=-10}I'm so wet... I'm so wet... Oh my god...{/size} Ahn~!"
    moon "{size=-10}我好湿......我好湿......哦我天......{/size}啊~!"

# game/scripts/bar.rpy:3311
translate schinese barmooncattalk_d06dd5b7:

    # c "Take off your clothes."
    c "把你的衣服脱了."

# game/scripts/bar.rpy:3312
translate schinese barmooncattalk_504ede54:

    # moon "A-And you'll put your finger inside...?"
    moon "那-那你会把手指放进来吗......?"

# game/scripts/bar.rpy:3313
translate schinese barmooncattalk_fabe4767:

    # c "Maybe..."
    c "也许......"

# game/scripts/bar.rpy:3314
translate schinese barmooncattalk_48ca5d2e:

    # moon "Nn..."
    moon "嗯......"

# game/scripts/bar.rpy:3317
translate schinese barmooncattalk_70c1bd2d:

    # moon "Weeelllll...?"
    moon "好吗......?"

# game/scripts/bar.rpy:3320
translate schinese barmooncattalk_3596ecbe:

    # moon "Ahh! Hnnn!"
    moon "啊!嗯!"

# game/scripts/bar.rpy:3321
translate schinese barmooncattalk_74a00694:

    # c "Are you always this wet?"
    c "你一直都这么湿吗?"

# game/scripts/bar.rpy:3322
translate schinese barmooncattalk_ec9252bd:

    # moon "{size=-10}...Y-Yes... Lately...{/size}"
    moon "{size=-10}......是-是的......最近..{/size}"

# game/scripts/bar.rpy:3323
translate schinese barmooncattalk_df72d121:

    # c "What's that?"
    c "真的吗?"

# game/scripts/bar.rpy:3324
translate schinese barmooncattalk_f8ba98f1:

    # moon "Hnnnn... Yes... I don't know why...!"
    moon "嗯......是的......我也不知道为什么......!"

# game/scripts/bar.rpy:3325
translate schinese barmooncattalk_a27c545f:

    # moon "I-It just- Hn! ...Happens... S-Sometimes..."
    moon "只-只是-嗯!......有时候......会-会发生......"

# game/scripts/bar.rpy:3326
translate schinese barmooncattalk_686def15:

    # c "Like when?"
    c "比如什么时候?"

# game/scripts/bar.rpy:3327
translate schinese barmooncattalk_0d207bce:

    # moon "D-Don't ask stupid questions! I-It just happens! Ah~!"
    moon "别-别问那些傻问题了!它-它就是会发生!啊~!"

# game/scripts/bar.rpy:3328
translate schinese barmooncattalk_095ceeb6:

    # c "Have you tried fixing it yourself?"
    c "你有试过修好你自己吗?"

# game/scripts/bar.rpy:3329
translate schinese barmooncattalk_8ff7e2d8:

    # moon "I s-said... Don't ask stupid questions...! HNN!"
    moon "我说-说了......别问那些傻问题了......!嗯!"

# game/scripts/bar.rpy:3334
translate schinese barmooncattalk_11aa970e_2:

    # moon "Sss!"
    moon "嘶!"

# game/scripts/bar.rpy:3339
translate schinese barmooncattalk_1129bef8:

    # moon "Nya!!"
    moon "喵!!"

# game/scripts/bar.rpy:3341
translate schinese barmooncattalk_9d272580:

    # moon "Ohh!"
    moon "噢!"

# game/scripts/bar.rpy:3342
translate schinese barmooncattalk_162715b8:

    # moon "{size=-10}...Wh... Why does it feel so good... Hnnn...{/size} Ahh~"
    moon "{size=-10}......为......为什么感觉这么舒服......嗯......{/size}啊~"

# game/scripts/bar.rpy:3345
translate schinese barmooncattalk_7030489d:

    # moon "Ahhh~ Nnn! {size=-10}D... Damn...!{/size}"
    moon "啊~嗯!{size=-10}操......操......!{/size}"

# game/scripts/bar.rpy:3346
translate schinese barmooncattalk_0f43440d:

    # moon "Hnnn! Nnn! Nngh!!"
    moon "嗯!嗯!嗯!!"

# game/scripts/bar.rpy:3347
translate schinese barmooncattalk_38f669d0:

    # moon "Fffuhh...! Nnguh! Nnn!"
    moon "操......!嗯!嗯!"

# game/scripts/bar.rpy:3348
translate schinese barmooncattalk_ee3b9ca7:

    # moon "I... I... I'm so..."
    moon "我......我......我好......"

# game/scripts/bar.rpy:3353
translate schinese barmooncattalk_f724519e_6:

    # moon "Ah!"
    moon "啊!"

# game/scripts/bar.rpy:3354
translate schinese barmooncattalk_83a9c09b:

    # moon "What was-"
    moon "什-"

# game/scripts/bar.rpy:3359
translate schinese barmooncattalk_70354428:

    # moon "Ngh!!"
    moon "嗯!!"

# game/scripts/bar.rpy:3360
translate schinese barmooncattalk_bd244b4a:

    # moon "...Please... Don't make me beg..."
    moon "......求你了......不要再让我求了......"

# game/scripts/bar.rpy:3364
translate schinese barmooncattalk_84381bf3_8:

    # moon "Enough!"
    moon "够了!"

# game/scripts/bar.rpy:3370
translate schinese barmooncattalk_0dc45d60_8:

    # moon "Oh! Sorry!"
    moon "噢!对不起!"

# game/scripts/bar.rpy:3373
translate schinese barmooncattalk_13a409dc_9:

    # c "...Maybe that was too much..."
    c "......也许有点过头了......"

# game/scripts/bar.rpy:3379
translate schinese barmooncattalk_ba314112_16:

    # c "That's enough for now."
    c "够了."

# game/scripts/bar.rpy:3380
translate schinese barmooncattalk_71d89851_8:

    # moon "Nnn, okay..."
    moon "嗯,好吧......"

# game/scripts/bar.rpy:3384
translate schinese barmooncattalk_ba314112_17:

    # c "That's enough for now."
    c "够了."

# game/scripts/bar.rpy:3385
translate schinese barmooncattalk_7e63ec25_9:

    # moon "Okay..."
    moon "好吧......"

# game/scripts/bar.rpy:3394
translate schinese barmooncattalk_d97a7eae:

    # moon "Please...!"
    moon "求你了......!"

# game/scripts/bar.rpy:3397
translate schinese barmooncattalk_4984c9d6_8:

    # moon "..."
    moon "......"

# game/scripts/bar.rpy:3403
translate schinese barmooncattalk_0e55b2b0:

    # moon "...I'm waiting..."
    moon "......我还在等......"

# game/scripts/bar.rpy:3410
translate schinese barmooncattalk_8ec36b9f:

    # moon "Hmmm! Ahhh!"
    moon "嗯!啊!"

# game/scripts/bar.rpy:3411
translate schinese barmooncattalk_422d6d5e:

    # moon "Sh... Shouldn't you spank me first...?"
    moon "你......你不应该先打我屁股吗......?"

# game/scripts/bar.rpy:3412
translate schinese barmooncattalk_5a985f75:

    # moon "I- Ah~ ...I mean... It would make more sense to start with that... Nhh...!"
    moon "我-啊~......我是说......那样开始更合理一点......嗯......!"

# game/scripts/bar.rpy:3413
translate schinese barmooncattalk_d9b7485c:

    # moon "But... I'm not complaining..."
    moon "但是......我不是在抱怨......"

# game/scripts/bar.rpy:3419
translate schinese barmooncattalk_9f46ae8e:

    # moon "Nnnnuh! Nghuh!"
    moon "嗯!嗯!"

# game/scripts/bar.rpy:3420
translate schinese barmooncattalk_912da63b:

    # moon "I-Inside...? Already...!? Hnn...!"
    moon "进-进来了......?已经进来了......!?嗯......!"

# game/scripts/bar.rpy:3421
translate schinese barmooncattalk_8e37b046:

    # c "You looked wet enough."
    c "你看起来已经够湿了."

# game/scripts/bar.rpy:3422
translate schinese barmooncattalk_d29e7f6e:

    # moon "Mmmgh! Whatever...! Hnnah!"
    moon "嗯!算了......!嗯!"

# game/scripts/bar.rpy:3430
translate schinese barmooncattalk_8d264032:

    # c "Maybe you can cum next time."
    c "也许你下次就可以去了."

# game/scripts/bar.rpy:3431
translate schinese barmooncattalk_578564c8:

    # moon "Next time...!? That's so mean!"
    moon "下次......!?你太刻薄了!"

# game/scripts/bar.rpy:3443
translate schinese barmooncattalk_b6d539ff:

    # moon "Okay."
    moon "好吧."

# game/scripts/bar.rpy:3459
translate schinese moonspankfingering_222e216f:

    # moon "Hmmm...! No...! Don't tease me..."
    moon "嗯......!不要......!不要玩弄我......"

# game/scripts/bar.rpy:3460
translate schinese moonspankfingering_edde996f:

    # moon "Hahhhnn! Nnnhh... I want it inside..."
    moon "哈!嗯......我想要它进来......"

# game/scripts/bar.rpy:3461
translate schinese moonspankfingering_7774b6e4:

    # moon "Mmm-hmmm...! Hahhn! I'm so wet...!"
    moon "嗯-嗯......!哈!我好湿......!"

# game/scripts/bar.rpy:3467
translate schinese moonspankfingering_9db6aca0:

    # moon "Ahhh~ Keep going...! Nnngh!"
    moon "啊~继续......!嗯!"

# game/scripts/bar.rpy:3468
translate schinese moonspankfingering_2e006a6a:

    # moon "Ahhhn! Ahhh! Hmm!"
    moon "啊!啊!嗯!"

# game/scripts/bar.rpy:3469
translate schinese moonspankfingering_e83b5589:

    # moon "Faster...! Faster...!"
    moon "快点......!快点......!"

# game/scripts/bar.rpy:3477
translate schinese moonspankfingering_8d264032:

    # c "Maybe you can cum next time."
    c "也许你下次就可以去了."

# game/scripts/bar.rpy:3478
translate schinese moonspankfingering_578564c8:

    # moon "Next time...!? That's so mean!"
    moon "下次......!?那太刻薄了!"

# game/scripts/bar.rpy:3493
translate schinese moonspankrub_8ec36b9f:

    # moon "Hmmm! Ahhh!"
    moon "嗯!啊!"

# game/scripts/bar.rpy:3494
translate schinese moonspankrub_8333b64e:

    # moon "Ahhn~ Yes... Nhhh! That feels good...! Ahh!"
    moon "啊~是的......嗯!感觉好舒服......!啊!"

# game/scripts/bar.rpy:3495
translate schinese moonspankrub_91435085:

    # moon "Ah, my pussy... Feels soo good...! But... I want more...! Ahh~"
    moon "啊,我的小穴......感觉好舒服......!但是......我还想要更多......!啊~"

# game/scripts/bar.rpy:3501
translate schinese moonspankrub_51340ef0:

    # moon "Ahhh~ Ahhh! Yes! Yes!"
    moon "啊~啊!是的!是的!"

# game/scripts/bar.rpy:3502
translate schinese moonspankrub_2392c72b:

    # moon "Nggh! Ahnn! So good...!"
    moon "嗯!啊!好棒......!"

# game/scripts/bar.rpy:3503
translate schinese moonspankrub_40ac5cd2:

    # moon "Ahh...! Nyyyah~! Mmmnggh!"
    moon "啊......!喵~!嗯!"

# game/scripts/bar.rpy:3504
translate schinese moonspankrub_cd974be9:

    # moon "S... Spank me... Spank me...!"
    moon "打......打我......打我屁股......!"

# game/scripts/bar.rpy:3512
translate schinese moonspankrub_8d264032:

    # c "Maybe you can cum next time."
    c "也许你下次就可以去了."

# game/scripts/bar.rpy:3513
translate schinese moonspankrub_578564c8:

    # moon "Next time...!? That's so mean!"
    moon "下次......!?那太刻薄了!"

# game/scripts/bar.rpy:3527
translate schinese moonspankspank_8ec36b9f:

    # moon "Hmmm! Ahhh!"
    moon "嗯!啊!"

# game/scripts/bar.rpy:3528
translate schinese moonspankspank_02df2e54:

    # moon "Ahhn~ Yes... Nhhh! Tease me...! Fffmm!"
    moon "啊~是的......嗯!玩弄我吧......!操!"

# game/scripts/bar.rpy:3529
translate schinese moonspankspank_642e6d3b:

    # moon "Sss! Ahh! Oh, my pussy...! Ahh~"
    moon "嘶!啊!噢,我的小穴......!啊~"

# game/scripts/bar.rpy:3535
translate schinese moonspankspank_9f46ae8e:

    # moon "Nnnnuh! Nghuh!"
    moon "嗯!嗯!"

# game/scripts/bar.rpy:3536
translate schinese moonspankspank_9ee55c9d:

    # moon "I-Inside...? Hnn...!"
    moon "进-进来了......?嗯......!"

# game/scripts/bar.rpy:3537
translate schinese moonspankspank_8e37b046:

    # c "You looked wet enough."
    c "你看起来已经够湿了."

# game/scripts/bar.rpy:3538
translate schinese moonspankspank_d29e7f6e:

    # moon "Mmmgh! Whatever...! Hnnah!"
    moon "嗯!算了......!嗯!"

# game/scripts/bar.rpy:3546
translate schinese moonspankspank_8d264032:

    # c "Maybe you can cum next time."
    c "也许你下次就可以去了."

# game/scripts/bar.rpy:3547
translate schinese moonspankspank_578564c8:

    # moon "Next time...!? That's so mean!"
    moon "下次......!?那太刻薄了!"

# game/scripts/bar.rpy:3554
translate schinese moonspankred_6b7e561c:

    # c "Take off everything."
    c "把衣服全脱了."

# game/scripts/bar.rpy:3555
translate schinese moonspankred_7e63ec25:

    # moon "Okay..."
    moon "好的......"

# game/scripts/bar.rpy:3560
translate schinese moonspankred_3c5f9753:

    # moon "Mmmmmm~ I'm all red again..."
    moon "嗯~我的屁股又红了......"

# game/scripts/bar.rpy:3567
translate schinese moonspankred_9d1cf4b1:

    # moon "Nyahh... Hnn!"
    moon "喵......嗯!"

# game/scripts/bar.rpy:3568
translate schinese moonspankred_118b2320:

    # moon "Hmmmm~ Ahhh! Make my ass redder...! Nnnh!"
    moon "嗯~啊!让我的屁股变得更红吧......!嗯!"

# game/scripts/bar.rpy:3569
translate schinese moonspankred_d0bf0ccb:

    # moon "Ooohfff, Nnn...! Your finger feels so big inside me..."
    moon "噢,嗯......你的手指在我里面好大......"

# game/scripts/bar.rpy:3570
translate schinese moonspankred_081750c1:

    # moon "Ehh... It's not even two fingers... And yet... Hnnn!"
    moon "嗯......这还没......两根手指......嗯!"

# game/scripts/bar.rpy:3580
translate schinese moonspankred_8d264032:

    # c "Maybe you can cum next time."
    c "也许你下次就可以去了."

# game/scripts/bar.rpy:3581
translate schinese moonspankred_578564c8:

    # moon "Next time...!? That's so mean!"
    moon "下次......!?那太刻薄了!"

# game/scripts/bar.rpy:3590
translate schinese moonspankred2_6f7e423a:

    # moon "This teasing is too much... It's too much...! It's too much!!"
    moon "这种玩弄太过分了......太过分了......!太过分了!!"

# game/scripts/bar.rpy:3591
translate schinese moonspankred2_dd06ab75:

    # moon "I'm so... So... So close... Please... Put your cock inside me..."
    moon "我已经......已经......已经要去了......求你了......把你的肉棒放进来吧......"

# game/scripts/bar.rpy:3595
translate schinese moonspankred2_25978520:

    # moon "Ahhh! Hnn! Nngh!"
    moon "啊!嗯!嗯!"

# game/scripts/bar.rpy:3596
translate schinese moonspankred2_6f7e423a_1:

    # moon "This teasing is too much... It's too much...! It's too much!!"
    moon "这种玩弄太过分了....太过分了....!太过分了!!"

# game/scripts/bar.rpy:3597
translate schinese moonspankred2_dd06ab75_1:

    # moon "I'm so... So... So close... Please... Put your cock inside me..."
    moon "我已经......已经......已经要去了......求你了......把你的肉棒放进来吧......"

# game/scripts/bar.rpy:3606
translate schinese moonspankred2_8d264032:

    # c "Maybe you can cum next time."
    c "也许你下次就可以去了."

# game/scripts/bar.rpy:3607
translate schinese moonspankred2_578564c8:

    # moon "Next time...!? That's so mean!"
    moon "下一次......!?那太刻薄了!"

# game/scripts/bar.rpy:3616
translate schinese moonspankred2_8d264032_1:

    # c "Maybe you can cum next time."
    c "也许你下次就可以去了."

# game/scripts/bar.rpy:3617
translate schinese moonspankred2_578564c8_1:

    # moon "Next time...!? That's so mean!"
    moon "下一次......!?那太刻薄了!"

# game/scripts/bar.rpy:3628
translate schinese moonspankred3_9d1cf4b1:

    # moon "Nyahh... Hnn!"
    moon "喵......嗯!"

# game/scripts/bar.rpy:3629
translate schinese moonspankred3_118b2320:

    # moon "Hmmmm~ Ahhh! Make my ass redder...! Nnnh!"
    moon "嗯~啊~让我的屁股变得更红吧......!嗯!"

# game/scripts/bar.rpy:3630
translate schinese moonspankred3_d0bf0ccb:

    # moon "Ooohfff, Nnn...! Your finger feels so big inside me..."
    moon "噢,嗯......!你的手指在我里面好大......"

# game/scripts/bar.rpy:3631
translate schinese moonspankred3_081750c1:

    # moon "Ehh... It's not even two fingers... And yet... Hnnn!"
    moon "嗯......这还没......两根手指......嗯!"

# game/scripts/bar.rpy:3641
translate schinese moonspankred3_8d264032:

    # c "Maybe you can cum next time."
    c "也许你下次就可以去了."

# game/scripts/bar.rpy:3642
translate schinese moonspankred3_578564c8:

    # moon "Next time...!? That's so mean!"
    moon "下一次......!?那太刻薄了!"

# game/scripts/bar.rpy:3648
translate schinese moonspankredfuck_0bc7a786:

    # moon "Mmmm... I'm ready..."
    moon "嗯......我准备好了......"

# game/scripts/bar.rpy:3650
translate schinese moonspankredfuck_fb396743:

    # moon "Aaahhhhh...!"
    moon "啊......!"

# game/scripts/bar.rpy:3652
translate schinese moonspankredfuck_c933433e:

    # moon "Ahh! Mmm!"
    moon "啊!嗯!"

# game/scripts/bar.rpy:3653
translate schinese moonspankredfuck_4c92816c:

    # moon "Hn! Sex... In front of people... This is so weird...!"
    moon "嗯!做爱......在大家面前做爱......这太奇怪了......!"

# game/scripts/bar.rpy:3654
translate schinese moonspankredfuck_c2c6e211:

    # moon "Ahhn~! Are they looking...? Wait! I don't want to know...! Mmm!"
    moon "啊~!他们在看着吗......?等等!我不想知道......!嗯!"

# game/scripts/bar.rpy:3657
translate schinese moonspankredfuck_d95d9693:

    # moon "Hnnnn! It feels so good, having sex after spankings...! Ah! Sex feels so good after a spanking!"
    moon "嗯!被打屁股之后做爱,感觉好舒服......!啊!被打屁股之后做爱感觉好舒服!"

# game/scripts/bar.rpy:3658
translate schinese moonspankredfuck_a2a9f131:

    # moon "Mmm, I'm so wet...! Hahh...! Hahhh!"
    moon "嗯,我好湿......!哈......!哈!"

# game/scripts/bar.rpy:3659
translate schinese moonspankredfuck_e5a19bcf:

    # moon "Hmm! Cum on my ass, [name]! Ahh! Please...!"
    moon "嗯!射到我的屁股上吧,[name]!啊!求你了......!"

# game/scripts/bar.rpy:3663
translate schinese moonspankredfuck_c4d4e1dc:

    # moon "Nyah!" with ssshake
    moon "喵!" with ssshake

# game/scripts/bar.rpy:3664
translate schinese moonspankredfuck_b05fbbe6:

    # moon "Nnnmmm... Should I clean it up...?"
    moon "嗯......我是不是应该清理一下......?"

# game/scripts/bar.rpy:3670
translate schinese moonspankcatfuck_0bc7a786:

    # moon "Mmmm... I'm ready..."
    moon "嗯......我准备好了......"

# game/scripts/bar.rpy:3672
translate schinese moonspankcatfuck_fb396743:

    # moon "Aaahhhhh...!"
    moon "啊......!"

# game/scripts/bar.rpy:3674
translate schinese moonspankcatfuck_c933433e:

    # moon "Ahh! Mmm!"
    moon "啊!嗯!"

# game/scripts/bar.rpy:3675
translate schinese moonspankcatfuck_4c92816c:

    # moon "Hn! Sex... In front of people... This is so weird...!"
    moon "嗯!做爱......在大家面前做爱......感觉好奇怪......!"

# game/scripts/bar.rpy:3676
translate schinese moonspankcatfuck_c2c6e211:

    # moon "Ahhn~! Are they looking...? Wait! I don't want to know...! Mmm!"
    moon "啊~!他们在看着吗......?等等!我不想知道......!嗯!"

# game/scripts/bar.rpy:3679
translate schinese moonspankcatfuck_d95d9693:

    # moon "Hnnnn! It feels so good, having sex after spankings...! Ah! Sex feels so good after a spanking!"
    moon "嗯!被打屁股之后做爱,感觉好舒服......!啊!被打屁股之后做爱感觉好舒服!"

# game/scripts/bar.rpy:3680
translate schinese moonspankcatfuck_a2a9f131:

    # moon "Mmm, I'm so wet...! Hahh...! Hahhh!"
    moon "嗯,我好湿......!哈......!哈!"

# game/scripts/bar.rpy:3681
translate schinese moonspankcatfuck_e5a19bcf:

    # moon "Hmm! Cum on my ass, [name]! Ahh! Please...!"
    moon "嗯!射到我的屁股上吧,[name]!啊!求你了......!"

# game/scripts/bar.rpy:3685
translate schinese moonspankcatfuck_c4d4e1dc:

    # moon "Nyah!" with ssshake
    moon "喵!" with ssshake

# game/scripts/bar.rpy:3686
translate schinese moonspankcatfuck_b05fbbe6:

    # moon "Nnnmmm... Should I clean it up...?"
    moon "嗯......我是不是应该清理一下......?"

# game/scripts/bar.rpy:3696
translate schinese barmoon_e49e664a:

    # moon "Hello!"
    moon "你好!"

# game/scripts/bar.rpy:3699
translate schinese barmoontalk_1c88bfd9:

    # moon "I'm doing amazing!"
    moon "我做的很棒!"

# game/scripts/bar.rpy:3702
translate schinese barmoontalk_f44f19ec:

    # "She's not ready yet."
    "她还没准备好."

# game/scripts/bar.rpy:3705
translate schinese barmoontalk_dc35cdf5:

    # moon "It still stings, [name]... Give me time to recover first."
    moon "现在还是有点痛,[name]......给我点时间恢复一下吧."

# game/scripts/bar.rpy:3713
translate schinese barmoontalk_5deb73b6:

    # moon "There are people here, [name]... Shouldn't we do this when nobody is around?"
    moon "这里有人,[name]......我们不应该在没人的时候做这个吗?"

# game/scripts/bar.rpy:3714
translate schinese barmoontalk_0206afc4:

    # c "I'm sure you can agree the humiliation just adds to the punishment."
    c "我相信你会同意的,羞辱也刚好增加了惩罚的部分."

# game/scripts/bar.rpy:3715
translate schinese barmoontalk_4984c9d6:

    # moon "..."
    moon "......"

# game/scripts/bar.rpy:3716
translate schinese barmoontalk_7e63ec25:

    # moon "Okay..."
    moon "好吧......"

# game/scripts/bar.rpy:3718
translate schinese barmoontalk_208f70a7:

    # moon "I can take more...!"
    moon "我还可以再忍受一会......!"

# game/scripts/bar.rpy:3720
translate schinese barmoontalk_00e1eca0:

    # moon "Please stop looking at me, everyone!"
    moon "大家请不要再看我了!"

# game/scripts/bar.rpy:3722
translate schinese barmoontalk_dad59b7f:

    # moon "Fine... Enough... I think that's enough..."
    moon "好吧......够了......我想已经够了......"

# game/scripts/bar.rpy:3726
translate schinese barmoontalk_84381bf3:

    # moon "Enough!"
    moon "够了!"

# game/scripts/bar.rpy:3732
translate schinese barmoontalk_0dc45d60:

    # moon "Oh! Sorry!"
    moon "噢!对不起!"

# game/scripts/bar.rpy:3735
translate schinese barmoontalk_13a409dc:

    # c "...Maybe that was too much..."
    c "......也许有点过头了......"

# game/scripts/bar.rpy:3741
translate schinese barmoontalk_ba314112:

    # c "That's enough for now."
    c "够了."

# game/scripts/bar.rpy:3742
translate schinese barmoontalk_71d89851:

    # moon "Nnn, okay..."
    moon "嗯,好吧......"

# game/scripts/bar.rpy:3746
translate schinese barmoontalk_ba314112_1:

    # c "That's enough for now."
    c "够了."

# game/scripts/bar.rpy:3747
translate schinese barmoontalk_7e63ec25_1:

    # moon "Okay..."
    moon "好吧......"

# game/scripts/bar.rpy:3754
translate schinese barmoontalk_84058823:

    # c "I need to do 'Waitress Training: All Eyes on You!' first."
    c "我得先完成'服务员训练:众目睽睽!'"

# game/scripts/bar.rpy:3756
translate schinese barmoontalk_7238be3c:

    # c "Change outfits. I'm going to spank you."
    c "把衣服换了.我要打你屁股."

# game/scripts/bar.rpy:3760
translate schinese barmoontalk_41a59255:

    # moon "This outfit is so humiliating..."
    moon "这件衣服好羞耻......"

# game/scripts/bar.rpy:3761
translate schinese barmoontalk_0206afc4_1:

    # c "I'm sure you can agree the humiliation just adds to the punishment."
    c "我相信你会同意的,羞辱也刚好增加了惩罚的部分."

# game/scripts/bar.rpy:3762
translate schinese barmoontalk_aeccfdc8:

    # moon "Right, right..."
    moon "好吧,好吧......"

# game/scripts/bar.rpy:3764
translate schinese barmoontalk_f4ff0555:

    # moon "Everyone is staring at me..."
    moon "每个人都在看着我......"

# game/scripts/bar.rpy:3766
translate schinese barmoontalk_c37bea86:

    # moon "Stop looking at me!"
    moon "别看我了!"

# game/scripts/bar.rpy:3768
translate schinese barmoontalk_dad59b7f_1:

    # moon "Fine... Enough... I think that's enough..."
    moon "好吧......够了......我想已经够了......"

# game/scripts/bar.rpy:3772
translate schinese barmoontalk_84381bf3_1:

    # moon "Enough!"
    moon "够了!"

# game/scripts/bar.rpy:3778
translate schinese barmoontalk_0dc45d60_1:

    # moon "Oh! Sorry!"
    moon "噢!对不起!"

# game/scripts/bar.rpy:3781
translate schinese barmoontalk_13a409dc_1:

    # c "...Maybe that was too much..."
    c "......也许有点太过了......"

# game/scripts/bar.rpy:3787
translate schinese barmoontalk_ba314112_2:

    # c "That's enough for now."
    c "够了."

# game/scripts/bar.rpy:3788
translate schinese barmoontalk_71d89851_1:

    # moon "Nnn, okay..."
    moon "嗯,好吧......"

# game/scripts/bar.rpy:3792
translate schinese barmoontalk_ba314112_3:

    # c "That's enough for now."
    c "够了."

# game/scripts/bar.rpy:3793
translate schinese barmoontalk_7e63ec25_2:

    # moon "Okay..."
    moon "好吧......"

# game/scripts/bar.rpy:3798
translate schinese barmoontalk_7238be3c_1:

    # c "Change outfits. I'm going to spank you."
    c "把衣服换了.我要打你屁股."

# game/scripts/bar.rpy:3802
translate schinese barmoontalk_da9cf4ef:

    # moon "Do we really have to do this again?"
    moon "我们真的得再做一次吗?"

# game/scripts/bar.rpy:3803
translate schinese barmoontalk_1ec63ee4:

    # c "We can stop if you'd like..."
    c "如果你想的话我们可以停下......"

# game/scripts/bar.rpy:3804
translate schinese barmoontalk_ec997992:

    # moon "I guess we can at least go at least once more..."
    moon "我想我们至少可以再来一次......"

# game/scripts/bar.rpy:3807
translate schinese barmoontalk_7966257c:

    # moon "Hnnn... [name]..."
    moon "嗯......[name]......"

# game/scripts/bar.rpy:3808
translate schinese barmoontalk_68f145e3:

    # c "Are you getting wet?"
    c "你湿了吗?"

# game/scripts/bar.rpy:3809
translate schinese barmoontalk_8bd2acfd:

    # moon "What!? Stop looking there!"
    moon "什么!?别看那里!"

# game/scripts/bar.rpy:3811
translate schinese barmoontalk_cfa2dbf5:

    # moon "This feels strange..."
    moon "感觉好奇怪......"

# game/scripts/bar.rpy:3813
translate schinese barmoontalk_dad59b7f_2:

    # moon "Fine... Enough... I think that's enough..."
    moon "好吧......够了......我想已经够了......"

# game/scripts/bar.rpy:3817
translate schinese barmoontalk_84381bf3_2:

    # moon "Enough!"
    moon "够了!"

# game/scripts/bar.rpy:3823
translate schinese barmoontalk_0dc45d60_2:

    # moon "Oh! Sorry!"
    moon "噢!对不起!"

# game/scripts/bar.rpy:3826
translate schinese barmoontalk_13a409dc_2:

    # c "...Maybe that was too much..."
    c "......也许有点太过了......"

# game/scripts/bar.rpy:3832
translate schinese barmoontalk_ba314112_4:

    # c "That's enough for now."
    c "够了."

# game/scripts/bar.rpy:3833
translate schinese barmoontalk_71d89851_2:

    # moon "Nnn, okay..."
    moon "嗯,好吧......"

# game/scripts/bar.rpy:3837
translate schinese barmoontalk_ba314112_5:

    # c "That's enough for now."
    c "够了."

# game/scripts/bar.rpy:3838
translate schinese barmoontalk_7e63ec25_3:

    # moon "Okay..."
    moon "好吧......"

# game/scripts/bar.rpy:3844
translate schinese barmoontalk_7238be3c_2:

    # c "Change outfits. I'm going to spank you."
    c "把衣服换了.我要打你屁股."

# game/scripts/bar.rpy:3848
translate schinese barmoontalk_236a448e:

    # moon "[name], I feel a lot less guilty about what I've done now. I think the spankings actually helped!"
    moon "[name],我现在对我做的那些事不再那么内疚了.我觉得打屁股是真的有帮助的!"

# game/scripts/bar.rpy:3853
translate schinese barmoontalk_d7b8ec89:

    # moon "Gah!"
    moon "咕!"

# game/scripts/bar.rpy:3855
translate schinese barmoontalk_37f78821:

    # moon "Hnn!"
    moon "嗯!"

# game/scripts/bar.rpy:3857
translate schinese barmoontalk_f724519e:

    # moon "Ah!"
    moon "啊!"

# game/scripts/bar.rpy:3859
translate schinese barmoontalk_11aa970e:

    # moon "Sss!"
    moon "嘶!"

# game/scripts/bar.rpy:3861
translate schinese barmoontalk_b268297c:

    # moon "AHH!"
    moon "啊!"

# game/scripts/bar.rpy:3867
translate schinese barmoontalk_d7b8ec89_1:

    # moon "Gah!"
    moon "咕!"

# game/scripts/bar.rpy:3869
translate schinese barmoontalk_37f78821_1:

    # moon "Hnn!"
    moon "嗯!"

# game/scripts/bar.rpy:3871
translate schinese barmoontalk_f724519e_1:

    # moon "Ah!"
    moon "啊!"

# game/scripts/bar.rpy:3873
translate schinese barmoontalk_11aa970e_1:

    # moon "Sss!"
    moon "嘶!"

# game/scripts/bar.rpy:3875
translate schinese barmoontalk_b268297c_1:

    # moon "AHH!"
    moon "啊!"

# game/scripts/bar.rpy:3876
translate schinese barmoontalk_796ac522:

    # c "That's good to hear."
    c "那听起来很不错."

# game/scripts/bar.rpy:3878
translate schinese barmoontalk_54066883:

    # moon "Nnh..."
    moon "嗯......"

# game/scripts/bar.rpy:3879
translate schinese barmoontalk_455f825a:

    # c "Seems like you're enjoying it as well."
    c "看起来你也很享受."

# game/scripts/bar.rpy:3880
translate schinese barmoontalk_4965796b:

    # moon "Don't be silly..."
    moon "别傻了......"

# game/scripts/bar.rpy:3883
translate schinese barmoontalk_e67bfb60:

    # moon "Wait, what are you-"
    moon "等等,你在干什-"

# game/scripts/bar.rpy:3885
translate schinese barmoontalk_c44ef131:

    # moon "Ah!" with ssshake
    moon "啊!" with ssshake

# game/scripts/bar.rpy:3887
translate schinese barmoontalk_6ab22eb5:

    # moon "You'll die for that!"
    moon "你想找死吗!"

# game/scripts/bar.rpy:3888
translate schinese barmoontalk_d07a6095:

    # c "You'd just keep getting them wet..."
    c "你那里一直都是湿的......"

# game/scripts/bar.rpy:3890
translate schinese barmoontalk_cc285698:

    # moon "T-That only happened once!"
    moon "那-那只发生过一次!"

# game/scripts/bar.rpy:3891
translate schinese barmoontalk_f8cb617b:

    # c "Twice now."
    c "现在两次了."

# game/scripts/bar.rpy:3892
translate schinese barmoontalk_9bc5ace8:

    # c "Besides, I'm sure you can agree the humiliation just-"
    c "而且,我相信你同意羞辱只是-"

# game/scripts/bar.rpy:3894
translate schinese barmoontalk_aef6a62a:

    # moon "Don't give me that! That's my pussy! I don't want people seeing that!"
    moon "别跟我那么说!那是我的小穴!我不想要别人看到它!"

# game/scripts/bar.rpy:3896
translate schinese barmoontalk_0cea0e5d:

    # moon "Thank you..."
    moon "谢谢你......"

# game/scripts/bar.rpy:3898
translate schinese barmoontalk_d0b54138:

    # c "Are you going to get wet again?"
    c "你是不是又要湿了?"

# game/scripts/bar.rpy:3899
translate schinese barmoontalk_3f9f60ea:

    # moon "Of course not! Just punish me already!"
    moon "当然不会!直接来惩罚我吧!"

# game/scripts/bar.rpy:3902
translate schinese barmoontalk_59932a8c:

    # c "That was fast."
    c "太快了."

# game/scripts/bar.rpy:3903
translate schinese barmoontalk_4f0bbe92:

    # moon "What was?"
    moon "什么?"

# game/scripts/bar.rpy:3904
translate schinese barmoontalk_711dc129:

    # c "You're wet again."
    c "你又湿了."

# game/scripts/bar.rpy:3906
translate schinese barmoontalk_eba3d098:

    # moon "Rgh! Stop looking! Are we done?"
    moon "呃!别看了!我们完事了吗?"

# game/scripts/bar.rpy:3910
translate schinese barmoontalk_84381bf3_3:

    # moon "Enough!"
    moon "够了!"

# game/scripts/bar.rpy:3916
translate schinese barmoontalk_0dc45d60_3:

    # moon "Oh! Sorry!"
    moon "噢!对不起!"

# game/scripts/bar.rpy:3919
translate schinese barmoontalk_13a409dc_3:

    # c "...Maybe that was too much..."
    c "......也许有点太过了......"

# game/scripts/bar.rpy:3925
translate schinese barmoontalk_ba314112_6:

    # c "That's enough for now."
    c "够了."

# game/scripts/bar.rpy:3926
translate schinese barmoontalk_71d89851_3:

    # moon "Nnn, okay..."
    moon "嗯,好吧......"

# game/scripts/bar.rpy:3930
translate schinese barmoontalk_ba314112_7:

    # c "That's enough for now."
    c "够了."

# game/scripts/bar.rpy:3931
translate schinese barmoontalk_7e63ec25_4:

    # moon "Okay..."
    moon "好吧......"

# game/scripts/bar.rpy:3937
translate schinese barmoontalk_7238be3c_3:

    # c "Change outfits. I'm going to spank you."
    c "把衣服换了.我要打你屁股."

# game/scripts/bar.rpy:3941
translate schinese barmoontalk_4984c9d6_1:

    # moon "..."
    moon "......"

# game/scripts/bar.rpy:3942
translate schinese barmoontalk_51e1fcda:

    # c "I haven't even done anything yet..."
    c "我什么都还没做呢......"

# game/scripts/bar.rpy:3944
translate schinese barmoontalk_0ae93eca:

    # moon "Haven't done anything? You've spanked me so much! Look at what you've done!"
    moon "什么都没做?你打我屁股那么多次了!看看你都干了什么!"

# game/scripts/bar.rpy:3945
translate schinese barmoontalk_7043ba7a:

    # moon "Actually wait, don't look!"
    moon "等等,别看!"

# game/scripts/bar.rpy:3946
translate schinese barmoontalk_3e0716eb:

    # c "I've only spanked you, it's not my fault if it turns you on."
    c "我只是打了你的屁股,你兴奋了又不是我的错."

# game/scripts/bar.rpy:3947
translate schinese barmoontalk_3ff210ff:

    # moon "T-Turns me on!? No! That..."
    moon "我-我兴奋了!?不是!那......"

# game/scripts/bar.rpy:3948
translate schinese barmoontalk_b0d66984:

    # moon "That's just not true! You broke my pussy or something!"
    moon "那不是真的!是你搞坏了我的小穴什么的!"

# game/scripts/bar.rpy:3949
translate schinese barmoontalk_d12b8add:

    # c "I haven't touched your pussy..."
    c "我都没碰过你的小穴......"

# game/scripts/bar.rpy:3950
translate schinese barmoontalk_73140e0f:

    # moon "It's the vibrations from the spankings! You do it too hard! This would happen to any girl!"
    moon "是打屁股传过去的!你太大力了!任何女孩都会这样子的!"

# game/scripts/bar.rpy:3952
translate schinese barmoontalk_db282ef6:

    # moon "...Just... Take them off... I'm tired of cleaning them every day..."
    moon "......就......直接把它们拿下来吧......我已经厌倦了每天清洗它们......"

# game/scripts/bar.rpy:3954
translate schinese barmoontalk_4984c9d6_2:

    # moon "..."
    moon "......"

# game/scripts/bar.rpy:3956
translate schinese barmoontalk_3980b5f5:

    # c "You have a nice ass."
    c "屁股不错."

# game/scripts/bar.rpy:3958
translate schinese barmoontalk_be0de4d2:

    # moon "Rghh..."
    moon "呃......"

# game/scripts/bar.rpy:3960
translate schinese barmoontalk_048f42ff:

    # moon "Just punish me already..."
    moon "就直接来惩罚我吧......"

# game/scripts/bar.rpy:3965
translate schinese barmoontalk_4ad7b887:

    # c "Don't say it like that."
    c "别那样说."

# game/scripts/bar.rpy:3966
translate schinese barmoontalk_c1205426:

    # moon "Excuse me?"
    moon "那怎么说?"

# game/scripts/bar.rpy:3967
translate schinese barmoontalk_00d76987:

    # c "Say 'Please spank my tight ass [name].'"
    c "说:'请打我的小紧屁股吧[name].'"

# game/scripts/bar.rpy:3969
translate schinese barmoontalk_ed082e3a:

    # moon "What!? Why should I do that?"
    moon "什么!?为什么我要这么做?"

# game/scripts/bar.rpy:3970
translate schinese barmoontalk_c6e9fdc4:

    # c "Because it would be hot?"
    c "因为那样很火辣? "

# game/scripts/bar.rpy:3971
translate schinese barmoontalk_40da1b81:

    # moon "RGH!"
    moon "呃!"

# game/scripts/bar.rpy:3974
translate schinese barmoontalk_de6d2bc0:

    # moon "You deserved that one!"
    moon "你活该!"

# game/scripts/bar.rpy:3978
translate schinese barmoontalk_13a409dc_4:

    # c "...Maybe that was too much..."
    c "......也许有点太过了......"

# game/scripts/bar.rpy:3984
translate schinese barmoontalk_c396f73a:

    # moon "It stings more than usual..."
    moon "比平时更痛了......"

# game/scripts/bar.rpy:3986
translate schinese barmoontalk_f59df506:

    # c "Are you getting wetter?"
    c "你也更湿了?"

# game/scripts/bar.rpy:3987
translate schinese barmoontalk_13f97163:

    # moon "Don't say such things! Punish me!"
    moon "别说那些话!惩罚我!"

# game/scripts/bar.rpy:3988
translate schinese barmoontalk_6ccb6e7f:

    # c "Again."
    c "再来."

# game/scripts/bar.rpy:3989
translate schinese barmoontalk_f498d30b:

    # moon "What?"
    moon "什么?"

# game/scripts/bar.rpy:3990
translate schinese barmoontalk_fc17c8cf:

    # c "Say it again."
    c "再说一次."

# game/scripts/bar.rpy:3991
translate schinese barmoontalk_9b847ee1:

    # moon "P-Punish me...?"
    moon "惩-惩罚我......?"

# game/scripts/bar.rpy:3993
translate schinese barmoontalk_dad59b7f_3:

    # moon "Fine... Enough... I think that's enough..."
    moon "好吧......够了......我想已经够了......"

# game/scripts/bar.rpy:3997
translate schinese barmoontalk_84381bf3_4:

    # moon "Enough!"
    moon "够了!"

# game/scripts/bar.rpy:4003
translate schinese barmoontalk_0dc45d60_4:

    # moon "Oh! Sorry!"
    moon "噢!对不起!"

# game/scripts/bar.rpy:4006
translate schinese barmoontalk_13a409dc_5:

    # c "...Maybe that was too much..."
    c "......也许有点太过了......"

# game/scripts/bar.rpy:4012
translate schinese barmoontalk_ba314112_8:

    # c "That's enough for now."
    c "够了."

# game/scripts/bar.rpy:4013
translate schinese barmoontalk_71d89851_4:

    # moon "Nnn, okay..."
    moon "嗯,好吧......"

# game/scripts/bar.rpy:4017
translate schinese barmoontalk_ba314112_9:

    # c "That's enough for now."
    c "够了."

# game/scripts/bar.rpy:4018
translate schinese barmoontalk_7e63ec25_5:

    # moon "Okay..."
    moon "好吧......"

# game/scripts/bar.rpy:4024
translate schinese barmoontalk_7238be3c_4:

    # c "Change outfits. I'm going to spank you."
    c "把衣服换了.我要打你屁股."

# game/scripts/bar.rpy:4028
translate schinese barmoontalk_4984c9d6_3:

    # moon "..."
    moon "......"

# game/scripts/bar.rpy:4029
translate schinese barmoontalk_27cdeed0:

    # c "Want me to take your panties off?"
    c "想让我把你内裤脱了吗?"

# game/scripts/bar.rpy:4030
translate schinese barmoontalk_e9b8976d:

    # moon "Yes..."
    moon "是的......"

# game/scripts/bar.rpy:4031
translate schinese barmoontalk_0d7444fd:

    # c "I'm surprised you even come to work with panties still."
    c "我很惊奇你居然还穿着内裤来上班."

# game/scripts/bar.rpy:4033
translate schinese barmoontalk_4984c9d6_4:

    # moon "..."
    moon "......"

# game/scripts/bar.rpy:4039
translate schinese barmoontalk_00d76987_1:

    # c "Say 'Please spank my tight ass [name].'"
    c "说'请打我的小紧屁股吧[name].'"

# game/scripts/bar.rpy:4041
translate schinese barmoontalk_0d562c6d:

    # moon "What!?"
    moon "什么!?"

# game/scripts/bar.rpy:4042
translate schinese barmoontalk_0206afc4_2:

    # c "I'm sure you can agree the humiliation just adds to the punishment."
    c "我相信你会同意的,羞辱也刚好增加了惩罚的部分."

# game/scripts/bar.rpy:4043
translate schinese barmoontalk_925a615a:

    # moon "Stop saying that!"
    moon "别说了!"

# game/scripts/bar.rpy:4045
translate schinese barmoontalk_44e50adc:

    # moon "Fine... Whatever..."
    moon "好吧......随便了......"

# game/scripts/bar.rpy:4046
translate schinese barmoontalk_8217c2e5:

    # moon "Please spank my tight ass... [name]..."
    moon "请打我的小紧屁股吧......[name]......"

# game/scripts/bar.rpy:4047
translate schinese barmoontalk_9696ccb2:

    # c "Good girl."
    c "好女孩."

# game/scripts/bar.rpy:4049
translate schinese barmoontalk_dff7887f:

    # moon "Grr..."
    moon "呃......"

# game/scripts/bar.rpy:4052
translate schinese barmoontalk_ae13a16b:

    # moon "...Do you like spanking me?"
    moon "......你喜欢打我屁股吗?"

# game/scripts/bar.rpy:4053
translate schinese barmoontalk_297d93f3:

    # c "What?"
    c "什么?"

# game/scripts/bar.rpy:4054
translate schinese barmoontalk_838f40c4:

    # moon "You seem to really enjoy it..."
    moon "你看起来真的很享受......"

# game/scripts/bar.rpy:4055
translate schinese barmoontalk_c2da66bb:

    # moon "I don't know if you just want to punish me... Or if you just like seeing my ass... Or if you like hurting me..."
    moon "我不知道你是不是只是想惩罚我......还是说你只是喜欢看我的屁股......或者你喜欢伤害我......"

# game/scripts/bar.rpy:4058
translate schinese barmoontalk_68f4bff9:

    # moon "Oh..."
    moon "噢......"

# game/scripts/bar.rpy:4059
translate schinese barmoontalk_da436381:

    # moon "I mean... Right."
    moon "我是说......好吧."

# game/scripts/bar.rpy:4061
translate schinese barmoontalk_4984c9d6_5:

    # moon "..."
    moon "......"

# game/scripts/bar.rpy:4062
translate schinese barmoontalk_931d1bcc:

    # moon "You... You really think so...?"
    moon "你......你真的这么觉得吗......?"

# game/scripts/bar.rpy:4063
translate schinese barmoontalk_3bc3e1f4:

    # moon "Mmm..."
    moon "嗯......"

# game/scripts/bar.rpy:4064
translate schinese barmoontalk_3ac179ce:

    # moon "Spank me, please..."
    moon "请打我屁股吧......"

# game/scripts/bar.rpy:4066
translate schinese barmoontalk_a3068798:

    # moon "...I see..."
    moon "......我懂了......"

# game/scripts/bar.rpy:4067
translate schinese barmoontalk_f60c322b:

    # moon "Get back to it, then..."
    moon "那就继续吧......"

# game/scripts/bar.rpy:4070
translate schinese barmoontalk_cef922d0:

    # moon "*gasp*!"
    moon "*惊叹*!"

# game/scripts/bar.rpy:4072
translate schinese barmoontalk_bfa75403:

    # moon "What are you-"
    moon "你在干什-"

# game/scripts/bar.rpy:4073
translate schinese barmoontalk_4f2bc13e:

    # c "-Just cleaning you up."
    c "-只是在帮你清理一下."

# game/scripts/bar.rpy:4074
translate schinese barmoontalk_da11c7b7:

    # moon "That's a terrible lie! You're not supposed to-"
    moon "那是个糟糕的借口!你不应该-"

# game/scripts/bar.rpy:4076
translate schinese barmoontalk_f4c0ab1a:

    # moon "-Hnn!"
    moon "-嗯!"

# game/scripts/bar.rpy:4077
translate schinese barmoontalk_488a4409:

    # moon "Mmmm... Ahhh..."
    moon "嗯......啊......"

# game/scripts/bar.rpy:4078
translate schinese barmoontalk_3b49566c:

    # moon "Th... That... Ohh... Ohhh..."
    moon "那......那......噢......噢......"

# game/scripts/bar.rpy:4079
translate schinese barmoontalk_d15034ef:

    # moon "Mmm... Nn... That's... Ah..."
    moon "嗯......嗯......那......啊......"

# game/scripts/bar.rpy:4083
translate schinese barmoontalk_7b3a4d59:

    # moon "You're not supposed to touch me there, remember?" with ssshake
    moon "你不应该碰我那里的,记得吗?" with ssshake

# game/scripts/bar.rpy:4084
translate schinese barmoontalk_881c312e:

    # c "Sorry, you seemed like you enjoyed it."
    c "对不起,你看起来很享受的."

# game/scripts/bar.rpy:4086
translate schinese barmoontalk_030731d7:

    # moon "Shut up... Spank me..."
    moon "闭嘴......打我屁股......"

# game/scripts/bar.rpy:4091
translate schinese barmoontalk_84381bf3_5:

    # moon "Enough!"
    moon "够了!"

# game/scripts/bar.rpy:4097
translate schinese barmoontalk_0dc45d60_5:

    # moon "Oh! Sorry!"
    moon "噢!对不起!"

# game/scripts/bar.rpy:4100
translate schinese barmoontalk_13a409dc_6:

    # c "...Maybe that was too much..."
    c "......也许有点过头了......"

# game/scripts/bar.rpy:4106
translate schinese barmoontalk_ba314112_10:

    # c "That's enough for now."
    c "够了."

# game/scripts/bar.rpy:4107
translate schinese barmoontalk_71d89851_5:

    # moon "Nnn, okay..."
    moon "嗯,好的......"

# game/scripts/bar.rpy:4111
translate schinese barmoontalk_ba314112_11:

    # c "That's enough for now."
    c "够了."

# game/scripts/bar.rpy:4112
translate schinese barmoontalk_7e63ec25_6:

    # moon "Okay..."
    moon "好的......"

# game/scripts/bar.rpy:4118
translate schinese barmoontalk_7238be3c_5:

    # c "Change outfits. I'm going to spank you."
    c "把衣服换了.我要打你屁股."

# game/scripts/bar.rpy:4122
translate schinese barmoontalk_d19f855d:

    # c "..."
    c "......"

# game/scripts/bar.rpy:4123
translate schinese barmoontalk_5e35f542:

    # c "You're not wearing panties?"
    c "你没穿内裤?"

# game/scripts/bar.rpy:4124
translate schinese barmoontalk_ab40fe7b:

    # moon "...I... Forgot to bring an extra pair today..."
    moon "......我......我今天忘多带一条了......"

# game/scripts/bar.rpy:4125
translate schinese barmoontalk_d0ae06d2:

    # c "Is that so?"
    c "真的吗?"

# game/scripts/bar.rpy:4127
translate schinese barmoontalk_5146e411:

    # moon "Shut up and spank me..."
    moon "闭嘴,打我屁股......"

# game/scripts/bar.rpy:4130
translate schinese barmoontalk_cef922d0_1:

    # moon "*gasp*!"
    moon "*惊叹*!"

# game/scripts/bar.rpy:4132
translate schinese barmoontalk_bfa75403_1:

    # moon "What are you-"
    moon "你在干什-"

# game/scripts/bar.rpy:4133
translate schinese barmoontalk_4f2bc13e_1:

    # c "-Just cleaning you up."
    c "-只是在帮你清理一下."

# game/scripts/bar.rpy:4134
translate schinese barmoontalk_007e83d9:

    # moon "You said that last time! That excuse still won't-"
    moon "你上次就这么说了!这个借口不能-"

# game/scripts/bar.rpy:4136
translate schinese barmoontalk_f4c0ab1a_1:

    # moon "-Hnn!"
    moon "-嗯!"

# game/scripts/bar.rpy:4137
translate schinese barmoontalk_4ce77e5e:

    # c "Hm?"
    c "嗯?"

# game/scripts/bar.rpy:4138
translate schinese barmoontalk_e4de1040:

    # moon "Ehhh... Ss... Shut up..."
    moon "呃......嘶......闭嘴......"

# game/scripts/bar.rpy:4139
translate schinese barmoontalk_b5780022:

    # moon "Ohh... Nnn... Oh my... Mmm..."
    moon "噢......嗯......哦我天......嗯......"

# game/scripts/bar.rpy:4140
translate schinese barmoontalk_62f96dc5:

    # moon "S... St... Sto... Mmm... Nn..."
    moon "停......停......停......嗯......嗯......"

# game/scripts/bar.rpy:4142
translate schinese barmoontalk_0f908422:

    # moon "W-What are you doing!?"
    moon "你-你在干什么!?"

# game/scripts/bar.rpy:4143
translate schinese barmoontalk_4e1a0168:

    # c "It sounded like you wanted me to stop."
    c "听起来你想让我停下啊."

# game/scripts/bar.rpy:4144
translate schinese barmoontalk_2aeb2c32:

    # moon "I never...! I- I didn't... I..."
    moon "我没有......!我-我没有......我......"

# game/scripts/bar.rpy:4145
translate schinese barmoontalk_4984c9d6_6:

    # moon "..."
    moon "......"

# game/scripts/bar.rpy:4147
translate schinese barmoontalk_1bdc3bd8:

    # moon "...I never said stop..."
    moon "......我没说停下......"

# game/scripts/bar.rpy:4149
translate schinese barmoontalk_5bde8cb4:

    # moon "Mmmmmm..."
    moon "嗯......"

# game/scripts/bar.rpy:4150
translate schinese barmoontalk_6941d226:

    # moon "Hahhh...! Nnn...! That's... So... Nnn..."
    moon "哈......!嗯......!那......好......嗯......"

# game/scripts/bar.rpy:4151
translate schinese barmoontalk_6d3c6a7f:

    # moon "[name]... Nnnmmm..."
    moon "[name]......嗯......"

# game/scripts/bar.rpy:4155
translate schinese barmoontalk_e3b3c07b:

    # moon "Ahh! Mmmm..." with dissolve
    moon "啊!嗯......" with dissolve

# game/scripts/bar.rpy:4156
translate schinese barmoontalk_eda4f0af:

    # moon "You're... Inside... Me..."
    moon "你......进入......我了......"

# game/scripts/bar.rpy:4157
translate schinese barmoontalk_30c9e87c:

    # moon "Ssnnn... Ohh... There..."
    moon "嘶......噢......那里......"

# game/scripts/bar.rpy:4158
translate schinese barmoontalk_bab4749a:

    # moon "Right... T-There... Ahh...! Hnn!"
    moon "就是......那-那里......啊......!嗯!"

# game/scripts/bar.rpy:4160
translate schinese barmoontalk_99fa2e42:

    # "*shlick*" with dissolve
    "*轻拍*" with dissolve

# game/scripts/bar.rpy:4161
translate schinese barmoontalk_90bed004:

    # moon "Mmmmm...?"
    moon "嗯......?"

# game/scripts/bar.rpy:4163
translate schinese barmoontalk_8a577270:

    # moon "Wait, what are you doing!? You're supposed to... Be spanking me!"
    moon "等等,你在干什么!?你应该......打我屁股的!"

# game/scripts/bar.rpy:4165
translate schinese barmoontalk_7c0e22f0:

    # moon "...So spank me..."
    moon "......打我屁股吧......"

# game/scripts/bar.rpy:4170
translate schinese barmoontalk_84381bf3_6:

    # moon "Enough!"
    moon "够了!"

# game/scripts/bar.rpy:4176
translate schinese barmoontalk_0dc45d60_6:

    # moon "Oh! Sorry!"
    moon "噢!对不起!"

# game/scripts/bar.rpy:4179
translate schinese barmoontalk_13a409dc_7:

    # c "...Maybe that was too much..."
    c "......也许有点太过了......"

# game/scripts/bar.rpy:4185
translate schinese barmoontalk_ba314112_12:

    # c "That's enough for now."
    c "够了."

# game/scripts/bar.rpy:4186
translate schinese barmoontalk_71d89851_6:

    # moon "Nnn, okay..."
    moon "嗯,好吧......"

# game/scripts/bar.rpy:4190
translate schinese barmoontalk_ba314112_13:

    # c "That's enough for now."
    c "够了."

# game/scripts/bar.rpy:4191
translate schinese barmoontalk_7e63ec25_7:

    # moon "Okay..."
    moon "好吧......"

# game/scripts/bar.rpy:4197
translate schinese barmoontalk_7238be3c_6:

    # c "Change outfits. I'm going to spank you."
    c "把衣服换了.我要打你屁股."

# game/scripts/bar.rpy:4201
translate schinese barmoontalk_1c6c7788:

    # c "No panties... Nice."
    c "没穿内裤......太棒了."

# game/scripts/bar.rpy:4202
translate schinese barmoontalk_244ff9be:

    # moon "...I forgot my... Nevermind..."
    moon "......我忘了我的......算了......"

# game/scripts/bar.rpy:4203
translate schinese barmoontalk_ba41627c:

    # c "Why don't you forget the rest of your clothes too?"
    c "为什么你不把其他衣服也忘了?"

# game/scripts/bar.rpy:4205
translate schinese barmoontalk_e3009336:

    # moon "What...!?"
    moon "什么......!?"

# game/scripts/bar.rpy:4206
translate schinese barmoontalk_5ba0cf3c:

    # c "Your ass has already been out there, the rest won't be much more."
    c "你的屁股都出来了,剩下的也没多少了......"

# game/scripts/bar.rpy:4208
translate schinese barmoontalk_b6c34a9b:

    # moon "That's ridiculous..."
    moon "那太荒谬了......"

# game/scripts/bar.rpy:4210
translate schinese barmoontalk_ea756623:

    # moon "...The rest of my clothes have nothing to do with spanking, [name]..."
    moon "......我剩下的衣服和打屁股无关,[name]......"

# game/scripts/bar.rpy:4212
translate schinese barmoontalk_7222fcf6:

    # moon "...But I'm sure you can agree the humiliation just adds to the punishment..."
    moon "......但我相信你会同意的,羞辱也刚好增加了惩罚的部分......"

# game/scripts/bar.rpy:4214
translate schinese barmoontalk_bdf3e631:

    # moon "...Among other effects..."
    moon "......还有别的影响......"

# game/scripts/bar.rpy:4217
translate schinese barmoontalk_3089766c:

    # moon "Interesting..."
    moon "有趣......"

# game/scripts/bar.rpy:4218
translate schinese barmoontalk_297d93f3_1:

    # c "What?"
    c "什么?"

# game/scripts/bar.rpy:4219
translate schinese barmoontalk_c16ea5bd:

    # moon "Oh, nothing..."
    moon "噢,没什么......"

# game/scripts/bar.rpy:4221
translate schinese barmoontalk_5b61b904:

    # moon "Um... Where's your phone?"
    moon "嗯......你的手机呢?"

# game/scripts/bar.rpy:4222
translate schinese barmoontalk_c98eb17c:

    # c "In my back pocket?"
    c "在我后面的口袋里?"

# game/scripts/bar.rpy:4223
translate schinese barmoontalk_68f4bff9_1:

    # moon "Oh..."
    moon "噢......"

# game/scripts/bar.rpy:4225
translate schinese barmoontalk_1d741ad6:

    # moon "Oh... I almost forgot, I'm naked aren't I?"
    moon "噢......我差点忘了,我是不是裸着?"

# game/scripts/bar.rpy:4226
translate schinese barmoontalk_ae2abe22:

    # c "You are."
    c "是的."

# game/scripts/bar.rpy:4227
translate schinese barmoontalk_21d00f0a:

    # moon "I was just so focused..."
    moon "我只是太专注了......"

# game/scripts/bar.rpy:4232
translate schinese barmoontalk_84381bf3_7:

    # moon "Enough!"
    moon "够了!"

# game/scripts/bar.rpy:4238
translate schinese barmoontalk_0dc45d60_7:

    # moon "Oh! Sorry!"
    moon "噢!对不起!"

# game/scripts/bar.rpy:4241
translate schinese barmoontalk_13a409dc_8:

    # c "...Maybe that was too much..."
    c "......也许有点过头了......"

# game/scripts/bar.rpy:4247
translate schinese barmoontalk_ba314112_14:

    # c "That's enough for now."
    c "够了."

# game/scripts/bar.rpy:4248
translate schinese barmoontalk_71d89851_7:

    # moon "Nnn, okay..."
    moon "嗯,好吧......"

# game/scripts/bar.rpy:4252
translate schinese barmoontalk_ba314112_15:

    # c "That's enough for now."
    c "够了."

# game/scripts/bar.rpy:4253
translate schinese barmoontalk_7e63ec25_8:

    # moon "Okay..."
    moon "好吧......"

# game/scripts/bar.rpy:4259
translate schinese barmoontalk_7238be3c_7:

    # c "Change outfits. I'm going to spank you."
    c "把衣服换了.我要打你屁股."

# game/scripts/bar.rpy:4263
translate schinese barmoontalk_038c8a31:

    # moon "...I should really buy new panties some day, hahah..."
    moon "......我真的得找个时间买点新内裤,哈哈哈......"

# game/scripts/bar.rpy:4264
translate schinese barmoontalk_4bd2b1f0:

    # c "They'd just get in the way."
    c "那它们只会碍手碍脚."

# game/scripts/bar.rpy:4266
translate schinese barmoontalk_c5bdbc04:

    # moon "Maybe they would..."
    moon "也许会吧......"

# game/scripts/bar.rpy:4268
translate schinese barmoontalk_6233657e:

    # moon "Besides, it hurts more without them..."
    moon "而且,没有它们疼多了......"

# game/scripts/bar.rpy:4270
translate schinese barmoontalk_747ace26:

    # moon "So I suppose you can continue..."
    moon "所以我想你可以继续......"

# game/scripts/bar.rpy:4272
translate schinese barmoontalk_a72ed496:

    # moon "W-Wait...! That's not spanking...!"
    moon "等-等等......!这不是打屁股......!"

# game/scripts/bar.rpy:4279
translate schinese barmoontalk_0debc013:

    # moon "Ahh..."
    moon "啊......"

# game/scripts/bar.rpy:4281
translate schinese barmoontalk_3bc3e1f4_1:

    # moon "Mmm..."
    moon "嗯......"

# game/scripts/bar.rpy:4283
translate schinese barmoontalk_8988c88e:

    # moon "Hnng..."
    moon "嗯......"

# game/scripts/bar.rpy:4285
translate schinese barmoontalk_37f78821_2:

    # moon "Hnn!"
    moon "嗯!"

# game/scripts/bar.rpy:4287
translate schinese barmoontalk_f724519e_2:

    # moon "Ah!"
    moon "啊!"

# game/scripts/bar.rpy:4289
translate schinese barmoontalk_322fa9f0:

    # c "Better?"
    c "好多了?"

# game/scripts/bar.rpy:4290
translate schinese barmoontalk_f1842f94:

    # moon "Hn..."
    moon "嗯......"

# game/scripts/bar.rpy:4291
translate schinese barmoontalk_6c4a37c7:

    # moon "...Yes..."
    moon "......是的......"

# game/scripts/bar.rpy:4298
translate schinese barmoontalk_814e5466:

    # moon "Hahhn!"
    moon "哈!"

# game/scripts/bar.rpy:4300
translate schinese barmoontalk_33c32dd1:

    # moon "Nn, mmm..."
    moon "嗯,嗯......"

# game/scripts/bar.rpy:4302
translate schinese barmoontalk_082ce360:

    # moon "Ohh..."
    moon "噢......"

# game/scripts/bar.rpy:4304
translate schinese barmoontalk_f724519e_3:

    # moon "Ah!"
    moon "啊!"

# game/scripts/bar.rpy:4306
translate schinese barmoontalk_8c7e88fb:

    # moon "Hnnnn...!"
    moon "嗯......!"

# game/scripts/bar.rpy:4308
translate schinese barmoontalk_4a41c3f1:

    # moon "...Mmmm... That feels so good...!"
    moon "......嗯......感觉好舒服......!"

# game/scripts/bar.rpy:4309
translate schinese barmoontalk_0530d3da:

    # c "Sounds like you're opening up."
    c "听起来你终于放开了."

# game/scripts/bar.rpy:4310
translate schinese barmoontalk_e88e760f:

    # moon "...I don't know... It just feels good, okay?"
    moon "......我不知道......只是感觉很舒服,好吗?"

# game/scripts/bar.rpy:4311
translate schinese barmoontalk_59cc033d:

    # moon "Mmmm...! Ah!"
    moon "嗯......!啊!"

# game/scripts/bar.rpy:4313
translate schinese barmoontalk_4984c9d6_7:

    # moon "..."
    moon "......"

# game/scripts/bar.rpy:4315
translate schinese barmoontalk_6fc2d03e:

    # moon "...What are you doing...?"
    moon "......你在干什么......?"

# game/scripts/bar.rpy:4322
translate schinese barmoontalk_f724519e_4:

    # moon "Ah!"
    moon "啊!"

# game/scripts/bar.rpy:4327
translate schinese barmoontalk_7ee2ec2d:

    # moon "Mmmmm!!"
    moon "嗯!!"

# game/scripts/bar.rpy:4329
translate schinese barmoontalk_b114a925:

    # c "Beg."
    c "求我."

# game/scripts/bar.rpy:4331
translate schinese barmoontalk_07a8886c:

    # moon "...Like before...?"
    moon "......像之前那样吗......?"

# game/scripts/bar.rpy:4332
translate schinese barmoontalk_49bfd76d:

    # moon "......"
    moon "......"

# game/scripts/bar.rpy:4333
translate schinese barmoontalk_8fb6232d:

    # moon "........."
    moon "........."

# game/scripts/bar.rpy:4335
translate schinese barmoontalk_7b0439e5:

    # moon "Please spank me..."
    moon "请打我屁股吧......"

# game/scripts/bar.rpy:4337
translate schinese barmoontalk_88ae8d97:

    # moon "I... Um... I've been a bad girl...?"
    moon "我......嗯......我是个坏女孩......?"

# game/scripts/bar.rpy:4339
translate schinese barmoontalk_de6f8d15:

    # moon "...I broke all your glasses and need to be punished..."
    moon "......我打破了你所有的玻璃杯,我需要被惩罚......"

# game/scripts/bar.rpy:4341
translate schinese barmoontalk_f1c10d66:

    # moon "And... And rubbed... My wet pussy wants to... Be rubbed..."
    moon "还有......还有抚摸......我湿润的小穴......需要被抚摸......"

# game/scripts/bar.rpy:4346
translate schinese barmoontalk_f724519e_5:

    # moon "Ah!"
    moon "啊!"

# game/scripts/bar.rpy:4351
translate schinese barmoontalk_7ee2ec2d_1:

    # moon "Mmmmm!!"
    moon "嗯!!"

# game/scripts/bar.rpy:4353
translate schinese barmoontalk_139c6bba:

    # c "Good."
    c "很好."

# game/scripts/bar.rpy:4354
translate schinese barmoontalk_95cc95c1:

    # moon "Ahh... Yes! Mmmm!!"
    moon "啊......是的!嗯!!"

# game/scripts/bar.rpy:4355
translate schinese barmoontalk_a770cc75:

    # moon "{size=-10}I'm so wet... I'm so wet... Oh my god...{/size} Ahn~!"
    moon "{size=-10}我好湿......我好湿......哦我天......{/size}啊~!"

# game/scripts/bar.rpy:4357
translate schinese barmoontalk_d06dd5b7:

    # c "Take off your clothes."
    c "把你的衣服脱了."

# game/scripts/bar.rpy:4358
translate schinese barmoontalk_504ede54:

    # moon "A-And you'll put your finger inside...?"
    moon "那-那你会把手指放进来吗......?"

# game/scripts/bar.rpy:4359
translate schinese barmoontalk_fabe4767:

    # c "Maybe..."
    c "也许......"

# game/scripts/bar.rpy:4360
translate schinese barmoontalk_48ca5d2e:

    # moon "Nn..."
    moon "嗯......"

# game/scripts/bar.rpy:4363
translate schinese barmoontalk_70c1bd2d:

    # moon "Weeelllll...?"
    moon "好吗......?"

# game/scripts/bar.rpy:4366
translate schinese barmoontalk_3596ecbe:

    # moon "Ahh! Hnnn!"
    moon "啊!嗯!"

# game/scripts/bar.rpy:4367
translate schinese barmoontalk_74a00694:

    # c "Are you always this wet?"
    c "你一直都这么湿吗?"

# game/scripts/bar.rpy:4368
translate schinese barmoontalk_ec9252bd:

    # moon "{size=-10}...Y-Yes... Lately...{/size}"
    moon "{size=-10}......是-是的......最近..{/size}"

# game/scripts/bar.rpy:4369
translate schinese barmoontalk_df72d121:

    # c "What's that?"
    c "真的吗?"

# game/scripts/bar.rpy:4370
translate schinese barmoontalk_f8ba98f1:

    # moon "Hnnnn... Yes... I don't know why...!"
    moon "嗯......是的......我也不知道为什么......!"

# game/scripts/bar.rpy:4371
translate schinese barmoontalk_a27c545f:

    # moon "I-It just- Hn! ...Happens... S-Sometimes..."
    moon "只-只是-嗯!......有时候......会-会发生......"

# game/scripts/bar.rpy:4372
translate schinese barmoontalk_686def15:

    # c "Like when?"
    c "比如什么时候?"

# game/scripts/bar.rpy:4373
translate schinese barmoontalk_0d207bce:

    # moon "D-Don't ask stupid questions! I-It just happens! Ah~!"
    moon "别-别问那些傻问题了!它-它就是会发生!啊~!"

# game/scripts/bar.rpy:4374
translate schinese barmoontalk_095ceeb6:

    # c "Have you tried fixing it yourself?"
    c "你有试过修好你自己吗?"

# game/scripts/bar.rpy:4375
translate schinese barmoontalk_8ff7e2d8:

    # moon "I s-said... Don't ask stupid questions...! HNN!"
    moon "我说-说了......别问那些傻问题了......!嗯!"

# game/scripts/bar.rpy:4380
translate schinese barmoontalk_11aa970e_2:

    # moon "Sss!"
    moon "嘶!"

# game/scripts/bar.rpy:4385
translate schinese barmoontalk_1129bef8:

    # moon "Nya!!"
    moon "喵!!"

# game/scripts/bar.rpy:4387
translate schinese barmoontalk_9d272580:

    # moon "Ohh!"
    moon "噢!"

# game/scripts/bar.rpy:4388
translate schinese barmoontalk_162715b8:

    # moon "{size=-10}...Wh... Why does it feel so good... Hnnn...{/size} Ahh~"
    moon "{size=-10}......为......为什么感觉这么舒服......嗯......{/size}啊~"

# game/scripts/bar.rpy:4391
translate schinese barmoontalk_7030489d:

    # moon "Ahhh~ Nnn! {size=-10}D... Damn...!{/size}"
    moon "啊~嗯!{size=-10}操......操......!{/size}"

# game/scripts/bar.rpy:4392
translate schinese barmoontalk_0f43440d:

    # moon "Hnnn! Nnn! Nngh!!"
    moon "嗯!嗯!嗯!!"

# game/scripts/bar.rpy:4393
translate schinese barmoontalk_38f669d0:

    # moon "Fffuhh...! Nnguh! Nnn!"
    moon "操......!嗯!嗯!"

# game/scripts/bar.rpy:4394
translate schinese barmoontalk_ee3b9ca7:

    # moon "I... I... I'm so..."
    moon "我......我......我好......"

# game/scripts/bar.rpy:4399
translate schinese barmoontalk_f724519e_6:

    # moon "Ah!"
    moon "啊!"

# game/scripts/bar.rpy:4400
translate schinese barmoontalk_83a9c09b:

    # moon "What was-"
    moon "什-"

# game/scripts/bar.rpy:4405
translate schinese barmoontalk_70354428:

    # moon "Ngh!!"
    moon "嗯!!"

# game/scripts/bar.rpy:4406
translate schinese barmoontalk_bd244b4a:

    # moon "...Please... Don't make me beg..."
    moon "......求你了......不要再让我求了......"

# game/scripts/bar.rpy:4410
translate schinese barmoontalk_84381bf3_8:

    # moon "Enough!"
    moon "够了!"

# game/scripts/bar.rpy:4416
translate schinese barmoontalk_0dc45d60_8:

    # moon "Oh! Sorry!"
    moon "噢!对不起!"

# game/scripts/bar.rpy:4419
translate schinese barmoontalk_13a409dc_9:

    # c "...Maybe that was too much..."
    c "......也许有点太过了......"

# game/scripts/bar.rpy:4425
translate schinese barmoontalk_ba314112_16:

    # c "That's enough for now."
    c "够了."

# game/scripts/bar.rpy:4426
translate schinese barmoontalk_71d89851_8:

    # moon "Nnn, okay..."
    moon "嗯,好吧......"

# game/scripts/bar.rpy:4430
translate schinese barmoontalk_ba314112_17:

    # c "That's enough for now."
    c "够了."

# game/scripts/bar.rpy:4431
translate schinese barmoontalk_7e63ec25_9:

    # moon "Okay..."
    moon "好吧......"

# game/scripts/bar.rpy:4437
translate schinese barmoontalk_7238be3c_8:

    # c "Change outfits. I'm going to spank you."
    c "把衣服换了.我要打你屁股."

# game/scripts/bar.rpy:4441
translate schinese barmoontalk_d97a7eae:

    # moon "Please...!"
    moon "求你了......!"

# game/scripts/bar.rpy:4444
translate schinese barmoontalk_4984c9d6_8:

    # moon "..."
    moon "......"

# game/scripts/bar.rpy:4450
translate schinese barmoontalk_0e55b2b0:

    # moon "...I'm waiting..."
    moon "......我还在等......"

# game/scripts/bar.rpy:4457
translate schinese barmoontalk_8ec36b9f:

    # moon "Hmmm! Ahhh!"
    moon "嗯!啊!"

# game/scripts/bar.rpy:4458
translate schinese barmoontalk_422d6d5e:

    # moon "Sh... Shouldn't you spank me first...?"
    moon "你......你不应该先打我屁股吗......?"

# game/scripts/bar.rpy:4459
translate schinese barmoontalk_5a985f75:

    # moon "I- Ah~ ...I mean... It would make more sense to start with that... Nhh...!"
    moon "我-啊~......我是说......那样开始更合理一点......嗯......!"

# game/scripts/bar.rpy:4460
translate schinese barmoontalk_d9b7485c:

    # moon "But... I'm not complaining..."
    moon "但是......我不是在抱怨......"

# game/scripts/bar.rpy:4466
translate schinese barmoontalk_9f46ae8e:

    # moon "Nnnnuh! Nghuh!"
    moon "嗯!嗯!"

# game/scripts/bar.rpy:4467
translate schinese barmoontalk_912da63b:

    # moon "I-Inside...? Already...!? Hnn...!"
    moon "进-进来了......?已经进来了......!?嗯......!"

# game/scripts/bar.rpy:4468
translate schinese barmoontalk_8e37b046:

    # c "You looked wet enough."
    c "你看起来已经够湿了."

# game/scripts/bar.rpy:4469
translate schinese barmoontalk_d29e7f6e:

    # moon "Mmmgh! Whatever...! Hnnah!"
    moon "嗯!算了......!嗯!"

# game/scripts/bar.rpy:4477
translate schinese barmoontalk_8d264032:

    # c "Maybe you can cum next time."
    c "也许你下次就可以去了."

# game/scripts/bar.rpy:4478
translate schinese barmoontalk_578564c8:

    # moon "Next time...!? That's so mean!"
    moon "下次......!?那太刻薄了!"

# game/scripts/bar.rpy:4491
translate schinese barmoontalk_2b8eac25:

    # moon "Right!"
    moon "好的!"

# game/scripts/bar.rpy:4505
translate schinese marelcomp_404de09a:

    # m "Woo! Alright, you ready to lose, woman?"
    m "喔!好了,你准备好认输了吗,女人?"

# game/scripts/bar.rpy:4506
translate schinese marelcomp_9bdfdae9:

    # e "You're going down young lady!"
    e "你才是倒下的那个,年轻的小姐!"

# game/scripts/bar.rpy:4507
translate schinese marelcomp_45a7cc2e:

    # m "Go!"
    m "来!"

# game/scripts/bar.rpy:4509
translate schinese marelcomp_88615573:

    # m "*gulp*!"
    m "*咕噜*!"

# game/scripts/bar.rpy:4510
translate schinese marelcomp_01440cc7:

    # e "*gulp*!"
    e "*咕噜*!"

# game/scripts/bar.rpy:4512
translate schinese marelcomp_1d3483fe:

    # e "Ready to give up?" with ssshake
    e "准备放弃了吗?" with ssshake

# game/scripts/bar.rpy:4513
translate schinese marelcomp_f1b5510c:

    # m "Never!"
    m "才不!"

# game/scripts/bar.rpy:4515
translate schinese marelcomp_88615573_1:

    # m "*gulp*!"
    m "*咕噜*!"

# game/scripts/bar.rpy:4516
translate schinese marelcomp_01440cc7_1:

    # e "*gulp*!"
    e "*咕噜*!"

# game/scripts/bar.rpy:4518
translate schinese marelcomp_41b2c21a:

    # m "Easy!" with ssshake
    m "太简单了!" with ssshake

# game/scripts/bar.rpy:4519
translate schinese marelcomp_d95e992a:

    # e "Same here!"
    e "我也一样!"

# game/scripts/bar.rpy:4521
translate schinese marelcomp_88615573_2:

    # m "*gulp*!"
    m "*咕噜*!"

# game/scripts/bar.rpy:4522
translate schinese marelcomp_01440cc7_2:

    # e "*gulp*!"
    e "*咕噜*!"

# game/scripts/bar.rpy:4525
translate schinese marelcomp_b4bb1a1d:

    # e "Ohhh..." with ssshake
    e "噢......" with ssshake

# game/scripts/bar.rpy:4526
translate schinese marelcomp_01e80206:

    # m "You're going down!"
    m "你倒下了!"

# game/scripts/bar.rpy:4528
translate schinese marelcomp_05d4d17c:

    # e "Not easily!"
    e "没那么容易!"

# game/scripts/bar.rpy:4530
translate schinese marelcomp_88615573_3:

    # m "*gulp*!"
    m "*咕噜*!"

# game/scripts/bar.rpy:4531
translate schinese marelcomp_01440cc7_3:

    # e "*gulp*!"
    e "*咕噜*!"

# game/scripts/bar.rpy:4533
translate schinese marelcomp_f80085e7:

    # e "It feels like my throat is burning..." with ssshake
    e "我的喉咙好像在燃烧......" with ssshake

# game/scripts/bar.rpy:4534
translate schinese marelcomp_0a799024:

    # m "Shut up and drink!"
    m "闭嘴,喝!"

# game/scripts/bar.rpy:4536
translate schinese marelcomp_88615573_4:

    # m "*gulp*!"
    m "*咕噜*!"

# game/scripts/bar.rpy:4537
translate schinese marelcomp_01440cc7_4:

    # e "*gulp*!"
    e "*咕噜*!"

# game/scripts/bar.rpy:4539
translate schinese marelcomp_1c41824e:

    # e "Nn! I can't take another drink!" with ssshake
    e "嗯!我再也喝不了了!" with ssshake

# game/scripts/bar.rpy:4540
translate schinese marelcomp_3de9563c:

    # m "Hah! I win! Now get to sucking!"
    m "哈哈!我赢了!现在去口交吧!"

# game/scripts/bar.rpy:4541
translate schinese marelcomp_4cb5765c:

    # e "Okay..."
    e "好吧......"

# game/scripts/bar.rpy:4543
translate schinese marelcomp_0f600867:

    # e "Well [name], it looks like I lost..."
    e "嗯[name],看起来我输了......"

# game/scripts/bar.rpy:4544
translate schinese marelcomp_5f8d0ecb:

    # e "Let's get to it, then!"
    e "那就开始吧!"

# game/scripts/bar.rpy:4546
translate schinese marelcomp_6ffbe535:

    # e "Aghhk! Mmmghh!"
    e "啊!嗯!"

# game/scripts/bar.rpy:4547
translate schinese marelcomp_ef4ee7d9:

    # m "This is so hot!"
    m "这太火辣了!"

# game/scripts/bar.rpy:4548
translate schinese marelcomp_e8292414:

    # e "NNGHK! Ghhpphh! Mmm, [name]!"
    e "嗯!咕!嗯,[name]!"

# game/scripts/bar.rpy:4551
translate schinese marelcomp_d43e3c76:

    # e "Mmmgph! [name]! Thish is amashing!"
    e "嗯![name]!台棒了!"

# game/scripts/bar.rpy:4552
translate schinese marelcomp_2771141e:

    # m "This is making me so wet..."
    m "这让我变得好湿......"

# game/scripts/bar.rpy:4553
translate schinese marelcomp_aa566afa:

    # e "Mmmph! Mmmp! GHK!"
    e "嗯!嗯!咕!"

# game/scripts/bar.rpy:4554
translate schinese marelcomp_8514d234:

    # e "Oofgh... I'm getting so dizzy..."
    e "噢......我好晕......"

# game/scripts/bar.rpy:4555
translate schinese marelcomp_a09e815c:

    # m "Cum all over her tits, [name]! Do it! Do it!"
    m "全射到她的奶子上吧,[name]!射吧!射吧!"

# game/scripts/bar.rpy:4557
translate schinese marelcomp_7d670ce7:

    # e "Nnn!" with ssshake
    e "嗯!" with ssshake

# game/scripts/bar.rpy:4558
translate schinese marelcomp_b899d67d:

    # m "Yes! Amazing!"
    m "是的!太棒了!"

# game/scripts/bar.rpy:4563
translate schinese marelcomp_839c51d9:

    # m "Ahh..." with ssshake
    m "啊......" with ssshake

# game/scripts/bar.rpy:4564
translate schinese marelcomp_474c8259:

    # e "Give up already!"
    e "放弃吧!"

# game/scripts/bar.rpy:4566
translate schinese marelcomp_1a0a36d7:

    # m "Shut it, woman!"
    m "闭嘴,女人!"

# game/scripts/bar.rpy:4568
translate schinese marelcomp_88615573_5:

    # m "*gulp*!"
    m "*咕噜*!"

# game/scripts/bar.rpy:4569
translate schinese marelcomp_01440cc7_5:

    # e "*gulp*!"
    e "*咕噜*!"

# game/scripts/bar.rpy:4571
translate schinese marelcomp_5bc1b2c0:

    # m "Nnh, my throat's on fire!" with ssshake
    m "嗯,我的喉咙在燃烧!" with ssshake

# game/scripts/bar.rpy:4573
translate schinese marelcomp_88615573_6:

    # m "*gulp*!"
    m "*咕噜*!"

# game/scripts/bar.rpy:4574
translate schinese marelcomp_01440cc7_6:

    # e "*gulp*!"
    e "*咕噜*!"

# game/scripts/bar.rpy:4576
translate schinese marelcomp_aaccad71:

    # m "Agh! No more! I can't take it..." with sshake
    m "啊!别来了!我受不了了......" with sshake

# game/scripts/bar.rpy:4577
translate schinese marelcomp_99fe0a2d:

    # e "Well, get to sucking young lady!"
    e "嗯,那就去口交吧,年轻的小姐!"

# game/scripts/bar.rpy:4578
translate schinese marelcomp_7d785ee6:

    # m "Right, okay!"
    m "好吧,好吧!"

# game/scripts/bar.rpy:4580
translate schinese marelcomp_5eab0ac3:

    # m "I lost on purpose, [name]..."
    m "我故意输的,[name]......"

# game/scripts/bar.rpy:4582
translate schinese marelcomp_f312154c:

    # m "Ghlllsh... Gphh..."
    m "咕噜嘶......咕......"

# game/scripts/bar.rpy:4583
translate schinese marelcomp_c8bc4ae1:

    # m "Nnguh! [name]..."
    m "嗯![name]......"

# game/scripts/bar.rpy:4584
translate schinese marelcomp_2f75fc86:

    # e "Don't stop now, you're in too deep now!"
    e "现在别停下,你现在已经很深了!"

# game/scripts/bar.rpy:4585
translate schinese marelcomp_82590478:

    # m "Anggh! Mmphh!"
    m "啊!嗯!"

# game/scripts/bar.rpy:4588
translate schinese marelcomp_d817d331:

    # e "Has he still not cum yet? Maria... I thought you were better than this!"
    e "他还没射吗?玛丽亚......我以为你可以做得更好的!"

# game/scripts/bar.rpy:4589
translate schinese marelcomp_5fc27f16:

    # m "Shut up, lady!"
    m "闭嘴,女人!"

# game/scripts/bar.rpy:4590
translate schinese marelcomp_8babc9cd:

    # m "Oogh! Nngh... Ooohh, I'm dizzy now..."
    m "噢!嗯......噢,我现在好晕......"

# game/scripts/bar.rpy:4591
translate schinese marelcomp_9e06540f:

    # e "[name], cum all over her!"
    e "[name],全射她身上吧!"

# game/scripts/bar.rpy:4593
translate schinese marelcomp_4c34829b:

    # m "Ahhh! Mmm..." with ssshake
    m "啊!嗯......" with ssshake

# game/scripts/bar.rpy:4598
translate schinese marelcomp_b21fec66:

    # e "That was fun!"
    e "那太好玩了!"

# game/scripts/bar.rpy:4599
translate schinese marelcomp_e93bd02d:

    # m "It was!"
    m "是的!"

# game/scripts/bar.rpy:4611
translate schinese moon_treehouse_2ec6edb9:

    # moon "[name]! Good day, how are you?"
    moon "[name]!日安,你今天过得怎么样?"

# game/scripts/bar.rpy:4612
translate schinese moon_treehouse_7c0edd3b:

    # c "Hey Moon, what's the occasion?"
    c "嘿文,有什么事情吗?"

# game/scripts/bar.rpy:4613
translate schinese moon_treehouse_7b028faa:

    # moon "I have some time off today so I thought we could go on a little adventure!"
    moon "我今天有点时间,所以我想我们可以去进行一次小小的冒险!"

# game/scripts/bar.rpy:4617
translate schinese moon_treehouse_ba74095a:

    # moon "Sweet! I knew you would say that."
    moon "太好了!我就知道你会那么说的."

# game/scripts/bar.rpy:4620
translate schinese moon_treehouse_bbc9f603:

    # moon "Oh, okay... give me a call when you want to hang out next time."
    moon "噢,好吧......下次想出去玩的时候打个电话给我."

# game/scripts/bar.rpy:4625
translate schinese moon12cont_7638b914:

    # moon "Okay, so the place I have in mind isn't too far from Caliber city. It's actually right next to my dorm, I visit this spot all the time with my study group. I'll give you directions!"
    moon "好了,我想去的那个地方离楷利泊市不远,实际上它就在我的宿舍旁边,我和我的学习小组经常去那个地方.我给你指路!"

# game/scripts/bar.rpy:4627
translate schinese moon12cont_270a3c95:

    # "You follow Moon's directions to her dorm. After parking, she leads you into a forest on the other side of the street. 10 minutes later you arrive..."
    "你跟着文的指示去了她的宿舍.停车后,她带你走进街道另一边的森林.10分钟后,你们到达了......"

# game/scripts/bar.rpy:4629
translate schinese moon12cont_50b77542:

    # moon "Here we are! My own little hideout. Well, it belongs to everyone in my study group, but I get to use it whenever I want to."
    moon "我们到了!这是我自己的小藏身处.好吧,它属于我们学习小组的每个人,但是只要我想,我就可以使用它."

# game/scripts/bar.rpy:4631
translate schinese moon12cont_b6a70d37:

    # c "A treehouse, wow. It looks well built."
    c "一个树屋,哇哦.看起来建的不错."

# game/scripts/bar.rpy:4632
translate schinese moon12cont_20fd1fae:

    # moon "We wanted to test our engineering skills together, so we found some land no one was using, and a year later, here it is!"
    moon "我们想要一起测试我们的工程技能,所以我们找了一些无人认领的土地,一年后,它就建好了!"

# game/scripts/bar.rpy:4633
translate schinese moon12cont_ff7b92b6:

    # moon "It has a bedroom, a toilet, a shower, and even a mini bar! We have also decided to store a few nonperishub- ...nonperlishi- ...nonperusblublu- ...nonpershlibles...?"
    moon "它有一个卧室,一个厕所,一个浴室,甚至还有一个迷你酒吧!我们还决定在这里放一些不依附-......不衣服-......不义父-......不已付......?"

# game/scripts/bar.rpy:4634
translate schinese moon12cont_a74ec3f5:

    # c "Nonperishables."
    c "不易腐烂的东西."

# game/scripts/bar.rpy:4635
translate schinese moon12cont_3d962656:

    # moon "That's it! Heheh..."
    moon "对了!呵呵呵......"

# game/scripts/bar.rpy:4637
translate schinese moon12cont_2e73f8fd:

    # c "So you just built this for fun? You didn't get graded for this at all?"
    c "所以你们只是为了好玩建的?你们没得到任何学分吗?"

# game/scripts/bar.rpy:4638
translate schinese moon12cont_8b6caefc:

    # moon "Nope, this was just a fun side project. Which is actually why I was coming here to begin with, I'm working on something else!"
    moon "没有,这只是一个有趣的项目罢了.这其实也是我一开始要来这里的原因,我在做别的东西!"

# game/scripts/bar.rpy:4639
translate schinese moon12cont_94af93af:

    # c "Do you come here often to work on things?"
    c "你经常来这里研究吗?"

# game/scripts/bar.rpy:4640
translate schinese moon12cont_0e0d536e:

    # moon "Yep. Since my walls are paper thin, it's best to have an office. So this is it!"
    moon "是的.因为我那里的墙跟纸糊的一样,所以最好得有个办公室.这里就是了!"

# game/scripts/bar.rpy:4641
translate schinese moon12cont_38f1da48:

    # moon "Unfortunately I couldn't find an ethernet cable long enough to reach out here, otherwise I would call this place home instead."
    moon "不幸的是,我找不到能够得到这里的网线,否则我就把这里当成家了."

# game/scripts/bar.rpy:4643
translate schinese moon12cont_556bf10c:

    # c "Hey, what's that?"
    c "嘿,那是什么?"

# game/scripts/bar.rpy:4644
translate schinese moon12cont_c18d7ef6:

    # moon "Oh that thing? That's been here much longer than we have. Wanna go check it out real quick?"
    moon "噢那个啊?那东西在这里的时间比我们还久.想去看看它的真面目吗?"

# game/scripts/bar.rpy:4645
translate schinese moon12cont_9f85b645:

    # c "Yeah."
    c "是的."

# game/scripts/bar.rpy:4647
translate schinese moon12cont_b9bcc21a:

    # c "Wow, what is it?"
    c "哇哦,这是什么?"

# game/scripts/bar.rpy:4648
translate schinese moon12cont_764f85a4:

    # moon "It's a post-industrial era mecha-tank. If my memory of history class is correct, those were mainly used in the third uprising. It's probably over 80 years old."
    moon "这是后工业时代的机械坦克.如果我对历史课的记忆没有错的话,那些东西主要用于第三次革命.它可能已经有80多年的历史了."

# game/scripts/bar.rpy:4649
translate schinese moon12cont_0d7b2280:

    # c "Oh, that's what you meant by it's been here longer than we have."
    c "噢,所以这就是你说的它在这里的时间比我们久."

# game/scripts/bar.rpy:4650
translate schinese moon12cont_d6fb53d3:

    # moon "Mhm. They were really slow, but each arm had a tank barrel and accuracy was very high! But once tanks were invented, these were quickly out-classed. Now it just holds my flower."
    moon "嗯.它们很慢,但是每只手臂上都有一根坦克炮管,精准度很高.但是坦克发明后,这些东西很快就被淘汰了.现在它只能拿来种花."

# game/scripts/bar.rpy:4651
translate schinese moon12cont_3e5a2ea0:

    # moon "Such a shame though! I would have loved to see those in battle, they're so cool!"
    moon "尽管如此,真是太可惜了!我很想看看这些东西在战斗中的表现,它们太酷了!"

# game/scripts/bar.rpy:4652
translate schinese moon12cont_5fd60228:

    # c "Have you gone inside?"
    c "你进去过吗?"

# game/scripts/bar.rpy:4653
translate schinese moon12cont_01ac3a87:

    # moon "Obviously! Who do you think you're talking to? I've stripped it for parts already..."
    moon "当然了!你以为你在和谁说话啊?我都把它一部分零件扒下来了......"

# game/scripts/bar.rpy:4654
translate schinese moon12cont_1d8fd8c0:

    # c "I'm surprised these old war mechs are just left here to rust. Is this the only one you've found?"
    c "我很惊讶这些旧时的战争机器居然就被丢在这里慢慢生锈.你就发现了这一个吗?"

# game/scripts/bar.rpy:4655
translate schinese moon12cont_a08e2284:

    # moon "There's a few scattered around the forest, but this one is the most intact that I could find around here."
    moon "还有几个散落在森林里,但是这是我在这附近能找到的最完整的一个."

# game/scripts/bar.rpy:4656
translate schinese moon12cont_777be316:

    # c "Huh, this must be an old battlefield then."
    c "嗯,那这里一定是古战场了."

# game/scripts/bar.rpy:4657
translate schinese moon12cont_48f273fb:

    # moon "You think so? That would explain the gun I found a few months ago."
    moon "你这么觉得吗?那这就可以解释我前几个月发现的那把枪了."

# game/scripts/bar.rpy:4658
translate schinese moon12cont_74815e89:

    # c "I wonder what happened here... how these mechs fell in battle."
    c "我想知道这里发生了什么......这些机器是如何在战争中倒下的."

# game/scripts/bar.rpy:4659
translate schinese moon12cont_42724509:

    # moon "Who knows. All I know is, it's badass! And very useful for spare parts. We were even able to build a motorcycle from it!"
    moon "谁知道呢?我只知道它太酷了!而且它作为备用零件很有用.我们甚至可以用它造一辆摩托车!"

# game/scripts/bar.rpy:4660
translate schinese moon12cont_4e24c2ca:

    # c "Really?"
    c "真的吗?"

# game/scripts/bar.rpy:4661
translate schinese moon12cont_19150209:

    # moon "Yeah! Let me show you, it's right over there!"
    moon "是的!让我给你看看,就在这里!"

# game/scripts/bar.rpy:4663
translate schinese moon12cont_ff905f99:

    # moon "There she is, our own {i}Woody{/i} Davidson!"
    moon "这就是她了,我们{i}伍迪{/i}-戴维森!(Woody Davidson可以表示人名,同时woody是木制的意思,davidson是一个摩托车品牌)"

# game/scripts/bar.rpy:4664
translate schinese moon12cont_546761c1:

    # moon "To tell you the truth, it was mostly my friend who made it. I don't know anything about vehicles. I made the wheels though!"
    moon "告诉你真相吧,其实主要还是我朋友做的.我对车一窍不通.不过我做了车轮!"

# game/scripts/bar.rpy:4665
translate schinese moon12cont_82a672a0:

    # c "Does it work?"
    c "它可以运转吗?"

# game/scripts/bar.rpy:4666
translate schinese moon12cont_1ff55387:

    # moon "Heh, kind of. You can walk faster than it drives, but it does sorta work in the forest."
    moon "呵呵,大概吧.你走路都比它快,但是它在森林里确实有点作用."

# game/scripts/bar.rpy:4667
translate schinese moon12cont_9449096c:

    # moon "It's mostly a... downhill type of vehicle."
    moon "它更像是......一种得走下坡的载具."

# game/scripts/bar.rpy:4668
translate schinese moon12cont_cad0d6b8:

    # c "Still, I'm impressed. You're all so talented."
    c "不过我还是很佩服.你们都很有天赋."

# game/scripts/bar.rpy:4669
translate schinese moon12cont_d827b11c:

    # moon "Thank you!"
    moon "谢谢你!"

# game/scripts/bar.rpy:4671
translate schinese moon12cont_b4257010:

    # moon "Would you like to go up now? You'll LOVE the view! It's even better at night, you can actually see the stars here!"
    moon "你现在想上去吗?你会喜欢上这里的景色的!这里晚上就更棒了,你真的可以看到这里的星星!"

# game/scripts/bar.rpy:4672
translate schinese moon12cont_281e042b:

    # c "Yeah, let's go."
    c "好啊,我们走吧."

# game/scripts/bar.rpy:4674
translate schinese moon12cont_2c2d80f1:

    # moon "Here's the lounge! This is where we go to eat together, and some of us like to do our homework here."
    moon "这里是休息室!这是我们一起吃饭的地方,我们有些人喜欢在这里做作业."

# game/scripts/bar.rpy:4675
translate schinese moon12cont_82aaad41:

    # c "Some of this wood feels soggy..."
    c "这里有些木头感觉湿漉漉的......"

# game/scripts/bar.rpy:4676
translate schinese moon12cont_6bcd2465:

    # moon "Oh, yeah, don't step on those parts. Safety first!"
    moon "噢,确实,不要踩到那些部分.安全第一!"

# game/scripts/bar.rpy:4678
translate schinese moon12cont_f05cd3b6:

    # c "Hey, is that Caliber? It's closer than I thought."
    c "嘿,那是楷利泊市吗?这比我想象中的要近很多."

# game/scripts/bar.rpy:4679
translate schinese moon12cont_04814627:

    # moon "You think this view is cool? Just wait..."
    moon "你觉得这个景色很酷吗?等着吧......"

# game/scripts/bar.rpy:4680
translate schinese moon12cont_0ce5eba6:

    # c "I don't think you could impress me more if you tried."
    c "我觉得你没法再给我什么令我印象更深刻的东西了."

# game/scripts/bar.rpy:4681
translate schinese moon12cont_d65cbec8:

    # moon "Who says I was trying to impress you, anyway? Maybe I brought you here to {i}murder you...{/i}"
    moon "谁说我想给你留下印象的?也许我带你来这里是想{i}谋杀你......{/i}"

# game/scripts/bar.rpy:4682
translate schinese moon12cont_25e9635c:

    # c "Right..."
    c "好吧......"

# game/scripts/bar.rpy:4683
translate schinese moon12cont_d1768b31:

    # moon "Hm? Don't think I could overpower you?"
    moon "嗯?你不觉得我可以压倒你吗?"

# game/scripts/bar.rpy:4684
translate schinese moon12cont_97111e83:

    # moon "Heheh, just kidding~ what a drag it would be to dig a hole big enough to fit you, right? I don't even have a shovel..."
    moon "呵呵呵,我只是在开玩笑~得挖多大的坑才能装得下你啊,对吧?我甚至都没有铲子......"

# game/scripts/bar.rpy:4685
translate schinese moon12cont_0c040564:

    # moon "Come on slowpoke, the view is this way!"
    moon "来吧,别慢吞吞的,风景在这边!"

# game/scripts/bar.rpy:4687
translate schinese moon12cont_eecd07f7:

    # moon "And there she is! Caliber city! If you look very close, you can see my dorm from here."
    moon "这就是她了!楷利泊市!如果你看得够仔细的话,你可以从这里看到我的宿舍."

# game/scripts/bar.rpy:4688
translate schinese moon12cont_7017d5e5:

    # c "It's a beautiful view."
    c "风景真美."

# game/scripts/bar.rpy:4689
translate schinese moon12cont_c401a5b0:

    # moon "I know, right? Sometimes I just come out here to think... It's so quiet here, I feel like I almost escape even myself when I'm here."
    moon "我就知道,对吧?有时候我这只是来这里思考一下......这里太安静了,我觉得我在这里的时候甚至都有些灵魂出窍了"

# game/scripts/bar.rpy:4691
translate schinese moon12cont_8c331d2c:

    # moon "Do you have an area where you can get away from everything and just be alone?"
    moon "你有一个可以远离所有人自己一个人呆着的地方吗?"

# game/scripts/bar.rpy:4692
translate schinese moon12cont_187df1ee:

    # c "Me? ...No, I'm always doing something with someone."
    c "我?......没有,我一直都在和别人做事."

# game/scripts/bar.rpy:4693
translate schinese moon12cont_967be8f5:

    # moon "Then this is your official new hideout! You can come here whenever you want."
    moon "那这就是你的藏身处了!你想什么时候来都可以."

# game/scripts/bar.rpy:4694
translate schinese moon12cont_950ef16f:

    # c "Oh, thanks."
    c "噢,谢谢."

# game/scripts/bar.rpy:4695
translate schinese moon12cont_fb007433:

    # moon "You can even help yourself to one of our vintage alcohols. Circa, last year! Would you like some?"
    moon "你甚至可以随便喝我们的精酿酒.大概是,去年产的!你想来点吗?"

# game/scripts/bar.rpy:4696
translate schinese moon12cont_33374f80:

    # c "Sure, I'll have a bit."
    c "当然了,我要来点."

# game/scripts/bar.rpy:4697
translate schinese moon12cont_6a8fe583:

    # moon "Right this way."
    moon "这边走."

# game/scripts/bar.rpy:4699
translate schinese moon12cont_4f68caa8:

    # moon "Good! Just sit tight and I'll serve up something delicious."
    moon "很好!坐下吧,我给你上点美味的东西."

# game/scripts/bar.rpy:4700
translate schinese moon12cont_cc336bad:

    # moon "I just need to unload a few things from my backpack first, one moment..."
    moon "我先腾一下背包先,等一下......"

# game/scripts/bar.rpy:4702
translate schinese moon12cont_3113a05e:

    # c "You know, you're way too overqualified to be my waitress, you should be building the next floor of my hotel."
    c "你知道吗,作为我的服务员完全配不上你的能力,你可以帮我的酒店更上一层楼的."

# game/scripts/bar.rpy:4703
translate schinese moon12cont_84ec5e09:

    # moon "Heheh, thanks. Maybe you should give me a raise if you want me sticking around."
    moon "呵呵呵,谢谢.如果你想让我坚持下去的话你也许可以给我升下职."

# game/scripts/bar.rpy:4705
translate schinese moon12cont_fe9e80b3:

    # moon "Let me wash these glasses, I don't know how long they've been sitting out. Safety first!"
    moon "我洗一下杯子,我不知道它们都在这里放多久了.安全第一!"

# game/scripts/bar.rpy:4706
translate schinese moon12cont_98e9d43c:

    # c "How long did it take for your study group to build this?"
    c "你们的学习小组建造这里花了多久?"

# game/scripts/bar.rpy:4707
translate schinese moon12cont_b27d736f:

    # moon "Oh, well we're still adding on to it each time we come here. Some of us bring chairs... tables... I'm usually the one to bring the alcohol."
    moon "噢,嗯我们每次过来的时候还在往上面添砖加瓦.我们有的带了椅子......桌子......我一般是带酒的那个."

# game/scripts/bar.rpy:4709
translate schinese moon12cont_e9f38b74:

    # moon "Let's see... whiskey... wine... tequila..."
    moon "我看看......威士忌......红酒......龙舌兰......"

# game/scripts/bar.rpy:4710
translate schinese moon12cont_3679ebfa:

    # moon "Ah, this should do!"
    moon "啊,这就是了!"

# game/scripts/bar.rpy:4712
translate schinese moon12cont_9d3fe680:

    # moon "An unopened bottle of wine. I think I brought this a few months ago. What do you say?"
    moon "一瓶没开过的红酒.我想我应该是几个月前带过来的.你怎么说?"

# game/scripts/bar.rpy:4713
translate schinese moon12cont_c02b2861:

    # c "Let's do it."
    c "开吧."

# game/scripts/bar.rpy:4715
translate schinese moon12cont_54ead5a8:

    # moon "This wine was made in Syl'anar, elvish wine just tastes better, don't you agree?"
    moon "这酒是在西尔阿纳尔本地产的,精灵酒的口感更好,你觉得呢?"

# game/scripts/bar.rpy:4716
translate schinese moon12cont_c48dc32f:

    # c "Sure."
    c "确实."

# game/scripts/bar.rpy:4718
translate schinese moon12cont_6eecd4b9:

    # moon "Don't tell me you can't taste the difference?"
    moon "别告诉我你尝不出任何区别?"

# game/scripts/bar.rpy:4720
translate schinese moon12cont_7931c7a5:

    # moon "Well, you're about to find out. Cheers!"
    moon "嗯,你会发现的.干杯!"

# game/scripts/bar.rpy:4721
translate schinese moon12cont_a209eace:

    # c "Cheers."
    c "干杯."

# game/scripts/bar.rpy:4723
translate schinese moon12cont_b17d73fe:

    # "This wine tastes sweeter than you would expect. It's quite good."
    "这种酒尝起来比你想象中的要甜一点.它真的很不错."

# game/scripts/bar.rpy:4725
translate schinese moon12cont_44305b62:

    # moon "Pretty tasty, right?"
    moon "很美味,对吧?"

# game/scripts/bar.rpy:4726
translate schinese moon12cont_e2e3d51a:

    # c "It was nice."
    c "确实很棒."

# game/scripts/bar.rpy:4727
translate schinese moon12cont_ca060614:

    # moon "Let me know if you would like some more. Seriously, we have like seven bottles here."
    moon "如果你还想要的话告诉我.实际上,我们这里有大概7瓶这样的."

# game/scripts/bar.rpy:4728
translate schinese moon12cont_21484ff5:

    # c "So what are you working on back there?"
    c "所以你们在这里研究什么?"

# game/scripts/bar.rpy:4729
translate schinese moon12cont_0f862ef4:

    # moon "Oh that? Come around, let me show you."
    moon "哦那个啊?过来,我给你看看."

# game/scripts/bar.rpy:4731
translate schinese moon12cont_f6cb52b9:

    # moon "This baby has been my personal project lately. It's a mecha-tank virtual visor, the same one soldiers would use to pilot that mecha-tank you saw."
    moon "这个机体是我最近的个人项目.它是机械坦克的虚拟面板,就是和你见过的那种用于给士兵驾驶的机械坦克一样的机械坦克."

# game/scripts/bar.rpy:4732
translate schinese moon12cont_2ef14929:

    # moon "It let the pilot see through the eyes of the mecha-tank, though it only saw colorful blobs of motion. But my goal is to upgrade it once it's in working condition. It needs thermal vision if it's going to be useful."
    moon "它让驾驶员可以通过机械坦克的眼睛看到外面,虽然它只能看到一些运动的彩色斑点.但是我的目标是把它升级一次让它可以到工作状态.它还得加上热视仪."

# game/scripts/bar.rpy:4733
translate schinese moon12cont_81c59f90:

    # c "Planning to refurbish that mecha-tank?"
    c "你打算翻修那个机械坦克?"

# game/scripts/bar.rpy:4734
translate schinese moon12cont_beded6b1:

    # moon "Nope, that thing is too far gone. But with any luck, I may be able to sell this visor for $2,000. People will pay a lot for antiques like this, and I just found it on the ground next to a rock."
    moon "不是,它已经没救了.但是如果我幸运的话,我可以把这个面板以2,000$的价格卖出去.人们会为这种古董出高价的,而我刚好就在一颗石头旁边发现了它."

# game/scripts/bar.rpy:4735
translate schinese moon12cont_373606fc:

    # moon "This has sort of become a hobby lately, and it's nice to have some extra cash around for things. Government support will only pay for the basics, but not a steak."
    moon "最近这都快成我的爱好了,而且来点外快总是好事.政府的资助只够买可以基本上生存下来的东西,买块牛排都不够."

# game/scripts/bar.rpy:4736
translate schinese moon12cont_c3769b9f:

    # c "Is money getting tight for you?"
    c "你手头这么紧吗?"

# game/scripts/bar.rpy:4737
translate schinese moon12cont_fe49a162:

    # moon "Oh, don't worry about me. The government is basically sponsoring my education, so I have a few years left of relative comfort."
    moon "噢,别担心我.政府已经资助了我的学费,所以我在离开舒适区前还有几年."

# game/scripts/bar.rpy:4738
translate schinese moon12cont_5dcf3c03:

    # moon "Besides, you pay me enough and I would be doing this anyway. It's just fun! You can join me if you want."
    moon "而且,只要你给我的钱够,我也会继续下去的.这很有趣!如果你想的话你也可以加入我."

# game/scripts/bar.rpy:4739
translate schinese moon12cont_e25f5b82:

    # c "I wouldn't want to break it..."
    c "我不想搞坏它......"

# game/scripts/bar.rpy:4740
translate schinese moon12cont_048ccbb8:

    # moon "It's easier than it looks, but fair enough heheh."
    moon "没有看起来那么难的,但是你说的有理,呵呵呵."

# game/scripts/bar.rpy:4742
translate schinese moon12cont_658dc6d5:

    # moon "Come on, there's something else I want to show you. Our most recent addition to the treehouse!"
    moon "来吧,我还有别的东西想给你看的.我们最近在树屋上新增的东西!"

# game/scripts/bar.rpy:4744
translate schinese moon12cont_bd82ed7a:

    # moon "We decided we wanted to be able to sleep out here all night when we want to get away from things, so we built a whole house dedicated to it, bridge and everything."
    moon "我们决定如果我们想要远离一切的话,我们应该可以在这里过夜,所以我们直接建了一整个房间,桥梁以及其他配套的一切东西."

# game/scripts/bar.rpy:4746
translate schinese moon12cont_1952db58:

    # moon "Watch your step! It's a bit wobbly, but you can trust it."
    moon "注意脚下!这里有点湿,但是还是很牢固的."

# game/scripts/bar.rpy:4747
translate schinese moon12cont_c4d62fcc:

    # c "When did your study group finish this addition?"
    c "你的学习小组什么时候完成这个附加房间的?"

# game/scripts/bar.rpy:4748
translate schinese moon12cont_de9e217a:

    # moon "A few weeks ago. I haven't even had the opportunity to sleep here yet."
    moon "几周前.我甚至还没在这里睡过."

# game/scripts/bar.rpy:4749
translate schinese moon12cont_51617b73:

    # c "Did you chop the wood yourself?"
    c "木头是你自己砍的吗?"

# game/scripts/bar.rpy:4750
translate schinese moon12cont_3c7474e4:

    # moon "Haha, of course not! Tom is our wood guy. I'm not {i}that{/i} cool..."
    moon "哈哈,当然不是!汤姆是我们的伐木工.我可没{i}那么{/i}酷......"

# game/scripts/bar.rpy:4751
translate schinese moon12cont_fc6a64f8:

    # c "Could have fooled me."
    c "差点就骗到我了."

# game/scripts/bar.rpy:4753
translate schinese moon12cont_27f26da6:

    # moon "And here's the bedroom! Bed-building? We opted for bunkbeds to maximize our space."
    moon "这里就是卧室!床吗?我们选择了双层床,这样就可以最大化利用我们的空间."

# game/scripts/bar.rpy:4754
translate schinese moon12cont_abba0dd0:

    # c "That's a shame..."
    c "真是遗憾......"

# game/scripts/bar.rpy:4755
translate schinese moon12cont_9a00a290:

    # moon "Huh? Why?"
    moon "嗯?为什么?"

# game/scripts/bar.rpy:4756
translate schinese moon12cont_ba10b63a:

    # c "We can't do much together on a bunkbed."
    c "我们在双层床上做不了什么."

# game/scripts/bar.rpy:4757
translate schinese moon12cont_01fc6b9e:

    # moon "Ohh, your mind is in the gutter~ heheh..."
    moon "噢,你的思想可真龌龊~呵呵呵......"

# game/scripts/bar.rpy:4758
translate schinese moon12cont_c3de5871:

    # moon "We wanted to go all out, so we furnished the whole thing! Mostly with wood we found in a junkyard... but it works. And the best part? It was free!"
    moon "我们想要全力以赴,所以我们把整个地方都装修好了!主要是用到了我们在垃圾场找到的木头......但是它很好用.而且最重要的是?它是免费的!"

# game/scripts/bar.rpy:4760
translate schinese moon12cont_d0c52c9f:

    # moon "These beds are very comfortable though! We didn't skimp on them at all. My father always said {i}'your bed is where you spend a third of your life, make sure it's good!{/i}"
    moon "不过这些床很舒服!我们可是花了大手笔在它们上面.我爸爸总是说{i}'你生命中有三分之一的时间都在床上,你得确保它是最好的!'{/i}"

# game/scripts/bar.rpy:4761
translate schinese moon12cont_9287b8cf:

    # moon "But you're right... these beds are too small for two..."
    moon "但是你说得对......这个床对两个人来说太小了......"

# game/scripts/bar.rpy:4763
translate schinese moon12cont_ee6aec64:

    # moon "This couch could be used as a bed though, and it's very sturdy too."
    moon "这个沙发倒是可以当床用,而且也很结实."

# game/scripts/bar.rpy:4765
translate schinese moon12cont_b9473970:

    # moon "But I could think of a few other things we could do on it~"
    moon "但是我想到了一些我们可以在上面做的其他事情~"

# game/scripts/bar.rpy:4766
translate schinese moon12cont_27de75a7:

    # c "You sure the floor won't fall out from under us?"
    c "你确定地板不会让我们掉下去吗?"

# game/scripts/bar.rpy:4767
translate schinese moon12cont_5da28cc2:

    # moon "Heheh, I'm willing to take the risk if you are~"
    moon "呵呵呵,如果你愿意,我也愿意冒这个险~"

# game/scripts/bar.rpy:4768
translate schinese moon12cont_295c4682:

    # moon "At least take off my pants, it's so hot..."
    moon "至少也得把我裤子脱了吧,这里好热......"

# game/scripts/bar.rpy:4770
translate schinese moon12cont_73e97bb2:

    # c "How's that?"
    c "如何?"

# game/scripts/bar.rpy:4771
translate schinese moon12cont_a3ea32fb:

    # moon "Nnnh, there. Much better..."
    moon "嗯,那里.好多了......"

# game/scripts/bar.rpy:4773
translate schinese moon12cont_9e94e821:

    # c "Are you ready to test the structural integrity of this place?"
    c "你准备好测试这个地方的结构完整性了吗?"

# game/scripts/bar.rpy:4774
translate schinese moon12cont_429a941d:

    # moon "Just don't go too rough, heheh. I think we can do it if we're careful..."
    moon "只要别太粗暴,呵呵.我想如果我们小心一点的话,我们可以做......"

# game/scripts/bar.rpy:4776
translate schinese moon12cont_354abadd:

    # moon "Hnnn, NNNNGH~! Ahh...!" with ssshake
    moon "嗯,嗯~!啊......!" with ssshake

# game/scripts/bar.rpy:4778
translate schinese moon12cont_6269d9ce:

    # moon "Ahh, ahhh...! Fuck, that's so good~! MMMH!"
    moon "啊,啊......!操,好舒服~!嗯!"

# game/scripts/bar.rpy:4779
translate schinese moon12cont_9db2d881:

    # moon "Nnnmn, we get to be as loud as we want out here! Hahhh...! One of the many perks of living in a forest, mmmh!"
    moon "嗯,我们可以在这里尽情喊叫!哈啊......!这就是生活再森林里的众多好处之一,嗯!"

# game/scripts/bar.rpy:4780
translate schinese moon12cont_01d0d2cf:

    # moon "Hahhh! Nnngh...! I've always wanted to fuck out here... Hnnn! Mmmmn... I love fucking outside~"
    moon "哈!嗯......!我一直想在这里做爱......嗯!嗯......我喜欢在外面做爱~"

# game/scripts/bar.rpy:4781
translate schinese moon12cont_62ddf461:

    # moon "Aaah, hnnnnm~! You feel so good inside me! Ahhh...! Nnnh, just like that..."
    moon "啊,嗯~!你在我体内的感觉真好!啊......!嗯,就像这样......"

# game/scripts/bar.rpy:4784
translate schinese moon12cont_895132d0:

    # moon "Come on, hnngh! Is that all you got!? FUCK ME!!"
    moon "来吧,嗯!你就这点本事吗!?操我!!"

# game/scripts/bar.rpy:4786
translate schinese moon12cont_464579e2:

    # moon "Aaaagh~! Yes, YES! Just like that! Mmmmh! Don't stop...!" with ssshake
    moon "啊~!是的,是的!就是这样!嗯!不要停......!" with ssshake

# game/scripts/bar.rpy:4787
translate schinese moon12cont_6ac9393d:

    # moon "Mmmn, yeah! I'm so wet for you, [name]...! Mmmn, mmmn~ Yes, yes, yes!"
    moon "嗯,是的!我因为你变得好湿,[name]......!嗯,嗯~是的,是的,是的!"

# game/scripts/bar.rpy:4788
translate schinese moon12cont_7cfa63b7:

    # moon "Ahh, fuck! I should take you to more places with me~ Nnnm...! Hahhh! I love your cock so much, [name]!"
    moon "啊,操!我应该多带你来这里的~嗯......!哈!我好喜欢你的肉棒,[name]!"

# game/scripts/bar.rpy:4792
translate schinese moon12cont_979d9413:

    # moon "*gasp*!" with ssshake
    moon "*惊叹*!" with ssshake

# game/scripts/bar.rpy:4793
translate schinese moon12cont_43c18362:

    # moon "Are you cumming inside!?" with ssshake
    moon "你射进来了!?" with ssshake

# game/scripts/bar.rpy:4794
translate schinese moon12cont_12569d1a:

    # moon "W-Wait, I don't want to get pregnant...! I'm not on the pill...!" with ssshake
    moon "等-等等,我不想要怀孕的......!我没带药......!" with ssshake

# game/scripts/bar.rpy:4796
translate schinese moon12cont_39178755:

    # moon "Hnn, oh well... it felt really good at least..."
    moon "嗯,哦嗯......至少感觉很舒服......"

# game/scripts/bar.rpy:4801
translate schinese moon12cont_d4bb2ad5:

    # moon "Yes, cum on me, cum on me...!"
    moon "是的,射给我,射给我......!"

# game/scripts/bar.rpy:4803
translate schinese moon12cont_a7ec3074:

    # moon "Ahh...! Nnnm..."
    moon "啊......!嗯......"

# game/scripts/bar.rpy:4804
translate schinese moon12cont_1d0fcba4:

    # moon "Enjoy yourself? Heheh, I know I did..."
    moon "享受吗?呵呵呵,我知道我做得好的......"

# game/scripts/bar.rpy:4806
translate schinese moon12cont_608aa24f:

    # moon "Nnnnm... wow, that really tired me out..."
    moon "嗯......哇哦,这真的让我累坏了......"

# game/scripts/bar.rpy:4807
translate schinese moon12cont_575899e1:

    # moon "I'm going to take a little nap... you can rest on one of the beds too, if you'd like..."
    moon "我要休息一下......如果你想的话,你也可以去床上休息一下......"

# game/scripts/bar.rpy:4809
translate schinese moon12cont_411fd4b8:

    # "You and Moon pass out for the next hour..."
    "接下来的几小时,你和文都睡着了......"

# game/scripts/bar.rpy:4810
translate schinese moon12cont_54be8fa7:

    # "...{w}...{w}..."
    "......{w}......{w}......"

# game/scripts/bar.rpy:4812
translate schinese moon12cont_43f8fb53:

    # moon "Our time together was so enjoyable today, [name]! Thanks for joining me on such short notice."
    moon "我们今天在一起的时间真愉快,[name]!谢谢你能这么快加入我."

# game/scripts/bar.rpy:4815
translate schinese moon12cont_f4d4d014:

    # level "Moon loved spending time with you!"
    level "文喜欢和你待在一起!"

# game/scripts/bar.rpy:4823
translate schinese moon_gaming_e36c6700:

    # c "{i}(Hey, what's Kali doing in here? I didn't think she liked alcohol too much.){/i}"
    c "{i}(嘿,凯莉在这里干什么?我以为她不喜欢喝酒来着.){/i}"

# game/scripts/bar.rpy:4825
translate schinese moon_gaming_4515cfbe:

    # c "Hey Kali, what's up?"
    c "嘿凯莉,怎么了?"

# game/scripts/bar.rpy:4826
translate schinese moon_gaming_6f4dec3a:

    # k "Oh hey. Here to join me for a drink?"
    k "哦嘿.来和我喝一杯吗?"

# game/scripts/bar.rpy:4827
translate schinese moon_gaming_7bc3d205:

    # c "I might. When did you start drinking?"
    c "可能吧.你什么时候开始喝酒的?"

# game/scripts/bar.rpy:4829
translate schinese moon_gaming_facc850e:

    # k "Oh, I was just bored, didn't know what else to do... so I came here."
    k "噢,我只是有点无聊,不知道要干什么......所以我就来这里了."

# game/scripts/bar.rpy:4830
translate schinese moon_gaming_74e3743e:

    # k "It might be hard to believe, but it feels so much more peaceful here than in my room with Lucy talking my ear off..."
    k "你可能不信,但我觉得这里比在我的房间里听露西在我耳边叽叽喳喳平静得多......"

# game/scripts/bar.rpy:4831
translate schinese moon_gaming_551499ac:

    # c "It's that bad, huh?"
    c "有这么糟,嗯?"

# game/scripts/bar.rpy:4832
translate schinese moon_gaming_2538dfed:

    # k "Yes, it is. I just wish she would grow up already..."
    k "是的,就是这么糟.我只是希望她可以长大......"

# game/scripts/bar.rpy:4833
translate schinese moon_gaming_9954e242:

    # c "Did something happen?"
    c "怎么了?"

# game/scripts/bar.rpy:4834
translate schinese moon_gaming_36eaef8f:

    # k "*sigh*... I was just playing games on my computer and she kept bugging me about something really stupid, and I was already getting pissed from my team in game. I just decided I had enough and came here..."
    k "*叹气*......我在玩电脑游戏,她却一直在跟我唠叨那些愚蠢的事情,而当时我已经被我的傻逼队友惹恼了.我受够了,所以决定来这里......"

# game/scripts/bar.rpy:4835
translate schinese moon_gaming_39211605:

    # k "It doesn't taste too bad, actually... I was expecting fruity gasoline, but this is tasty."
    k "实际上,尝起来不是很坏......我以为是会像是水果味的汽油,但是尝起来还不错."

# game/scripts/bar.rpy:4836
translate schinese moon_gaming_fb58f502:

    # c "I'm glad you like it."
    c "我很高兴你喜欢它."

# game/scripts/bar.rpy:4837
translate schinese moon_gaming_aae5fd4e:

    # k "*sigh*, but yeah... I wish Lucy and I could get along better... I wish we had things in common, but it's almost like we were raised by completely different parents..."
    k "*叹气*,但是,是的......我希望露西可以和我好好相处......我希望我们能有共同点,但是我们两个就像是被完全不同的父母抚养的一样......"

# game/scripts/bar.rpy:4839
translate schinese moon_gaming_2006bb08:

    # moon "Hey, did you say you were playing games on your computer? You must be Kali! I've heard about you."
    moon "嘿,你说你在电脑上玩游戏?你一定是凯莉吧!?我听说过你."

# game/scripts/bar.rpy:4840
translate schinese moon_gaming_bb7d648e:

    # k "Huh? Who are you?"
    k "嗯?你是谁?"

# game/scripts/bar.rpy:4841
translate schinese moon_gaming_58b74007:

    # moon "I'm Ji-yeon Moon! But you can call me Moon. [name] told me about you!"
    moon "我是文智妍!但是你可以叫我文.[name]告诉过我你的事情!"

# game/scripts/bar.rpy:4843
translate schinese moon_gaming_1fd0d615:

    # k "Is that so?"
    k "真的吗?"

# game/scripts/bar.rpy:4844
translate schinese moon_gaming_8f6b55d2:

    # moon "Don't worry, nothing bad. It turns out we have something in common."
    moon "别担心,没什么坏事.不过我们好像有点共同点."

# game/scripts/bar.rpy:4846
translate schinese moon_gaming_22cdf01f:

    # k "Oh, really?"
    k "噢,真的吗?"

# game/scripts/bar.rpy:4847
translate schinese moon_gaming_b778ffcb:

    # moon "Yep, I play games as well. Sometimes I join tourneys. Just local ones."
    moon "是的,我也玩游戏.有时候我还会参加比赛.不过只是本地的."

# game/scripts/bar.rpy:4848
translate schinese moon_gaming_2a7320e3:

    # k "Woah, really? You don't seem the type."
    k "喔,真的吗?你看起来不像是那种人."

# game/scripts/bar.rpy:4849
translate schinese moon_gaming_f9d7dd3b:

    # moon "Just because I don't wear glasses like you? I can play games! And I'm very competitive, too!"
    moon "就因为我不像你那样戴眼镜吗?我会打游戏的!而且我也挺有竞争力的!"

# game/scripts/bar.rpy:4850
translate schinese moon_gaming_9e037934:

    # k "I've never joined a tournament before. What do you play?"
    k "我之前从来没参加过比赛.你玩的什么?"

# game/scripts/bar.rpy:4851
translate schinese moon_gaming_819d1b36:

    # moon "Just about everything except simulations. I love shooting and racing games."
    moon "除了模拟游戏什么都玩.我喜欢射击和赛车游戏."

# game/scripts/bar.rpy:4852
translate schinese moon_gaming_af0f4e13:

    # k "Nice, me too. You work here, right? How come I've never seen you around?"
    k "太好了,我也是.你在这里工作,对吧?我怎么从来没见过你?"

# game/scripts/bar.rpy:4853
translate schinese moon_gaming_f00a3531:

    # moon "I don't live here, I'm just a waitress. But I'm pretty much always in the bar."
    moon "我不住在这里,我只是一个服务员.但是我大部分时间都在酒吧里."

# game/scripts/bar.rpy:4854
translate schinese moon_gaming_dcb88c48:

    # k "Oh, that explains it."
    k "噢,那就可以解释了."

# game/scripts/bar.rpy:4855
translate schinese moon_gaming_2b683dfa:

    # k "I don't have anything else to do, and I'd rather not drink myself to sleep... Do you want to play something? We have a console in the lobby."
    k "我现在无事可做,但是我也不想喝得酩酊大醉......你想玩点什么吗?我们大厅里有个游戏机."

# game/scripts/bar.rpy:4856
translate schinese moon_gaming_05950cb6:

    # moon "*gasp*, really? I would love to! I've been dying for real competition, so you better be good!"
    moon "*惊叹*,真的吗?我愿意!我一直都想来一次真正的比赛,所以你最好有那个本事!"

# game/scripts/bar.rpy:4859
translate schinese moon_gaming_59e72d2c:

    # k "Are you sure you don't want to play, [name]?"
    k "你确定你不想玩吗,[name]?"

# game/scripts/bar.rpy:4860
translate schinese moon_gaming_4787f125:

    # c "I'll just watch for now."
    c "我现在就看看吧."

# game/scripts/bar.rpy:4861
translate schinese moon_gaming_64782318:

    # moon "{size=-10}{i}He's too scared to lose to a couple of girls, heheh.{/i}{/size}"
    moon "{size=-10}{i}他太害怕了,怕输给几个女孩,呵呵呵.{/i}{/size}"

# game/scripts/bar.rpy:4862
translate schinese moon_gaming_dd51a8e6:

    # k "How about this? Have you played it before?"
    k "这个怎么样?你之前玩过吗?"

# game/scripts/bar.rpy:4863
translate schinese moon_gaming_fe249c99:

    # moon "Of course! I play it all the time."
    moon "当然了!我一直玩这个的."

# game/scripts/bar.rpy:4864
translate schinese moon_gaming_c3c0caef:

    # k "You should be good then. No excuses if you lose."
    k "那你应该会玩.输了可别找借口."

# game/scripts/bar.rpy:4866
translate schinese moon_gaming_38aaa8bf:

    # em "Hey guys... what's going on?"
    em "嘿伙计们......你们在干什么?"

# game/scripts/bar.rpy:4867
translate schinese moon_gaming_daa3febc:

    # k "Hey Emma, want to join?"
    k "嘿艾玛,想加入吗?"

# game/scripts/bar.rpy:4869
translate schinese moon_gaming_b1115d58:

    # em "Um, maybe... What is this?"
    em "嗯,大概吧......这是什么?"

# game/scripts/bar.rpy:4870
translate schinese moon_gaming_53a06ad9:

    # k "It's a racing game, have you ever played it?"
    k "这是个赛车游戏,你玩过吗?"

# game/scripts/bar.rpy:4871
translate schinese moon_gaming_8c7a854c:

    # em "No, but it looks interesting."
    em "没有,但是看起来很有趣."

# game/scripts/bar.rpy:4872
translate schinese moon_gaming_be046e68:

    # em "I think I'll watch you two for now. I've never seen this before, so I would just suck anyway..."
    em "我想我现在还是看着你们两个玩吧.我之前从来没玩过这个,所以我只会玩的很烂......"

# game/scripts/bar.rpy:4873
translate schinese moon_gaming_2f5fd8e2:

    # moon "That's fair. I usually beat my friends so bad they hate me for it. So it wouldn't be good for our relationship..."
    moon "那确实.我通常会把我朋友打得很惨,所以他们讨厌我.这样对我们的关系没啥好处的......"

# game/scripts/bar.rpy:4875
translate schinese moon_gaming_1358545b:

    # game "3..."
    game "3......"

# game/scripts/bar.rpy:4876
translate schinese moon_gaming_547b9630:

    # k "Those are some big words."
    k "说大话呢."

# game/scripts/bar.rpy:4877
translate schinese moon_gaming_c6a10de1:

    # game "2..."
    game "2......"

# game/scripts/bar.rpy:4878
translate schinese moon_gaming_9e7b1dc5:

    # moon "Hmph, they're perfectly proportional words!"
    moon "哼,我说的可全是真的!"

# game/scripts/bar.rpy:4879
translate schinese moon_gaming_58f3a190:

    # game "1..."
    game "1......"

# game/scripts/bar.rpy:4880
translate schinese moon_gaming_69fff9cd:

    # k "We'll see about that."
    k "走着瞧."

# game/scripts/bar.rpy:4881
translate schinese moon_gaming_52896c8a:

    # game "GO!!!"
    game "{b}开始!!!{/b}"

# game/scripts/bar.rpy:4882
translate schinese moon_gaming_9394af7e:

    # k "Damn it, I didn't boost...!"
    k "妈的,起步喷没喷出来......!"

# game/scripts/bar.rpy:4883
translate schinese moon_gaming_be2f4d6a:

    # moon "Hah! You suck!"
    moon "哈哈!你可真烂!"

# game/scripts/bar.rpy:4884
translate schinese moon_gaming_186552ee:

    # k "That's fine, it just gives me a chance to hit you with a shell."
    k "没事,这刚好给了我用个龟壳(马里奥赛车的一种道具)打你的机会."

# game/scripts/bar.rpy:4885
translate schinese moon_gaming_aff874e9:

    # moon "Not with this guarding my back!"
    moon "在我的防守下可不行!"

# game/scripts/bar.rpy:4886
translate schinese moon_gaming_aa194c08:

    # em "Are you guys allowed to kill each other?"
    em "你们是想杀了对方吗?"

# game/scripts/bar.rpy:4887
translate schinese moon_gaming_e1f0178b:

    # k "No, but we can hit each other's cars. You never die in this."
    k "没有,但是我们可以撞到对方的车.在这里面是不会死的."

# game/scripts/bar.rpy:4889
translate schinese moon_gaming_cdd9e689:

    # k "Got you!" with ssshake
    k "抓到了!" with ssshake

# game/scripts/bar.rpy:4890
translate schinese moon_gaming_390c27fc:

    # moon "Rrrgh!"
    moon "呃!"

# game/scripts/bar.rpy:4891
translate schinese moon_gaming_97a7ec46:

    # k "First place!"
    k "第一!"

# game/scripts/bar.rpy:4892
translate schinese moon_gaming_222ff8ec:

    # em "One of you is a princess and one of you is a dragon, I don't get how that's fair..."
    em "你们一个是公主,另一个是龙,这怎么是公平游戏......(应该是马里奥赛车)"

# game/scripts/bar.rpy:4893
translate schinese moon_gaming_b1599c4e:

    # moon "YEAH! HAD TO PICK THE OVER POWERED CHARACTER, HUH!?"
    moon "是的!你用了那些超模角色了,嗯!?"

# game/scripts/bar.rpy:4894
translate schinese moon_gaming_ccdeffb1:

    # k "Here come the excuses..."
    k "别找借口......"

# game/scripts/bar.rpy:4895
translate schinese moon_gaming_5ea1e5ec:

    # moon "Not excuses... the game just isn't fair sometimes. It's built that way!"
    moon "这可不是借口......游戏有时候就是不公平的.它本来就是建立在这上面的!"

# game/scripts/bar.rpy:4896
translate schinese moon_gaming_c30a200f:

    # moon "You'll see, I'll get a homing shell and you'll eat my dirt! Then you'll see who is making excuses!"
    moon "走着瞧,我会拿个追踪龟壳的,你就等着在后面吃灰吧!然后你就知道是谁在找借口了!"

# game/scripts/bar.rpy:4897
translate schinese moon_gaming_569737e4:

    # em "Who are they? Are you playing with real people?"
    em "他们是谁?你们在和真人玩家玩吗?"

# game/scripts/bar.rpy:4898
translate schinese moon_gaming_42acf3c2:

    # k "Of course, it's an online game."
    k "当然了,这是在线游戏."

# game/scripts/bar.rpy:4899
translate schinese moon_gaming_7be67771:

    # em "Oh, that's cool. You two are good then, I haven't seen either of you go below 5th place yet."
    em "噢,那太酷了.你们两个都很好,我都没看过你们两个谁低过第五名过."

# game/scripts/bar.rpy:4900
translate schinese moon_gaming_38113359:

    # k "Some of them are probably kids, so it's not a huge achievement..."
    k "他们有些可能只是小孩,这不是什么很大的成就......"

# game/scripts/bar.rpy:4901
translate schinese moon_gaming_afa7dd57:

    # moon "{b}{u}DIEEEEE!!!{/u}{/b}"
    moon "{b}{u}死吧!!!{/u}{/b}"

# game/scripts/bar.rpy:4903
translate schinese moon_gaming_aadfa9cb:

    # k "What? *gasp*! NO! You bitch..." with ssshake
    k "什么?*惊叹*!不!你个婊子......" with ssshake

# game/scripts/bar.rpy:4904
translate schinese moon_gaming_3d50fc81:

    # moon "Muahahahah! Get owned!"
    moon "哇哈哈哈哈!优势在我!"

# game/scripts/bar.rpy:4905
translate schinese moon_gaming_2ed7edbd:

    # em "Heheh, this looks so fun!"
    em "呵呵呵,看起来很好玩!"

# game/scripts/bar.rpy:4906
translate schinese moon_gaming_e5899321:

    # k "Dammit! Dammit, no! Give me a boost or something! Fuck these coins!"
    k "妈的!妈的,不要!给我个加速什么的!妈的怎么都是金币!"

# game/scripts/bar.rpy:4907
translate schinese moon_gaming_62ff0c20:

    # moon "You're going to lose~"
    moon "你要输咯~"

# game/scripts/bar.rpy:4908
translate schinese moon_gaming_c1c74a4e:

    # em "Go faster, Kali! What are you doing?"
    em "快点,凯莉!你在干什么?"

# game/scripts/bar.rpy:4909
translate schinese moon_gaming_f70ab331:

    # k "I can't hold the button down any harder than this!"
    k "我没法把按钮按的更大力了!"

# game/scripts/bar.rpy:4910
translate schinese moon_gaming_d06bd631:

    # k "Ergh, I'm getting so unlucky!"
    k "呃,我好倒霉!"

# game/scripts/bar.rpy:4911
translate schinese moon_gaming_f13e82df:

    # moon "Now who is making excuses? Hah!"
    moon "现在是谁在找借口?哈哈!"

# game/scripts/bar.rpy:4912
translate schinese moon_gaming_1f3a0a72:

    # game "PLAYER 2 WINS."
    game "玩家2获胜."

# game/scripts/bar.rpy:4914
translate schinese moon_gaming_aa22acdd:

    # moon "YES! I win!"
    moon "是的!我赢了!"

# game/scripts/bar.rpy:4915
translate schinese moon_gaming_5adfc593:

    # k "All luck, no skill..."
    k "全是运气,毫无技巧......"

# game/scripts/bar.rpy:4916
translate schinese moon_gaming_ce676473:

    # moon "{u}Pure{/u} skill."
    moon "{u}完完全全{/u}是技巧噢."

# game/scripts/bar.rpy:4917
translate schinese moon_gaming_7328a75c:

    # em "This looks fun, can I join you guys next round?"
    em "看起来很好玩,下一局我可以加入你们吗?"

# game/scripts/bar.rpy:4919
translate schinese moon_gaming_2584aeca:

    # k "Sure, grab a controller."
    k "当然了,去拿个控制器吧."

# game/scripts/bar.rpy:4920
translate schinese moon_gaming_d41a7f1a:

    # moon "As long as you don't mind losing to me..."
    moon "只要你不介意会输给我......"

# game/scripts/bar.rpy:4921
translate schinese moon_gaming_a24064f2:

    # em "Don't worry, I catch on quick. You'll be losing soon enough."
    em "别担心,我学得很快的.你很快就会输的."

# game/scripts/bar.rpy:4922
translate schinese moon_gaming_65bf1165:

    # moon "Oooh, she's confident."
    moon "噢,她很自信嘛."

# game/scripts/bar.rpy:4924
translate schinese moon_gaming_57f0afc1:

    # "Kali and Moon give Emma a quick explaination of the controls..."
    "凯莉和文给艾玛快速地解释了一下控制方法......"

# game/scripts/bar.rpy:4926
translate schinese moon_gaming_1358545b_1:

    # game "3..."
    game "3......"

# game/scripts/bar.rpy:4927
translate schinese moon_gaming_8bbf2a8f:

    # k "You're going down this time, Moon!"
    k "这次你可要倒霉了,文!"

# game/scripts/bar.rpy:4928
translate schinese moon_gaming_c6a10de1_1:

    # game "2..."
    game "2......"

# game/scripts/bar.rpy:4929
translate schinese moon_gaming_90243c9c:

    # moon "Not even in your dreams!"
    moon "做梦去吧!"

# game/scripts/bar.rpy:4930
translate schinese moon_gaming_58f3a190_1:

    # game "1..."
    game "1......"

# game/scripts/bar.rpy:4931
translate schinese moon_gaming_99b2070e:

    # em "Okay, wait, um... how do I boost again?"
    em "好了,等等,嗯......我怎么喷射来着?"

# game/scripts/bar.rpy:4932
translate schinese moon_gaming_52896c8a_1:

    # game "GO!!!"
    game "{b}开始!!!{/b}"

# game/scripts/bar.rpy:4933
translate schinese moon_gaming_add0f254:

    # em "Ah~!"
    em "啊~!"

# game/scripts/bar.rpy:4934
translate schinese moon_gaming_7ee673f4:

    # em "Wait, I'm in 7th! That's not too bad!"
    em "等等,我是第七名!这太糟糕了!"

# game/scripts/bar.rpy:4935
translate schinese moon_gaming_72e39304:

    # moon "You started in 7th, heh."
    moon "你就是在第七名的位置开始的,呵呵."

# game/scripts/bar.rpy:4936
translate schinese moon_gaming_32956232:

    # em "That's not fair, why did you start in 1st?"
    em "这不公平,为什么你就在第一名的位置?"

# game/scripts/bar.rpy:4937
translate schinese moon_gaming_44eb49a7:

    # moon "Don't worry, just pick up an item. You'll get something good."
    moon "别担心,去拿个道具.你会拿到点好东西的."

# game/scripts/bar.rpy:4939
translate schinese moon_gaming_480e2f06:

    # em "Woah! I turned into a bullet! I'm going so fast!"
    em "喔!我变成了一颗子弹!我好快!"

# game/scripts/bar.rpy:4940
translate schinese moon_gaming_5ceed862:

    # k "Hey! Don't hit me!"
    k "嘿!别撞我!"

# game/scripts/bar.rpy:4941
translate schinese moon_gaming_5463ff89:

    # em "Heheh, I didn't try."
    em "呵呵呵,我不是故意的."

# game/scripts/bar.rpy:4942
translate schinese moon_gaming_044281a4:

    # moon "Uh oh, you're getting beaten by the noob, Kali?"
    moon "啊噢,你被菜鸟打败了,凯莉?"

# game/scripts/bar.rpy:4943
translate schinese moon_gaming_b435fc73:

    # k "Shut up..."
    k "闭嘴......"

# game/scripts/bar.rpy:4944
translate schinese moon_gaming_6af9218c:

    # moon "New rules! Every time you get hit by a shell, you take a shot!"
    moon "新规则!每次你被龟壳撞到了,你就要喝酒!"

# game/scripts/bar.rpy:4945
translate schinese moon_gaming_08811cd5:

    # k "You can't make up new rules in the middle of the game."
    k "你不能在游戏还在进行的时候添加新规则."

# game/scripts/bar.rpy:4946
translate schinese moon_gaming_01b88fb8:

    # moon "Why not? It would be fun! Let's make things interesting..."
    moon "为什么不行?那会很好玩的!让我们把事情变得更有趣点吧......"

# game/scripts/bar.rpy:4947
translate schinese moon_gaming_7222b401:

    # k "How about you eat my homing shell instead!"
    k "那要不要来试试我的自动追踪龟壳!"

# game/scripts/bar.rpy:4949
translate schinese moon_gaming_56b52457:

    # moon "NOO!!" with ssshake
    moon "不!!" with ssshake

# game/scripts/bar.rpy:4950
translate schinese moon_gaming_ba87696a:

    # k "1st place!"
    k "第一名!"

# game/scripts/bar.rpy:4951
translate schinese moon_gaming_eb019795:

    # moon "4th? 5th!? 6th!?"
    moon "第四名?第五名!?第六名!?"

# game/scripts/bar.rpy:4952
translate schinese moon_gaming_f12bf5a1:

    # em "Bye loser!"
    em "再见失败者!"

# game/scripts/bar.rpy:4953
translate schinese moon_gaming_21d0cb29:

    # moon "Aaagh!"
    moon "啊!"

# game/scripts/bar.rpy:4954
translate schinese moon_gaming_c932c539:

    # moon "You'll regret that, Kali!"
    moon "你会后悔的,凯莉!"

# game/scripts/bar.rpy:4955
translate schinese moon_gaming_0766179c:

    # k "Good luck, we're on the last lap."
    k "祝你好运,我们已经是最后一圈了."

# game/scripts/bar.rpy:4956
translate schinese moon_gaming_c964fac0:

    # em "Why aren't any of my shells hitting you!?"
    em "为什么我的龟壳没有打到你!?"

# game/scripts/bar.rpy:4957
translate schinese moon_gaming_b26f4fc5:

    # k "Get better aim."
    k "瞄准点."

# game/scripts/bar.rpy:4958
translate schinese moon_gaming_e7708c07:

    # em "That's not helpful!"
    em "那没用!"

# game/scripts/bar.rpy:4959
translate schinese moon_gaming_17446f8f:

    # moon "I'm in 4th again, I can still bring this back!"
    moon "我又到第四了.我还可以回来!"

# game/scripts/bar.rpy:4960
translate schinese moon_gaming_4ff11e64:

    # moon "Nngh, move losers! I need my vengeance!"
    moon "嗯,走开失败者!我要复仇!"

# game/scripts/bar.rpy:4961
translate schinese moon_gaming_4ada5e49:

    # em "Hey, is that you behind me?"
    em "嘿,你在我后面吗?"

# game/scripts/bar.rpy:4962
translate schinese moon_gaming_a6489e48:

    # moon "Help me kill Kali and I'll let you win this round!"
    moon "帮我杀了凯莉,我就让你赢这一局!"

# game/scripts/bar.rpy:4963
translate schinese moon_gaming_1ba6a103:

    # em "Okay, how?"
    em "好吧,怎么搞?"

# game/scripts/bar.rpy:4964
translate schinese moon_gaming_26b4a807:

    # k "Hey, you can't team in this!"
    k "嘿,你们不能在这样组队!"

# game/scripts/bar.rpy:4965
translate schinese moon_gaming_2643458e:

    # moon "Just throw shit at her!"
    moon "给她随便丢点东西!"

# game/scripts/bar.rpy:4966
translate schinese moon_gaming_853548ed:

    # em "Hmmm, okay. Am I hitting her?"
    em "嗯,好吧.我打到了她吗?"

# game/scripts/bar.rpy:4967
translate schinese moon_gaming_ce08b45e:

    # moon "AAGH! That was a banana! You hit me!" with ssshake
    moon "啊!那是个香蕉皮!你打到我了!" with ssshake

# game/scripts/bar.rpy:4968
translate schinese moon_gaming_e1fbdae1:

    # em "Eh, wasn't much of a loss."
    em "呃,这也没什么损失."

# game/scripts/bar.rpy:4969
translate schinese moon_gaming_0b34eaca:

    # moon "No, no! I'm going to lose now!"
    moon "不,不!我现在要输了!"

# game/scripts/bar.rpy:4970
translate schinese moon_gaming_e46950a4:

    # game "PLAYER 1 WINS!"
    game "玩家1获胜!"

# game/scripts/bar.rpy:4972
translate schinese moon_gaming_43604cc5:

    # moon "That was shameful..."
    moon "真遗憾......"

# game/scripts/bar.rpy:4973
translate schinese moon_gaming_6acee86b:

    # em "I got 3rd! That's pretty good, right?"
    em "我第三!很不错,对吧?"

# game/scripts/bar.rpy:4974
translate schinese moon_gaming_426cce4b:

    # moon "Yeah... better than me..."
    moon "是的......比我好......"

# game/scripts/bar.rpy:4975
translate schinese moon_gaming_ec1cb1b3:

    # k "Best two out of three?"
    k "三局两胜?"

# game/scripts/bar.rpy:4977
translate schinese moon_gaming_c6de263c:

    # moon "You're on! Start it already!"
    moon "来!开!"

# game/scripts/bar.rpy:4978
translate schinese moon_gaming_bae9ea80:

    # k "Alright, you ready Emma?"
    k "好了,你准备好了吗艾玛?"

# game/scripts/bar.rpy:4979
translate schinese moon_gaming_019b4c4c:

    # em "Yeah! Let's play."
    em "是的!我们开始吧."

# game/scripts/bar.rpy:4980
translate schinese moon_gaming_18fba6a4:

    # game "3... 2... 1..."
    game "3......2......1......"

# game/scripts/bar.rpy:4981
translate schinese moon_gaming_38a93156:

    # game "GO!!!" with ssshake
    game "{b}开始!!!{/b}" with ssshake

# game/scripts/bar.rpy:4982
translate schinese moon_gaming_85f4da57:

    # em "*gasp*! Hey, I did the thing! I boosted!"
    em "*惊叹*!嘿,我做到了!我喷射了!"

# game/scripts/bar.rpy:4983
translate schinese moon_gaming_b5684f50:

    # moon "Good job, you catch on quickly..."
    moon "干得好,你学得很快......"

# game/scripts/bar.rpy:4985
translate schinese moon_gaming_ec6a9d2c:

    # em "Ah! Hey! Who just hit me!?" with ssshake
    em "啊!嘿!谁打我!?" with ssshake

# game/scripts/bar.rpy:4986
translate schinese moon_gaming_5b705ee4:

    # moon "Not me."
    moon "不是我."

# game/scripts/bar.rpy:4987
translate schinese moon_gaming_5a924c52:

    # k "I'm ahead of you, couldn't have been me."
    k "我在你前面,不可能是我."

# game/scripts/bar.rpy:4988
translate schinese moon_gaming_b5ba248b:

    # em "Rrgh, it was some loser named elfsimp69. What did I do to them!?"
    em "呃,是某个叫'精灵舔狗69'(elfsimp69)的人.我对他们做了什么!?"

# game/scripts/bar.rpy:4989
translate schinese moon_gaming_6a6f7e3a:

    # moon "They probably just want your spot, heh."
    moon "他们可能只是想要你的位置,呵呵."

# game/scripts/bar.rpy:4990
translate schinese moon_gaming_6d3e6692:

    # em "Eat fireball, nerd!"
    em "食我大火球,菜鸡!"

# game/scripts/bar.rpy:4992
translate schinese moon_gaming_1a8bc263:

    # em "*gasp*! I hit them!"
    em "*惊叹*!我打到他们了!"

# game/scripts/bar.rpy:4993
translate schinese moon_gaming_79f3b471:

    # moon "Grats, but..."
    moon "恭喜你,但是......"

# game/scripts/bar.rpy:4995
translate schinese moon_gaming_31f162dd:

    # em "HEYYY!" with ssshake
    em "嘿!" with ssshake

# game/scripts/bar.rpy:4996
translate schinese moon_gaming_706874cc:

    # moon "Sorry! I'm taking my revenge on Kali! You were just in the way..."
    moon "对不起!我要向凯莉复仇!你挡我路了......"

# game/scripts/bar.rpy:4997
translate schinese moon_gaming_e30f3c76:

    # em "I fought hard for that spot, you just made an enemy for life!"
    em "我为了这个位置奋斗了多久,你刚刚结下了一个终生的敌人!"

# game/scripts/bar.rpy:4998
translate schinese moon_gaming_1649c35f:

    # moon "Gonna have to catch me first~"
    moon "先抓到我再说~"

# game/scripts/bar.rpy:4999
translate schinese moon_gaming_dc1525cf:

    # em "Rrrgh...!"
    em "呃......!"

# game/scripts/bar.rpy:5000
translate schinese moon_gaming_3f3ba847:

    # k "Is that you throwing shit at me, Moon?"
    k "是你在给我丢东西吗,文?"

# game/scripts/bar.rpy:5001
translate schinese moon_gaming_35e9498f:

    # moon "Yep, I'm in 2nd now."
    moon "是的,我现在第二了."

# game/scripts/bar.rpy:5002
translate schinese moon_gaming_316ca057:

    # k "How did you catch up so fast?"
    k "你怎么这么快赶上来的?"

# game/scripts/bar.rpy:5003
translate schinese moon_gaming_047a4963:

    # moon "Guess I got lucky with boosts."
    moon "大概是因为我运气好吃了几个喷射吧."

# game/scripts/bar.rpy:5004
translate schinese moon_gaming_875d9782:

    # em "Let's kill her, Kali!"
    em "让我杀了她,凯莉!"

# game/scripts/bar.rpy:5005
translate schinese moon_gaming_b3af4c51:

    # k "Please, be my guest."
    k "请吧."

# game/scripts/bar.rpy:5006
translate schinese moon_gaming_4a8e0e1c:

    # moon "Tsk, you sure are keeping your defenses up, Kali."
    moon "啧,你真的一直在防着啊,凯莉."

# game/scripts/bar.rpy:5007
translate schinese moon_gaming_0017e8dd:

    # k "I'm not letting you take my spot..."
    k "我不会让你夺走我的位置的......"

# game/scripts/bar.rpy:5008
translate schinese moon_gaming_0c10a859:

    # moon "We'll see about that! I just need a lucky item and you'll lose!"
    moon "走着瞧!我只需要运气好点拿个道具你就输了!"

# game/scripts/bar.rpy:5009
translate schinese moon_gaming_573a1f59:

    # em "Don't forget about me, I still haven't forgiven you!"
    em "别忘了我,我还没原谅你!"

# game/scripts/bar.rpy:5011
translate schinese moon_gaming_bc54c8f0:

    # em "Ah! Why am I small!? Where did my items go!?" with ssshake
    em "啊!为什么我变小了!?我的道具呢!?" with ssshake

# game/scripts/bar.rpy:5012
translate schinese moon_gaming_3af74c76:

    # k "Ergh, great... we all lost our items."
    k "呃,好吧......我们的道具都没了."

# game/scripts/bar.rpy:5013
translate schinese moon_gaming_ac18e2f9:

    # moon "Aaand we're slow..."
    moon "而且我们都变慢了......"

# game/scripts/bar.rpy:5014
translate schinese moon_gaming_af9e0a50:

    # em "..."
    em "......"

# game/scripts/bar.rpy:5015
translate schinese moon_gaming_82091822:

    # k "..."
    k "......"

# game/scripts/bar.rpy:5016
translate schinese moon_gaming_4984c9d6:

    # moon "..."
    moon "......"

# game/scripts/bar.rpy:5018
translate schinese moon_gaming_964126bb:

    # moon "Finally! And it looks like your defenses are gone, Kali!"
    moon "终于!看起来你的防御失效了,凯莉!"

# game/scripts/bar.rpy:5019
translate schinese moon_gaming_a1a4920d:

    # k "Shit, you're right."
    k "操了,你说得对."

# game/scripts/bar.rpy:5020
translate schinese moon_gaming_a157fdc3:

    # em "Hey! I'm in third! I think I see you, Moon."
    em "嘿!我第三了!我想我看见你了,文."

# game/scripts/bar.rpy:5021
translate schinese moon_gaming_504acd39:

    # moon "Don't fuck me up, noob! I'm taking 1st!"
    moon "别搞我,菜鸟!我要拿第一!"

# game/scripts/bar.rpy:5022
translate schinese moon_gaming_216c5af6:

    # em "What if I want it?"
    em "如果我要呢?"

# game/scripts/bar.rpy:5023
translate schinese moon_gaming_37bb860e:

    # moon "THEN YOU'LL HAVE TO TEAR IT FROM MY COLD DEAD HANDS!"
    moon "{b}那就从我的尸体上踏过去吧!{/b}"

# game/scripts/bar.rpy:5024
translate schinese moon_gaming_5c9a5f6d:

    # moon "Try to dodge these fireballs, Kali!"
    moon "试着躲开这些火球吧,凯莉!"

# game/scripts/bar.rpy:5025
translate schinese moon_gaming_88e054e1:

    # k "Aagh! FUCK! NO!!!" with ssshake
    k "啊!操!不!!!" with ssshake

# game/scripts/bar.rpy:5026
translate schinese moon_gaming_54628e74:

    # moon "1st! 1st!"
    moon "第一!第一!"

# game/scripts/bar.rpy:5027
translate schinese moon_gaming_560ecad1:

    # em "Oh, see ya Kali."
    em "噢,再见凯莉."

# game/scripts/bar.rpy:5028
translate schinese moon_gaming_5c012d57:

    # k "Rrrgh! Fuck..."
    k "呃!操......"

# game/scripts/bar.rpy:5029
translate schinese moon_gaming_ce1c2648:

    # moon "I'm gonna win!"
    moon "我要赢了!"

# game/scripts/bar.rpy:5030
translate schinese moon_gaming_83037c61:

    # em "Not if I have anything to do with it!"
    em "那得先问问我!"

# game/scripts/bar.rpy:5031
translate schinese moon_gaming_122d1d4b:

    # moon "Oh yeah? What do you have?"
    moon "噢真的吗?你有什么?"

# game/scripts/bar.rpy:5032
translate schinese moon_gaming_f5239a04:

    # em "I have... two coins!"
    em "我有......两枚金币!"

# game/scripts/bar.rpy:5033
translate schinese moon_gaming_747a3830:

    # em "Wha- hey! Why can't I throw them at you? Am I eating them!? That doesn't help!"
    em "什-嘿!为什么我不能把它丢向你?我是在吃它们吗!?这完全没用啊!"

# game/scripts/bar.rpy:5034
translate schinese moon_gaming_469a88b8:

    # moon "Hahaha!"
    moon "哈哈哈!"

# game/scripts/bar.rpy:5035
translate schinese moon_gaming_a06b55c7:

    # game "PLAYER 2 WINS!"
    game "玩家2获胜!"

# game/scripts/bar.rpy:5037
translate schinese moon_gaming_21f6a513:

    # moon "I win!"
    moon "我赢了!"

# game/scripts/bar.rpy:5038
translate schinese moon_gaming_4b80909e:

    # k "*sigh*... good game..."
    k "*叹气*......gg......"

# game/scripts/bar.rpy:5040
translate schinese moon_gaming_140926b0:

    # em "That was really fun! I rarely get the chance to play games. We should do this again some time!"
    em "太好玩了!我一直没什么机会玩游戏.我们应该找时间再玩一次!"

# game/scripts/bar.rpy:5041
translate schinese moon_gaming_d0d84585:

    # k "That sounds fun."
    k "听起来不错."

# game/scripts/bar.rpy:5042
translate schinese moon_gaming_a35eb8c2:

    # moon "So eager to lose to me, hm? Heh, don't worry. You're pretty good for a beginner actually!"
    moon "这么想输给我,嗯?呵呵,别担心.实际上你作为新手已经很强了!"

# game/scripts/bar.rpy:5043
translate schinese moon_gaming_e26da559:

    # k "Yeah, good job Emma."
    k "是的,干得好艾玛."

# game/scripts/bar.rpy:5044
translate schinese moon_gaming_553b5360:

    # em "Thanks. I should practice a bit more though..."
    em "谢谢.但是我应该多加练习......"

# game/scripts/bar.rpy:5045
translate schinese moon_gaming_dbf1f32f:

    # k "I can teach you the basic sometime if you'd like, and a few tips n' tricks as well. The rest is up to you though."
    k "如果你想的话我可以教你一下基础操作,还有一些建议和技巧.不过剩下的就师傅领进门,修行看个人了."

# game/scripts/bar.rpy:5046
translate schinese moon_gaming_69b45b1d:

    # em "That would be awesome, thanks Kali!"
    em "那太棒了,谢谢你凯莉!"

# game/scripts/bar.rpy:5051
translate schinese moon_gaming_939e1d6d:

    # level "Moon enjoys her time at the hotel more!"
    level "文更喜欢她在酒店里的时光了!"

# game/scripts/bar.rpy:5059
translate schinese moon_dorm_b7b84f7f:

    # "You drive to the mystery location... after heading south for 15 minutes you arrive at what appears to be a public housing unit for the community college next door."
    "你驱车去了那个神秘地点......向南行驶15分钟后,你到达了社区学院(一种提供两年大学通识教育和技术培训的学校)隔壁的公共住房单元."

# game/scripts/bar.rpy:5060
translate schinese moon_dorm_c836f8e9:

    # "Not sure where to go next, you text Moon..."
    "你不知道要去哪里,就发了条短信给文......"

# game/scripts/bar.rpy:5061
translate schinese moon_dorm_db6d3b75:

    # c "I'm here, now what?"
    c "我到了,然后呢?"

# game/scripts/bar.rpy:5062
translate schinese moon_dorm_c7b9c948:

    # moon "Already?"
    moon "到了?"

# game/scripts/bar.rpy:5063
translate schinese moon_dorm_3e51c26e:

    # c "You said now, didn't you?"
    c "你说要现在,不是吗?"

# game/scripts/bar.rpy:5064
translate schinese moon_dorm_6991efa7:

    # moon "Yes, okay! Just one sec."
    moon "确实,好的!等我一会."

# game/scripts/bar.rpy:5065
translate schinese moon_dorm_54be8fa7:

    # "...{w}...{w}..."
    "......{w}......{w}......"

# game/scripts/bar.rpy:5066
translate schinese moon_dorm_10731390:

    # moon "Room 524, east wing. Don't knock too loud."
    moon "东侧的524号房.敲门不要敲太大声."

# game/scripts/bar.rpy:5067
translate schinese moon_dorm_daf1e33a:

    # c "{i}(This is where she lives? East wing... room 524... okay.){/i}"
    c "{i}(这是她住的地方吗?东侧......524号房......好的.){/i}"

# game/scripts/bar.rpy:5068
translate schinese moon_dorm_ed76a37e:

    # "After finding room 524, you knock three times. But just before you can finish the last knock, Moon opens the door, grabs your arm, and rushes you passed the kitchen into her room."
    "找到524号房间后,你敲了三次门.但就在你敲完最后一次门之前,文打开了门,抓住你的胳膊,抓着你冲过厨房进入她的房间里."

# game/scripts/bar.rpy:5070
translate schinese moon_dorm_35b82d8c:

    # moon "Phew! Here's my room!"
    moon "呼!这是我的房间!"

# game/scripts/bar.rpy:5071
translate schinese moon_dorm_33c354c2:

    # c "What's with the rush?"
    c "为什么冲这么快?"

# game/scripts/bar.rpy:5073
translate schinese moon_dorm_f4d57304:

    # moon "Oh, I just didn't want my roommate hearing you... she's sleeping."
    moon "噢,我只是不想要我的室友听到你......她在睡觉."

# game/scripts/bar.rpy:5074
translate schinese moon_dorm_c89d67c5:

    # c "Well you have a nice place. Very clean."
    c "嗯,你这地方不错.很干净啊."

# game/scripts/bar.rpy:5076
translate schinese moon_dorm_e50b1c79:

    # moon "Thanks! Been living here for a few years now."
    moon "谢谢!我在这已经住了几年了."

# game/scripts/bar.rpy:5077
translate schinese moon_dorm_589654e5:

    # c "To be honest, It's a little weird for me to not see you in your work uniform."
    c "说实在的,对我来说看见你没穿工作服有点怪怪的."

# game/scripts/bar.rpy:5078
translate schinese moon_dorm_40bbc59c:

    # moon "And you're still wearing the same thing as ever, I see."
    moon "而你却还穿着一样的衣服,我懂了."

# game/scripts/bar.rpy:5079
translate schinese moon_dorm_1af2a742:

    # c "You even kept the hair pin..."
    c "你甚至还留着发夹......"

# game/scripts/bar.rpy:5081
translate schinese moon_dorm_4984c9d6:

    # moon "..."
    moon "......"

# game/scripts/bar.rpy:5083
translate schinese moon_dorm_281670c2:

    # moon "Heh, yep..."
    moon "呵呵,是的......"

# game/scripts/bar.rpy:5084
translate schinese moon_dorm_8dda2683:

    # moon "Ummmmm..."
    moon "嗯......"

# game/scripts/bar.rpy:5085
translate schinese moon_dorm_9121a1f9:

    # c "So, did you need help with anything?"
    c "所以,你需要帮忙吗?"

# game/scripts/bar.rpy:5087
translate schinese moon_dorm_9ff7bb75:

    # moon "...No, just bored..."
    moon "......不是,我只是无聊了......"

# game/scripts/bar.rpy:5089
translate schinese moon_dorm_868230b0:

    # moon "...Got too angry at that stupid game...{w}\nI can call you over whenever I want, right?"
    moon "......玩那些傻逼游戏给我玩上火了......{w}\n我什么时候想打给你都行,对吧?"

# game/scripts/bar.rpy:5092
translate schinese moon_dorm_6a400715:

    # moon "I didn't think you would."
    moon "我也觉得你不会的."

# game/scripts/bar.rpy:5094
translate schinese moon_dorm_6bd06bf3:

    # moon "*sigh*, you can't fit me into your busy schedule from time to time?"
    moon "*叹气*,你就不能在你繁忙的日程安排里抽出时间安排一下我吗?"

# game/scripts/bar.rpy:5095
translate schinese moon_dorm_5fe2b739:

    # c "It really depends on the day."
    c "那真的得看那天情况如何."

# game/scripts/bar.rpy:5096
translate schinese moon_dorm_7e63ec25:

    # moon "Okay..."
    moon "好吧......"

# game/scripts/bar.rpy:5098
translate schinese moon_dorm_37f98624:

    # moon "Nice!"
    moon "太好了!"

# game/scripts/bar.rpy:5100
translate schinese moon_dorm_de1697b7:

    # c "Uh, what's up with the sound proofing?"
    c "呃,这里隔音效果如何?"

# game/scripts/bar.rpy:5101
translate schinese moon_dorm_0ecfe7f1:

    # moon "Oh... um... heh, funny story."
    moon "噢......嗯......呵呵,有趣的故事."

# game/scripts/bar.rpy:5102
translate schinese moon_dorm_856e068a:

    # moon "Sometimes I get a little... too angry when playing... my roommate hates it when I get loud..."
    moon "有时候我有点......玩游戏太生气了......我室友很讨厌我大声叫喊......"

# game/scripts/bar.rpy:5103
translate schinese moon_dorm_d851fbe6:

    # c "At least you're being considerate."
    c "至少你考虑得很周到."

# game/scripts/bar.rpy:5104
translate schinese moon_dorm_67085aac:

    # moon "Yep!{w} \n{size=-10}{i}It may also be covering a few holes...\nThese are really cheap walls...{/i}{/size}"
    moon "是的!{w}\n{size=-10}{i}不过上面可能有几个洞......\n这些墙真的很便宜......{/i}{/size}"

# game/scripts/bar.rpy:5106
translate schinese moon_dorm_39a43d58:

    # c "I think I know someone you might like."
    c "我想我知道你可能会喜欢谁了."

# game/scripts/bar.rpy:5107
translate schinese moon_dorm_e5c59d4d:

    # moon "Really? Who?"
    moon "真的吗?谁?"

# game/scripts/bar.rpy:5108
translate schinese moon_dorm_f9926702:

    # c "Kali, she lives at my hotel. She's on her computer all day."
    c "凯莉,她住在我的酒店里.她整天在搞她的电脑."

# game/scripts/bar.rpy:5109
translate schinese moon_dorm_28c6458f:

    # moon "Really? I don't remember her..."
    moon "真的吗?我不认得她啊......"

# game/scripts/bar.rpy:5110
translate schinese moon_dorm_915c077b:

    # c "She's... choosy with her beverages. Prefers soda."
    c "她......她对饮料很挑剔.她更喜欢苏打水."

# game/scripts/bar.rpy:5112
translate schinese moon_dorm_25c7eaa7:

    # moon "Ohhh, well if she wants something sugary, there's plenty of options! Tell her to drop by some day, I'll show her my skills!"
    moon "噢,嗯,如果她想要一些含糖的东西的话,我这里有很多选择!告诉她哪天来找我,我让她看看我的本事!"

# game/scripts/bar.rpy:5113
translate schinese moon_dorm_d522c5e0:

    # c "I definitely will."
    c "我一定会的."

# game/scripts/bar.rpy:5115
translate schinese moon_dorm_556bf10c:

    # c "Hey, what's that?"
    c "嘿,那是什么?"

# game/scripts/bar.rpy:5116
translate schinese moon_dorm_366c2711:

    # moon "Oh, Junior?"
    moon "噢,尤尼奥尔?(原文Junior,意为初级的)"

# game/scripts/bar.rpy:5118
translate schinese moon_dorm_ae721fa6:

    # moon "One of my first completed projects! It was a team effort to be honest with you, but I got to keep him!"
    moon "这是我最早完成的项目之一!说实话,这是一整个团队努力的结果,但是我留下了他!"

# game/scripts/bar.rpy:5119
translate schinese moon_dorm_89e22d18:

    # c "Does he work?"
    c "他可以工作吗?"

# game/scripts/bar.rpy:5120
translate schinese moon_dorm_e864e211:

    # moon "Yeah, he will walk around and wave at you, but he doesn't have any AI or anything. I'm just an engineer."
    moon "是的,他会走来走去,或者向你挥手,但是他没有任何AI在里面.毕竟我只是个工程师."

# game/scripts/bar.rpy:5122
translate schinese moon_dorm_2fae629c:

    # c "So you work on robots?"
    c "所以你在研究机器人?"

# game/scripts/bar.rpy:5123
translate schinese moon_dorm_8bcbcbca:

    # moon "Not anymore, I've moved on to, um... - what's the expression... - greener pastures!"
    moon "没有了,我转到,嗯......-应该怎么说来着......-更好的东西上了!"

# game/scripts/bar.rpy:5124
translate schinese moon_dorm_e037f495:

    # moon "I study architecture, buildings and bridges."
    moon "我在研究建筑设计,土木工程和桥梁工程."

# game/scripts/bar.rpy:5125
translate schinese moon_dorm_98855229:

    # moon "Want to know a secret? The future looks bright, literally! The next generation of buildings will be techno-fied."
    moon "想知道个秘密吗?真的,前途很光明的!下一代的建筑将是科技化的."

# game/scripts/bar.rpy:5126
translate schinese moon_dorm_55b48736:

    # c "How do you know this?"
    c "你怎么知道的?"

# game/scripts/bar.rpy:5127
translate schinese moon_dorm_be9bf2b4:

    # moon "Because I'm working on them! Maybe not... directly, but we aren't studying last generation's architecture, that's for sure!"
    moon "因为我在研究它们啊!也许并不是很......直接,但是我们不是在研究上一代的建筑,这是肯定的!"

# game/scripts/bar.rpy:5128
translate schinese moon_dorm_d05a85f5:

    # moon "Imagine entire cities with two levels! Skyscrapers with ads two miles high, like a big TV on each side!"
    moon "想象一下,整个城市分成两层!摩天大楼里的广告有两英尺高,就像大电视一样!"

# game/scripts/bar.rpy:5129
translate schinese moon_dorm_aecc6613:

    # c "So you're going to be the reason no one gets any sleep?"
    c "所以你会成为人们没法睡觉的原因?"

# game/scripts/bar.rpy:5130
translate schinese moon_dorm_cec1207c:

    # moon "Heheh, good thing I don't live in a city I guess?"
    moon "呵呵呵,我想幸好我没住城里?"

# game/scripts/bar.rpy:5132
translate schinese moon_dorm_78a4c7a9:

    # moon "You must understand, more people are congregating in cities each day, there will even be a need for living spaces on top of and under major bridges. Like a house/bridge hybrid."
    moon "你得明白,随着每天都会有更多的人涌入城市,甚至会有人需要在桥梁底部和顶部生活.就像是房子和桥梁的混合体."

# game/scripts/bar.rpy:5133
translate schinese moon_dorm_5ea157d5:

    # c "You're really trying to cram as many people in one spot as you can, aren't you?"
    c "你真的很想把尽可能多的人塞进一个地方里,对吧?"

# game/scripts/bar.rpy:5134
translate schinese moon_dorm_a6268835:

    # moon "I guess if there's a demand, someone will be there to supply it."
    moon "我想,只要有需求,那就会有人来供给."

# game/scripts/bar.rpy:5136
translate schinese moon_dorm_0936d363:

    # moon "That someone will be me! I want to be designing entire cities from the ground up!"
    moon "那个人就是我!我想从头开始设计整个城市!"

# game/scripts/bar.rpy:5137
translate schinese moon_dorm_79c8c50e:

    # c "That's a very ambitious dream."
    c "这是一个非常雄心勃勃的梦想."

# game/scripts/bar.rpy:5138
translate schinese moon_dorm_0a54e98a:

    # moon "There's more than enough space in Syl'anar, and with the recent push for human children from the government, there will be more than enough demand too!"
    moon "西尔阿纳尔的空间绰绰有余,加上最近政府对人类儿童福祉的推动,也会带来更多的需求的!"

# game/scripts/bar.rpy:5139
translate schinese moon_dorm_d6f29a72:

    # c "Interesting... what's your dream city?"
    c "有趣......你梦想里的城市是什么样的?"

# game/scripts/bar.rpy:5140
translate schinese moon_dorm_6498292d:

    # moon "I would very much like to mimic the scenery I grew up with. In a way, there were towers around me too. Stone towers, with wooden bridges connecting each for safe travel."
    moon "我很想模仿我成长过程中所遇到的风景.某种程度上来说,我的周围也充满摩天大楼.它们是石制的塔楼,每座塔楼之间有木桥连接,以便安全出行."

# game/scripts/bar.rpy:5141
translate schinese moon_dorm_2c9ea994:

    # moon "My dream is to have each tower connected to the other at the top! You would never have to touch the ground if you really wanted to."
    moon "我的梦想是让每座摩天大楼的顶部和另一座连接起来!如果你真的想的话,你甚至可以永远不接触到地表."

# game/scripts/bar.rpy:5142
translate schinese moon_dorm_bb9dbecd:

    # moon "Greenery too! There are many trees native to my country that can withstand the heavy winds at high altitudes, it should help make cities a little livelier!"
    moon "绿化也是!我的国家有很多原生的树木,可以抵御高海拔地区的大风,它应该有助于让城市变得更有生气!"

# game/scripts/bar.rpy:5143
translate schinese moon_dorm_7f580a28:

    # c "That's impressive, I hope to see it one day."
    c "那太惊人了,我希望有一天我可以看到它."

# game/scripts/bar.rpy:5145
translate schinese moon_dorm_b4fcef56:

    # moon "Heh, me too... people in my field don't actually expect to see their work completed."
    moon "呵呵,我也是......我这个领域的人一般是没法看到他们的杰作完成的."

# game/scripts/bar.rpy:5146
translate schinese moon_dorm_5cd8af09:

    # c "Really? That's rough..."
    c "真的吗?那太痛苦了......"

# game/scripts/bar.rpy:5147
translate schinese moon_dorm_56856ac0:

    # moon "In theory we should be able to build and support a metropolis in thirty years, especially with the push for human children. But it's more of the fact that this has never been done before..."
    moon "理论上,我们应该可以在30年内建造并支持一个大都市,尤其是在新生的人类儿童的推动下.但是更大的问题是,实际上,这种事情以前从来没有发生过......"

# game/scripts/bar.rpy:5149
translate schinese moon_dorm_9d83f950:

    # moon "It would take dozens of architects, engineers, and designers to even start planning and likely years after that just to ensure the blueprints are perfect. Every nut and bolt... So we keep our expectations in check."
    moon "甚至还要花几十个建筑师,工程师和设计师来开始规划,之后可能还需要好几年的时间来确保蓝图的完美.即使是每一个螺母和螺栓......所以我们要保持我们的期望."

# game/scripts/bar.rpy:5150
translate schinese moon_dorm_bbdedd0b:

    # c "Wow, what about space? Any projects up there?"
    c "喔,那太空呢?有计划吗?"

# game/scripts/bar.rpy:5151
translate schinese moon_dorm_3b86563e:

    # moon "Are you kidding? No way! That's way out of my field."
    moon "你在开玩笑吗?不可能的!这已经超出了我的领域."

# game/scripts/bar.rpy:5153
translate schinese moon_dorm_25ac70fd:

    # c "You're majoring in biochemistry as well, right?"
    c "你主修生物化学专业,对吧?"

# game/scripts/bar.rpy:5154
translate schinese moon_dorm_946b3b9e:

    # moon "Yeah, but it's not my passion to tell you the truth. I like to think of myself as an architect!"
    moon "是的,但是还是把真相告诉你把,这并不是我的激情所在.我喜欢把自己当成建筑师!"

# game/scripts/bar.rpy:5155
translate schinese moon_dorm_5153b017:

    # moon "When I joined, I wasn't really sure what I wanted to do... typical freshman story..."
    moon "我加入大学的时候完全不确定自己想做什么......典型的新生故事......"

# game/scripts/bar.rpy:5156
translate schinese moon_dorm_ecc41ada:

    # c "You haven't quit though? You must still enjoy it."
    c "但是你没有放弃?你一定还是很享受的吧."

# game/scripts/bar.rpy:5157
translate schinese moon_dorm_69f876aa:

    # moon "Yeah! I have always liked the idea of making medicines! It was something my father did! Each time I would scratch myself on the harsh foliage, he would mix bees wax, mint, lavender, melaleuca and a few other spices to make a home made healing salve!"
    moon "是的!我一直喜欢制作药品!这也是我爸爸所做的事情!每次我因为抓到锋利的树叶弄伤自己的时候,他都会把蜂蜡,薄荷,薰衣草,白头翁,还有其他的草药混合起来,制成自制的治疗药膏!"

# game/scripts/bar.rpy:5158
translate schinese moon_dorm_db33a1f3:

    # c "Wow, he was very resourceful."
    c "哇哦,他很有办法呢."

# game/scripts/bar.rpy:5159
translate schinese moon_dorm_7d5115b0:

    # moon "You bet! His way of doing things was very primative though, what I'm doing is real science!"
    moon "你说得对!不过他的方式很原始,而我则是真正的科学!"

# game/scripts/bar.rpy:5161
translate schinese moon_dorm_8c546c48:

    # moon "I study vassen- vaxin... ehh..."
    moon "我学的是一秒-移瞄......呃......"

# game/scripts/bar.rpy:5163
translate schinese moon_dorm_6d202f34:

    # moon "Vaccination! When they inject your arm with healing juice to prevent you from gaining a sickness."
    moon "疫苗!就是那种他们给你手臂注射的那种药物,以防止你生病."

# game/scripts/bar.rpy:5164
translate schinese moon_dorm_6405c78c:

    # c "I think I understand."
    c "我想我理解了."

# game/scripts/bar.rpy:5165
translate schinese moon_dorm_e0f2fd9b:

    # moon "Mhm! It's pretty fun, we get to peer into the microscopic universe. Maybe I'll show you what a protein looks like some day?"
    moon "嗯!它真的很有趣,它让我们可以窥探宇宙的微观层面.也许有一天我可以让你看看蛋白质到底长什么样子?"

# game/scripts/bar.rpy:5166
translate schinese moon_dorm_a89a0d91:

    # c "That would be nice."
    c "那太好了."

# game/scripts/bar.rpy:5168
translate schinese moon_dorm_4ed77576:

    # c "Did you ever think you would be where you are now? In a society or just surrounded by all of this technology?"
    c "你以前有没有想过你现在会出现在这里?有没有想过自己真的会融入社会,甚至被这些科技所环绕?"

# game/scripts/bar.rpy:5169
translate schinese moon_dorm_608f72da:

    # moon "Definitely not, it's a dream come true!"
    moon "当然没有,但是这也是我的梦想!"

# game/scripts/bar.rpy:5171
translate schinese moon_dorm_3528e1b5:

    # moon "Not for my father though, it was his worst nightmare..."
    moon "不过不是我爸爸的,这对他来说是最可怕的噩梦......"

# game/scripts/bar.rpy:5172
translate schinese moon_dorm_d0ae06d2:

    # c "Is that so?"
    c "真的吗?"

# game/scripts/bar.rpy:5173
translate schinese moon_dorm_cd7d2135:

    # moon "They left all of this behind when my parents decided to 'return to the ancient way of life'. My father said it was all too much, wanted a simpler life."
    moon "我父母决定'回归古老的生活方式',把这些东西都抛掷脑后了.我爸爸说这些东西实在是太多了,他想要更简单的生活."

# game/scripts/bar.rpy:5174
translate schinese moon_dorm_34ac060c:

    # c "He willingly made his life worse?"
    c "他心甘情愿让自己的生活变得更糟糕吗?"

# game/scripts/bar.rpy:5176
translate schinese moon_dorm_b42bb5c0:

    # moon "Guess that's all subjective! We never paid taxes, I bet you can't say the same."
    moon "那都是主观臆断!我们都不用交税,你也不能这么说."

# game/scripts/bar.rpy:5177
translate schinese moon_dorm_14dd6c94:

    # c "You don't even sound upset that's how you were raised."
    c "你听起来并没有因为你成长的环境而不高兴啊."

# game/scripts/bar.rpy:5178
translate schinese moon_dorm_2584a6ef:

    # moon "Why should I be? I had a great upbringing! I was loved, well fed, and learned something new all the time. Still to this day, I reflect on those times and come to understand what I learned helps me today."
    moon "为什么我要?我有一个很好的成长环境!我被爱着,被喂养着,而且一直在学习新的东西.直到今天,我依然在怀念那些时光,并领悟到我那时候所学到的东西对我今天的帮助."

# game/scripts/bar.rpy:5179
translate schinese moon_dorm_23cbf4bf:

    # moon "My father wasn't a dumb man, he was just... different!"
    moon "我爸爸不是傻子,他只是有点......与众不同!"

# game/scripts/bar.rpy:5180
translate schinese moon_dorm_d533ad6f:

    # moon "It would have been a totally different story if I had stayed though, I knew that life wasn't for me."
    moon "如果我留下来的话,故事就完全不一样了,我知道那种生活并不适合我."

# game/scripts/bar.rpy:5181
translate schinese moon_dorm_535329d9:

    # c "You said it made his life worse when you left though?"
    c "你是说你离开后他的生活变得更糟了?"

# game/scripts/bar.rpy:5183
translate schinese moon_dorm_61058d50:

    # moon "He really did want to keep me away from technology, society, everything. He believed everything we needed was already around us, and he showed me that through his teachings."
    moon "他确实很想让我远离科技,社会和其他的一切东西.他相信我们所需要的一切都已经在我们身边,他通过他的教诲向我传达了这一点."

# game/scripts/bar.rpy:5184
translate schinese moon_dorm_29df8326:

    # moon "And he was correct, but what he failed to understand is... we have the ability to change things, make them better."
    moon "他是对的,但是他没有理解的是......我们有能力改变这些东西,让它们变得更好."

# game/scripts/bar.rpy:5185
translate schinese moon_dorm_4d620097:

    # moon "The medicines of today are just alterations of the medicines of yesterday. My father was already doing the one thing he tried to get away from. Those healing salves, the dinner he cooked for me... that never grew off the tree like that. {i}He made them better.{/i}"
    moon "今天的药物是在昨天的药物上做出的改动.我爸爸已经做过那些他试图摆脱的事情了.无论是那些治疗用的药膏还是他给我做的晚餐......它们都不是树上长出来的.{i}是他让它们变得更好.{/i}"

# game/scripts/bar.rpy:5187
translate schinese moon_dorm_2b189c1b:

    # moon "We do the same things! I just do it better than him."
    moon "我们也一样!只不过我做得比他更好."

# game/scripts/bar.rpy:5188
translate schinese moon_dorm_91e092ab:

    # c "That's an interesting way of looking at it, I see where you got your inspiration from."
    c "这么看问题确实有趣,我知道你的灵感是来自哪里了."

# game/scripts/bar.rpy:5189
translate schinese moon_dorm_b9cb54da:

    # moon "Yep!"
    moon "是的!"

# game/scripts/bar.rpy:5190
translate schinese moon_dorm_4984c9d6_1:

    # moon "..."
    moon "......"

# game/scripts/bar.rpy:5192
translate schinese moon_dorm_d19f855d:

    # c "..."
    c "......"

# game/scripts/bar.rpy:5193
translate schinese moon_dorm_1e3a082d:

    # moon "...So... {w}I wasn't entirely truthful when I told you I was just bored..."
    moon "......所以......{w}我告诉你我只是无聊的时候,我没有把话全说出来......"

# game/scripts/bar.rpy:5194
translate schinese moon_dorm_498da023:

    # moon "I was watching videos earlier...{w} with... {w}my hand between my legs..."
    moon "我之前在看视频......{w}还......{w}还把手放在两腿中间了......"

# game/scripts/bar.rpy:5195
translate schinese moon_dorm_81334995:

    # moon "I... couldn't get a certain thought out of my head..."
    moon "我......没法摆脱那个想法......"

# game/scripts/bar.rpy:5196
translate schinese moon_dorm_1cef78cf:

    # c "So what did you do?"
    c "所以你做了什么?"

# game/scripts/bar.rpy:5198
translate schinese moon_dorm_bbce081d:

    # moon "I called you..."
    moon "我打电话给你......"

# game/scripts/bar.rpy:5200
translate schinese moon_dorm_4984c9d6_2:

    # moon "..."
    moon "......"

# game/scripts/bar.rpy:5202
translate schinese moon_dorm_2ba2b123:

    # moon "I do not want you to think I'm after only one thing or another... {w}\nBut..."
    moon "我不想让你认为我只会做一件事......{w}\n但是......"

# game/scripts/bar.rpy:5204
translate schinese moon_dorm_dbc96652:

    # moon "I've never... - what's the word... - put a penis in my mouth..."
    moon "我从来没有......-那个词怎么说来着......-把肉棒放进我的嘴里......"

# game/scripts/bar.rpy:5206
translate schinese moon_dorm_4aa15cdd:

    # moon "Suddenly, the thought crossed my mind... {w}we've done worse, right?"
    moon "突然之间,我的脑海里闪过了一个念头......{w}我们都做更糟糕的事情了,对吧?"

# game/scripts/bar.rpy:5208
translate schinese moon_dorm_7d1dabd6:

    # moon "So I called you..."
    moon "所以我打电话叫了你......"

# game/scripts/bar.rpy:5209
translate schinese moon_dorm_c568f585:

    # c "I think the word you're looking for is... blowjob?"
    c "我想你想说的是......口交?"

# game/scripts/bar.rpy:5211
translate schinese moon_dorm_fe410df0:

    # moon "Yes, thank you!"
    moon "是的,谢谢你!"

# game/scripts/bar.rpy:5213
translate schinese moon_dorm_e999cbd7:

    # moon "I finished before I called you... I did not want to make a stupid decision."
    moon "我在打电话给你之前就已经完事了......我不想做一个愚蠢的决定."

# game/scripts/bar.rpy:5214
translate schinese moon_dorm_60523be5:

    # moon "Still... the thought stained my mind. I could not get it out..."
    moon "但是......那个想法还是在我头脑里.我没法摆脱它......"

# game/scripts/bar.rpy:5215
translate schinese moon_dorm_0ce97c90:

    # moon "It is clear to me now the only thing that will cleanse my mind, is..."
    moon "我现在很清楚,唯一可以净化我心灵的东西,就是......"

# game/scripts/bar.rpy:5217
translate schinese moon_dorm_4984c9d6_3:

    # moon "..."
    moon "......"

# game/scripts/bar.rpy:5218
translate schinese moon_dorm_bec974f9:

    # moon "May I?"
    moon "我可以吗?"

# game/scripts/bar.rpy:5219
translate schinese moon_dorm_e995a244:

    # c "You don't have to ask twice."
    c "你都没必要问."

# game/scripts/bar.rpy:5221
translate schinese moon_dorm_cc01b18a:

    # moon "Well... take it out then..."
    moon "嗯......那就把它拿出来吧......"

# game/scripts/bar.rpy:5223
translate schinese moon_dorm_35b2237f:

    # moon "Hmmm... it's bigger up close..."
    moon "嗯......这么近看起来更大了......"

# game/scripts/bar.rpy:5224
translate schinese moon_dorm_f3bd666d:

    # moon "I've never tried this before, but... I think I know what I'm doing..."
    moon "我之前从来没尝试过,但是......我想我知道我在干什么......"

# game/scripts/bar.rpy:5225
translate schinese moon_dorm_c3a7a74c:

    # moon "But don't raise your expecations too high..."
    moon "但是别太期待了......"

# game/scripts/bar.rpy:5227
translate schinese moon_dorm_1d7d7018:

    # moon "I... can feel you throbbing against my cheek... you're so hot..."
    moon "我......可以感觉到你在我脸颊上的跳动......你好热......"

# game/scripts/bar.rpy:5229
translate schinese moon_dorm_8f0cd903:

    # moon "Llln... Do you like when I tease this spot? \n{w}You seem to like it..."
    moon "噜嗯......你喜欢我挑逗这个地方吗?\n{w}你看起来很喜欢......"

# game/scripts/bar.rpy:5230
translate schinese moon_dorm_fa4c0924:

    # moon "Mmln... you're getting harder...\nI think it's working..."
    moon "嗯噜......你变得更硬了......\n我想这是有用的......"

# game/scripts/bar.rpy:5234
translate schinese moon_dorm_063a6058:

    # moon "Mmlnlmmmn... how is this? Do you like my tongue?"
    moon "嗯噜嗯......这个怎么样?你喜欢我的舌头吗?"

# game/scripts/bar.rpy:5235
translate schinese moon_dorm_714923b5:

    # moon "Mmmln... mmmph... it's so strange..."
    moon "嗯噜......嗯......好奇怪......"

# game/scripts/bar.rpy:5236
translate schinese moon_dorm_a23da6d4:

    # moon "I think I know something you'll like more..."
    moon "我觉得我知道你更喜欢什么东西......"

# game/scripts/bar.rpy:5238
translate schinese moon_dorm_0660ef29:

    # moon "Hnnghk!" with ssshake
    moon "嗯!" with ssshake

# game/scripts/bar.rpy:5239
translate schinese moon_dorm_f43e1cad:

    # moon "Nnngh...{w}\nMmnh...{w}\nHngh..."
    moon "嗯......{w}\n嗯......{w}\n嗯......"

# game/scripts/bar.rpy:5243
translate schinese moon_dorm_e4c2ba79:

    # moon "*gasp*! *cough*! Ehem!"
    moon "*惊叹*!*咳嗽*!嗯!"

# game/scripts/bar.rpy:5244
translate schinese moon_dorm_5180beb4:

    # moon "Haaah...! Nnnnh, that was hard... how far did I go?"
    moon "哈......!嗯,好难......我吞下去了多少?"

# game/scripts/bar.rpy:5245
translate schinese moon_dorm_62cea0fd:

    # c "You could do with a little more practice..."
    c "你可以练习一下......"

# game/scripts/bar.rpy:5247
translate schinese moon_dorm_6f703720:

    # moon "Was that a challenge...?"
    moon "这是在挑战我吗......?"

# game/scripts/bar.rpy:5248
translate schinese moon_dorm_6abc2302:

    # moon "I'm very competitive! You won't know what hit you before long...! Let me get a better angle..."
    moon "我可是很有能力!过不了多久你就知道你面前的到底是谁了......!让我换个更好的角度......"

# game/scripts/bar.rpy:5250
translate schinese moon_dorm_e8be2a98:

    # moon "Mmmmn! Hnnn...! How's this? Feel any better? Nnnh...!"
    moon "嗯!嗯......!这个怎么样?感觉舒服多了吗?嗯......!"

# game/scripts/bar.rpy:5251
translate schinese moon_dorm_b63df24d:

    # moon "Mmmph...! Aahhnngh...! Mmmn... this... feels really nice... Mmmn...!"
    moon "嗯......!啊......!嗯......这个......感觉好舒服......嗯......!"

# game/scripts/bar.rpy:5252
translate schinese moon_dorm_30faa807:

    # moon "Mmmpph! But I {u}will{/u} make you cum! Ahnn...! Nnngh! There's no way I would let my first blowjob experience end in failure! Mmmph!"
    moon "嗯!但是我{u}会{/u}让你射的!啊......!嗯!我可不会让我的第一次口交以失败告终!嗯!"

# game/scripts/bar.rpy:5255
translate schinese moon_dorm_8aa86653:

    # moon "Mmmngh! Nnngh...! [name], your cock is delicious...! Hnngh! Nnnmm...! It's everything I fantasize about..."
    moon "嗯!嗯......![name],你的肉棒好好吃......!嗯!嗯......!这就是我所幻想的事情......"

# game/scripts/bar.rpy:5256
translate schinese moon_dorm_ba73a962:

    # moon "Nnngh! Hnnngh! Yes! Mmmn! Is my mouth to your liking? I'm not bad, right! Llllmngh!"
    moon "嗯!嗯!是的!嗯!你喜欢我的嘴巴吗?我做得还不错,对吧!噜嗯!"

# game/scripts/bar.rpy:5257
translate schinese moon_dorm_f025a78a:

    # moon "Mmmn! Mmmn! Cum on my face, [name]...! Ahhh...! I want to see your hot cum on me...! Mmmngh!"
    moon "嗯!嗯!射在我的脸上吧,[name]......!嗯!我想要看见你那热热的精液射在我的脸上......!嗯!"

# game/scripts/bar.rpy:5261
translate schinese moon_dorm_778f5ee6:

    # moon "Hnnnghk!?" with ssshake
    moon "嗯!?" with ssshake

# game/scripts/bar.rpy:5262
translate schinese moon_dorm_38315754:

    # moon "Nnnnnnnnghk!" with ssshake
    moon "嗯!" with ssshake

# game/scripts/bar.rpy:5263
translate schinese moon_dorm_29ecec0c:

    # moon "Mmmn! Mmmmgh!" with ssshake
    moon "嗯!嗯!" with ssshake

# game/scripts/bar.rpy:5265
translate schinese moon_dorm_6d15ca15:

    # moon "*cough*! *cough*! You could have warned me...! Nnnngh..."
    moon "*咳嗽*!*咳嗽*!你应该警告我的......!嗯......"

# game/scripts/bar.rpy:5267
translate schinese moon_dorm_c9cf94d7:

    # moon "*sigh*... Nnnnnm..."
    moon "*叹气*......嗯......"

# game/scripts/bar.rpy:5268
translate schinese moon_dorm_89875f83:

    # moon "That was... really fun."
    moon "那还......挺好玩的."

# game/scripts/bar.rpy:5272
translate schinese moon_dorm_328f57b9:

    # moon "Nnnnm! I'm ready for your cum, [name]! Ahhh...!"
    moon "嗯!我已经准备好迎接你的精液了,[name]!啊......!"

# game/scripts/bar.rpy:5274
translate schinese moon_dorm_979d9413:

    # moon "*gasp*!" with ssshake
    moon "*惊叹*!" with ssshake

# game/scripts/bar.rpy:5276
translate schinese moon_dorm_fff1153c:

    # moon "*sigh*... Nnnnnm... you did it..."
    moon "*叹气*......嗯......你做到了......"

# game/scripts/bar.rpy:5277
translate schinese moon_dorm_28c6e358:

    # moon "That was... really fun!"
    moon "那还......挺好玩的!"

# game/scripts/bar.rpy:5279
translate schinese moon_dorm_34d66f6e:

    # moon "How did I do? Great right! Even for my first time!"
    moon "我做的怎么样?很棒对吧!即使是这是我的第一次!"

# game/scripts/bar.rpy:5280
translate schinese moon_dorm_ce737bda:

    # c "You did great."
    c "你做的很棒."

# game/scripts/bar.rpy:5281
translate schinese moon_dorm_14d98a4f:

    # moon "Thanks..."
    moon "谢谢......"

# game/scripts/bar.rpy:5282
translate schinese moon_dorm_5e3b8cd8:

    # moon "Umm... you're probably pretty tired now, right? It's already pretty late..."
    moon "嗯......你现在很累了,对吧?现在也很晚了......"

# game/scripts/bar.rpy:5283
translate schinese moon_dorm_c624cb20:

    # moon "If you want... you could sleep here for the night..."
    moon "如果你想要的话......你今晚可以在这睡觉......"

# game/scripts/bar.rpy:5284
translate schinese moon_dorm_5634653d:

    # c "Sure, I think I'll do that."
    c "当然了,我想我会那么做的."

# game/scripts/bar.rpy:5315
translate schinese moon_dorm_b9249e79:

    # "After a lovely night, Moon rushes you out before the day starts. Not before she pecks you on the cheek though."
    "一个美好的夜晚过后,文催促你在天亮前离开.不过不是在她亲完你的脸之前."

# game/scripts/bar.rpy:5317
translate schinese moon_dorm_a100f75e:

    # level "Moon likes you more!"
    level "文更喜欢你了!"

# game/scripts/bar.rpy:5327
translate schinese moonmarbj_41343e6a:

    # c "Hm? Where's Moon?"
    c "嗯?文呢?"

# game/scripts/bar.rpy:5329
translate schinese moonmarbj_ab2010ca:

    # c "Hey bartender, have you seen Moon?"
    c "嘿调酒师,你看见文了吗?"

# game/scripts/bar.rpy:5331
translate schinese moonmarbj_921bf1c4:

    # bart "Over there."
    bart "在那里."

# game/scripts/bar.rpy:5333
translate schinese moonmarbj_aa569f27:

    # m "...and lonely, mostly... When I was young I would rely on my smarts to get attention. It's always been a problem for me..."
    m "......还有孤独,主要是......我年轻的时候,我会依靠我的聪明才智来获得关注.这对我来说一直都是个问题......"

# game/scripts/bar.rpy:5335
translate schinese moonmarbj_3edfcc6c:

    # moon "My father was very rough on me, I never had a strong motherly presence, so I kind of understand... I sometimes wish I had someone to hold me more. I can't imagine what it's like to grow up with no parents at all..."
    moon "我爸爸对我很严格,而且我也从来没有有过一个强势的妈妈,所以我多多少少有点理解......我有时候希望能有个人可以来多抱抱我.我没法想象在完全没有父母的情况下长大会是什么样子的......"

# game/scripts/bar.rpy:5336
translate schinese moonmarbj_da4dd66b:

    # moon "But any way you look at it, your desire for attention made you a very smart person, so there's a strong sri-,"
    moon "但是无论你怎么看待这件事,你对他人注意的渴望让你成为一个非常聪明的人,所以你还是有一线喜旺-,"

# game/scripts/bar.rpy:5338
translate schinese moonmarbj_9e78c8a9:

    # moon "sli- svil- err, uhh..."
    moon "习-细-呃,嗯......"

# game/scripts/bar.rpy:5339
translate schinese moonmarbj_5cf34b5c:

    # m "Silver lining?"
    m "一线希望?"

# game/scripts/bar.rpy:5341
translate schinese moonmarbj_040bfcdb:

    # moon "Yes, thank you~"
    moon "对,谢谢你~"

# game/scripts/bar.rpy:5343
translate schinese moonmarbj_c59b56a9:

    # m "Yeah, I mean... you're right..."
    m "是的,我是说......你是对的......"

# game/scripts/bar.rpy:5345
translate schinese moonmarbj_02562c09:

    # moon "Just like me! Not many people would think growing up in a jungle would be good for your health, but it's made me so resilient!"
    moon "就像我一样!没有几个人会认为在丛林里长大对健康会有什么好处,但是就是它才让我变得如此坚强!"

# game/scripts/bar.rpy:5346
translate schinese moonmarbj_d3107ef6:

    # m "Well..."
    m "嗯......"

# game/scripts/bar.rpy:5348
translate schinese moonmarbj_ae794acd:

    # moon "Just look on the bright side, okay? There's no point in dwelling on the bad. It already happened... now you're smarter as a result!"
    moon "看看好的那一面,好吗?纠结于那些坏事是没有意义的,因为它已经发生了......现在你会因此变得更聪明!"

# game/scripts/bar.rpy:5350
translate schinese moonmarbj_c34b6c3b:

    # moon "Now live your life as smart, confident, sexy Maria!"
    moon "现在作为聪明,自信,性感的玛丽亚继续生活下去吧!"

# game/scripts/bar.rpy:5352
translate schinese moonmarbj_bb403ad7:

    # m "Sexy...?"
    m "性感......?"

# game/scripts/bar.rpy:5353
translate schinese moonmarbj_d386f5ad:

    # moon "I'm just trying to give you confidence...!"
    moon "我只是想给你点自信心......!"

# game/scripts/bar.rpy:5354
translate schinese moonmarbj_4bffbbe9:

    # m "It's working..."
    m "那确实有用......"

# game/scripts/bar.rpy:5356
translate schinese moonmarbj_6045c813:

    # moon "Yay~"
    moon "耶~"

# game/scripts/bar.rpy:5357
translate schinese moonmarbj_86e2bc48:

    # c "Hey you two."
    c "嘿你们两个."

# game/scripts/bar.rpy:5359
translate schinese moonmarbj_86dac188:

    # m "Oh, hey [name]!"
    m "噢,嘿[name]!"

# game/scripts/bar.rpy:5360
translate schinese moonmarbj_1e89fde5:

    # moon "Hello, [name]! Join us!"
    moon "你好,[name]!加入我们吧!"

# game/scripts/bar.rpy:5362
translate schinese moonmarbj_29905314:

    # m "Come to join us for a drink?"
    m "想来加入我们喝一杯吗?"

# game/scripts/bar.rpy:5364
translate schinese moonmarbj_0988a85e:

    # m "Or me, since I'm drinking alone apparently..."
    m "或者说是我,毕竟很显然只有我在喝酒......"

# game/scripts/bar.rpy:5365
translate schinese moonmarbj_1d15c3fb:

    # moon "It would just go to waste on me... and I don't even think I'm allowed to drink on the job..."
    moon "它在我身上就是浪费......而且我不认为我可以在工作的时候喝酒......"

# game/scripts/bar.rpy:5367
translate schinese moonmarbj_b891c885:

    # m "Oh well, guess I'm drinking alone..."
    m "噢那好吧,那我就自己喝了......"

# game/scripts/bar.rpy:5368
translate schinese moonmarbj_294b8f18:

    # m "Sure would be fun to see you drunk though, for a change... It must be possible."
    m "虽然看到你喝醉了一定很有趣......而且那是一定有可能的."

# game/scripts/bar.rpy:5369
translate schinese moonmarbj_27475bbf:

    # moon "Sure, I am human after all."
    moon "那肯定,我毕竟还是个人类."

# game/scripts/bar.rpy:5371
translate schinese moonmarbj_d7dbd324:

    # m "You know what? Enough is enough! I've seen you drink like crazy, and you never get drunk! Tonight is the night!"
    m "你知道吗?够了!我见过你疯狂地喝酒,但是我从来没见过你喝醉过!今晚就是个好机会!"

# game/scripts/bar.rpy:5372
translate schinese moonmarbj_3c873be0:

    # moon "O-Oh, I don't know..."
    moon "噢-哦,我不知道......"

# game/scripts/bar.rpy:5373
translate schinese moonmarbj_0deb1d00:

    # m "Don't you worry! I've been practicing my mixology skills... I made something special for {i}you{/i}..."
    m "别担心!我一直在练习我的调酒技巧......我给{i}你{/i}准备了点特别的东西......"

# game/scripts/bar.rpy:5375
translate schinese moonmarbj_8c70a1a5:

    # m "Bartender! Bring the blue bottle!"
    m "调酒师!把那瓶蓝色的拿过来!"

# game/scripts/bar.rpy:5376
translate schinese moonmarbj_fb66d701:

    # bart "Okay!"
    bart "好的!"

# game/scripts/bar.rpy:5378
translate schinese moonmarbj_5c9d48b3:

    # m "We're gonna get so fucked up!"
    m "我们要大搞一通了!"

# game/scripts/bar.rpy:5380
translate schinese moonmarbj_21b966f7:

    # bart "Here you are, ladies."
    bart "给你,小姐."

# game/scripts/bar.rpy:5381
translate schinese moonmarbj_909152d6:

    # moon "I-... Is it safe...? I've never seen it served to customers..."
    moon "这-......这安全吗......?我之前从来没见过它提供给顾客过......"

# game/scripts/bar.rpy:5382
translate schinese moonmarbj_40072b17:

    # m "I mix drinks, not poison. Bottoms up!"
    m "我调的是酒,不是毒.干杯!"

# game/scripts/bar.rpy:5383
translate schinese moonmarbj_812bfb84:

    # moon "C-Can I...?"
    moon "我-我可以吗......?"

# game/scripts/bar.rpy:5384
translate schinese moonmarbj_a4d6a9a1:

    # c "Yeah, go ahead."
    c "可以,去吧."

# game/scripts/bar.rpy:5385
translate schinese moonmarbj_da9b4df0:

    # moon "S-Sure, why not... it's been so long!"
    moon "好-好的,为什么不呢......已经这么久了!"

# game/scripts/bar.rpy:5387
translate schinese moonmarbj_01e23d1f:

    # moon "*gulp*! *gulp*!"
    moon "*咕噜*!*咕噜*!"

# game/scripts/bar.rpy:5388
translate schinese moonmarbj_a3f326a7:

    # m "Daaaamn!"
    m "操了!"

# game/scripts/bar.rpy:5389
translate schinese moonmarbj_58653d50:

    # moon "*gulp*! *gulp*! *gulp*!"
    moon "*咕噜*!*咕噜*!*咕噜*!"

# game/scripts/bar.rpy:5391
translate schinese moonmarbj_0c2aeeaa:

    # moon "Ghaaahk!" with ssshake
    moon "啊!" with ssshake

# game/scripts/bar.rpy:5392
translate schinese moonmarbj_094192d3:

    # m "Strong, right?"
    m "很烈,对吧?"

# game/scripts/bar.rpy:5393
translate schinese moonmarbj_8365e4c1:

    # moon "Ooogh, I feel like I just drank... spicy gasoline...!"
    moon "哦,我感觉我好像喝了......辣味的汽油......!"

# game/scripts/bar.rpy:5395
translate schinese moonmarbj_c872e0cc:

    # moon "Here... Egh, try some..."
    moon "这里......呃,试一点吧......"

# game/scripts/bar.rpy:5396
translate schinese moonmarbj_b3762817:

    # m "Go ahead... if you dare!"
    m "去吧......只要你敢的话!"

# game/scripts/bar.rpy:5398
translate schinese moonmarbj_8537b737:

    # c "Are you going to have any?"
    c "你喝吗?"

# game/scripts/bar.rpy:5399
translate schinese moonmarbj_e3074847:

    # m "I have my wine, don't you worry..."
    m "我有我自己的酒,你不用担心......"

# game/scripts/bar.rpy:5400
translate schinese moonmarbj_ecdb97c1:

    # c "Cheers..."
    c "干杯......"

# game/scripts/bar.rpy:5402
translate schinese moonmarbj_78699188:

    # c "*gulp* *gulp*" with ssshake
    c "*咕噜**咕噜*" with ssshake

# game/scripts/bar.rpy:5404
translate schinese moonmarbj_330791e0:

    # c "Ooof... that's- *cough*! Rough...."
    c "哦......好-*咳嗽*!烈......"

# game/scripts/bar.rpy:5405
translate schinese moonmarbj_00670cd8:

    # m "Right?"
    m "对吧?"

# game/scripts/bar.rpy:5407
translate schinese moonmarbj_58653d50_1:

    # moon "*gulp*! *gulp*! *gulp*!"
    moon "*咕噜*!*咕噜*!*咕噜*!"

# game/scripts/bar.rpy:5408
translate schinese moonmarbj_8750edcd:

    # m "Wow, you like it, huh?"
    m "哇哦,你喜欢,嗯?"

# game/scripts/bar.rpy:5410
translate schinese moonmarbj_0e14a918:

    # moon "BLEGHK! No, not at all...!" with ssshake
    moon "呜!不,一点也不......!" with ssshake

# game/scripts/bar.rpy:5411
translate schinese moonmarbj_5c7288db:

    # moon "But I can't get, um- {i}hammered{/i}? Without a lot of it, so... ehhhhhhhhhh..."
    moon "但是如果我不喝多的话,我就没法,嗯-{i}喝个烂醉{/i}?所以......嗯......"

# game/scripts/bar.rpy:5412
translate schinese moonmarbj_9624c1aa:

    # m "Heheheh..."
    m "呵呵呵呵......"

# game/scripts/bar.rpy:5414
translate schinese moonmarbj_0c77e1eb:

    # m "Come on, it's not so bad... It's only 95 percent alcohol..."
    m "来吧,没那么糟糕......里面只有95%%的酒精......"

# game/scripts/bar.rpy:5415
translate schinese moonmarbj_64be9e7b:

    # moon "Eghk! Holy shit...! Can you even legally sell this? *cough*"
    moon "呃!我了个大操......!这玩意能合法出售吗?*咳嗽*!"

# game/scripts/bar.rpy:5416
translate schinese moonmarbj_612503dc:

    # m "I dunno..."
    m "我不知道......"

# game/scripts/bar.rpy:5418
translate schinese moonmarbj_b35fe0b7:

    # moon "*GULP*! *GULP*! *GULP*! *GULP*!" with ssshake
    moon "*咕噜*!*咕噜*!*咕噜*!*咕噜*!" with ssshake

# game/scripts/bar.rpy:5420
translate schinese moonmarbj_078caf64:

    # moon "Hooshi syo- *hic* shtrongk..." with ssshake
    moon "我了革大草,好-*打嗝*好烈的就......" with ssshake

# game/scripts/bar.rpy:5421
translate schinese moonmarbj_fdad4fef:

    # m "Haha, what was that?"
    m "哈哈,怎么样?"

# game/scripts/bar.rpy:5422
translate schinese moonmarbj_099cf244:

    # moon "So strong... eghhh..."
    moon "好烈......呃......"

# game/scripts/bar.rpy:5423
translate schinese moonmarbj_db5c38e5:

    # m "Maybe that's enough for you..."
    m "也许这对你来说已经够多了......"

# game/scripts/bar.rpy:5425
translate schinese moonmarbj_f177e462:

    # moon "Maybe you're- *hic* right..."
    moon "也许你说得-*打嗝*对......"

# game/scripts/bar.rpy:5426
translate schinese moonmarbj_6e1bb30d:

    # m "Not so tough now, are you? Heheh."
    m "你还好,对吧?呵呵呵."

# game/scripts/bar.rpy:5427
translate schinese moonmarbj_0752b1fe:

    # moon "My tummy feels like it's on fire..."
    moon "我的肚子感觉像着火一样......"

# game/scripts/bar.rpy:5428
translate schinese moonmarbj_ca2c3b20:

    # m "That means it's working..."
    m "那说明有效果了......"

# game/scripts/bar.rpy:5430
translate schinese moonmarbj_a2302f95:

    # moon "Are you feeling... anything?"
    moon "你有感觉到......什么吗?"

# game/scripts/bar.rpy:5431
translate schinese moonmarbj_1aad05b1:

    # c "Yeah, a little."
    c "是的,有点."

# game/scripts/bar.rpy:5432
translate schinese moonmarbj_45a98564:

    # moon "I've never had something this- *hic*... strong before..."
    moon "我之前从来没喝过这么-*打嗝*......烈的东西......"

# game/scripts/bar.rpy:5434
translate schinese moonmarbj_06d01854:

    # moon "Nnngh, can we go somewhere quieter...? All this music is making my head pound..."
    moon "嗯,我们能不能去个更安静点的地方......?这里的音乐让我的头砰砰直跳......"

# game/scripts/bar.rpy:5435
translate schinese moonmarbj_4bb1e53f:

    # m "Awww, quitting already?"
    m "啊,打算退出了?"

# game/scripts/bar.rpy:5436
translate schinese moonmarbj_1ea25d2f:

    # moon "No way, n-nothing can take me oooooooouugh... out..."
    moon "不可能,什-什么都不能把我扯~......出去......"

# game/scripts/bar.rpy:5437
translate schinese moonmarbj_97f9f58a:

    # m "Mhm, okay... sure let's go."
    m "嗯,好吧......我们走吧."

# game/scripts/bar.rpy:5438
translate schinese moonmarbj_0cea0e5d:

    # moon "Thank you..."
    moon "谢谢你......"

# game/scripts/bar.rpy:5443
translate schinese moonmarbj_e45127da:

    # moon "Ooogh, are we there yet?"
    moon "哦,我们到了吗?"

# game/scripts/bar.rpy:5444
translate schinese moonmarbj_cd4d2ed6:

    # m "Yep, we're here..."
    m "是的,我们到了......"

# game/scripts/bar.rpy:5446
translate schinese moonmarbj_add9353b:

    # moon "Ohh, the hot springs...? That's nice..."
    moon "哦,温泉啊......?那太棒了......"

# game/scripts/bar.rpy:5448
translate schinese moonmarbj_9ce06e99:

    # m "Aaaah!"
    m "啊!"

# game/scripts/bar.rpy:5449
translate schinese moonmarbj_cbd47132:

    # moon "Wha- did you just take your top off!?"
    moon "什-你是不是把你的上衣脱掉了!?"

# game/scripts/bar.rpy:5451
translate schinese moonmarbj_c295c526:

    # m "I'm getting comfortable...!"
    m "我这样很舒服......!"

# game/scripts/bar.rpy:5452
translate schinese moonmarbj_14c5550d:

    # m "And it's not like the customers are coming here, come on!"
    m "而且,顾客也不会来这里,来吧!"

# game/scripts/bar.rpy:5454
translate schinese moonmarbj_74242015:

    # m "Mmmmmmmngh! That feels so good..."
    m "嗯!感觉好舒服......"

# game/scripts/bar.rpy:5455
translate schinese moonmarbj_1a131396:

    # moon "W-What are you doing...!?"
    moon "你-你在干什么......!?"

# game/scripts/bar.rpy:5457
translate schinese moonmarbj_51f56bd3:

    # moon "Ahh!"
    moon "啊!"

# game/scripts/bar.rpy:5458
translate schinese moonmarbj_5278d04c:

    # m "Gettin' nuuuuuuuude~..."
    m "给我全脱了~......"

# game/scripts/bar.rpy:5460
translate schinese moonmarbj_6054c4c2:

    # m "Come on, you're getting in with me!"
    m "来吧,你和我一起进来!"

# game/scripts/bar.rpy:5461
translate schinese moonmarbj_31520427:

    # moon "B-But I didn't bring a swimuit!"
    moon "但-但是我没带泳衣!"

# game/scripts/bar.rpy:5462
translate schinese moonmarbj_5feb669a:

    # m "Awww, cute..."
    m "啊,真可爱......"

# game/scripts/bar.rpy:5463
translate schinese moonmarbj_a5c92b75:

    # moon "H-Huh?"
    moon "嗯-嗯?"

# game/scripts/bar.rpy:5465
translate schinese moonmarbj_e7d3d140:

    # m "Hurry up, [name]!"
    m "快点,[name]!"

# game/scripts/bar.rpy:5466
translate schinese moonmarbj_6f7c4713:

    # moon "Why are we gehhk- *hic*! ...Why are we getting naked?"
    moon "为什么我们要-*打嗝*!......为什么我们要裸着?"

# game/scripts/bar.rpy:5467
translate schinese moonmarbj_ddf2805b:

    # m "It's fun! Shhhhhh!"
    m "因为很好玩!唰!"

# game/scripts/bar.rpy:5469
translate schinese moonmarbj_2f7e54e1:

    # m "Aaaah... it feels so nice when you're drunk too..."
    m "啊......你喝醉的时候也会感觉很好的......"

# game/scripts/bar.rpy:5471
translate schinese moonmarbj_fdfee004:

    # m "C'mon sourpuss, what are you waiting for?"
    m "来吧,别哭着脸了,你还在等什么?"

# game/scripts/bar.rpy:5472
translate schinese moonmarbj_80128404:

    # moon "Ehhhh..."
    moon "呃......"

# game/scripts/bar.rpy:5473
translate schinese moonmarbj_e76018a1:

    # m "You're not chickening out now, are you...?"
    m "你不是在退缩,对吧......?"

# game/scripts/bar.rpy:5474
translate schinese moonmarbj_0717fd8d:

    # moon "I've just... never..."
    moon "我只是......从来没有......"

# game/scripts/bar.rpy:5476
translate schinese moonmarbj_571755a8:

    # "*SPLASH*!" with ssshake
    "*哗*!" with ssshake

# game/scripts/bar.rpy:5478
translate schinese moonmarbj_d90a28ce:

    # moon "Ahh, noo!" with ssshake
    moon "啊,不要!" with ssshake

# game/scripts/bar.rpy:5480
translate schinese moonmarbj_7562dda1:

    # moon "Rrrg..."
    moon "呃......"

# game/scripts/bar.rpy:5481
translate schinese moonmarbj_735627e0:

    # m "Damn... looks like you have to let your clothes dry anyway!"
    m "妈的......看起来你无论如何都得弄干你的衣服了!"

# game/scripts/bar.rpy:5482
translate schinese moonmarbj_d515f2bd:

    # moon "Fine... you leave me with no choice..."
    moon "好吧......你让我别无选择......"

# game/scripts/bar.rpy:5484
translate schinese moonmarbj_91194f0d:

    # m "Trust me babe, the water feels amazing on your skin..."
    m "相信我宝贝,这里的水会让你的皮肤感觉很好的......"

# game/scripts/bar.rpy:5485
translate schinese moonmarbj_bfbcb850:

    # moon "Don't call me {i}babe{/i}!"
    moon "别叫我{i}宝贝{/i}!"

# game/scripts/bar.rpy:5486
translate schinese moonmarbj_d1623484:

    # m "But you're so cuuuute~"
    m "但是你真的好可爱~"

# game/scripts/bar.rpy:5488
translate schinese moonmarbj_e2c9d193:

    # moon "Aaaaah..."
    moon "啊......"

# game/scripts/bar.rpy:5489
translate schinese moonmarbj_fe345868:

    # m "Great, huh?"
    m "很不错吧,嗯?"

# game/scripts/bar.rpy:5491
translate schinese moonmarbj_3e85a18f:

    # moon "You were right..."
    moon "你说的对......"

# game/scripts/bar.rpy:5493
translate schinese moonmarbj_6fb31e80:

    # m "I'm always right..."
    m "我一直都是对的......"

# game/scripts/bar.rpy:5494
translate schinese moonmarbj_4984c9d6:

    # moon "..."
    moon "......"

# game/scripts/bar.rpy:5496
translate schinese moonmarbj_12295b26:

    # m "How nice of you to finally join us."
    m "你终于加入了我们,太好了."

# game/scripts/bar.rpy:5497
translate schinese moonmarbj_8e25b7f4:

    # c "You two looked like you were having fun."
    c "你们两个看起来很开心啊."

# game/scripts/bar.rpy:5498
translate schinese moonmarbj_7e8e2631:

    # moon "Yeah..."
    moon "是的......"

# game/scripts/bar.rpy:5500
translate schinese moonmarbj_bd9268e0:

    # m "Could be having more though..."
    m "不过还可以更开心......"

# game/scripts/bar.rpy:5501
translate schinese moonmarbj_951fae67:

    # moon "Ehehehe... no..."
    moon "呵呵呵......别......"

# game/scripts/bar.rpy:5503
translate schinese moonmarbj_b132dfd2:

    # m "*sigh*, to this day I still can't figure out why you decided to hire such a boring person..."
    m "*叹气*,直到今天我还是想不通,你为什么要雇佣这么一个无聊的人......"

# game/scripts/bar.rpy:5504
translate schinese moonmarbj_9b454b62:

    # moon "*gasp*! I am not boring! What part of growing up isolated in a jungle is {i}boring{/i}!?"
    moon "*惊叹*!我才不无聊!在丛林里与世隔绝地成长中哪一部分是{i}无聊{/i}的!?"

# game/scripts/bar.rpy:5506
translate schinese moonmarbj_4653d798:

    # m "*yawwwn*... I'd be more interested if you told me a monkey fucked you..."
    m "*哈欠*......你要是告诉我有只猴子把你上了,那我才更感兴趣......"

# game/scripts/bar.rpy:5507
translate schinese moonmarbj_fa88ff6a:

    # moon "Agh, gross... you're too horny!"
    moon "啊,恶心......你太色情了!"

# game/scripts/bar.rpy:5509
translate schinese moonmarbj_5fba4ebe:

    # m "So what if I am? You're still boring..."
    m "所以我是又如何?你还是很无聊......"

# game/scripts/bar.rpy:5510
translate schinese moonmarbj_56f36352:

    # moon "That's not true! I've done things..."
    moon "那不是真的!我做过一些事情......"

# game/scripts/bar.rpy:5511
translate schinese moonmarbj_d74c09b9:

    # m "Oh yeah? Like what..."
    m "哦是吗?比如什么......"

# game/scripts/bar.rpy:5513
translate schinese moonmarbj_0432c510:

    # moon "There was that time he spanked me... a lot."
    moon "比如让他打我的屁股......很多次."

# game/scripts/bar.rpy:5514
translate schinese moonmarbj_a101c7cd:

    # m "Ooooooh?"
    m "哦~?"

# game/scripts/bar.rpy:5516
translate schinese moonmarbj_31a6c3f2:

    # m "That's not too special..."
    m "那也没什么大不了的......"

# game/scripts/bar.rpy:5517
translate schinese moonmarbj_0723360d:

    # moon "Well, he spanked me until... my ass was red..."
    moon "嗯,他打我的屁股......直到我的屁股变红了......"

# game/scripts/bar.rpy:5518
translate schinese moonmarbj_7fb90130:

    # m "Hmmm? You like it rough? No you don't..."
    m "嗯?你喜欢粗暴点的?不,你不喜欢......"

# game/scripts/bar.rpy:5519
translate schinese moonmarbj_67ccd5a3:

    # moon "{i}{size=-10}Well not at first...{/size}{/i}"
    moon "{i}{size=-10}嗯,一开始确实不喜欢......{/size}{/i}"

# game/scripts/bar.rpy:5520
translate schinese moonmarbj_ac0aab65:

    # m "I love being spanked... I love it when my ass gets red..."
    m "我喜欢被打屁股......我喜欢我的屁股变红......"

# game/scripts/bar.rpy:5521
translate schinese moonmarbj_76587e2f:

    # m "You would just cry after one slap..."
    m "而你被打一巴掌就会哭......"

# game/scripts/bar.rpy:5522
translate schinese moonmarbj_60c388b0:

    # moon "Not true! No... I like rough..."
    moon "不是!不......我喜欢粗暴的......"

# game/scripts/bar.rpy:5524
translate schinese moonmarbj_cca51ec4:

    # m "Show me!"
    m "让我看看!"

# game/scripts/bar.rpy:5525
translate schinese moonmarbj_f498d30b:

    # moon "What?"
    moon "什么?"

# game/scripts/bar.rpy:5527
translate schinese moonmarbj_0985f31a:

    # m "Show me, pleeeeease?"
    m "让我看看,请?"

# game/scripts/bar.rpy:5528
translate schinese moonmarbj_f9316dab:

    # moon "Fine, I'll prove it to you..."
    moon "好吧,我会证明给你看的......"

# game/scripts/bar.rpy:5530
translate schinese moonmarbj_314cca20:

    # m "Really? Do it! Bend over, I wanna see your ass!"
    m "真的吗?做吧!趴下,我要看看你的屁股!"

# game/scripts/bar.rpy:5531
translate schinese moonmarbj_423557a4:

    # moon "Sit down, horny girl!"
    moon "坐下,你个饥渴的女孩!"

# game/scripts/bar.rpy:5533
translate schinese moonmarbj_aa915b2d:

    # m "Aaaah~!"
    m "啊~!"

# game/scripts/bar.rpy:5534
translate schinese moonmarbj_a07d606e:

    # moon "It's nothing to me...! I could last way longer than you!"
    moon "这对我来说没什么......!我可以比你坚持得更久!"

# game/scripts/bar.rpy:5535
translate schinese moonmarbj_f50edce0:

    # m "I doubt that..."
    m "我表示怀疑......"

# game/scripts/bar.rpy:5537
translate schinese moonmarbj_caf680d2:

    # moon "Spank me!"
    moon "打我屁股!"

# game/scripts/bar.rpy:5539
translate schinese moonmarbj_4b28d3f7:

    # moon "{i}Can you believe her? She thinks I'm boring! It's not true!{/i}"
    moon "{i}你相信她吗?她觉得我很无聊!那不是真的!{/i}"

# game/scripts/bar.rpy:5540
translate schinese moonmarbj_fe648b6b:

    # m "Mmmmmmmn...!"
    m "嗯......!"

# game/scripts/bar.rpy:5544
translate schinese moonmarbj_61a74dc8:

    # moon "Ahh!" with ssshake
    moon "啊!" with ssshake

# game/scripts/bar.rpy:5548
translate schinese moonmarbj_0bd0fdb8:

    # moon "Mmmn!" with ssshake
    moon "嗯!" with ssshake

# game/scripts/bar.rpy:5550
translate schinese moonmarbj_bf7b413b:

    # moon "See, this is nothing!"
    moon "看吧,这没什么!"

# game/scripts/bar.rpy:5552
translate schinese moonmarbj_2b486236:

    # m "Yeah... I see..."
    m "是的......我看见了......"

# game/scripts/bar.rpy:5554
translate schinese moonmarbj_632e4811:

    # m "I see him going real soft..."
    m "我看见他真的很轻柔......"

# game/scripts/bar.rpy:5555
translate schinese moonmarbj_dc809df8:

    # moon "Go harder, [name]! Urgh!"
    moon "再大力点,[name]!呃!"

# game/scripts/bar.rpy:5557
translate schinese moonmarbj_8b1f170b:

    # moon "Aah!" with sshake
    moon "啊!" with sshake

# game/scripts/bar.rpy:5558
translate schinese moonmarbj_0e50e2df:

    # m "Mmmmmn..."
    m "嗯......"

# game/scripts/bar.rpy:5559
translate schinese moonmarbj_80822c93:

    # moon "NNGH!" with sshake
    moon "嗯!" with sshake

# game/scripts/bar.rpy:5561
translate schinese moonmarbj_87f18afa:

    # m "Fuck! I can't take it anymore!" with ssshake
    m "草!我再也忍不住了!" with ssshake

# game/scripts/bar.rpy:5563
translate schinese moonmarbj_4130ae8d:

    # moon "Aaah! M-Maria!?"
    moon "啊!玛-玛丽亚!?"

# game/scripts/bar.rpy:5565
translate schinese moonmarbj_7ed6bac9:

    # moon "Ahhh...! W-What are you doing...?"
    moon "啊......!你-你在干什么......?"

# game/scripts/bar.rpy:5566
translate schinese moonmarbj_7e222ace:

    # moon "Are you {i}trying{/i} to lick my ass? Mmmn...!?"
    moon "你是{i}想{/i}舔我的屁股吗?嗯......!?"

# game/scripts/bar.rpy:5567
translate schinese moonmarbj_4ac04626:

    # m "Lllmn! Mmmn!"
    m "噜嗯!嗯!"

# game/scripts/bar.rpy:5569
translate schinese moonmarbj_859f914b:

    # moon "Hahhh...! Hnnngh...! That feels so weird...! Mmmmngh!"
    moon "哈......!嗯......!感觉好奇怪......!嗯!"

# game/scripts/bar.rpy:5571
translate schinese moonmarbj_8cfcc47e:

    # m "I think she's almost ready for you..."
    m "我觉得她已经为你准备好了......"

# game/scripts/bar.rpy:5573
translate schinese moonmarbj_a75937c2:

    # moon "What...? I- I don't... I haven't..."
    moon "什么......?我-我没有......我还没有......"

# game/scripts/bar.rpy:5574
translate schinese moonmarbj_1d165932:

    # m "Tonight can be your first..."
    m "今晚可以是你的第一次......"

# game/scripts/bar.rpy:5576
translate schinese moonmarbj_6759686e:

    # moon "Mmmmngh...! Hahhh...!"
    moon "嗯......!哈......!"

# game/scripts/bar.rpy:5577
translate schinese moonmarbj_971c769f:

    # m "Mmmm! You're so tight...!"
    m "嗯!你好紧......!"

# game/scripts/bar.rpy:5579
translate schinese moonmarbj_a8484134:

    # moon "Just because I'm drunk doesn't mean I couldn't-"
    moon "我喝醉了又不是不能-"

# game/scripts/bar.rpy:5581
translate schinese moonmarbj_2a95e930:

    # moon "Mmmngh!" with ssshake
    moon "嗯!" with ssshake

# game/scripts/bar.rpy:5582
translate schinese moonmarbj_13465a60:

    # m "What was that?"
    m "什么?"

# game/scripts/bar.rpy:5584
translate schinese moonmarbj_80ab1dfc:

    # moon "Did you... put another finger in...?"
    moon "你......是不是把另一个手指放进来了......?"

# game/scripts/bar.rpy:5585
translate schinese moonmarbj_d36c2f36:

    # m "Mhm..."
    m "嗯......"

# game/scripts/bar.rpy:5587
translate schinese moonmarbj_4998ce0a:

    # moon "Mmmmmgh...!"
    moon "嗯......!"

# game/scripts/bar.rpy:5588
translate schinese moonmarbj_561df4b8:

    # m "You like it when I stretch your ass, don't you...?"
    m "你喜欢我弄你的屁股,对吧......?"

# game/scripts/bar.rpy:5589
translate schinese moonmarbj_f8babd87:

    # moon "Don't stop..."
    moon "不要停......"

# game/scripts/bar.rpy:5591
translate schinese moonmarbj_ca681c0c:

    # moon "Mmmmmngh!"
    moon "嗯!"

# game/scripts/bar.rpy:5592
translate schinese moonmarbj_17c39fe2:

    # m "Who knew you were such an anal slut..."
    m "谁知道你居然是这样的菊穴骚货......"

# game/scripts/bar.rpy:5593
translate schinese moonmarbj_49c8bd58:

    # m "You would love for a big, meaty cock to fill you up, wouldn't you...?"
    m "你会喜欢一个有大又粗的肉棒来填满你的的,对吧......?"

# game/scripts/bar.rpy:5594
translate schinese moonmarbj_0ec00733:

    # moon "Mmmmmn...!"
    moon "嗯......!"

# game/scripts/bar.rpy:5595
translate schinese moonmarbj_e4090a5b:

    # m "To stretch you... paint your insides with hot cum...?"
    m "玩弄你......用热热的精液填满你的内部......?"

# game/scripts/bar.rpy:5596
translate schinese moonmarbj_bf94e1ec:

    # moon "Ahhh...!"
    moon "啊......!"

# game/scripts/bar.rpy:5597
translate schinese moonmarbj_04bc4ba1:

    # moon "Yes...!"
    moon "是的......!"

# game/scripts/bar.rpy:5598
translate schinese moonmarbj_9d5e78dd:

    # moon "Mmn! Yes, I do...!"
    moon "嗯!是的,我要......!"

# game/scripts/bar.rpy:5600
translate schinese moonmarbj_2b47c302:

    # m "That should be enough..."
    m "应该可以了......"

# game/scripts/bar.rpy:5601
translate schinese moonmarbj_ac8ac204:

    # moon "Mmmmm..."
    moon "嗯......"

# game/scripts/bar.rpy:5603
translate schinese moonmarbj_32c98ff4:

    # m "I think she's ready for you..."
    m "我觉得她已经为你准备好了......"

# game/scripts/bar.rpy:5604
translate schinese moonmarbj_d57d9fe3:

    # moon "Wait..."
    moon "等等......"

# game/scripts/bar.rpy:5606
translate schinese moonmarbj_e452875d:

    # moon "Can you... take me to your room, [name]? We can do it there..."
    moon "你可以......带我去你的房间里吗,[name]?我们可以在那里做......"

# game/scripts/bar.rpy:5607
translate schinese moonmarbj_c48dc32f:

    # c "Sure."
    c "当然可以."

# game/scripts/bar.rpy:5609
translate schinese moonmarbj_239ea740:

    # "You, Moon and Maria drukenly stumble towards your room..."
    "你,文还有玛丽亚摇摇晃晃地走向了你的房间......"

# game/scripts/bar.rpy:5611
translate schinese moonmarbj_60955b5d:

    # moon "Mmmmmn... so comfortable..."
    moon "嗯......很舒服......"

# game/scripts/bar.rpy:5612
translate schinese moonmarbj_6c08d2c4:

    # m "Are you ready, Moon...?"
    m "你准备好了吗,文......?"

# game/scripts/bar.rpy:5614
translate schinese moonmarbj_6ea884d6:

    # moon "Mmmm... fuck me..."
    moon "嗯......操我......"

# game/scripts/bar.rpy:5615
translate schinese moonmarbj_b285bea1:

    # moon "Fuck my ass... put it in..."
    moon "操我的屁股......把它放进来......"

# game/scripts/bar.rpy:5617
translate schinese moonmarbj_182f7ea2:

    # moon "Hnnnnggh! Haaah!!" with ssshake
    moon "嗯!哈!!" with ssshake

# game/scripts/bar.rpy:5619
translate schinese moonmarbj_bb6d9cca:

    # moon "Aaaaaaaaah!!!" with ssshake
    moon "啊!!!" with ssshake

# game/scripts/bar.rpy:5620
translate schinese moonmarbj_52b2d20d:

    # moon "Oww! Tight...! Tight! Mmmmmnn!"
    moon "哦!好紧......!好紧!嗯!"

# game/scripts/bar.rpy:5621
translate schinese moonmarbj_9c34e215:

    # m "Mmmmm, that's so hot...!"
    m "嗯,好色啊......!"

# game/scripts/bar.rpy:5622
translate schinese moonmarbj_083fe0d4:

    # m "Doesn't it feel amazing, stretching your ass like that...?"
    m "感觉不是很棒吗,那样玩弄你的屁股......?"

# game/scripts/bar.rpy:5624
translate schinese moonmarbj_cfc69322:

    # moon "Hnnnghk! Aaaaah!" with ssshake
    moon "嗯!啊!" with ssshake

# game/scripts/bar.rpy:5625
translate schinese moonmarbj_937c04eb:

    # m "Mmm! Yes! Split that bitch in half!"
    m "嗯!是的!把那个婊子操成两半吧!"

# game/scripts/bar.rpy:5626
translate schinese moonmarbj_7039a362:

    # moon "Agh...! Ahh...! It feels... so much bigger in there than... than my... Mmmmn! Fuuuck!"
    moon "啊......!啊......!感觉......更大了......比在我的......嗯!操!"

# game/scripts/bar.rpy:5627
translate schinese moonmarbj_8ff45aad:

    # moon "Start... start moving...! Hnnnngh...!"
    moon "开始......开始动起来吧......!嗯......!"

# game/scripts/bar.rpy:5629
translate schinese moonmarbj_93edb3a4:

    # moon "Aaaagh~! Hnnngh! Fuck...!"
    moon "啊~!嗯!操......!"

# game/scripts/bar.rpy:5630
translate schinese moonmarbj_00e51a33:

    # m "That's it, girl... take that giant cock up your ass! Mmm!"
    m "好了,女孩......把那个大肉棒放你屁股上把!嗯!"

# game/scripts/bar.rpy:5631
translate schinese moonmarbj_afc7102d:

    # moon "Nnnngh! You're- Agh!... rubbing yourself so close to me...! MMMM!"
    moon "嗯!你-啊!......在离我这么近的地方抚摸自己......!嗯!"

# game/scripts/bar.rpy:5632
translate schinese moonmarbj_b50b8038:

    # m "The expression on your face is too cute...! I just want to smother your face between my legs, mmm...!"
    m "你脸上的表情太可爱了......!我好想把你的脸埋在我的腿里,嗯......!"

# game/scripts/bar.rpy:5633
translate schinese moonmarbj_a50d01b0:

    # m "You should focus on yourself though... Mmm... how's that cock feel...?"
    m "不过你应该专注在你自己身上......嗯......肉棒感觉如何啊......?"

# game/scripts/bar.rpy:5634
translate schinese moonmarbj_ffb508f4:

    # moon "Good... mmmn...! Big... ahh! Hard...!"
    moon "好舒服......嗯......!好大......啊!好硬......!"

# game/scripts/bar.rpy:5635
translate schinese moonmarbj_66941a1d:

    # m "Mmmmm, just what you were hoping for?"
    m "嗯,这就是你期待的吗?"

# game/scripts/bar.rpy:5636
translate schinese moonmarbj_208d38fe:

    # moon "Agh! Yes...! Yes! Yes! Ahhh!"
    moon "啊!是的......!是的!是的!啊!"

# game/scripts/bar.rpy:5639
translate schinese moonmarbj_c5c1d956:

    # moon "Hnnnngh! Deeper! Ahhh...! Fill me, [name]! Ahh! Ahh! D-Don't stop...!"
    moon "嗯!再深点!啊......!填满我吧,[name]!啊!啊!不-不要停......!"

# game/scripts/bar.rpy:5640
translate schinese moonmarbj_aa7a1fdb:

    # m "Fucking fill this anal slut! Fuck her!"
    m "他妈的把这个菊穴骚货填满吧!操她!"

# game/scripts/bar.rpy:5641
translate schinese moonmarbj_eaa9eccd:

    # moon "Cum...! Inside...! My ass...! Ahhh! Cum...! Inside...! My ass...!"
    moon "射......!进......!我的屁股......!啊!射......!进......!我的屁股......!"

# game/scripts/bar.rpy:5642
translate schinese moonmarbj_2e11c46d:

    # moon "Aaagh! I-I'm going to cum...! Aaah! Aaaah! Fuuuck...! Don't stop!"
    moon "啊!我-我要去了......!啊!啊!操......!不要停!"

# game/scripts/bar.rpy:5643
translate schinese moonmarbj_5675d23b:

    # m "Yes...! Make this bitch cum! Fuck!"
    m "是的......!让这个婊子去吧!妈的!"

# game/scripts/bar.rpy:5645
translate schinese moonmarbj_d8236100:

    # moon "[linoname]...! [linoname]...! [linoname]! [linoname]! [linoname]!"
    moon "[linoname]......![linoname]......![linoname]![linoname]![linoname]!"

# game/scripts/bar.rpy:5647
translate schinese moonmarbj_45722ac7:

    # moon "[name]...! [name]...! [name]! [name]! [name]!"
    moon "[name]......![name]......![name]![name]![name]!"

# game/scripts/bar.rpy:5650
translate schinese moonmarbj_237e90c3:

    # moon "AAAAAAAAAAAAAAGH! [linoname]!!!" with sshake
    moon "啊啊啊啊啊![linoname]!!!" with sshake

# game/scripts/bar.rpy:5652
translate schinese moonmarbj_dff2e52a:

    # moon "AAAAAAAAAAAAAAGH! [name]!!!" with sshake
    moon "啊啊啊啊啊![name]!!!" with sshake

# game/scripts/bar.rpy:5654
translate schinese moonmarbj_384e281a:

    # m "FUCK! MMMMMMNNN! Fuuuck...!" with ssshake
    m "操!嗯!操......!" with ssshake

# game/scripts/bar.rpy:5655
translate schinese moonmarbj_6bf0033c:

    # moon "Cum in me...! Cum in me...! Yes! Yesssss!" with ssshake
    moon "射给我......!射给我......!是的!是的!" with ssshake

# game/scripts/bar.rpy:5657
translate schinese moonmarbj_75c3c171:

    # moon "Aghhh...! Ahhh...! Hahhh...!" with ssshake
    moon "啊......!啊......!哈......!" with ssshake

# game/scripts/bar.rpy:5658
translate schinese moonmarbj_73429bf9:

    # m "Hahhh... Mmmmmmmmn... fuck..."
    m "哈......嗯......操......"

# game/scripts/bar.rpy:5660
translate schinese moonmarbj_43cd0365:

    # m "Maybe you aren't so boring after all..."
    m "也许你并不无聊......"

# game/scripts/bar.rpy:5661
translate schinese moonmarbj_e1d4c59a:

    # moon "Hahhh... Nnnngh... I told you... I don't lose...! Ahhh..."
    moon "哈......嗯......我都告诉你了......我才没输......!啊......"

# game/scripts/bar.rpy:5663
translate schinese moonmarbj_d179a6f2:

    # m "Mmm, I guess you win, then..."
    m "嗯,我想你赢了,嗯......"

# game/scripts/bar.rpy:5664
translate schinese moonmarbj_6ddc76f4:

    # moon "Nnngh... good... and don't ever doubt me again...!"
    moon "嗯......好的......还有不要再质疑我了......!"

# game/scripts/bar.rpy:5666
translate schinese moonmarbj_1dc77450:

    # moon "And you... you...!"
    moon "还有你......你......!"

# game/scripts/bar.rpy:5668
translate schinese moonmarbj_cefd44fb:

    # moon "{i}{size=-5}...That felt really good... I came really hard... thank you...{/size}{/i}"
    moon "{i}{size=-5}......感觉真的很舒服......我高潮得很猛烈......谢谢你......{/size}{/i}"

# game/scripts/bar.rpy:5670
translate schinese moonmarbj_070491af:

    # moon "URGHHHH! Back to work...! Goodbye everyone..."
    moon "呃!要回去工作了......!再见各位......"

# game/scripts/bar.rpy:5672
translate schinese moonmarbj_799af532:

    # m "Oh no, you can't work like that..."
    m "哦不,你不能那样子去工作......"

# game/scripts/bar.rpy:5674
translate schinese moonmarbj_292a3cba:

    # moon "Huh? What are you saying...? I don't want to get fired...!"
    moon "嗯?你在说什么......?我不想被炒鱿鱼......!"

# game/scripts/bar.rpy:5675
translate schinese moonmarbj_52f3d5ff:

    # m "I don't think you have to worry about that tonight..."
    m "我觉得你今晚不需要担心这个问题......"

# game/scripts/bar.rpy:5677
translate schinese moonmarbj_ff735f58:

    # m "C'mon, let's go sleep in my bed... you shouldn't be driving."
    m "来吧,在我床上睡一会吧......你也不能开车."

# game/scripts/bar.rpy:5678
translate schinese moonmarbj_889a0e1a:

    # moon "But I'm not even tired...!"
    moon "但是我还不累......!"

# game/scripts/bar.rpy:5679
translate schinese moonmarbj_28955434:

    # m "Goodnight [name]..."
    m "晚安[name]......"

# game/scripts/bar.rpy:5680
translate schinese moonmarbj_8f16e46a:

    # c "Goodnight you two."
    c "你们两个晚安."

# game/scripts/bar.rpy:5683
translate schinese moonmarbj_34a31b59:

    # level "Your relationship with Moon has grown!"
    level "你和文的关系等级提升了!"

# game/scripts/bar.rpy:5704
translate schinese moondate_a80a3b30:

    # moon "Agh... Hmmm... Tsk..."
    moon "啊......嗯......啧......"

# game/scripts/bar.rpy:5705
translate schinese moondate_05c7ebc9:

    # c "Moon?"
    c "文?"

# game/scripts/bar.rpy:5706
translate schinese moondate_5bad8f17:

    # moon "Ah?"
    moon "啊?"

# game/scripts/bar.rpy:5708
translate schinese moondate_fc2357e2:

    # moon "Nnnnnn..."
    moon "嗯......"

# game/scripts/bar.rpy:5709
translate schinese moondate_853e6a9c:

    # c "{i}(That's a powerful glare...){/i}"
    c "{i}(好重的目光......){/i}"

# game/scripts/bar.rpy:5710
translate schinese moondate_9f1a8d54:

    # c "That's a nice outfit..."
    c "衣服不错......"

# game/scripts/bar.rpy:5712
translate schinese moondate_a1244888:

    # moon "Hmph!"
    moon "嗯!"

# game/scripts/bar.rpy:5713
translate schinese moondate_58d569ea:

    # c "Why aren't you working? Is there a reason you're dressed up?"
    c "为什么你没在工作?你为什么打扮得这么漂亮?"

# game/scripts/bar.rpy:5715
translate schinese moondate_07b50b7b:

    # moon "I took the day off to ask you out on a date!"
    moon "我请了一天假跟你约会!"

# game/scripts/bar.rpy:5716
translate schinese moondate_67741a21:

    # c "Really? I'm getting mixed signals..."
    c "真的吗?我好像听到了别的东西哦......"

# game/scripts/bar.rpy:5717
translate schinese moondate_3029cdfa:

    # c "Wait, when did you call off of work? I didn't get a call."
    c "等等,你什么时候请假的?我没接到你的电话啊."

# game/scripts/bar.rpy:5718
translate schinese moondate_57fdd0c9:

    # moon "I-If I called you, you would have known, idiot! It was for the best!"
    moon "如-如果我打给你,你就会知道的,笨蛋!这么做是最好的!"

# game/scripts/bar.rpy:5719
translate schinese moondate_0c9596cb:

    # c "Okay, but why are you upset?"
    c "好吧,但是为什么你看起来不高兴?"

# game/scripts/bar.rpy:5721
translate schinese moondate_3342c491:

    # moon "Because I'm not sure about you..."
    moon "因为我不确定你的意思......"

# game/scripts/bar.rpy:5723
translate schinese moondate_7ab38679:

    # moon "I know you have a thing with Maria, and maybe others too. I don't know what to make of that, but I will question you. You also slept with me before any proper dates, so I'm here to make up for that!"
    moon "我知道你和玛丽亚有关系,也许还有其他人.我不知道我要怎么理解,但是我会问问你的.而且你是在和我约会前就和我上床了,所以我是来弥补这件事的!"

# game/scripts/bar.rpy:5725
translate schinese moondate_f2c2531b:

    # moon "...So come with me on a date, now. I won't let you get away without one."
    moon "......所以跟我去约会吧,现在.我不会让你走的."

# game/scripts/bar.rpy:5726
translate schinese moondate_8d4c7382:

    # c "Okay, but... where would we-"
    c "好吧,但是......我们要去-"

# game/scripts/bar.rpy:5728
translate schinese moondate_695f3758:

    # moon "I said now! You're dressed fine as you are." with ssshake
    moon "我说现在!穿你现在的衣服就可以了." with ssshake

# game/scripts/bar.rpy:5730
translate schinese moondate_456d28c5:

    # moon "That's my car, hop in."
    moon "那是我的车,进去."

# game/scripts/bar.rpy:5731
translate schinese moondate_04a0ba8d:

    # c "You're driving?"
    c "你开车?"

# game/scripts/bar.rpy:5732
translate schinese moondate_37859f0c:

    # moon "Yes, of course. Don't ask where we're going, either. It's a surprise."
    moon "是的,当然了.也别问我们要去哪.这是个惊喜."

# game/scripts/bar.rpy:5735
translate schinese moondate_fc4c6365:

    # moon "Tadaaaa~ what do you think!? I had to reserve a table three days in advance!"
    moon "嗒哒~你觉得如何!?我不得不提前三天才预定到了桌子!"

# game/scripts/bar.rpy:5736
translate schinese moondate_02de81b7:

    # c "It's amazing. I wasn't expecting somewhere so fancy."
    c "这太棒了.我从来没想过会是这么高级的地方."

# game/scripts/bar.rpy:5738
translate schinese moondate_848baffb:

    # moon "I'm glad you think so! It's my favorite restaurant in all of Syl'anar! But you haven't tried amazing until you've had their burgers!"
    moon "我很高兴你这么认为!这是我在整个西尔阿纳尔最喜欢的餐厅!但是你得尝尝它们的汉堡才,那才是真正令人惊奇的东西!"

# game/scripts/bar.rpy:5739
translate schinese moondate_a9f43bbe:

    # c "A burger?"
    c "汉堡?"

# game/scripts/bar.rpy:5740
translate schinese moondate_5d019abf:

    # moon "It's the taste that counts!"
    moon "味道才是最重要的!"

# game/scripts/bar.rpy:5742
translate schinese moondate_5895c7ab:

    # moon "Are you cheating on Maria!?" with ssshake
    moon "你是在欺骗玛丽亚吗!?" with ssshake

# game/scripts/bar.rpy:5743
translate schinese moondate_f30d15f9:

    # c "Woah, what?"
    c "喔,什么?"

# game/scripts/bar.rpy:5744
translate schinese moondate_ef4e39c2:

    # moon "I don't like cheaters! You need to be committed to the person you love!"
    moon "我不喜欢骗子!你需要对你所爱之人做出承诺!"

# game/scripts/bar.rpy:5745
translate schinese moondate_71b2ae7a:

    # c "Relax, I'm not cheating on her with you..."
    c "放松点,我没欺骗她......"

# game/scripts/bar.rpy:5746
translate schinese moondate_9151da15:

    # c "I... own a harem."
    c "我......有个后宫."

# game/scripts/bar.rpy:5748
translate schinese moondate_f541d8fc:

    # moon "Oh, really? I didn't know that was common here. Apologies!"
    moon "哦,真的吗?我不知道那种东西在这里是很正常的.我向你道歉!"

# game/scripts/bar.rpy:5750
translate schinese moondate_19a7afb0:

    # moon "Where is the waiter? I'm starving..."
    moon "侍者呢?我快饿死了......"

# game/scripts/bar.rpy:5751
translate schinese moondate_0a7572e3:

    # c "You took that better than I thought you would."
    c "你接受得比我想象中的要好."

# game/scripts/bar.rpy:5753
translate schinese moondate_fe156999:

    # moon "Oh really? Harems are tradition where I come from, so it's nothing new to me."
    moon "哦,真的吗?在我的家乡,后宫是那里的传统,所以这对我来说并不新鲜."

# game/scripts/bar.rpy:5754
translate schinese moondate_f246a941:

    # c "I didn't know that, did your father own one?"
    c "我不知道,你爸爸有后宫吗?"

# game/scripts/bar.rpy:5755
translate schinese moondate_3fbf6875:

    # moon "No, hahah! You need to be kind of rich to own a harem."
    moon "没有,哈哈哈!你得有点钱才可以拥有一个后宫."

# game/scripts/bar.rpy:5757
translate schinese moondate_fa007c8a:

    # moon "After all, no one finds monkey hunting to be an attractive skill..."
    moon "毕竟,没人会认为抓猴子是一项有魅力的技能的......"

# game/scripts/bar.rpy:5759
translate schinese moondate_9197ccc4:

    # c "Did you really hunt monkeys?"
    c "你真的捕猎猴子吗?"

# game/scripts/bar.rpy:5760
translate schinese moondate_6e24819e:

    # moon "Don't believe me? Of course I did!"
    moon "不相信我?我当然有了!"

# game/scripts/bar.rpy:5761
translate schinese moondate_a0f3cc15:

    # moon "I didn't like killing them, but you have to survive in times like one I grew up in. They were always a last resort anyway, and quite rare to catch."
    moon "我不喜欢杀它们,但是在我长大的那个时间,你必须活下来.反正它们也是我们最后的手段,而且它们很难抓得到."

# game/scripts/bar.rpy:5763
translate schinese moondate_12338c72:

    # moon "I mostly ate turkey. Fruits and vegetables were on the menu as well, I just like to tell people I ate monkeys because it creeps them out!"
    moon "我主要是吃火鸡.还有水果和蔬菜,我只是喜欢告诉别人我吃过猴子,因为这样会吓他们一大跳!"

# game/scripts/bar.rpy:5765
translate schinese moondate_5b889971:

    # moon "A lovely conversation to have over dinner, isn't it?"
    moon "晚餐前可爱的对话,不是吗?"

# game/scripts/bar.rpy:5766
translate schinese moondate_a1f0da02:

    # c "Yeah..."
    c "是的......"

# game/scripts/bar.rpy:5767
translate schinese moondate_4e3d7195:

    # c "So, you seem fine with harems?"
    c "所以,你对后宫没问题?"

# game/scripts/bar.rpy:5768
translate schinese moondate_bd70e2b6:

    # moon "What matters most is the love. I don't see why a person can't love multiple people at once, right?"
    moon "最重要的是爱.我不明白为什么一个人不能同时爱多个人,不是吗?"

# game/scripts/bar.rpy:5769
translate schinese moondate_f5f7c27d:

    # c "Right."
    c "确实."

# game/scripts/bar.rpy:5771
translate schinese moondate_5f97f928:

    # moon "There's my reasoning, I'm sure you can agree, haha!"
    moon "这就是我的原因,我相信你会同意的,哈哈!"

# game/scripts/bar.rpy:5773
translate schinese moondate_2842f7c2:

    # waiter "Good day and welcome, I shall be your waiter for today. Do you know what you would like to have tonight? Or would you like to take a look at our menu?"
    waiter "日安,欢迎你们,我会是你们今天的服务员.你们今晚想吃什么吗?或者你们想看看我们的菜单?"

# game/scripts/bar.rpy:5774
translate schinese moondate_a5057aa4:

    # moon "I'll have the Drow Supreme with no tomatoes."
    moon "我要没有西红柿的卓尔至尊堡."

# game/scripts/bar.rpy:5779
translate schinese moondate_6532f100:

    # moon "*gasp*! You'll love it!"
    moon "*惊叹*!你会喜欢它的!"

# game/scripts/bar.rpy:5780
translate schinese moondate_25a16e8a:

    # waiter "Very well, right away."
    waiter "很好,马上就来."

# game/scripts/bar.rpy:5784
translate schinese moondate_25a16e8a_1:

    # waiter "Very well, right away."
    waiter "很好,马上就来."

# game/scripts/bar.rpy:5788
translate schinese moondate_25a16e8a_2:

    # waiter "Very well, right away."
    waiter "很好,马上就来."

# game/scripts/bar.rpy:5791
translate schinese moondate_a8ddc131:

    # moon "How many women are in your harem, [name]?"
    moon "你的后宫里有几个女人,[name]?"

# game/scripts/bar.rpy:5793
translate schinese moondate_d1f7a1e9:

    # c "That's a bit of a difficult question... I haven't gotten to know everyone too well yet. At least five, maybe eight."
    c "这是个有点困难的问题......我还没有和每个人都很熟.至少五个,可能有八个."

# game/scripts/bar.rpy:5795
translate schinese moondate_b8ae5227:

    # c "That's a bit of a difficult question... I haven't gotten to know all of them too well yet. At least four maybe."
    c "这是个有点困难的问题......我还没有和每个人都很熟.至少四个吧大概."

# game/scripts/bar.rpy:5796
translate schinese moondate_48974e4d:

    # moon "Really? That's a lot! This whole time I thought you were hitting on the other girls, I had no idea you were already in a relationship with them."
    moon "真的吗?那可太多了!这段时间我一直以为你在搭讪其他女孩,我不知道你已经和她们在一起了."

# game/scripts/bar.rpy:5798
translate schinese moondate_3d4dd72e:

    # moon "Although, to be honest... I don't find the idea of being in a harem exciting... I would prefer a one-on-one relationship, I don't think I would like sharing the things I own with so many people."
    moon "但是,说实在的......我不觉得后宫是一个令人兴奋的想法......我更喜欢一对一的关系,我不觉得我会喜欢和这么多人分享我拥有的东西."

# game/scripts/bar.rpy:5799
translate schinese moondate_f79468f3:

    # moon "However... I do not think of it as a deal breaker either."
    moon "但是......我也不觉得这破坏了交易."

# game/scripts/bar.rpy:5801
translate schinese moondate_9c71546d:

    # moon "Would I have to, eh... be naked with Maria?"
    moon "我是不是必须,嗯......和玛丽亚赤身裸体待在一起?"

# game/scripts/bar.rpy:5803
translate schinese moondate_274bb953:

    # moon "Sorry, perhaps we shouldn't talk about such embarassing things over food."
    moon "对不起,也许我们不应该在吃饭的时候谈论这种令人尴尬的话题."

# game/scripts/bar.rpy:5804
translate schinese moondate_f19a9c13:

    # c "I wouldn't be opposed to it, I think you two would look sexy together."
    c "我不反对,我觉得你们两个在一起会很性感的."

# game/scripts/bar.rpy:5806
translate schinese moondate_1df3298b:

    # moon "Speaking hypothetically... I wouldn't be opposed to it either if it was something the head of the harem wanted, but I wouldn't seek it out, she's my friend..."
    moon "假设一下......如果这是后宫之主需要的东西,我不会反对,但是我不会主动去做的,她是我的朋友......"

# game/scripts/bar.rpy:5807
translate schinese moondate_cb8b9ab8:

    # moon "May we change the subject, please?"
    moon "我们可以换个话题吗,请?"

# game/scripts/bar.rpy:5808
translate schinese moondate_14b7a690:

    # c "How is college life going for you?"
    c "你的大学生活怎么样了?"

# game/scripts/bar.rpy:5810
translate schinese moondate_fc25ae89:

    # moon "It's going very well! I'll be graduating soon, my roommate and I have been glued to our books lately. I hope I'll be able to find a good job soon."
    moon "很好!我很快就要毕业了,我和我的室友最近都扑到书上面了.我希望我很快就可以找到一份好工作."

# game/scripts/bar.rpy:5811
translate schinese moondate_bb64dd94:

    # c "You already want out of the bar?"
    c "你已经想离开酒吧了?"

# game/scripts/bar.rpy:5812
translate schinese moondate_fc0598d0:

    # moon "Of course not! You don't expect me to be a waitress forever, do you? Being a waitress isn't my final step, I still have a long way to go."
    moon "当然不是!不过你不会希望我永远当个服务员吧?做服务员不少我的最后一步,我还有很长的路要走."

# game/scripts/bar.rpy:5813
translate schinese moondate_f20b08b4:

    # c "What is your final step?"
    c "你的最后一步是什么?"

# game/scripts/bar.rpy:5815
translate schinese moondate_1f50ba54:

    # moon "I don't know, I haven't decided... I like both engineering and biochemistry, but I'm not sure how many opportunities I have to combine the two."
    moon "我不知道,我还没决定......我既喜欢工程学,也喜欢生物化学,但是我不确定我有多少机会可以把这两者结合起来."

# game/scripts/bar.rpy:5818
translate schinese moondate_c74bc53e:

    # waiter "Here you are, sir."
    waiter "这是你的,先生."

# game/scripts/bar.rpy:5819
translate schinese moondate_b2577a95:

    # moon "Wow, it looks delicious!"
    moon "哇哦,看起来好美味!"

# game/scripts/bar.rpy:5820
translate schinese moondate_dcba3e5d:

    # "You take a bite..."
    "你吃了一口......"

# game/scripts/bar.rpy:5822
translate schinese moondate_9b27f647:

    # c "Turkey?"
    c "火鸡肉?"

# game/scripts/bar.rpy:5823
translate schinese moondate_9873fa6d:

    # moon "Mhm!"
    moon "嗯!"

# game/scripts/bar.rpy:5825
translate schinese moondate_7df8542e:

    # moon "It reminds me of the food my father made me when I was young."
    moon "它让我想起了我爸爸在我年轻的时候给我做的食物."

# game/scripts/bar.rpy:5826
translate schinese moondate_6901f315:

    # c "He made burgers?"
    c "他做汉堡?"

# game/scripts/bar.rpy:5828
translate schinese moondate_66e561bf:

    # moon "No, haha! He didn't make burgers, but he did prepare meals with turkey in them. Most restaurants don't get the flavor right, but this place has it perfect!"
    moon "不是,哈哈!他不做汉堡,但是他确实做了有火鸡的饭菜.大多数餐馆的味道都不太对,但是这个地方的味道很完美!"

# game/scripts/bar.rpy:5831
translate schinese moondate_c74bc53e_1:

    # waiter "Here you are, sir."
    waiter "这是你的,先生."

# game/scripts/bar.rpy:5832
translate schinese moondate_b2577a95_1:

    # moon "Wow, it looks delicious!"
    moon "哇哦,看起来好美味!"

# game/scripts/bar.rpy:5835
translate schinese moondate_c74bc53e_2:

    # waiter "Here you are, sir."
    waiter "这是你的,先生."

# game/scripts/bar.rpy:5836
translate schinese moondate_b2577a95_2:

    # moon "Wow, it looks delicious!"
    moon "哇哦,看起来好美味!"

# game/scripts/bar.rpy:5838
translate schinese moondate_569c0087:

    # c "How is yours?"
    c "你的怎么样?"

# game/scripts/bar.rpy:5839
translate schinese moondate_08f3e5a0:

    # moon "Wonderful."
    moon "完美."

# game/scripts/bar.rpy:5841
translate schinese moondate_aa0240d0:

    # moon "Oh, maybe I can create some new and interesting drinks with my biochemistry skills!"
    moon "哦,也许我可以用我的生物化学技能制造一些有趣的新型饮料!"

# game/scripts/bar.rpy:5842
translate schinese moondate_d9673365:

    # c "So long as you don't poison anyone, that sounds like a great idea."
    c "只要你不会让人中毒,这听起来似乎是个好主意."

# game/scripts/bar.rpy:5844
translate schinese moondate_5cba6af5:

    # moon "Why would I poison anyone? I major in biochemistry not idiocy."
    moon "为什么我会让人中毒?我学的是生物化学又不是白痴学."

# game/scripts/bar.rpy:5846
translate schinese moondate_9fde3549:

    # moon "I know what I'm doing, you can trust me!"
    moon "我知道我在干什么,你可以相信我!"

# game/scripts/bar.rpy:5847
translate schinese moondate_fb3d85f5:

    # c "Have you ever tried?"
    c "你试过吗?"

# game/scripts/bar.rpy:5848
translate schinese moondate_40312dd1:

    # moon "Well, I have mixed some drinks before, but I haven't tried anything too crazy. The tools required to make such complex things can be quite expensive."
    moon "嗯,我之前调过一些饮料,但是我还没有尝试过一些太过疯狂的东西.制作这种复杂的东西所需的工具可能相当昂贵."

# game/scripts/bar.rpy:5850
translate schinese moondate_50376d38:

    # moon "Buuuut... if I did have such tools, I'm sure I could make something crazy delicious! I could even make them addictive."
    moon "但是......如果我有那些工具的话,我肯定可以做出一些很疯狂的美味佳肴!我甚至可以让别人上瘾."

# game/scripts/bar.rpy:5851
translate schinese moondate_e35a8781:

    # c "You can make drugs?"
    c "你会制毒?"

# game/scripts/bar.rpy:5853
translate schinese moondate_96ccce8f:

    # moon "No no, don't get me wrong, never illegal drugs!"
    moon "不是不是,别误会,我这不是非法毒品!"

# game/scripts/bar.rpy:5854
translate schinese moondate_9b851947:

    # moon "I can just use the same stuff they put in android orgasms. It's edible, not phsyically addictive and can be easily cured with another drug, it's fine! There's not that many side effects!"
    moon "我可以用那些和他们用于智能机器人高潮时的那些东西.它可使用,不会上瘾,即使上瘾也很容易用药物治好,这很好!还没有那么多的副作用!"

# game/scripts/bar.rpy:5855
translate schinese moondate_f63b7434:

    # c "I'll have to think about that."
    c "我得考虑考虑."

# game/scripts/bar.rpy:5857
translate schinese moondate_d190978e:

    # moon "It's illegal if you don't mention how addictive the drink is by the way."
    moon "如果你不提这种饮料有成瘾性的话,那确实违法."

# game/scripts/bar.rpy:5858
translate schinese moondate_c80a6807:

    # c "How addictive can you make them?"
    c "你可以让它们有多大的成瘾性?"

# game/scripts/bar.rpy:5859
translate schinese moondate_9d4925e4:

    # moon "Very, hahah!"
    moon "很大,哈哈哈!"

# game/scripts/bar.rpy:5861
translate schinese moondate_40ba61d2:

    # c "What dangerous things can you make?"
    c "你可以做什么危险的东西吗?"

# game/scripts/bar.rpy:5862
translate schinese moondate_97f0978a:

    # moon "I can make a variety of explosive materials!"
    moon "我可以制造各种爆炸材料!"

# game/scripts/bar.rpy:5863
translate schinese moondate_4d44bf69:

    # c "Can you make someone go to sleep?"
    c "你可以让人入睡吗?"

# game/scripts/bar.rpy:5865
translate schinese moondate_d2612575:

    # moon "W-Why are you asking?"
    moon "你-你问这个干什么?"

# game/scripts/bar.rpy:5866
translate schinese moondate_dd49ee01:

    # c "Morbid curiosity."
    c "病态的好奇心."

# game/scripts/bar.rpy:5868
translate schinese moondate_3a751d1d:

    # moon "I wouldn't need drugs to make someone go to sleep anyway, so don't get any ideas."
    moon "反正我不需要药物来让人入睡,所以你还是别想了."

# game/scripts/bar.rpy:5869
translate schinese moondate_3bb327cc:

    # c "I'm just curious."
    c "我只是好奇."

# game/scripts/bar.rpy:5871
translate schinese moondate_7ab4c8ee:

    # moon "Hahah, I'm just pulling your leg! Yes, I could probably synthesize some harder drugs, but I never will. They're not good for anything."
    moon "哈哈哈,我只是在开个玩笑.是的,我也许可以合成一些更复杂的药物,但是我永远都不会这样做.它们对任何事情都没有好处."

# game/scripts/bar.rpy:5873
translate schinese moondate_e5079bb2:

    # c "What are you studying for your engineering classes?"
    c "你在工程课上学什么?"

# game/scripts/bar.rpy:5874
translate schinese moondate_af489dcd:

    # moon "Oh, I'm majoring in civil engineering. My favorite subjects are bridges, towers, and airports. Basically anything that involves me climbing to great heights!"
    moon "哦,我主修土木工程.我最喜欢的项目是桥梁,塔楼和机场.基本上都是涉及到高度的东西."

# game/scripts/bar.rpy:5875
translate schinese moondate_2467493b:

    # c "Because it reminds you of climbing rocks and trees as a child?"
    c "因为这让你想起了小时候爬石头和爬树?"

# game/scripts/bar.rpy:5877
translate schinese moondate_30b983ea:

    # moon "You were listening!"
    moon "你有听我的话!"

# game/scripts/bar.rpy:5879
translate schinese moondate_d6c911ea:

    # moon "It all reminds me of simpler times, and I'm pretty great at climbing anyway."
    moon "这一切让我想起了我的小时候,而且我在攀爬方面也很厉害."

# game/scripts/bar.rpy:5881
translate schinese moondate_03808035:

    # moon "I have a question for you now. If it is true you are interested in me as more than just an object, that would mean I would have to integrate into your harem, yes?"
    moon "我现在有一个问题要问你,如果你是真的对我感兴趣,而不是光喜欢我的外表,那就是说我必须加入你的后宫,对吗?"

# game/scripts/bar.rpy:5882
translate schinese moondate_5ba8c8fd:

    # moon "So the question must be asked, what kind of leader are you, [name]?"
    moon "所以我必须问你,你是什么样的人,[name]?"

# game/scripts/bar.rpy:5884
translate schinese moondate_1540012c:

    # moon "So the question must be asked, what kind of leader are you, [name]?" nointeract
    moon "所以我必须问你,你是什么样的人,[name]?" nointeract

# game/scripts/bar.rpy:5886
translate schinese moondate_26e6311d:

    # moon "Ah, how very stereotypical."
    moon "啊,真够刻板印象的."

# game/scripts/bar.rpy:5887
translate schinese moondate_3efad275:

    # moon "There are many valuable aspects of a traditional relationship, but I'm not the type of girl to serve a man's every beck and call."
    moon "传统关系有许多有价值的地方,但我不少那种男人一叫我就得去服务的那种女孩."

# game/scripts/bar.rpy:5888
translate schinese moondate_2e4777b8:

    # moon "To be honest, I never expected full equality in a harem anyway. So long as I'm not your slave-girl and have my own freedoms, I don't mind!"
    moon "说实在的,无论如何,我也从未想过能在后宫里可以完全平等.只要我不是你的女奴,有我自己的自由,我就不介意!"

# game/scripts/bar.rpy:5891
translate schinese moondate_1806d8d2:

    # moon "What!? I hope you aren't hurting anyone!"
    moon "什么!?我希望你没伤害别人!"

# game/scripts/bar.rpy:5892
translate schinese moondate_bf4832b6:

    # c "Only when they like it."
    c "只有她们喜欢才会."

# game/scripts/bar.rpy:5894
translate schinese moondate_994736c9:

    # moon "Oh, so you're that kind of rough, huh? Well don't expect me to go down without a fight!"
    moon "哦,所以你是那种粗暴的人,嗯?好吧,那可别指望我不战而降!"

# game/scripts/bar.rpy:5897
translate schinese moondate_84c04637:

    # moon "How lovely, that is not something I expected of the head of a harem. It's quite unique where I come from."
    moon "真可爱,这不是我印象中的后宫之主的做法.在我的家乡里,这是很特别的."

# game/scripts/bar.rpy:5898
translate schinese moondate_6f2bfd6e:

    # c "What is it usually like?"
    c "一般是什么样的?"

# game/scripts/bar.rpy:5899
translate schinese moondate_07e6300f:

    # moon "Things have changed since I was a young girl. When I was growing up, I usually saw 1 man with 3 to 7 women, and they were barely more than slaves to be honest with you, hahah!"
    moon "自从我还年轻的时候事情就已经改变了.在我成长的过程中,我经常看到一个男人带着三个到七个女人,说实在的,她们的数量都快赶上奴隶了,哈哈!"

# game/scripts/bar.rpy:5900
translate schinese moondate_3aecfa0a:

    # moon "I read an article a couple weeks ago that said harems have halved in size in the last two decades. They say it's because of rapid development."
    moon "我几周前读了一篇文章,说后宫在过去20年里规模减少了很多.他们说这是因为经济的快速发展."

# game/scripts/bar.rpy:5904
translate schinese moondate_c974ec56:

    # moon "Oh yeah? I bet I could snap your cock with my ass!!"
    moon "哦,真的吗?那我会用我的屁股折断你的肉棒!!"

# game/scripts/bar.rpy:5906
translate schinese moondate_d9c09749:

    # moon "{size=-10}{i}Oh shit... everyone definitely heard that!{/i}{/size}"
    moon "{size=-10}{i}哦操了......大家肯定都听到了!{/i}{/size}"

# game/scripts/bar.rpy:5907
translate schinese moondate_d463a99a:

    # moon "Are you done with your food? I think this is a good time to go now...!"
    moon "你吃完了吗?我想现在可以走了......!"

# game/scripts/bar.rpy:5908
translate schinese moondate_7160acc1:

    # c "Sure, haha."
    c "当然可以,哈哈."

# game/scripts/bar.rpy:5910
translate schinese moondate_34a31b59:

    # level "Your relationship with Moon has grown!"
    level "你和文的关系等级提升了!"

# game/scripts/bar.rpy:5914
translate schinese moondate_cb95de8d:

    # moon "This date was lovely, [name], I hope to spend more time with you in the future but I should really get back to studying! My roommate is probably pissed at me... Shall we go home?"
    moon "这次约会真不错,[name],我希望我以后能多和你相处,但是我真的应该回去学习了!我的室友可能对我很生气......我们回家吧?"

# game/scripts/bar.rpy:5915
translate schinese moondate_c48dc32f:

    # c "Sure."
    c "当然可以."

# game/scripts/bar.rpy:5917
translate schinese moondate_34a31b59_1:

    # level "Your relationship with Moon has grown!"
    level "你和文的关系等级提升了!"

# game/scripts/bar.rpy:5927
translate schinese moonfuck_735b8578:

    # moon "You!" with ssshake
    moon "你!" with ssshake

# game/scripts/bar.rpy:5928
translate schinese moonfuck_61731d2a:

    # c "What? Why are you in my room?"
    c "什么?你在我房间里干什么?"

# game/scripts/bar.rpy:5929
translate schinese moonfuck_5ab4c44e:

    # moon "I can't believe you! I can't believe you!"
    moon "我不能相信你!我不能相信你!"

# game/scripts/bar.rpy:5930
translate schinese moonfuck_f80627b2:

    # c "What's going on?"
    c "怎么了?"

# game/scripts/bar.rpy:5932
translate schinese moonfuck_93f4d377:

    # moon "What's going on!?"
    moon "怎么了!?"

# game/scripts/bar.rpy:5934
translate schinese moonfuck_88ebeb21:

    # moon "You spank me for days! You get me wet! You stare at my bare ass!"
    moon "你打了我好几天的屁股!你都把我弄湿了!你还一直盯着我的光屁股!"

# game/scripts/bar.rpy:5935
translate schinese moonfuck_45ad80fb:

    # c "Oh, uh-"
    c "哦,呃-"

# game/scripts/bar.rpy:5937
translate schinese moonfuck_6f383a4d:

    # moon "You even rubbed my pussy! You weren't supposed to do that, remember?"
    moon "你甚至还揉了我的小穴!你不应该那么做的,记得吗?"

# game/scripts/bar.rpy:5938
translate schinese moonfuck_9bb0350c:

    # c "Well-"
    c "嗯-"

# game/scripts/bar.rpy:5939
translate schinese moonfuck_58d33a6a:

    # moon "At first it was fine, it was just a punishment after all..."
    moon "一开始还好,毕竟这是一种惩罚......"

# game/scripts/bar.rpy:5941
translate schinese moonfuck_bc1adccc:

    # moon "But then you got me wet!" with ssshake
    moon "但是之后你让我湿了!" with ssshake

# game/scripts/bar.rpy:5943
translate schinese moonfuck_4a3e7a4f:

    # moon "You got me horny! Do you have an excuse for that!?"
    moon "你让我欲火焚身!你还有什么借口吗!?"

# game/scripts/bar.rpy:5944
translate schinese moonfuck_5681801e:

    # c "I-"
    c "我-"

# game/scripts/bar.rpy:5945
translate schinese moonfuck_e5372c18:

    # moon "Do you know how many panties I went through!?"
    moon "你知道我换了多少条内裤吗!?"

# game/scripts/bar.rpy:5947
translate schinese moonfuck_c2989c97:

    # moon "I stopped coming to work with them because I would just ruin them!" with ssshake
    moon "我不再在工作的时候穿它们是因为我会毁了它们!" with ssshake

# game/scripts/bar.rpy:5949
translate schinese moonfuck_afeccdc7:

    # moon "I saw your big bulge in your pants, don't act like you didn't want to either! You were clearly attracted to my ass but you never wanted to go further!"
    moon "我看到了你裤子里鼓起来的东西了,别装的好像你也不想!你显然被我的屁股吸引了,但是你从来没有想要过进一步发展!"

# game/scripts/bar.rpy:5950
translate schinese moonfuck_dd15bdbc:

    # c "Uh-"
    c "呃-"

# game/scripts/bar.rpy:5951
translate schinese moonfuck_1839a633:

    # moon "Do you know how many times I went home to finish myself off because you didn't have the balls to go further!?"
    moon "你知道有多少次因为你没有胆量再进一步,我回到家里得自己解决吗!?"

# game/scripts/bar.rpy:5953
translate schinese moonfuck_16d9651d:

    # moon "I had to start spanking {i}myself{/i} to get anywhere..."
    moon "我甚至不得不开始打我{i}自己{/i}的屁股......"

# game/scripts/bar.rpy:5955
translate schinese moonfuck_384581d2:

    # moon "My roommate caught me, and it's your fault!" with ssshake
    moon "我的室友抓到了我,这都是你的错!" with ssshake

# game/scripts/bar.rpy:5957
translate schinese moonfuck_00055609:

    # moon "I don't know what's worse!"
    moon "我不知道还有什么更糟糕的了!"

# game/scripts/bar.rpy:5959
translate schinese moonfuck_8b85e388:

    # moon "The fact that I let this happen..."
    moon "我让这一切发生了,这确实是事实......"

# game/scripts/bar.rpy:5961
translate schinese moonfuck_63ff2cbd:

    # moon "OR THE FACT YOU NEVER FINISHED ME OFF!" with ssshake
    moon "但是你也没有再进一步,这也是事实!" with ssshake

# game/scripts/bar.rpy:5963
translate schinese moonfuck_3a1e5daf:

    # moon "The teasing... The edging... It's too much!"
    moon "挑逗......玩弄......太多了!"

# game/scripts/bar.rpy:5965
translate schinese moonfuck_55c018aa:

    # c "Moon, What are you doing?"
    c "文,你在干什么?"

# game/scripts/bar.rpy:5966
translate schinese moonfuck_3388f985:

    # moon "Surviving!"
    moon "求生!"

# game/scripts/bar.rpy:5968
translate schinese moonfuck_4c2588f8:

    # moon "Mmmmmf! Mm...!" with ssshake
    moon "嗯!嗯......!" with ssshake

# game/scripts/bar.rpy:5972
translate schinese moonfuck_4984c9d6:

    # moon "..."
    moon "......"

# game/scripts/bar.rpy:5973
translate schinese moonfuck_f0c2c2bb:

    # moon "...I kissed you..."
    moon "......我亲了你......"

# game/scripts/bar.rpy:5975
translate schinese moonfuck_fdac74e2:

    # moon "...I- I wore something special... Do you want to see it?"
    moon "......我-我穿了一些特殊的东西......你想看看吗?"

# game/scripts/bar.rpy:5976
translate schinese moonfuck_4e8c7062:

    # c "Of course."
    c "当然了."

# game/scripts/bar.rpy:5978
translate schinese moonfuck_4984c9d6_1:

    # moon "..."
    moon "......"

# game/scripts/bar.rpy:5980
translate schinese moonfuck_2bb00ef7:

    # moon "...You're sure? You want this too...?"
    moon "......你确定吗?你也想要......?"

# game/scripts/bar.rpy:5981
translate schinese moonfuck_3f262cee:

    # c "Absolutely!"
    c "当然了!"

# game/scripts/bar.rpy:5984
translate schinese moonfuck_cefb62b0:

    # moon "...What do you think...?"
    moon "......你觉得怎么样......?"

# game/scripts/bar.rpy:5987
translate schinese moonfuck_45ea0c48:

    # c "Wow..."
    c "哇哦......"

# game/scripts/bar.rpy:5989
translate schinese moonfuck_f4aa94cc:

    # moon "Hahah... It's been a while for me... I thought this lingerie may have been a stupid idea. It's my first time wearing a bra, so..."
    moon "哈哈哈......我有这个想法已经有一段时间了......我觉得穿内衣是个愚蠢的想法.这是我第一次穿胸罩,所以......"

# game/scripts/bar.rpy:5990
translate schinese moonfuck_5c9b4066:

    # moon "But I guess it was worth it..."
    moon "但是我想这值得......"

# game/scripts/bar.rpy:5991
translate schinese moonfuck_3778ee2d:

    # moon "Where are your condoms?"
    moon "你的避孕套呢?"

# game/scripts/bar.rpy:5992
translate schinese moonfuck_8cb710d7:

    # c "I don't have any."
    c "我没有."

# game/scripts/bar.rpy:5994
translate schinese moonfuck_f498d30b:

    # moon "What?"
    moon "什么?"

# game/scripts/bar.rpy:5995
translate schinese moonfuck_7e390e8e:

    # moon "But... I didn't bring any either! Someone else has to have some, right? Maybe Maria?"
    moon "但是......我也没有带啊!别人肯定有,对吧?玛丽亚也许有?"

# game/scripts/bar.rpy:5996
translate schinese moonfuck_c846993a:

    # c "Calm down, we don't need one."
    c "冷静点,我们不需要."

# game/scripts/bar.rpy:5998
translate schinese moonfuck_49ad8350:

    # moon "Nnnff..."
    moon "嗯......"

# game/scripts/bar.rpy:6000
translate schinese moonfuck_aa184e35:

    # moon "...Fine. We can do it raw... But I'm not on birth control."
    moon "......好吧.我们可以无套......但是我现在可没有做避孕措施."

# game/scripts/bar.rpy:6001
translate schinese moonfuck_2b711de9:

    # moon "Are you ready?"
    moon "你准备好了吗?"

# game/scripts/bar.rpy:6002
translate schinese moonfuck_4aabd39f:

    # c "Take off your lingerie first."
    c "把你的内衣脱了先."

# game/scripts/bar.rpy:6004
translate schinese moonfuck_75bdd465:

    # moon "But I bought them for just this reason!"
    moon "但是我买它们就是为了这个的!"

# game/scripts/bar.rpy:6005
translate schinese moonfuck_e32348dc:

    # c "You buy them so you can take them off."
    c "你买了它们就是为了脱掉的."

# game/scripts/bar.rpy:6006
translate schinese moonfuck_0d92dad6:

    # moon "Hm, fine."
    moon "嗯,好吧."

# game/scripts/bar.rpy:6009
translate schinese moonfuck_803af1b5:

    # moon "Am I small, or are you big...?"
    moon "是我太小了,还是你太大了......?"

# game/scripts/bar.rpy:6010
translate schinese moonfuck_e2554a0d:

    # c "A bit of both."
    c "都有吧."

# game/scripts/bar.rpy:6011
translate schinese moonfuck_cbf3a57c:

    # moon "...That's quite scary, you know?"
    moon "......那还挺吓人的,你知道吗?"

# game/scripts/bar.rpy:6013
translate schinese moonfuck_c9b3115c:

    # moon "Don't move, okay? It's been years for me..."
    moon "别动,好吗?我好几年没这么做了......"

# game/scripts/bar.rpy:6015
translate schinese moonfuck_798fca6b:

    # moon "Hnnnnn...!"
    moon "嗯......!"

# game/scripts/bar.rpy:6017
translate schinese moonfuck_69cef320:

    # moon "Ehh...! The tip is in..."
    moon "呃......!前面进来了......"

# game/scripts/bar.rpy:6019
translate schinese moonfuck_bf94e1ec:

    # moon "Ahhh...!"
    moon "啊......!"

# game/scripts/bar.rpy:6021
translate schinese moonfuck_5ed5d25f:

    # moon "Hnnnnn...!!"
    moon "嗯......!!"

# game/scripts/bar.rpy:6023
translate schinese moonfuck_dbf4d9be:

    # moon "HNNGAHH!" with sshake
    moon "嗯!" with sshake

# game/scripts/bar.rpy:6025
translate schinese moonfuck_42d61b2a:

    # moon "Hahhh...! I came a little... Ah!"
    moon "哈......!我小高潮了一下......啊!"

# game/scripts/bar.rpy:6027
translate schinese moonfuck_fb4e25fa:

    # moon "Ahhhn~"
    moon "啊~"

# game/scripts/bar.rpy:6028
translate schinese moonfuck_f14abb98:

    # moon "Ohhh my god! Finally! Fuck! MM!"
    moon "哦我天!终于!操!嗯!"

# game/scripts/bar.rpy:6029
translate schinese moonfuck_dc5b8746:

    # moon "That feels so good...! Hnngh, I needed this so much!"
    moon "感觉好舒服......!嗯,我好需要它!"

# game/scripts/bar.rpy:6030
translate schinese moonfuck_bb8262a3:

    # moon "I... I had forgotten the feeling of cock... Hnn!"
    moon "我......我已经忘记了肉棒的感觉了......嗯!"

# game/scripts/bar.rpy:6033
translate schinese moonfuck_863ffa1c:

    # moon "Hahhhhn!!! HNN!" with ssshake
    moon "哈!!!嗯!" with ssshake

# game/scripts/bar.rpy:6034
translate schinese moonfuck_cc8761c1:

    # moon "Ssssnn! AH! I came again...! HNGUH!"
    moon "嘶!啊!我又去了......!嗯!"

# game/scripts/bar.rpy:6035
translate schinese moonfuck_322d9799:

    # moon "I don't know why... I don't understand why your spanking did this to me..."
    moon "我不知道为什么......我不理解为什么你打我屁股会有这种影响......"

# game/scripts/bar.rpy:6036
translate schinese moonfuck_05188f76:

    # moon "HN! M-Maybe it was the humiliation... Or the exhibition... Or the pain... I don't know... But I love it...!"
    moon "嗯!也-也许是因为羞耻......或者说暴露......或者是疼痛......我不知道......但是我喜欢它......!"

# game/scripts/bar.rpy:6040
translate schinese moonfuck_56fdf3c7:

    # moon "Ah! What are you doing...!?" with ssshake
    moon "啊!你在干什么......!?" with ssshake

# game/scripts/bar.rpy:6041
translate schinese moonfuck_66d2e02f:

    # moon "Hnnnnnuh...! You're going too fast!"
    moon "嗯......!你太快了!"

# game/scripts/bar.rpy:6042
translate schinese moonfuck_add04def:

    # moon "Ahh! AHH! You're going to make me cum! Slow down!"
    moon "啊!啊!你要让我去了!慢点!"

# game/scripts/bar.rpy:6043
translate schinese moonfuck_d2dd4d97:

    # c "But don't you want to cum?"
    c "但是你不需要去吗?"

# game/scripts/bar.rpy:6044
translate schinese moonfuck_d03252e3:

    # moon "Hnnn! Yes, but...! I just came twice! NNNNAH!"
    moon "嗯!是的,但是......!我已经去两次了!嗯!"

# game/scripts/bar.rpy:6045
translate schinese moonfuck_1db00cbb:

    # moon "Hahhh! Ahhh! Ahh!! AHH!"
    moon "哈!啊!啊!!啊!"

# game/scripts/bar.rpy:6047
translate schinese moonfuck_84526f61:

    # moon "HNNNN!!! HNNNNNNNNN!!!" with ssshake
    moon "嗯!!!嗯!!!" with ssshake

# game/scripts/bar.rpy:6048
translate schinese moonfuck_3dc4c2ce:

    # moon "GAAAAHHH!!" with ssshake
    moon "咕!!" with ssshake

# game/scripts/bar.rpy:6050
translate schinese moonfuck_8789cec0:

    # moon "Ehhhhhh........." with ssshake
    moon "呃........." with ssshake

# game/scripts/bar.rpy:6051
translate schinese moonfuck_4bd3eb81:

    # moon "Hahhh... Hahh... I... I can't move... Ahhh..."
    moon "哈......哈......我......我动不了了......啊......"

# game/scripts/bar.rpy:6053
translate schinese moonfuck_98254a08:

    # moon "Nnnn... What are you doing?"
    moon "嗯......你在干什么?"

# game/scripts/bar.rpy:6054
translate schinese moonfuck_0de1bd1d:

    # c "Helping you move."
    c "帮你动."

# game/scripts/bar.rpy:6056
translate schinese moonfuck_44688b1e:

    # moon "...But this is... So personal..."
    moon "......但是这是......个人行为......"

# game/scripts/bar.rpy:6058
translate schinese moonfuck_0758f585:

    # moon "Mmmm!" with ssshake
    moon "嗯!" with ssshake

# game/scripts/bar.rpy:6060
translate schinese moonfuck_aa5949b4:

    # moon "Mmmmmmmmmmmm~"
    moon "嗯~"

# game/scripts/bar.rpy:6061
translate schinese moonfuck_a33f2b9f:

    # moon "Hmmm~ Hmmmm!!"
    moon "嗯~嗯!!"

# game/scripts/bar.rpy:6062
translate schinese moonfuck_7773e4cf:

    # moon "Mmmphh... Sho good...!"
    moon "嗯......好棒......!"

# game/scripts/bar.rpy:6065
translate schinese moonfuck_bc7927b7:

    # moon "Hahhmmmm! Mmmnn!"
    moon "哈!嗯!"

# game/scripts/bar.rpy:6066
translate schinese moonfuck_e53964ee:

    # moon "Cuh... Cum inshide...! Nnm!"
    moon "射......射进来......!嗯!"

# game/scripts/bar.rpy:6068
translate schinese moonfuck_ee96c65b:

    # moon "MMMM!" with ssshake
    moon "嗯!" with ssshake

# game/scripts/bar.rpy:6069
translate schinese moonfuck_697b036b:

    # moon "Hmmmm...!" with ssshake
    moon "嗯......!" with ssshake

# game/scripts/bar.rpy:6070
translate schinese moonfuck_9b6271d3:

    # moon "Mppphh..." with ssshake
    moon "嗯......" with ssshake

# game/scripts/bar.rpy:6072
translate schinese moonfuck_3b905c2d:

    # moon "Ahhhh... Nnnn... Hahhh..."
    moon "啊......嗯......哈......"

# game/scripts/bar.rpy:6073
translate schinese moonfuck_12512fea:

    # moon "You shouldn't have cum inside me..."
    moon "你不应该射进来的......"

# game/scripts/bar.rpy:6074
translate schinese moonfuck_4329a785:

    # c "You asked me to."
    c "你叫我的."

# game/scripts/bar.rpy:6075
translate schinese moonfuck_28a36296:

    # moon "You shouldn't have listened... Nnn..."
    moon "你不应该听的......嗯......"

# game/scripts/bar.rpy:6076
translate schinese moonfuck_62b3ca73:

    # c "Did you get a new hair clip?"
    c "你换新发夹了?"

# game/scripts/bar.rpy:6078
translate schinese moonfuck_268c8583:

    # moon "What...? Oh. Yeah..."
    moon "什么......?哦.是的......"

# game/scripts/bar.rpy:6079
translate schinese moonfuck_f543e862:

    # c "You should wear it more."
    c "你应该多戴戴的."

# game/scripts/bar.rpy:6081
translate schinese moonfuck_79662552:

    # moon "...Maybe I will..."
    moon "......也许我会的......"

# game/scripts/bar.rpy:6082
translate schinese moonfuck_378bbe00:

    # moon "Can I sleep here for a couple of minutes?"
    moon "我可以在这里睡几分钟吗?"

# game/scripts/bar.rpy:6083
translate schinese moonfuck_4e8c7062_1:

    # c "Of course."
    c "当然可以."

# game/scripts/bar.rpy:6087
translate schinese moonfuck_376f4188:

    # "You've become closer with Moon."
    "你和文更亲密了."

# game/scripts/bar.rpy:6088
translate schinese moonfuck_24c6ce3f:

    # "New scenes for Moon are available in the Bar."
    "酒吧中可以触发文的新场景."

# game/scripts/bar.rpy:6095
translate schinese moondrink_4984c9d6:

    # moon "..."
    moon "......"

# game/scripts/bar.rpy:6096
translate schinese moondrink_7ed74d8a:

    # c "Yes...?"
    c "怎么了......?"

# game/scripts/bar.rpy:6098
translate schinese moondrink_e344880c:

    # moon "What's your name?"
    moon "你叫什么名字?"

# game/scripts/bar.rpy:6101
translate schinese moondrink_b3af96aa:

    # moon "It's good to meet you [name]! Um, again."
    moon "很高兴认识你,[name]!呃,再次认识你."

# game/scripts/bar.rpy:6104
translate schinese moondrink_eb8d867e:

    # moon "Oh... That's funny..."
    moon "哦......有趣......"

# game/scripts/bar.rpy:6106
translate schinese moondrink_174e1c4c:

    # moon "But I don't know your name! What is it?"
    moon "但是我还是不知道你的名字!所以是什么?"

# game/scripts/bar.rpy:6107
translate schinese moondrink_0e5b3a10:

    # c "[name]."
    c "[name]."

# game/scripts/bar.rpy:6108
translate schinese moondrink_b3af96aa_1:

    # moon "It's good to meet you [name]! Um, again."
    moon "很高兴认识你,[name]!呃,再次认识你."

# game/scripts/bar.rpy:6110
translate schinese moondrink_5cd04ec2:

    # moon "What are you drinking?"
    moon "你在喝什么?"

# game/scripts/bar.rpy:6111
translate schinese moondrink_4e5a4202:

    # c "Why?"
    c "为什么问这个?"

# game/scripts/bar.rpy:6113
translate schinese moondrink_ab5b9ffa:

    # moon "I've never been a fan of beer. Tastes like gasoline to me..."
    moon "我不喜欢啤酒.对我来说,它们就像汽油一样......"

# game/scripts/bar.rpy:6114
translate schinese moondrink_7c58a589:

    # c "Wine then?"
    c "红酒呢?"

# game/scripts/bar.rpy:6116
translate schinese moondrink_a20cf4f4:

    # moon "Oh, no! I prefer Vodka."
    moon "哦,不!我更喜欢伏特加."

# game/scripts/bar.rpy:6117
translate schinese moondrink_4e24c2ca:

    # c "Really?"
    c "真的吗?"

# game/scripts/bar.rpy:6118
translate schinese moondrink_0c5d7ee3:

    # moon "Oh, think I can't handle it?"
    moon "哦,觉得我承受不了是吗?"

# game/scripts/bar.rpy:6119
translate schinese moondrink_ed1e847f:

    # c "Well..."
    c "嗯......"

# game/scripts/bar.rpy:6121
translate schinese moondrink_2162d3d6:

    # moon "I could drink you under the table..."
    moon "我可以把你喝倒......"

# game/scripts/bar.rpy:6122
translate schinese moondrink_bf235757:

    # c "Is that a challenge?"
    c "这是个挑战吗?"

# game/scripts/bar.rpy:6124
translate schinese moondrink_1f06d9bd:

    # moon "Hmmm... No. I wouldn't want to be charged with manslaughter..."
    moon "嗯......不是.我可不想被指控过失杀人......"

# game/scripts/bar.rpy:6125
translate schinese moondrink_fdc6bb4f:

    # c "Oh, you're funny..."
    c "哦,你可真有趣......"

# game/scripts/bar.rpy:6127
translate schinese moondrink_40586e75:

    # moon "Is that so?"
    moon "真的吗?"

# game/scripts/bar.rpy:6129
translate schinese moondrink_14b02ce3:

    # moon "I'm not known for my jokes, [name]."
    moon "我可不是以笑话出名的,[name]."

# game/scripts/bar.rpy:6130
translate schinese moondrink_8cb4ab5b:

    # c "What are you known for then?"
    c "那你是以什么出名的?"

# game/scripts/bar.rpy:6132
translate schinese moondrink_1779cfcb:

    # moon "My iron liver."
    moon "我的铁肝."

# game/scripts/bar.rpy:6133
translate schinese moondrink_94ad5704:

    # moon "I could drink raw ethanol and never get drunk!"
    moon "我可以直接喝无水乙醇,而且永远不会喝醉!"

# game/scripts/bar.rpy:6134
translate schinese moondrink_0d70e605:

    # moon "Well... That was just a joke. I couldn't really drink raw ethanol..."
    moon "嗯......那只是开个玩笑.我可不能喝无水乙醇......"

# game/scripts/bar.rpy:6136
translate schinese moondrink_7d2f3cd6:

    # moon "But I'm serious! It runs in the family. It takes ten times the normal amount of alcohol for me to get drunk!"
    moon "但是我是认真的!这是家族遗传的.我需要十倍于正常量的酒精才会喝醉!"

# game/scripts/bar.rpy:6137
translate schinese moondrink_1bbf35ff:

    # c "Not even a little tipsy?"
    c "在那之前甚至都不会醉?"

# game/scripts/bar.rpy:6138
translate schinese moondrink_95fe8000:

    # moon "Nope! No side effects at all! Impressive, right?"
    moon "不!而且没有副作用!这很令人印象深刻,对吧?"

# game/scripts/bar.rpy:6139
translate schinese moondrink_6efe446b:

    # c "If it's true, then... Yeah."
    c "如果那是真的的话,那......确实."

# game/scripts/bar.rpy:6141
translate schinese moondrink_22122b2f:

    # moon "Is {i}that{/i} a challenge?"
    moon "{i}这{/i}是挑战吗?"

# game/scripts/bar.rpy:6143
translate schinese moondrink_ba26ce8b:

    # moon "Just kidding!"
    moon "开玩笑的!"

# game/scripts/bar.rpy:6145
translate schinese moondrink_da615de7:

    # moon "I'll let you get back to your drink [name]."
    moon "我会让你继续喝你的酒的,[name]."

# game/scripts/bar.rpy:6147
translate schinese moondrink_450fcff4:

    # moon "Just make sure you don't drink yourself to death~!"
    moon "只要确保你不会把自己喝死就行~!"

# game/scripts/bar.rpy:6154
translate schinese moontrain1_b306f49f:

    # moon "Hello boss!"
    moon "你好老板!"

# game/scripts/bar.rpy:6155
translate schinese moontrain1_03592692:

    # c "Moon, we need to train you."
    c "文,我们需要训练你."

# game/scripts/bar.rpy:6157
translate schinese moontrain1_293c9d5f:

    # moon "What do you mean? Am I not doing good enough?"
    moon "你这是什么意思?我做的不够好吗?"

# game/scripts/bar.rpy:6158
translate schinese moontrain1_d059c348:

    # c "No, you're doing fine. But there's always room for improvement."
    c "不,你做得很好.但是事情总是有可以改进的地方嘛."

# game/scripts/bar.rpy:6160
translate schinese moontrain1_3ff065f5:

    # moon "Well, if you say so! What do I need to do?"
    moon "好吧,如果你这么说的话!我需要怎么做?"

# game/scripts/bar.rpy:6161
translate schinese moontrain1_fc4e1109:

    # c "Today I'll be teaching you how to always listen."
    c "今天我会教你怎么耳听八方."

# game/scripts/bar.rpy:6162
translate schinese moontrain1_7abc8828:

    # moon "I always listen, Mr. [name]..."
    moon "我一直都在听着,[name]先生......"

# game/scripts/bar.rpy:6163
translate schinese moontrain1_a50c63eb:

    # c "Do you?"
    c "真的吗?"

# game/scripts/bar.rpy:6164
translate schinese moontrain1_b07595e8:

    # moon "Of course!"
    moon "当然了!"

# game/scripts/bar.rpy:6165
translate schinese moontrain1_c2792e65:

    # c "So you haven't heard that guy asking for another beer?"
    c "所以你没听到那个人要求再来一杯啤酒?"

# game/scripts/bar.rpy:6167
translate schinese moontrain1_91b4c325:

    # moon "What...?"
    moon "什么......?"

# game/scripts/bar.rpy:6168
translate schinese moontrain1_e1af66dc:

    # pat "Hello!? Beer please!"
    pat "喂!?请来一杯啤酒!"

# game/scripts/bar.rpy:6170
translate schinese moontrain1_f4d8d188:

    # moon "A-Ahh... Sorry boss... I guess I was paying too much attention to our conversation..."
    moon "啊-啊......对不起老板......我想我太注意我们的谈话了......"

# game/scripts/bar.rpy:6171
translate schinese moontrain1_915077e6:

    # c "Don't worry about me, get him a beer. And tell him it's on the house."
    c "别担心我,去给他拿杯啤酒吧,然后告诉他我请客."

# game/scripts/bar.rpy:6172
translate schinese moontrain1_346f6971:

    # moon "R-Right... Sorry..."
    moon "好-好的......对不起......"

# game/scripts/bar.rpy:6174
translate schinese moontrain1_828a7d7b:

    # moon "I'm so sorry for the wait sir! Here's your beer!"
    moon "对不起让你久等了,先生!这是你的啤酒!"

# game/scripts/bar.rpy:6175
translate schinese moontrain1_97746990:

    # moon "Oh, and it's on the house as well!"
    moon "哦,还有这杯免单!"

# game/scripts/bar.rpy:6176
translate schinese moontrain1_844f6a17:

    # pat "Oh, thank you very much!"
    pat "哦,谢谢你!"

# game/scripts/bar.rpy:6177
translate schinese moontrain1_37af331d:

    # moon "Enjoy!"
    moon "请享用!"

# game/scripts/bar.rpy:6179
translate schinese moontrain1_95dd568b:

    # moon "I'm very sorry boss! I hope you won't fire me!"
    moon "我很抱歉,老板!我希望你不会开除我!"

# game/scripts/bar.rpy:6180
translate schinese moontrain1_7dc38413:

    # c "It's fine, that's what training is for."
    c "没关系的,这就是训练的目的."

# game/scripts/bar.rpy:6181
translate schinese moontrain1_e458f82a:

    # c "Now, I'm going to sit around here and watch your performance for a couple of hours."
    c "现在,我要坐在这里,看你接下来几个小时的表现."

# game/scripts/bar.rpy:6182
translate schinese moontrain1_b07595e8_1:

    # moon "Of course!"
    moon "好的!"

# game/scripts/bar.rpy:6184
translate schinese moontrain1_73be9c93:

    # "You stand in a corner trying your best not to draw any attention from Moon, or the patrons. Being able to see the whole room, you keep an eye on Moon's performance, you notice whenever she talks, she seems to keep her ears closed..."
    "你站在一个角落里,尽量不引起文或者顾客的注意.由于你可以看到整个房间,你一直都在关注着文的表现,你注意到每当她说话的时候,她的耳朵似乎就像是被关上了一样......"

# game/scripts/bar.rpy:6185
translate schinese moontrain1_9f118427:

    # "You inform her of this imperfection and she tries her best to keep her ears always on, while still providing the humbling feeling of small talk with the patrons."
    "你向她告知了这一不完善的地方,然后她就尽力保持她的耳朵一直在倾听,同时依然在和顾客交谈时保持谦卑."

# game/scripts/bar.rpy:6186
translate schinese moontrain1_0b0e22a8:

    # "After a couple of hours, she gets better and better. Always making a mental note of the orders each patron calls for, even when chatting."
    "几小时后,她变得越来越好了.她可以在头脑里记下每个顾客要求的订单,甚至在聊天的时候也是如此."

# game/scripts/bar.rpy:6187
translate schinese moontrain1_3461c4fe:

    # "As even more time passes, she's easily able to tell where the orders are coming up."
    "随着时间的推移,她很容易就可以分辨出订单在哪里."

# game/scripts/bar.rpy:6188
translate schinese moontrain1_387b8b36:

    # "Moon is now able to always listen, picking up on even the faintest of voices, and their direction. Even if she does trip sometimes..."
    "文现在可以耳听八方了,即使是最微弱的声音,也能捕捉到它们的方向.虽然她有时候会被绊倒就是了......"

# game/scripts/bar.rpy:6190
translate schinese moontrain1_e63c89d5:

    # c "Good job, Moon! I think you've improved quite a lot."
    c "干得好,文!我觉得你进步很大."

# game/scripts/bar.rpy:6191
translate schinese moontrain1_c1678d0b:

    # c "The more you listen, the faster everyone can get their order. This means they'll be likely to order even more now."
    c "你听得越多,大家就能越快获得他们的订单.这也意味着他们现在会有可能买更多的东西."

# game/scripts/bar.rpy:6192
translate schinese moontrain1_dd953866:

    # moon "Thank you boss! I'm glad you see my improvement!"
    moon "谢谢你老板!我很高兴你看到了我的进步!"

# game/scripts/bar.rpy:6193
translate schinese moontrain1_c06a2ba4:

    # level "Moon has become a better waitress!"
    level "文成为了更好的服务员了!"

# game/scripts/bar.rpy:6200
translate schinese moontrain2_b306f49f:

    # moon "Hello boss!"
    moon "你好老板!"

# game/scripts/bar.rpy:6201
translate schinese moontrain2_03592692:

    # c "Moon, we need to train you."
    c "文,我们需要训练你."

# game/scripts/bar.rpy:6202
translate schinese moontrain2_7520a63b:

    # moon "Oh, again? What will we be doing this time?"
    moon "哦,又要吗?这次是什么?"

# game/scripts/bar.rpy:6203
translate schinese moontrain2_4e301bb3:

    # c "Today I'll be teaching you how to always look."
    c "今天我要教你如何眼观六路."

# game/scripts/bar.rpy:6204
translate schinese moontrain2_69ec063c:

    # moon "Always look? Do you want me to grow a third eye? Haha!"
    moon "眼观六路?你想让我长出第三只眼吗?哈哈!"

# game/scripts/bar.rpy:6205
translate schinese moontrain2_11b2546d:

    # c "You're very clumsy, Moon."
    c "你很笨拙,文."

# game/scripts/bar.rpy:6207
translate schinese moontrain2_73974658:

    # moon "Clumsy? I'm not clumsy!"
    moon "笨拙?我可不笨拙!"

# game/scripts/bar.rpy:6208
translate schinese moontrain2_4d17a7c5:

    # c "That's not what my reports say..."
    c "我的报告可不是这么说的......"

# game/scripts/bar.rpy:6210
translate schinese moontrain2_b91b95fd:

    # moon "Ah, I guess it's unavoidable then... I'm sorry, Mr. [name]... These heels are really awkward to walk in..."
    moon "啊,我想那是不可避免的......我很抱歉,[name]先生......这双高跟鞋穿起来真的很别扭......"

# game/scripts/bar.rpy:6212
translate schinese moontrain2_6b260d56:

    # moon "Okay boss! What do you need me to do?"
    moon "好的老板!你需要我做什么?"

# game/scripts/bar.rpy:6213
translate schinese moontrain2_6342e0a2:

    # c "If your heels are the problem, it's a pretty simple fix. Take small steps, and land heel-to-toe."
    c "如果你的鞋跟就是问题所在的话,那很容易.小步小步走,先落脚跟再落脚掌."

# game/scripts/bar.rpy:6214
translate schinese moontrain2_55e0b418:

    # moon "Small steps? But wouldn't that slow me down?"
    moon "小步走?但是这样不会让我慢下来吗?"

# game/scripts/bar.rpy:6215
translate schinese moontrain2_814bf329:

    # c "One problem at a time. Go ahead and serve our patrons while I watch."
    c "一次只解决一个问题.去吧,去给我们的客人们服务,我会看着的."

# game/scripts/bar.rpy:6216
translate schinese moontrain2_c1710669:

    # moon "Yes sir!"
    moon "是的先生!"

# game/scripts/bar.rpy:6218
translate schinese moontrain2_2bd21fed:

    # "You watch as Moon goes about serving guests. From time to time she almost trips, but she's giving her strut extra attention now that you're overseeing her."
    "你看着文去服务客人.她时不时会差点被绊倒,但现在在你的监督下,她对自己的步态格外关注."

# game/scripts/bar.rpy:6220
translate schinese moontrain2_1663211f:

    # moon "Wah!"
    moon "喔!"

# game/scripts/bar.rpy:6222
translate schinese moontrain2_7c883d5b:

    # moon "Nnnguh!"
    moon "嗯!"

# game/scripts/bar.rpy:6224
translate schinese moontrain2_1dc9af3f:

    # moon "Oh! Oh no! No, no! Urgh!"
    moon "哦!哦不!不,不!嗯!"

# game/scripts/bar.rpy:6226
translate schinese moontrain2_ddec832a:

    # moon "Nnnn! Curse these stupid heels! Ohhh..."
    moon "嗯!傻逼高跟鞋!哦......"

# game/scripts/bar.rpy:6228
translate schinese moontrain2_3fc15ae0:

    # c "Are you okay?"
    c "你还好吗?"

# game/scripts/bar.rpy:6229
translate schinese moontrain2_657332c1:

    # moon "Yes, I'm fine.. But... I'm sorry boss..."
    moon "是的,我还好......但是......对不起,老板......"

# game/scripts/bar.rpy:6230
translate schinese moontrain2_9fe82e58:

    # c "We can always buy some more glass. Are you sure you're not hurt? Any bleeding?"
    c "玻璃杯我们可以再买的.你确定你没有受伤吗?有流血吗?"

# game/scripts/bar.rpy:6232
translate schinese moontrain2_55762caa:

    # moon "Y-Yes, I'm fine, thank you."
    moon "是-是的,我没事,谢谢你."

# game/scripts/bar.rpy:6233
translate schinese moontrain2_02dc4aeb:

    # c "Alright, keep trying, you'll improve."
    c "好的,继续试试吧,你会进步的."

# game/scripts/bar.rpy:6234
translate schinese moontrain2_2b8eac25:

    # moon "Right!"
    moon "好的!"

# game/scripts/bar.rpy:6236
translate schinese moontrain2_4bb58898:

    # "As time goes on, Moon's strut becomes more determined. She's growing more confident in walking in her heels. She rarely fumbles, and never trips."
    "随着时间的推移,文的步伐变得越来越稳定.她对穿着高跟鞋走路也越来越有信心.她失误已经很少了,而且没被绊倒过."

# game/scripts/bar.rpy:6238
translate schinese moontrain2_8f62a416:

    # c "Good job, Moon! You've improved a lot. Now that you're not breaking as many glasses, we'll save some money."
    c "干得好,文!你进步很大.现在你没有打碎那么多杯子了,我们可以省下一笔钱了."

# game/scripts/bar.rpy:6239
translate schinese moontrain2_8c1e2ea0:

    # moon "Thank you Mr. [name]!"
    moon "谢谢你[name]先生!"

# game/scripts/bar.rpy:6240
translate schinese moontrain2_c06a2ba4:

    # level "Moon has become a better waitress!"
    level "文成为了更好的服务员了!"

# game/scripts/bar.rpy:6247
translate schinese moontrain3_b306f49f:

    # moon "Hello boss!"
    moon "你好老板!"

# game/scripts/bar.rpy:6248
translate schinese moontrain3_fe30ae0c:

    # c "Today we're going to train you in speed."
    c "今天我们要训练你的速度."

# game/scripts/bar.rpy:6249
translate schinese moontrain3_1fe5482f:

    # moon "Does that mean I'll be able to take off these heels? Hahaha!"
    moon "这是不是意味着我可以脱掉这些高跟鞋了?哈哈哈!"

# game/scripts/bar.rpy:6250
translate schinese moontrain3_c8270a9e:

    # c "You'll be keeping the heels on, but you're going to need to learn how to efficiently serve each guest in a timely manner."
    c "你还得穿着高跟鞋,但是你要学会如何及时有效地给每个客人服务."

# game/scripts/bar.rpy:6252
translate schinese moontrain3_036fe038:

    # moon "Ah... That's going to be very hard Mr. [name]! My last training even slowed me down!"
    moon "啊......那太难了,[name]先生!我上次的训练都让我变慢了!"

# game/scripts/bar.rpy:6253
translate schinese moontrain3_d4bb803a:

    # c "That's exactly why we need to make you fast again."
    c "这就是为什么我们需要让你再次变快."

# game/scripts/bar.rpy:6255
translate schinese moontrain3_6bd986e0:

    # moon "Well... I don't see how it's possible for me to not only be faster, but also not trip..."
    moon "嗯......我不明白我怎么做到又快又不被绊倒......"

# game/scripts/bar.rpy:6257
translate schinese moontrain3_d337e975:

    # moon "But I guess that's why you're here! So what do I need to do?"
    moon "但是我想这就是你在这里的原因!我需要怎么做?"

# game/scripts/bar.rpy:6259
translate schinese moontrain3_bbfd8aa6:

    # "You explain to Moon that she needs to use all of her previous training in this one. Moon needs to always listen, and always be prepared. She needs to be at every patron's beck and call with alcohol in hand."
    "你向文解释说,她需要再这一次使用她以前所有训练的来的技巧.文要眼观六路,耳听八方.她需要手拿酒水,听从每一位顾客的召唤."

# game/scripts/bar.rpy:6260
translate schinese moontrain3_d64b091d:

    # "You inform Moon that being faster doesn't necessarily pertain to walking. Remembering a regular's face and at which time they visit can increase efficiency!"
    "你告诉文,更快不一定和走路的速度有关.记住每一个常客的面孔和来访时间,也可以提高工作效率!"

# game/scripts/bar.rpy:6261
translate schinese moontrain3_05129739:

    # "Paying attention to each patron's liquid level is also very important. If Moon's foresight can predict when a patron is about to order another beer, she can have it made and ordered before they even ask!"
    "关注每个顾客还剩下多少酒水也是非常重要的.如果文能够预测顾客什么时候需要在点一杯啤酒,那她就可以在他们开口之前就把啤酒做好点好!"

# game/scripts/bar.rpy:6262
translate schinese moontrain3_f94c1b8f:

    # "Knowing this, Moon puts this all into practice..."
    "知道了这一点之后,文把这些都付诸实践了......"

# game/scripts/bar.rpy:6263
translate schinese moontrain3_0e2b5e92:

    # "You sit down and micromanage Moon. You tell her where and how to look, and to remember their faces and pair them with their drink of choice."
    "你坐下来,对文进行一些细节上的教学.你告诉她在哪里以及如何寻找顾客,并记住他们的面孔,将他们与他们选择的饮料配对."

# game/scripts/bar.rpy:6264
translate schinese moontrain3_97b1e1e0:

    # "Instead of sitting down and looking at her phone in her off time, she stands at the bar's counter, meticulously, and sometimes awkwardly, watching the room and it's inhabitants. Always presuming a patron's next order."
    "她并没有在休息时间坐下来看手机,而是站在吧台前,一丝不苟地,有时候有点尴尬地观察房间和房间里的人.推测顾客的下一份订单会是什么."

# game/scripts/bar.rpy:6265
translate schinese moontrain3_43c04b6c:

    # "After a couple of hours, she turns into clockwork. Never having downtime. When you know what to look for, you should always be doing something. Moon is becoming a well oiled machine!"
    "几个小时后,她已经变成了某种机械装置.没有休息时间.当你知道你要寻找什么的时候,你就应该做什么.文正在变成一台上好油的机器!"

# game/scripts/bar.rpy:6267
translate schinese moontrain3_bd841a3b:

    # moon "Phew! How did I do boss?"
    moon "呼!我干的怎么样,老板?"

# game/scripts/bar.rpy:6268
translate schinese moontrain3_7ca97faf:

    # c "You're doing great! I can tell already effeciency has improved."
    c "你做得很好!我看得出来效率已经提高了."

# game/scripts/bar.rpy:6269
translate schinese moontrain3_c4274da3:

    # moon "I'm glad! I had no idea I could be so busy!"
    moon "我很高兴!我不知道我居然可以这么忙!"

# game/scripts/bar.rpy:6270
translate schinese moontrain3_0cc8d6aa:

    # c "Don't wear yourself out too much. Make sure to drink plenty of water and eat often."
    c "别让自己太累了.多喝水,多吃饭."

# game/scripts/bar.rpy:6271
translate schinese moontrain3_de499a95:

    # moon "Of course! Thank you boss!"
    moon "当然了!谢谢你老板!"

# game/scripts/bar.rpy:6272
translate schinese moontrain3_c06a2ba4:

    # level "Moon has become a better waitress!"
    level "文成为了更好的服务员了!"

# game/scripts/bar.rpy:6279
translate schinese moontrain4_ed2c5967:

    # moon "Hello boss! More training?"
    moon "你好老板!更多的训练吗?"

# game/scripts/bar.rpy:6280
translate schinese moontrain4_16a78c3b:

    # c "No, not this time. I bought an outfit for you that I'd like you to wear from time to time."
    c "不,这次不是.我给你买了一套新衣服,我希望你可以经常穿它."

# game/scripts/bar.rpy:6282
translate schinese moontrain4_14267fae:

    # moon "Oh, no, this isn't one of those titty bars, is it...?"
    moon "哦,不,这不是那种色情酒吧的,对吧......?"

# game/scripts/bar.rpy:6283
translate schinese moontrain4_53d1ea34:

    # c "Not... Exactly, no."
    c "不......完全不是."

# game/scripts/bar.rpy:6285
translate schinese moontrain4_f191075d:

    # moon "Oh! So just an updated uniform then?"
    moon "哦!所以只是更新一下制服?"

# game/scripts/bar.rpy:6286
translate schinese moontrain4_e9cb16d5:

    # c "Not exactly that, either."
    c "也不太是."

# game/scripts/bar.rpy:6287
translate schinese moontrain4_f743dfcd:

    # c "Here, try it on."
    c "来,试试吧."

# game/scripts/bar.rpy:6288
translate schinese moontrain4_8f9045f2:

    # moon "Okay! Be right back!"
    moon "好的!马上回来!"

# game/scripts/bar.rpy:6291
translate schinese moontrain4_d453898b:

    # moon "So... This is the outfit...?"
    moon "所以......这就是新衣服了......?"

# game/scripts/bar.rpy:6294
translate schinese moontrain4_56d0beb3:

    # c "That's right."
    c "是的."

# game/scripts/bar.rpy:6296
translate schinese moontrain4_636fc93f:

    # moon "The skirt is really short, and it comes with cat ears and a cat tail extention..."
    moon "这个裙子好短,还有附带了猫尾巴和猫耳朵......"

# game/scripts/bar.rpy:6300
translate schinese moontrain4_febc06a6:

    # moon "I love it! It's so cute!"
    moon "我喜欢它!它好可爱!"

# game/scripts/bar.rpy:6301
translate schinese moontrain4_fb58f502:

    # c "I'm glad you like it."
    c "我很高兴你喜欢它."

# game/scripts/bar.rpy:6302
translate schinese moontrain4_96024190:

    # moon "Will I be wearing this every day?"
    moon "我每天会穿这套服装吗"

# game/scripts/bar.rpy:6303
translate schinese moontrain4_bda73689:

    # c "Only when I say so."
    c "只有我要求的时候才需要."

# game/scripts/bar.rpy:6304
translate schinese moontrain4_54cde968:

    # moon "Oh, okay."
    moon "哦,好吧."

# game/scripts/bar.rpy:6306
translate schinese moontrain4_9039322b:

    # moon "Wait... Why am I dressed up like a black cat? You're not some weird pervert with a cat fetish, are you...?"
    moon "等等......为什么我打扮得像只黑猫一样,你不会是有什么恋猫癖吧......?"

# game/scripts/bar.rpy:6307
translate schinese moontrain4_a4fc9d59:

    # c "The outfit is meant to draw attention to you."
    c "这身衣服是为了让你吸引别人的注意."

# game/scripts/bar.rpy:6308
translate schinese moontrain4_30fc9a67:

    # moon "Well, I'm sure you hit the nail on the head with this then..."
    moon "嗯,我相信你说到点子上了......"

# game/scripts/bar.rpy:6309
translate schinese moontrain4_ac097dc3:

    # c "Not only will all eyes be on you, but patrons will be so focused on you that they'll drink without thinking about it."
    c "不仅所有的眼睛都会盯着你,而且顾客也会专注于你,这会让他们不假思索地喝酒."

# game/scripts/bar.rpy:6311
translate schinese moontrain4_29fdda13:

    # moon "Oh! So they'll be so horny they'll get thirsty!"
    moon "哦!所以他们会非常饥'渴'!"

# game/scripts/bar.rpy:6313
translate schinese moontrain4_4973e701:

    # moon "Wait, wouldn't that just promote harassment?"
    moon "等等,这不会增加骚扰吗?"

# game/scripts/bar.rpy:6314
translate schinese moontrain4_a4ae8e73:

    # c "No, no. The hope is that they'll forget why they're here and start passively drinking without much thought. When they run out of a drink without properly enjoying it, they'll order another."
    c "不,不会.他们会忘记他们为什么会在这里,然后开始无意识地喝酒.当他们喝完一杯酒却没有好好享用它时,他们会再点一杯的."

# game/scripts/bar.rpy:6316
translate schinese moontrain4_2ffa94a4:

    # moon "I guess that makes sense. Not that I have to worry about harassment anyway..."
    moon "我想那确实有道理.反正我也用不着担心骚扰的问题......"

# game/scripts/bar.rpy:6317
translate schinese moontrain4_ba365199:

    # c "Well, there's not much more this time."
    c "好吧,这次没有别的了."

# game/scripts/bar.rpy:6318
translate schinese moontrain4_8486df5a:

    # moon "Oh, that's it? No 'Do better Ms. Moon!' ?"
    moon "哦,真的吗?没有'做得更好,文小姐!'了?"

# game/scripts/bar.rpy:6319
translate schinese moontrain4_5491d8ed:

    # c "Haha, not this time. Just keep up the good work!"
    c "哈哈,这次没有.继续努力吧!"

# game/scripts/bar.rpy:6320
translate schinese moontrain4_09c71317:

    # moon "Okay boss!"
    moon "好的老板!"

# game/scripts/bar.rpy:6321
translate schinese moontrain4_c06a2ba4:

    # level "Moon has become a better waitress!"
    level "文成为了更好的服务员了!"

# game/scripts/bar.rpy:6329
translate schinese moonintro_5796facf:

    # moon "Here's your drink, sir!"
    moon "这是你的饮料,先生!"

# game/scripts/bar.rpy:6330
translate schinese moonintro_98522154:

    # pat "Oh, thank you!"
    pat "哦,谢谢你!"

# game/scripts/bar.rpy:6331
translate schinese moonintro_1bc298e6:

    # moon "You're very welcome!"
    moon "不用谢!"

# game/scripts/bar.rpy:6332
translate schinese moonintro_7c78bd3d:

    # pat "Waitress!"
    pat "服务员!"

# game/scripts/bar.rpy:6334
translate schinese moonintro_6d9be936:

    # moon "Another drink, sir?"
    moon "再来一杯吗,先生?"

# game/scripts/bar.rpy:6335
translate schinese moonintro_181da72a:

    # pat "Actually, I- *hic* I was thinking..."
    pat "实际上,我-*打嗝*我在想......"

# game/scripts/bar.rpy:6337
translate schinese moonintro_e6efa8c2:

    # pat "I could get a little bit of you..."
    pat "我想点你......"

# game/scripts/bar.rpy:6339
translate schinese moonintro_30b03fe6:

    # pat "Waddya say? Hm? Come back t-"
    pat "你怎么说?嗯?去-"

# game/scripts/bar.rpy:6341
translate schinese moonintro_5f702acb:

    # moon "Why don't you..." with sshake
    moon "为什么你不......" with sshake

# game/scripts/bar.rpy:6343
translate schinese moonintro_79236622:

    # moon "Stick to drinks?" with sshake
    moon "继续喝你的酒呢?" with sshake

# game/scripts/bar.rpy:6345
translate schinese moonintro_d6ee3e53:

    # pat "AGH! What the..."
    pat "啊!什么......"

# game/scripts/bar.rpy:6346
translate schinese moonintro_9907d821:

    # moon "Sorry!"
    moon "对不起!"

# game/scripts/bar.rpy:6348
translate schinese moonintro_201b3a65:

    # moon "Are you hurt? Would you like another beer?"
    moon "你受伤了吗?你想要再来一杯吗?"

# game/scripts/bar.rpy:6349
translate schinese moonintro_009392c9:

    # pat "Wuh... What? No!"
    pat "什......什么?不!"

# game/scripts/bar.rpy:6351
translate schinese moonintro_9a62c278:

    # pat "Get away from me! Don't hurt me!"
    pat "离我远点!不要伤害我!"

# game/scripts/bar.rpy:6353
translate schinese moonintro_68f4bff9:

    # moon "Oh..."
    moon "哦......"

# game/scripts/bar.rpy:6355
translate schinese moonintro_0bdacce4:

    # moon "Come back soon!"
    moon "欢迎下次光临!"

translate schinese strings:

    # game/scripts/bar.rpy:272
    old "Finish"
    new "完成"

    # game/scripts/bar.rpy:1613
    old "Took it up the ass"
    new "被肛交过"

    # game/scripts/bar.rpy:1613
    old "Kissed the same sex"
    new "亲吻同性"

    # game/scripts/bar.rpy:1613
    old "Shoplifted"
    new "入店盗窃"

    # game/scripts/bar.rpy:1613
    old "Had a threesome"
    new "3P过"

    # game/scripts/bar.rpy:1613
    old "Peed on someone"
    new "尿别人身上"

    # game/scripts/bar.rpy:1613
    old "Been arrested"
    new "被逮捕"

    # game/scripts/bar.rpy:1613
    old "Had sex in public"
    new "在公共场合做爱"

    # game/scripts/bar.rpy:1613
    old "Gone down on a girl"
    new "给一个女孩口交"

    # game/scripts/bar.rpy:1631
    old "Drink"
    new "喝"

    # game/scripts/bar.rpy:1631
    old "Don't Drink"
    new "不喝"

    # game/scripts/bar.rpy:2091
    old "Why don't you guys just take a nap?"
    new "为什么你们不休息会呢?"

    # game/scripts/bar.rpy:2091
    old "Great idea!"
    new "好主意!"

    # game/scripts/bar.rpy:2203
    old "Harder drink for Ellen"
    new "给艾兰烈酒"

    # game/scripts/bar.rpy:2203
    old "Harder drink for Maria"
    new "给玛丽亚烈酒"

    # game/scripts/bar.rpy:2339
    old "Alright"
    new "好的"

    # game/scripts/bar.rpy:2419
    old "Put her on her back"
    new "把她翻过来"

    # game/scripts/bar.rpy:2449
    old "Bend her over"
    new "把她按趴下"

    # game/scripts/bar.rpy:2482
    old "Drink please"
    new "来一杯"

    # game/scripts/bar.rpy:2494
    old "Another one"
    new "再来"

    # game/scripts/bar.rpy:2494
    old "Another one!"
    new "再来!"

    # game/scripts/bar.rpy:2494
    old "ANOTHER ONE!"
    new "再来!"

    # game/scripts/bar.rpy:2494
    old "ANOTHSUR WUNS!!"
    new "再赖!!"

    # game/scripts/bar.rpy:2494
    old "ARNTHERR OEN!!!"
    new "载濑!!!"

    # game/scripts/bar.rpy:2567
    old "{color=#8C8C8C}Call Ellen and Maria{/color}"
    new "{color=#8C8C8C}叫艾兰和玛丽亚{/color}"

    # game/scripts/bar.rpy:2567
    old "Call Ellen and Maria"
    new "叫艾兰和玛丽亚"

    # game/scripts/bar.rpy:2661
    old "Spank her "
    new "打她屁股"

    # game/scripts/bar.rpy:2661
    old "Spank her 1/10"
    new "打她屁股1/10"

    # game/scripts/bar.rpy:2661
    old "Spank her 2/10"
    new "打她屁股2/10"

    # game/scripts/bar.rpy:2661
    old "Spank her 3/10"
    new "打她屁股3/10"

    # game/scripts/bar.rpy:2661
    old "Spank her 4/10"
    new "打她屁股4/10"

    # game/scripts/bar.rpy:2661
    old "Spank her 5/10"
    new "打她屁股5/10"

    # game/scripts/bar.rpy:2661
    old "Spank her 6/10"
    new "打她屁股6/10"

    # game/scripts/bar.rpy:2661
    old "Spank her 7/10"
    new "打她屁股7/10"

    # game/scripts/bar.rpy:2661
    old "Spank her 8/10"
    new "打她屁股8/10"

    # game/scripts/bar.rpy:2661
    old "Spank her 9/10"
    new "打她屁股9/10"

    # game/scripts/bar.rpy:2661
    old "Spank her 10/10"
    new "打她屁股10/10"

    # game/scripts/bar.rpy:2661
    old "Take off the Cat Maid Outfit"
    new "把猫娘女仆装脱了"

    # game/scripts/bar.rpy:2919
    old "Make her ask for it"
    new "让她自己要"

    # game/scripts/bar.rpy:3013
    old "You need to be punished."
    new "你需要被惩罚"

    # game/scripts/bar.rpy:3013
    old "Your ass is amazing."
    new "你的屁股很不错"

    # game/scripts/bar.rpy:3013
    old "Hurting you is fun."
    new "伤害你很有趣"

    # game/scripts/bar.rpy:3270
    old "Make her beg"
    new "让她求"

    # game/scripts/bar.rpy:3404
    old "Rub her"
    new "揉搓她"

    # game/scripts/bar.rpy:3404
    old "Finger her"
    new "指入她"

    # game/scripts/bar.rpy:3404
    old "Fuck her"
    new "操她"

    # game/scripts/bar.rpy:3404
    old "Leave her"
    new "离开她"

    # game/scripts/bar.rpy:3697
    old "Wear the Cat Maid Oufit"
    new "穿上猫娘女仆装"

    # game/scripts/bar.rpy:5090
    old "I don't have an issue with that."
    new "我没有问题"

    # game/scripts/bar.rpy:5090
    old "If I have time..."
    new "如果我有时间的话......"

    # game/scripts/bar.rpy:5775
    old "What she said"
    new "按她所说的"

    # game/scripts/bar.rpy:5775
    old "Tacos"
    new "玉米饼"

    # game/scripts/bar.rpy:5883
    old "My girls are here to serve me."
    new "我的女孩要为我服务"

    # game/scripts/bar.rpy:5883
    old "Bruises are physical proof of my love."
    new "瘀伤是我爱意的体现"

    # game/scripts/bar.rpy:5883
    old "I would treat you with love and respect."
    new "我以爱和尊重对待你"

    # game/scripts/bar.rpy:5883
    old "I bet I could snap your choker with my cock."
    new "我可以用肉棒戳碎你的项圈"

    # game/scripts/bar.rpy:6099
    old "[name]."
    new "[name]."

    # game/scripts/bar.rpy:6099
    old "You can call me Mr. Moon."
    new "你可以叫我文先生."
# TODO: Translation updated at 2024-11-09 02:02

# game/scripts/bar.rpy:2562
translate schinese drinking2_a212d911:

    # "You don't have any Elvish Hops to deliver."
    "你没有精灵酒花."

# game/scripts/bar.rpy:2565
translate schinese drinking2_b30e9ba2:

    # m "Great! Now we'll save money on buying beer!"
    m "太好了!现在我们可以省下买啤酒的钱了!"

# game/scripts/bar.rpy:2567
translate schinese drinking2_665c7d94:

    # "You gained $[elvish_hops_value]!"
    "你获得了[elvish_hops_value]$!"

# game/scripts/bar.rpy:2669
translate schinese drinking_a212d911:

    # "You don't have any Elvish Hops to deliver."
    "你没有精灵酒花."

# game/scripts/bar.rpy:2672
translate schinese drinking_733b300d:

    # bart "Great! Thanks boss. We can save money on buying beer now!"
    bart "太好了!谢谢老板.现在我们可以省下买啤酒的钱了!"

# game/scripts/bar.rpy:2674
translate schinese drinking_665c7d94:

    # "You gained $[elvish_hops_value]!"
    "你获得了[elvish_hops_value]$!"

translate schinese strings:

    # game/scripts/bar.rpy:2488
    old "{color=#8C8C8C}Deliver Elvish Hops{/color}"
    new "{color=#8C8C8C}交付精灵酒花{/color}"

    # game/scripts/bar.rpy:2488
    old "Deliver Elvish Hops {color=#30EA3C}(+$[elvish_hops_value]){/color}"
    new "交付精灵酒花 {color=#30EA3C}(+[elvish_hops_value]$){/color}"
